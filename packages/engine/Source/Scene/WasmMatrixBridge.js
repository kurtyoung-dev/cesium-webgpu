...
class WasmMatrixBridge {
   ...
   */
  batchTransformPoints(matrix, pointsX, pointsY, pointsZ, count, outX, outY, outZ) {
    this._transformCount++;
    if (_wasmReady && count >= this._threshold) {
      this._lastWasmUsed = true;
      this._transformWasm(matrix, pointsX, pointsY, pointsZ, count, outX, outY, outZ);
      return;
    }
    this._lastWasmUsed = false;
    this._transformJS(matrix, pointsX, pointsY, pointsZ, count, outX, outY, outZ);
  }
  ...
  _transformWasm(matrix, px, py, pz, count, ox, oy, oz) {
    ...
    try {
      ...
      _wasmModule.batch_transform_points(ptr, pxPtr, pyPtr, pzPtr, count, oxPtr, oyPtr, ozPtr);

      ox.set(new Float32Array(buf, oxPtr, count));
      oy.set(new Float32Array(buf, oyPtr, count));
      oz.set(new Float32Array(buf, ozPtr, count));
    } catch (e) {
      console.warn('[CesiumJS:WASM:matrix] transform failed, using JS fallback:', e.message);
      this._transformJS(matrix, px, py, pz, count, ox, oy, oz);
    }
  }
  ...
  _multiplyWasm(models, view, count, out) {
    ...
    try {
      ...
      new Float32Array(buf, ptr, count * 16).set(models.subarray(0, count * 16));
      ...
      console.warn('[CesiumJS:WASM:matrix] multiply failed, using JS fallback:', e.message);
      this._multiplyJS(models, view, count, out);
    }
  }
