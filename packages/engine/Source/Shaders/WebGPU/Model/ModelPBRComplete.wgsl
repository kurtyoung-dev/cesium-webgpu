...
// `clippingEdgeWidth > 0`.
// AUDIT_2026_05_02 A.6 — polygon SDF clipping for models. Mirror of
// the WebGL `Shaders/Model/ModelClippingPolygonsStageFS.glsl` flow,
// adapted for WGSL and the simplified single-polygon-collection
// case. The SDF was generated by `WebGPUClippingPolygonCollection`
// over a 256×256 atlas keyed on geographic (lon, lat) — see
// `chunks/functions/csm_clipByPolygons.wgsl` for the same approach
// used by the globe.
//
// CRITICAL FRAME: input is WORLD-space position (meters from Earth
// center). The fragment converts to spherical (lon ∈ [-π, π],
// lat ∈ [-π/2, π/2]) and remaps to atlas UV. Multi-polygon-region
// dispatch (per-region atlas tiles, like the WebGL VS-side
// region-selection) is deferred — first-cut covers the common
// single-polygon case correctly. Multi-region collections will
// undersample some polygons until the per-region uv mapping lands
// (NEW-MODEL-CLIPPING-POLY-MULTI-REGION).
fn modelClipByPolygon(positionWC: vec3<f32>) -> bool {
  let count = effects.clippingPolygonCount;
  if (count == 0u) { return false; }
  // Earth-centered spherical coords. atan2(y, x) yields longitude in
  // [-π, π]; asin(z / length) yields latitude in [-π/2, π/2]. These
  // match the (longitude, latitude) the SDF compute pass packs.
  let r = length(positionWC);
  if (r <= 0.0) { return false; }
  let PI: f32 = 3.14159265358979;
  let lon = atan2(positionWC.y, positionWC.x);
  let lat = asin(clamp(positionWC.z / r, -1.0, 1.0));
  let u = (lon + PI) / (2.0 * PI);
  let v = (lat + PI * 0.5) / PI;
  ...
  // SDF >= 0.5 → outside polygon → discard. Matches the
  // CesiumJS-non-inverse default of `czm_clipPolygons`.
  return sdfValue >= 0.5;
}
