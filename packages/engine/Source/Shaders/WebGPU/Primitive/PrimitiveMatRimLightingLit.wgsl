...
// and effects). Ambient + rim stay unshadowed; only direct (diffuse + spec) is modulated.
...
fn fragmentMain(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // CSM Slice 2d — shadow the direct diffuse + specular. Ambient + rim
    // stay unshadowed (rim is a non-physical artistic effect).
    ...
    let finalColor = ambientTerm + directTerm + spec + rim;
