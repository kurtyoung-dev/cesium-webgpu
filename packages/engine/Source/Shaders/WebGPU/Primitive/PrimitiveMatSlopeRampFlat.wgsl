// PrimitiveMatSlopeRampFlat.wgsl
// Slope ramp material, no lighting
// Maps surface slope to a color ramp texture
// Without normals, slope cannot be computed — samples ramp at 0.5 (mid-slope)
...
fn fragmentMain(input: VertexOutput) -> @location(0) vec4<f32> {
    // No normals available — use mid-ramp fallback (slope ~45 degrees)
    let rampColor = textureSample(rampTexture, rampSampler, vec2<f32>(0.5, 0.5));
    return rampColor;
}
