...
// Ambient + emission stay unshadowed; only direct (diffuse + spec) is modulated.
//
...
// `channels` packs as vec3<f32> indices (r=0, g=1, b=2, a=3). Fabric
// order: channels, repeat.
...
fn fragmentMain(input: VertexOutput) -> FragOutput {
    ...
    // EmissionMap has no material `color` field — fabric only provides a
    // texture + channel swizzle ("rgb") + repeat. Lit base is white.
    let ambient = 0.5;
    ...
    // Emission swizzled by the fabric-specified channels, unshadowed —
    // surface glows.
    ...
    // The white lit base supplies albedo; F0/roughness are synthesized as a
    ...
    mrtOut.color = vec4<f32>(finalColor, texColor.a);
