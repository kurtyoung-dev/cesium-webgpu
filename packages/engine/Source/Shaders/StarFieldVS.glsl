...
// disables the depth test — the stars run in the environment pass, before
// the globe and opaque passes, which then overwrite the stars wherever
// solid geometry is present — again matching the SkyBox.
//
...
// Zenith atmospheric transmittance (per channel) and the camera's local-up
// direction in the Earth-fixed frame. Each star's slant transmittance is
// zenithTransmittance^airmass(elevation). Default (1,1,1) up leaves the field
// byte-identical (pow(1, x) === 1).
uniform vec3 u_zenithTransmittance;
uniform vec3 u_cameraUpFixed;
// Angular solar-glare star washout. Resolved once per frame on the
...
void main()
{
    ...
    // airmass X ≈ 1/sin(elevation), capped near the horizon at ~38 (the
    // Kasten-Young limit) to stay finite; slant transmittance =
    // zenithTransmittance^X. u_zenithTransmittance is (1,1,1) from orbit or
    // with the atmosphere hidden, so this is a byte-identical no-op there.
    ...
    // separation from the Sun is constant across its whole quad. Dotted
    // against the raw `directionFixed` attribute, not `dirFixed`: the
    // attribute is the TEME catalogue direction and the published Sun vector
    // is resolved into TEME, so this is the same dot the WGSL twin takes and
