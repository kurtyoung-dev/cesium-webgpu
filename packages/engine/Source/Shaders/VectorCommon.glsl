...
vec4 vectorPolylineRender(vec2 vectorUv, vec4 baseColor)
{
    ...
    bool uvJacobianUnusable =
        abs(uvJacobianDet) < 1.0e-20 ||
    ...
    // Every dFdx/dFdy in this function is evaluated above, so abandoning the
    // fragment here keeps them all in uniform control flow. The meters path
    // needs the same rejection: a singular Jacobian collapses both screen-space
    // derivative lengths and drives pixelsPerMeter to Inf.
    if (uvJacobianUnusable)
    {
        return baseColor;
    }
