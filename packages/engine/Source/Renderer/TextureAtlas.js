...
class TextureAtlas {
   ...
   */
  computeTextureCoordinates(index, result) {
    //>>includeStart('debug', pragmas.debug);
   ...
   * @private
   */
  _copyFromTexture(context, width, height, rectangles) {
   ...
   * @private
   */
  _copyImageToTexture({ index, image, resolve, reject }) {
    const texture = this._texture;
    const rectangle = this._rectangles[index];
   ...
   */
  addImage(id, image, width, height) {
    //>>includeStart('debug', pragmas.debug);
    Check.typeOf.string("id", id);
    Check.defined("image", image);
    //>>includeEnd('debug');
   ...
   */
  getCachedImageSubRegion(id, subRegion, imageIndex) {
...
/**
 * Info needed to add a queued image to the texture atlas when update operations are executed, typically at the end of a frame.
 * @private
 * @param {object} options Object with the following properties:
