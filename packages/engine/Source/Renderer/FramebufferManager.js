...
class FramebufferManager {
  ...
  isDirty(width, height, numSamples, pixelDatatype, pixelFormat) {
  ...
  update(context, width, height, numSamples, pixelDatatype, pixelFormat) {
    //>>includeStart('debug', pragmas.debug);
  ...
  setColorRenderbuffer(renderbuffer, index) {
    index = index ?? 0;
    //>>includeStart('debug', pragmas.debug);
    if (this._createColorAttachments) {
      throw new DeveloperError(
  ...
  setDepthRenderbuffer(renderbuffer) {
    //>>includeStart('debug', pragmas.debug);
  ...
  setDepthStencilRenderbuffer(renderbuffer) {
    //>>includeStart('debug', pragmas.debug);
    if (this._createDepthAttachments) {
      throw new DeveloperError(
  ...
  setDepthStencilTexture(texture) {
    //>>includeStart('debug', pragmas.debug);
    if (this._createDepthAttachments) {
      throw new DeveloperError(
   ...
   * @private
   */
  prepareTextures(context, blitStencil) {
    if (this._numSamples > 1) {
      this._multisampleFramebuffer.blitFramebuffers(context, blitStencil);
    }
  }

  clear(context, clearCommand, passState) {
