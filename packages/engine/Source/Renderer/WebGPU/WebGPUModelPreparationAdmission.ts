...
interface WebGPUModelPreparationWorkStatistics {
  ...
  /**
   * Light blocks packed this frame. One per model per view, because the block
   * lives in the group-0 arena rather than in each primitive's bind group. A
   * count that tracks primitive count instead is a regression, not a workload
   * change.
   */
  lightPacks: number;
  /**
   * Light blocks staged into the per-frame ring. Not a count of
   * `queue.writeBuffer` calls: the ring emits one write per dirty page for the
   * whole frame. Equals `lightPacks` by construction, because a staged block is
