 ...
 * The band needs its own altitude datum.
 *
 * `VolumetricFog.wgsl::densityInjection` reconstructs a froxel's `altitude` as
 * a height above the ellipsoid's inscribed sphere, whose radius is the
 * minimum of the three radii, the WGS84 polar radius of 6,356,752 m. That
 * frame is chosen so a camera over a pole can never produce a negative
 * altitude, and it is adequate for the base height fog, whose falloff scale is
 * about 10 km: the sphere-versus-ellipsoid offset just scales the density
 * globally.
 *
 * The ground-fog band's falloff scale is about 120 m, and that same offset is
 * 21,385 m at the equator, 10,215 m at 46.4 deg N, and 0 only at the poles.
 * Feeding the raw `altitude` into `exp(-altitude / 120)` therefore evaluates
 * `e^-85` at Alpine latitudes — a denormal, 2.1e-40, and an exact f32 zero
 * nearer the equator. The optical depth a ray accumulates over the whole
 * march then comes to around 1e-44, which leaves the f32 transmittance at
 * exactly 1.0 and the in-scatter at exactly 0, so the composite's
 * `scene.rgb * transmittance + scatteredLight` returns the scene colour bit
 * for bit and the band renders nothing at all.
 *
 * Expressing a ground datum in the same inscribed-sphere frame and subtracting
 * it in the shader avoids that: the sphere-versus-ellipsoid error is common to
 * both terms, so it cancels exactly and the band lands on the ground it is
 * named for.
 *
 * The datum is the camera-local ground elevation: the ellipsoid's geocentric
 * radius along the camera's radial direction, plus the terrain height directly
 * beneath the camera when a terrain surface can supply one, and sea level
 * otherwise. One scalar per frame, the standard cheap approximation for a
 ...
 * which a per-froxel-column datum reconstructed from the depth buffer would
 * address.
 */
 ...
 *
 * Derived, not tuned: `extinctionForVisibility(2000)`. 2 km is the mist-to-fog
 * boundary of the WMO visibility scale, so a fully saturated band reads as a
 * real fog while the near field stays legible, which is the acceptance
 * criterion for this effect: terrain visible through the haze, not a whiteout.
 * Intensity scales it linearly, so 0.3 gives about 6.7 km of visibility, a
 * haze, and 0.6 about 3.3 km, a mist.
 *
 * A tuned constant here is a trap. 1.2e-4, for instance, corresponds to 32 km
 * of visibility: clear air, not fog, and below the 8-bit quantisation floor of
 * a frame.
 */
