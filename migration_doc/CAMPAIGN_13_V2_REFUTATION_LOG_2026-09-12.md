...
| **U3** | GMGSI "~42 min latency, 7–8 MB NetCDF, 71°N–71°S" | The AWS registry page confirms ~8 km, hourly, the bucket ARN and the open-data statement — but states **no** latency, format, size or latitude range; and §8 already conceded the grid/projection are undocumented | §4.1 GMGSI row; §8 Unknown 4; added as a `C13-N05` precondition. **WHAT WOULD CONFIRM:** read one file's NetCDF header |
