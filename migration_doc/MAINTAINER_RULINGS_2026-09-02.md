...
| R-2026-09-02-26 | Sun disc (C12-38 engine finding: WebGPU disc ~3.1x darker than WebGL, darker than the sky) | Reconcile the two sun-disc clones FIRST: a read-only audit of `cesium-worker-sundisc` against the frozen `cesium-lane-sundisc2`, banked with a receipt (lane Ciryon, `migration_doc/branches/SUNDISC_RECONCILIATION_2026-09-02.md`). The freeze stands until that audit is read; no engine lane before it. |
| R-2026-09-02-27 | AO radius law at range (`DM-18`) | The target is parity with WebGL: the WebGPU HBAO pass adopts WebGL's depth-scaled radius law (lengthCap x sqrt(depth), gaussian weight) so both backends occlude at terrain range. Full bar; dispatch gated on the close-range and terrain captures from the `DM-17` Edge legs showing the gap. |
| R-2026-09-02-28 | Wave-end gate for the 2026-09-02 landings | Runs tonight, after the nine owed Edge legs, as the manual three-step of R-2026-08-29-2 (variant smoke test, Sandcastle2 sweep on both renderers, capture-and-diff without baseline refresh), banked under `Tools/visual-regression/output/wave-end/2026-09-02/`. |
| R-2026-09-02-29 | `C18-P3` funding | Two engine slices in the next window per lane Leofa's design (`migration_doc/C18_P3_PNTS_MODEL_ATTENUATION_DESIGN_2026-09-02.md`, reviewer Freca: P3-a alone, P3-b with P3-c), each with the full bar and the landed probe as the Edge leg. |
| R-2026-09-02-30 | chelate remote | The relocated Rust supervisor stays a local-only repository with no GitHub remote until the certification row is funded and passes; backups continue through the worker archive. |
| R-2026-09-02-31 | Sol's role when Codex usage returns | Bounded single-deliverable lanes only: Sonnet-shaped rows with a named spec and runner home, always under an Opus station-3 review; never the seat, never the Edge executor, never git. The narrowed per-repository trust and the provisioning trust flags apply to every Codex clone. |
| R-2026-09-02-32 | Codex home data (7.4 GB) | Maintainer-owned; Codex cleans it up on the maintainer's instruction. The seat deletes nothing there and reports the directory's size after each major Codex session. |
| R-2026-09-02-33 | Sun-disc clones after Ciryon's reconciliation | `cesium-lane-sundisc2` holds nothing unlanded (its option-A co-fade landed in full at Batch 1184): retired two-phase against the bank at `cesium-webgpu-worker-archive/sundisc-reconciliation-2026-09-02`. `cesium-worker-sundisc`'s option-B transmittance split and its three salvage items become row `C12-38b`; the clone stays until that row is dispatched. The freeze on sun-disc engine work stands until the discriminator sweeps (sun hidden; bloom off) are read. |
