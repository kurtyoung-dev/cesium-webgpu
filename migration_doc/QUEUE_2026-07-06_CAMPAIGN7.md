...
| 3 | NEW-PP-SILHOUETTE-ARRAY-EDGE-UNIFORMS | S | parity-bug | `createSilhouetteStage([edgeStages])` array form loses inner edge color/length on WebGPU (falls back to black / 0.25). Cheap: walk `stage._stages` to the first inner `czm_edge_detection` and pack from its uniforms. |
