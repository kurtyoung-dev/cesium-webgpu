...
#### 2.3 — Aerial-perspective froxel 3D LUT (`AERIAL-FROXEL`) — **P1, effort L** — ✅ SHIPPED Batch Q23 (32³ froxel volume baked once/frame by `AerialPerspectiveFroxel.wgsl`; `AerialPerspective.wgsl` does one trilinear fetch when `scene.aerialPerspectiveFroxel` is on; opt-in, default OFF byte-identical; froxel vs analytic visually indistinguishable per `probe-aerial-froxel.mjs`)
