...
| VW-N7 tranche 3c | GREEN (2026-08-28 ~23:10; evidence output/edge-tranche3c-2026-08-29/): NV legs 4 green + 1 reframed - the 1254/1255 defaults are live (nightDarkness 0.15, enableNightLights true), city-light emission is 1254's only orbit-visible change (WebGL lights-off vs pre-1254 default 0.0152%), street default byte-identical to 3a `-nd015`, app-managed stacks untouched (zero layers injected). Cross-backend ON agreement reframed to meanAbs (ON pair 0.336987 vs lights-off control 0.33862): the emission adds no divergence. First GLSL `APPLY_NIGHT_LIGHTS` compile clean. |
| VW-L3 tranche 3c | GREEN on the disc + MoonLight (2026-08-28 ~23:10): copper Moon on both backends (whole-disc R/B 36x vs 1.49; aim 0.000000 deg); MoonLight lightColor dims to 0.0451849 (22.1x, bit-identical both backends); SunLight control unchanged. NOT ESTABLISHED: luminance ratio 0.0531 vs 0.0452 predicted (threshold-dominated at 97.71% coverage), leg (e) pixel half, leg (f). Q-71 (setView zenith reflection) and Q-72 (WebGPU model mask, unconfirmed) filed. |
