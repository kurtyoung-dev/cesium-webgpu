...
| `C15-G7a` | WebGPU gsplat classification: the depth-write variant is never selected and the per-instance colour does not reach classification shading; the shadow volume is unclipped (the C15-G7 reds (a)+(b)) | M | PARTIAL LANDED (Batch 1389; Indis, reviewer Duinhir): the per-instance colour red is FIXED at the renderer altitude with the full bar; the depth-write variant is structurally unreachable for splats (the tile-update tagging predicate sees only commands pushed during traversal; the splat command is pushed afterwards by the primitive wrapper) and the shadow volume follows the same chain - design in the packet, one link cannot be fixed without degrading splat rendering; the probe re-run is the Edge leg | `C15-G7` |
| `C15-G7b` | `probe-gsplat-classification-depth.mjs` framing: frame the tower so `towerMaskPixels` is in the hundreds on both backends (distance from the tower's own extent, not the footprint sphere), keep every existing assertion, add a refusal when the mask is under a pre-registered floor | XS | LANDED (Batch 1387; Narvi, reviewer Mahtan; framing from the tower's own extent, refusal under a 100 px mask floor; live re-run owed) | `C15-G7` |
