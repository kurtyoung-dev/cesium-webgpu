...
| `C15-G7a` | WebGPU gsplat classification: the depth-write variant is never selected and the per-instance colour does not reach classification shading; the shadow volume is unclipped (the C15-G7 reds (a)+(b)) | M | DISPATCHED wave 2 2026-09-02 (Indis, Opus engine lead; full bar; diagnosis-first, WebGL byte-identical; the C15-G7 probe re-run by the Edge executor after C15-G7b lands) | `C15-G7` |
| `C15-G7b` | `probe-gsplat-classification-depth.mjs` framing: frame the tower so `towerMaskPixels` is in the hundreds on both backends (distance from the tower's own extent, not the footprint sphere), keep every existing assertion, add a refusal when the mask is under a pre-registered floor | XS | DISPATCHED wave 2 2026-09-02 (Narvi, Sonnet; probe-only; distance from the tower's own extent, refusal under a pre-registered mask floor) | `C15-G7` |
