...
- ✅ **NEW-SPLAT-SORT-CONSUME-INDEXES** (Batch 288 — Gaussian splats consume back-to-front sort). **NEW-CSM-SOFT-SHADOW-PCF** (Batch 289 — receive-side 3x3 PCF kernel CODE SHIPPED). ✅ **NEW-CSM-CAST-NO-DISPATCH-VIEWER** (Batch 296 — CAST SIDE FIXED: `SceneRenderer.executeShadowMapCastCommands` now populates the per-pass cast lists for BOTH backends before delegating GPU dispatch (was WebGL-only — the WebGPU context early-return skipped population), and appearance-primitive WebGPU commands carry `rte24` cast metadata → `_castDispatches` 0→584, caster self-shadows, 0 errors). Remaining: **NEW-CSM-GLOBE-RECEIVE-PROJECTION-MISS** (HIGH, surfaced Batch 296 — cast depth correct but the globe-terrain receiver's cascade lookup misses the caster's stored depth; primitive receiver self-shadows fine → globe-specific RTE/VP alignment). Remaining CSM_DESIGN slices: normal-shading clamp + altitude-adaptive splits (Slice 3) + VSM. **NEW-TAA-PIPELINE-ORDER-RECONCILE** (pre/post-tonemap decision + clamp retune), **NEW-POSTPROCESS-USER-WARN-PROD** (user GLSL stages silently dropped — un-strip the warning).
...
- **V-A Atmosphere fidelity:** NEW-ATMO-BRUNETON-FULL-LUTS (add multiple-scattering + irradiance LUTs) → NEW-ATMO-AERIAL-PERSPECTIVE-POSTPROCESS (unified per-pixel transmittance+inscatter over the whole scene — the biggest "looks like the talk" win) → NEW-ATMO-DERIVED-LIGHTING + MIXED-MASK (sun/sky light from the LUTs; post-process-Lambertian terrain + light-source-PBR models, the ISS pattern).
...
- **V-C Effects + bodies:** NEW-STARS-BRIGHT-CATALOG (Yale Bright Star Catalog) · NEW-SUN-MOON-FIDELITY (physical sun disc/limb + lens glare; moon PBR + earthshine) · NEW-EFFECTS-LIGHTSHAFTS-LENSGLARE (crepuscular rays, geometry lens glare) · NEW-MULTIBODY-ATMOSPHERE (Mars/airless-body parameter sets on the same LUT pipeline).
- License: MIT repo + BSD Bruneton reference → implement the technique, credit Bruneton + Takram in shader headers.
