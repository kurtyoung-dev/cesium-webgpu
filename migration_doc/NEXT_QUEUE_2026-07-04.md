...
| Q3 | **ATMOSPHERE-LUT-BGL-INCOMPAT** | M | scoped | opus | — | Sky LUT compute pipes `computeTransmittance`/`computeInscatter` built `layout:'auto'`, bound with explicit `AtmosphereLUT_BGL`; `SkyAtmosphere.wgsl:60` | capture uncaptured-GPU-error console during bake dispatch | give pipes explicit `pipelineLayout` or `getBindGroupLayout(0)`; ENABLE_SKY_INSCATTER_LUT still false → visually inert, fixes per-frame validation error |
...
| Q9 | **VOXEL-PICK-OCTREE-COMPOSITION (R-8a)** | M | scoped | opus | Q8 | `fragmentPickVoxelMain` still root-biased vs color march L3; pick should share color-march traversal, honor VOXEL_USER_CUSTOM_SHADER alpha | `probe-voxel-pick-lod` at L1+ refinement | pick returns the LEAF cell (not root) once frame refines; **NOTE**: depth-1 pick shipped B509 (see reconcile) — residual = reach color-march L3 + share traversal |
