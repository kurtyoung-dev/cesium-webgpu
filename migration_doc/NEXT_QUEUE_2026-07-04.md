...
| ~~Q15~~ | ~~**ATMO-AERIAL-PERSPECTIVE-NEARFIELD-TUNE**~~ **DONE (2026-07-04, C4-ATMO-AERIAL-NEARFIELD-TUNE)** | S | scoped | opus | — | `updateAerialPerspectiveFrameData` (WebGPUPostProcessStageCollection.ts) now re-reads `scene.aerialPerspectiveConfig.{intensity,inscatterScale}` every frame and pushes them via the existing `fx.setIntensity`/`setInscatterScale` setters (mirrors the ColdOptics/HeatShimmer per-frame `setIntensity` pattern). Config was previously read once at `addAerialPerspective` init → runtime mutations were inert. Off-gate: `aerialPerspectiveConfig` is undefined by default → new block skipped → byte-identical | `probe-aerial-runtime-config.mjs` PASS: intensity dial far ΔB=-0.21 (extinction dims at low inscatter — physical), inscatterScale dial far ΔB=+0.44, off-gate abs(mean Δ)=0.0000, 0 errors; base `probe-aerial-perspective.mjs` still PASS | near-field haze not over-heavy at high sun; depth-fade already correct (extinction blend `mix(1,extinction,hazeIntensity)` unchanged) |
| ~~Q16~~ | ~~**WASM-BRIDGE-BUNDLE-LOAD**~~ **PREMISE STALE (2026-07-04, C4-WASM-BRIDGE-BUNDLE-LOAD)** | M | scoped | opus | — | Fixed in Batch 274 (0593a6c8b1, NEW-WASM-BRIDGE-BUNDLE-LOAD, see DEFERRED_WORK L279): all 7 `Wasm*Bridge` route the glue `import()` through `resolveWasmGlueUrl()` = `buildModuleUrl("ThirdParty/Workers/cesium_wasm.js")`, so glue+binary load 200 in every format. The RTE SIMD kernel genuinely runs in the bundle (`simdActive:true`, `_lastWasmUsed:true`, byte-identical to the JS fround twin, no fallback warning). Not reimplemented. | `probe-wasm-bundle-load.mjs` GREEN at HEAD (all 7 bridges load, 0 `cesium_wasm*` 404s, RTE WASM==JS byte-identical + **new `simdActive` assertion** covering the "SIMD kernel actually loads" off-gate) | n/a runtime (probe strengthen + doc reconcile only). JS fallback stays byte-exact; flat-buffer hoist win B273 stands regardless |
...
| Q18 | ~~**GPU-CULLER-CONSUME-OR-DELETE**~~ **PREMISE STALE (2026-07-04)** | M | scoped | opus | — | ~~orphan dispatchers allocate eagerly, no live caller~~ — FALSE at HEAD: all four (gpuCull/HiZ/GPUSortKeys/PointCloudSort) are LAZILY allocated + gated with live consumers (RESOLVED Batches 209/210/211; PointCloudSort via `TimeDynamicPointCloud`→`sortByDistance`→`fr.sort`). See DEFERRED_WORK.md "NEW-GPU-CULLER-CONSUME-OR-DELETE" §"Q18 reconcile" | `Tools/visual-regression/probe-gpu-culler-consumers.mjs` (structural consumer/gate guard; PASS 10/10) | no code change — consume-decision stands; regression probe locks it in |
| ~~Q19~~ | ~~**TAKRAM-8-GEOMETRY-LENS-GLARE**~~ **PREMISE STALE (2026-07-04, C4-TAKRAM8-GEOMETRY-LENS-GLARE)** | M | scoped | opus | — | Lens-glare already ships: `createSunTexture()` in `WebGPUEnvironmentRenderer.js:203-286` bakes disc + soft glow halo + the six pre-rotated lens-flare bursts, a faithful port of WebGL `SunTextureFS.glsl` `addBurst()` (landed **Batch 214 / BUG-1**, 0e51182139). WebGL's moon (EllipsoidPrimitive) has NO glare → glare is a sun-only parity feature, already at parity. Not reimplemented. | `Tools/visual-regression/probe-sun-lens-glare.mjs` (new) GREEN: WebGPU sun disc + glow/flare halo present, halo extent ratio WebGPU/WebGL w=1.00 h=1.00, burst spikes visible in the read-back PNG | n/a runtime (probe + doc reconcile only). Cosmetic fill-in; disc/limb/glow core shipped B378 |
