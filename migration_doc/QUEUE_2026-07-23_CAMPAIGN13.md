...
| `C13-16` | Per-region cloud-type and vertical-deck mixtures  **ACCEPTANCE RUN ATTEMPTED 2026-08-06 (orchestrator): NOT YET DECIDED - probe needs tuning, no product verdict either way.** `probe-cloud-genus-morphology.mjs` (landed Batch 830) first failed at init with executeCalls=0 / initialized=false / pipelineReady=false, which reads exactly like a broken renderer but was the probe never calling its own `configure()` helper before `awaitProceduralReady` - every working cloud probe (probe-cloud-banding, probe-cloud-tour) configures FIRST. Fixed (Batch 836). It now initializes but exceeds the 420 s watchdog: six legs with paired on/off captures at `cloudQuality: 96` is too expensive. Tuning owed (lower the quality for the structural legs, which measure autocorrelation DIRECTION and do not need 96 steps, and/or split the legs across runs). The C13-16 Edge checklist remains entirely OWED - nothing here is evidence for or against the morphology.| P1 | weather/quality | L | W3 | `C13-14`, `C13-15` |
