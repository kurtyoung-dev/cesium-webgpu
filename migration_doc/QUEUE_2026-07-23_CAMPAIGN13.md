...
| `C13-GATE-B` | **PARTIAL / OPEN — every row now implemented; the gate itself is unverified** | `C13-03`, `C13-04`, and `C13-05` are complete. `C13-06` and `C13-07` are implemented AND probe-verified (2026-08-01). `C13-08` — the last row — is implemented with a 29/29 CPU twin and is AWAITING its orchestrator browser verification (checklist in §9's `C13-08` subsection); its regional-placement pixel gate **2026-08-02 ORCHESTRATOR GATE RUN (WebGPU): PASS on the row's criteria — uniform=1.0 CONUS-bounded field renders a clear regional response inside (bright-pixel fraction 0 -> 0.118 at the 250 km nadir metric) while the far-outside view stays byte-flat vs no-provider (0 -> 0), and getPackStats cross-checks exactly (global:false, observedTexels 798, filledTexels 31970, same-evaluation). Zero console errors. Remaining from the checklist: the antimeridian-straddle lane and the WebGL regression sweep.** is the one Gate B criterion ("a regional field is globally stretched") that nothing has yet exercised on the GPU. The gate does not close on the CPU twin alone. `C13-41` unblocks once that verification lands. |
...
The CPU twin (`weather-field-bounds.spec.mjs`, 29/29) proves the packer's bytes. It cannot prove the
...
4. **Antimeridian-straddling region.** Same probe, bounds `170°E → 170°W`, camera ON the seam: the
   observed band must be continuous across ±180° with no wall, and must not appear twice.
