...
- **(c) Routing every feature renderer through the central cache** — **STILL OPEN.** Audit confirms zero feature-renderer call sites currently use `context.webgpuPipelineCache`; every renderer keeps its own pipeline map (e.g. `WebGPUModelPipelineCache._pipelines`, the per-effect caches in `WebGPUPostProcessEffects`, the per-renderer pipelines in `WebGPUEllipsoidPrimitiveRenderer` / `WebGPUGroundPrimitiveRenderer` / `WebGPUGaussianSplatRenderer` / collections / globe surface). Routing them through the central cache requires also sharing `GPUShaderModule` handles across renderer instances — otherwise two models with identical material settings still materialise two pipelines because their shader modules differ. That's a per-renderer migration spanning multiple sessions.

Tracked as **FOLLOW-UP C-R7-RENDERER-MIGRATION** (per-renderer routing) + **FOLLOW-UP C-R7-SHADER-MODULE-DEDUP** (cross-renderer shader module sharing for actual dedup wins).
