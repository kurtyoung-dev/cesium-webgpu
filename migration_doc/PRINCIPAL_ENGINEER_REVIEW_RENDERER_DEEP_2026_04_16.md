...
- **(c) Routing every feature renderer through the central cache** — **PARTIALLY DONE (Batch 53, 2026-04-25).** First-cut migration of three representative renderers landed:
  - `WebGPUEllipsoidPrimitiveRenderer.ts` — color + pick descriptors now built once in `buildEllipsoidPipelineResources` and resolved through `context.webgpuPipelineCache.getPipeline(descriptor)`. Async resolution gracefully skips draw frames until pipelines materialize. Falls back to direct `device.createRenderPipeline()` when no cache is available (legacy callers, WebGL contexts).
  - `WebGPUGaussianSplatRenderer.ts` — color + OIT + pick descriptors all routed through the cache via `buildSplatPipelineResources` + `tryResolveSplatPipelines`. OIT failure remains non-fatal and doesn't block the color+pick ready signal.
  - `WebGPUDepthPlane.ts` — single depth-only pipeline routed via an optional `pipelineCache` parameter on `initialize()`. `WebGPUSceneRenderer._ensureResources` now passes `context.webgpuPipelineCache` so split-screen / multi-canvas instances dedupe identical depth-plane descriptors.

  The remaining 12+ renderers (`WebGPUModelPipelineCache`, `WebGPUPostProcessEffects`, the rest of the per-renderer pipelines under `Source/Renderer/WebGPU/`, etc.) keep their own pipeline maps. Migration is mechanical but multi-session; `WebGPUModelPipelineCache` also needs cross-renderer `GPUShaderModule` sharing before routing it gives any actual dedup win — otherwise two models with identical material settings still materialize two pipelines because their shader modules differ.
...
Tracked as **FOLLOW-UP C-R7-RENDERER-MIGRATION** (per-renderer routing — 12 remaining call sites) + **FOLLOW-UP C-R7-SHADER-MODULE-DEDUP** (cross-renderer shader module sharing for actual dedup wins).
