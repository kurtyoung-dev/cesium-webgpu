...
- **HDR pipeline interaction (RECONCILED 2026-06-23 — supersedes the 2026-04-11 note; NEW-TAA-PIPELINE-ORDER-RECONCILE, decided Batch 290)**: the HDR pipeline DID land — the post-process intermediates are now `rgba16float` (`_intermediateFormat`) in HDR. TAA accordingly runs **PRE-tonemap** in the linear/HDR domain (`WebGPUPostProcessPipeline.ts` calls `_taaEffect.execute()`, then pushes the tonemap stage; the resolve uses history in `_intermediateFormat`). The variance clamp was **NOT retuned**: the resolve operates in reversible **tonemap-weight space** (`TAA.wgsl` `tonemapWeight = c/(1+luma)` / `inverseTonemapWeight = c/(1-luma)`, the Karis HDR anti-firefly weight), which is well-defined ONLY on linear/HDR input — feeding it already-tonemapped SDR (luma→1) divides by ~0 → Inf/NaN. So the existing clamp constants are domain-correct as-is, and post-tonemap placement is INVALID, not merely suboptimal. (The old note's "TAA after tonemapping is the correct position" is obsolete.)
- **Phase 5 WGF-3 shader-f16 interaction (RECONCILED 2026-06-23)**: the f16 tonemapping variant runs **after** TAA (per the pre-tonemap order above), not before. TAA's input/history is linear/HDR (`rgba16float`), not SDR [0,1]; f16's ~3-decimal-digit mantissa is precision-adequate for the tonemap stage that follows the resolve. No interaction concern.
