...
- **CLT-B3 (S)** — Contain `computeTerminatorGlow` (§2 bug 3): port to GLSL as
  ...
  double-schedule.
- **CLT-B4 (M)** — Reconcile the terminator ramp law across backends (§2
  bug 2), untangle the imagery-alpha vs lighting expressions, correct the
  false WGSL comment, decide the camera-fade question; write the target law
  into `SHADER_PAIRS_LOCKSTEP.md`. **PREREQ DISCHARGED 2026-08-07 (Batch 919,
  CO-15):** the row's blocking half — the constant normal source — is fixed,
  so the ramp exists and the `+0.5` offset is measurable for the first time.
  What remains for B4 is the offset itself, the false comment, finding (c)'s
  vertex-normal gate and finding (d)'s camera fade. Expected second-run
  readings are pre-registered in `DEFERRED_WORK.md` under
  `NEW-WEBGPU-GLOBE-DAYNIGHT-RAMP-OFFSET`; the guards that pin the `+0.5` and
  the wrong comment ON PURPOSE (`daynight-terminator-law.spec.mjs` A2/A3/A5,
  `globe-daynight-normal-source.spec.mjs` E1) must move in the same edit.
