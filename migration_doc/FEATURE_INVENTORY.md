...
- Volumetric clouds Phase 6 main render path SHIPPED via `WebGPUProceduralCloudRenderer` (Schneider HG dual-lobe + Beer-Powder + 3D FBM); activation is the exclusive `CloudCollection.renderMode = VOLUMETRIC` (default `BILLBOARD`) after the cloud-unification epic (Batches 617–625) — `atmosphericConditions.clouds.enableVolumetric` (which proxies onto `globe.defaultCloudCollection`) defaults FALSE per B15 lock; the legacy `globe.showProceduralClouds`/`globe.cloud*` surface was REMOVED (slice 4B). Phase 6c (cloud shadows in volumetric fog froxel) + 6d (quality dial) + 6b (≥100km 2D fast-path crossfade, may be unnecessary given single-renderer arch) — deferred to future session (CELESTIAL §4.6)
