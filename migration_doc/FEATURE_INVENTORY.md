...
- WebGPUDerivedCommand — centralized pipeline-descriptor-variant factory (depth-only/log-depth/pick/HDR/shadow). The variant STRUCTURE half is live (`selectCommandVariant` + `cmd.derivedCommands.{picking,depth,shadows}` dispatch every frame); the factory half is unwired scaffolding (create* methods zero callers, flags unread, caches unpopulated) — planned first consumer is the NEW-COLLECTIONS-LOG-DEPTH epic; see DEFERRED_WORK NEW-DERIVEDCOMMAND-VARIANT-FACTORY (re-documented Batch 239) (SCAFFOLDED)
