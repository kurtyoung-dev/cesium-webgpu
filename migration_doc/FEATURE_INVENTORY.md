...
**2026-07-31 working-tree audit overlay:** local `main` and `origin/main` are
equal at Batch 771 (`fe990ab335`); the index is empty and the active work is
uncommitted. The current pure eclipse/globe/atmosphere Node set passes 138/138,
but focused Edge/Karma did not execute a test before timeout, so the older
Edge counts above remain historical evidence rather than a current dirty-tree
...
open. `C11-181` is implemented and verified but not landed. A valid moving
...
- Per-object cache device-loss recovery walk (Batch 197, corrected 2026-07-31) — the original walk clears literal `_webgpuCache` owners reached through scene/ground primitives plus shadow-map/post-process owners, but does not prove nested tileset-model, clipping, label-cache, or replacement-device lifetime coverage. Point/Label normal teardown is now covered in the dirty tree. (WIP / PARTIAL; `C11-20`)
