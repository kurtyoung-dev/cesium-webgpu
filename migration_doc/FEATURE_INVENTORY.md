...
- 🟠 HIZ-OCCLUSION-CONSUMER — **P0 black-screen FIXED (Batch 212); residual intermittent readback race remains.** Wave 0 found the Hi-Z/GPU-cull consumer black-screened any dense (≥2400 opaque-cmd) WebGPU scene (0 px, 702 validation errors caught by the Batch 208 gate). Batch 212 fixed the catastrophic causes — dense scenes now RENDER (0%→77% px) with no regression on normal scenes:
  ...
  - (3) gpuCull readback had no in-flight guard → re-copied into the mapped staging buffer → "[Buffer] used in submit while mapped". Fixed: per-frustum `_gpuCullReadbackInFlight` guard (mirrors `_hiZReadbackInFlight`).
  - **RESIDUAL (Batch 212, tracked):** ~174 intermittent readback-staging races/180 frames still fire ("used in submit while mapped" on `Occlusion_VisibilityStaging` + occasional `Visibility Readback`) — the async `mapAsync` readback is issued from the mid-frame dispatch BEFORE the frame's submit, and the HiZ staging buffer isn't double-buffered. Scene renders (the captured frame is clean) but the gate still flags these. Next fix: defer the readback mapAsync to post-submit and/or ring-buffer the staging targets. Interim safe state still available: `gpuCullingHint='never'`. probe-hiz-occlusion-consumer.mjs / -control.mjs. (BACKLOG-§7)
