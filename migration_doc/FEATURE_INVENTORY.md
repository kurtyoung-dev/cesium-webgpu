...
- WebGPUCSMRenderer — CSM Slice 1+2a; RTE-precise per-cascade VPs + slope-scaled bias; soft-shadow 3x3 PCF box kernel (Batch 289, `softShadows`/`pcfRadius` config + `effects.csmControl.y` → `sampleOneCascade` in all 25 receivers, matches WebGL `czm_shadowVisibility` USE_SOFT_SHADOWS; code shipped, visual-verify blocked on cast no-dispatch — see NEW-CSM-SOFT-SHADOW-PCF / NEW-CSM-CAST-NO-DISPATCH-VIEWER) (SHIPPED code; verify-blocked)
