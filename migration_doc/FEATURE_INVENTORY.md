...
- WebGPUPointPrimitiveRenderer — PointPrimitive collection (SHIPPED in 3D as of Batch 219 — bug-1 BV + bug-3 clip-z fix; bug 2 far-surface depth resolved via log-depth. Phase 3 Slices 2-3 (2026-06-13): projects + morph-blends in 2D/CV. Slice 2b (2026-06-13, NEW-COLLECTIONS-2DCV-COPLANAR-DEPTH): `noDepthTest` pipeline variant → COLUMBUS_VIEW coplanar point now renders (0 → 231 px). SCENE2D coplanar points still 0 px — same globe-pass multi-frustum overwrite as billboard, fix belongs in Globe & Imagery (MORPH-COLLECTIONS-AUDIT Slice 2b))
...
- WebGPUEllipsoidPrimitiveRenderer / WebGPUEllipsoidRenderer — analytic ellipsoid primitive. Log-depth plumbing added B266 (NEW-ELLIPSOIDPRIM-LOG-DEPTH; verified structurally). KNOWN BUG: a scene-placed EllipsoidPrimitive renders no visible pixels on WebGPU (`center` dropped — renderer reads `primitive.modelMatrix` not the Scene's `_computedModelMatrix`; + ray-cast quad emits no coverage even when placed via modelMatrix), tracked BUG-ELLIPSOIDPRIM-WEBGPU-INVISIBLE (§C). (SHIPPED-but-invisible)
