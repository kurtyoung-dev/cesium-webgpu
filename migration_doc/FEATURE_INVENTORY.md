...
- WebGPUGroundPrimitiveRenderer — depth-sample classifier (post-ADR-2026-04-28) (SHIPPED)
- WebGPUGroundPolylineRenderer — terrain-clamped polyline classifier; Arrow/Stripe/Image materials (SHIPPED)
- WebGPUVector3DTilePrimitiveRenderer — extruded polygon classification (SHIPPED)
- WebGPUVector3DTilePolylinesRenderer — non-clamped 3D polyline classification (SHIPPED)
- WebGPUVector3DTileClampedPolylinesRenderer — terrain-clamped polyline classifier on Vector3DTiles (SHIPPED)
...
- WebGPUVoxelRenderer — VoxelPrimitive ray-march (SHIPPED)
...
- WebGPUVolumetricFogRenderer — froxel-grid volumetric fog (Phase 5) (SCAFFOLDED)
- WebGPUProceduralCloudRenderer — procedural cloud-noise sampling effect (SCAFFOLDED)
...
- VolumetricFog.wgsl — froxel-grid density injection + scattering integration (SCAFFOLDED)
...
- VolumetricFogComposite.wgsl — composites froxel-grid result into scene color (SCAFFOLDED)
...
- C-R9-MODEL-FEATURE-PICK code wired but un-testable until b3dm-Model rendering path lands (blocked on NEW-BG-CONSOLIDATION) (C-R9-MODEL-FEATURE-PICK)
...
- KHR_lights_punctual not wired in WebGPU model path; shader hardcodes 1 sun + ambient (Phase-8 §2)
...
- Volumetric fog scaffolded but `enableVolumetricFog` defaults FALSE; 5a-5d froxel infrastructure landing in phases (CELESTIAL §4.8)
- Cloud shadows in fog (Phase 5e) bundled into volumetric clouds Phase 6c — not yet shipped (CELESTIAL §4.6/§4.8)
...
- FEAT-SURVEY-40 clustered forward lighting (depends on KHR_lights_punctual) (FEAT-SURVEY-40)
