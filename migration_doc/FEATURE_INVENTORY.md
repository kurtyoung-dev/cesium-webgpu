...
**Last full-catalog refresh:** 2026-05-07

**Incremental feature updates:** through launch anchor `a54cc06` (2026-07-11)
**Phase-0 note (2026-07-13):** individual batches kept many entries current, but the four-bucket catalog has not received a complete post-Campaign-7 reconciliation. Use [Current Rendering Functionality Baseline](CURRENT_RENDERING_FUNCTIONALITY_BASELINE_2026-07-13.md) for the pre-fix visual/semantic anchor and FAR-000/FAR-002 for the full status/provenance reconciliation; do not interpret this header as proof that every status tag is current.
**Cloud reconciliation (2026-07-24):** [Campaign 13](QUEUE_2026-07-23_CAMPAIGN13.md) is the live
execution authority for volumetric-cloud correctness, architecture, quality, and performance. Its
...
certification.
**Celestial S5/S6 certification overlay (2026-07-28):** this paragraph
supersedes older counts and interim architecture language in the long
historical rows below as of that certification date. Focused executable gates were S6 **51/51**, S5 RTE
**18/18**, S5 visual-source **4/4**, and dynamic-environment recovery **7/7**;
the protected Node set is **145/145**, the core C12-29 set is **134/134**, and
the performance contract is **23/23**. The focused Edge/Karma
dynamic-environment manager gate is **11/11**. The final scene-dimming run is
PASS on both backends with a terrain-responsive pixel control and zero console
errors.

Current S5 architecture is logical-`View`-owned and camera-independent: CPU f64
publishes a Sun common ray plus Moon-minus-Sun differential. Scene prepares
S1/S2/S6 and clears the S5 alias/memo; retained scene capture, main globe, and
pick each prepare S5 once against their exact owned terrain selection before
commands.
Selected-terrain coverage is two-stage: a provider-wide O(1) conservative
envelope rejects ordinary frames, while a globally possible shadow alone pays
the selected, skirt-inclusive rendered-mesh sphere scan. Custom-ellipsoid sky
orientation uses geodetic up. The `StarField` altitude bound follows the active
ellipsoid's `maximumRadius` but remains scalar/radial, not a fully triaxial
horizon model.

The 2026-07-28 exact-owner correction removes one duplicate O(1) broad test for
each rendered-globe logical `View`/frame and the repeated limb fit on active
intersecting frames; it is a bounded eclipse-path CPU cleanup, not an FPS
claim. The extended browser probe
passes on both renderers: one fixed-camera outside target records 81/81 globe
rays, exactly 36 stable skirted meshes, gate 3, zero body inverse ranges, a
non-vacuous S2-only control, and correction exact on WebGL/within one code value
on WebGPU. The first inside frame is gate 2 with 25 meshes; the reverse first
frame conservatively exposes two root fallbacks at gate 2 before settling back
to the exact 36-mesh gate-3 state. WebGPU correction and local lanes each cost
one allocation over gate 0 independent of tile count. Artifact:
`Tools/visual-regression/output/eclipse-globe-shadow-report.json`.
The repeated moving route proves current renderer parity, not an isolated S5
performance delta.

**2026-07-31 working-tree audit overlay (landing reconciled 2026-08-01 — the
changeset described here landed as Batches 772-781, `origin/main` =
`3900608bb9`; every open gate below is unchanged):** local `main` and
`origin/main` were equal at Batch 771 (`fe990ab335`) when this overlay was
written; the index was empty and the active work was uncommitted. The pure
eclipse/globe/atmosphere Node set passes 138/138,
but focused Edge/Karma did not execute a test before timeout, so the older
Edge counts above remain historical evidence rather than a current
claim. `C11-20` is PARTIAL: normal Point/Label native-cache teardown is fixed
and landed (Batch 778),
while nested model/tileset/clipping and replacement-device invalidation remain
open. `C11-181` is implemented and verified and landed (Batch 773); it is still
not complete. A valid moving
globe-only performance control is near parity; representative model/tileset
measurement remains open after correcting non-vacuous sampling and resident
camera-cleanup defects in the harness.

Correction-only gates 3/4 remain active carriers: one memoized 64-byte View
slice on WebGPU and the active bit-33 shader plus packed `mat4` on WebGL. Body
inverse ranges are zero and local common-ray/horizon/overlap/limb-fit ALU is
skipped; carrier cost cannot grow per tile/pass.

Dynamic-environment recovery now reports explicit `FAILED`, `SKY_ONLY`,
`SUBMITTED`, and `PARTIAL` outcomes. Its terrain/imagery content epoch catches
stable-provider changes; zero-draw replay records no success and allocates no
capture depth/encoder; repeated per-tile publication calls use an exact
same-frame identity/content fast return. These results do **not** constitute
replacement-device browser/device-loss certification, which remains open.
...
- MapMode2D / SceneMode / SceneTransitioner — 2D/Columbus/3D modes + transitions. WebGPU morph (transition) animation audited 2026-06-07 (webgpu-morph-review): globe terrain morph fixed at WebGL parity (Batch 216 — see §B-resolved); remaining gaps tracked in `DEFERRED_WORK.md` "Scene-mode morph pillar" (exaggeration-in-morph/CV skirt walls, PolylineCollection 2D/CV, billboard/label/point morph audit, TAA previousViewProjection at the mode flip, mix-jitter, WebMercator instanceof, model projectTo2D). glTF Model BASE 2D/CV placement shipped 2026-07-02 (MODEL-SCENE-MODES — mode-aware `_computedModelMatrix2D` + `_boundingSphere2D` pick in `WebGPUModelRenderer.js`; `probe-model-scene-modes.mjs`); `projectTo2D:true` accurate-2D remains open (MORPH-MODEL-PROJECT2D).
...
- WaterClassificationProvider seam (Scene/WaterClassificationProvider.ts; C7-LAKE-WATER-MASK, WATER-epic Phase 1) — renderer-agnostic interface for augmenting the terrain provider's per-tile water mask with supplementary client-side water datasets, applied on the CPU mask BYTES at the single shared upload point (`GlobeSurfaceTile.js` → `createWaterMaskTextureIfNeeded`, unified for both backends in Batch 512) so WebGL and WebGPU consume identical texels by construction. Phase-1 concrete impl `LakeWaterClassificationProvider` rasterizes Natural Earth 1:10m lake polygons (PUBLIC DOMAIN — global + North America supplement, preprocessed by `Tools/build-lake-water-mask.mjs` into the bundled ~900 KiB packed-polygon `Assets/WaterMask/ne10mLakes.bin`; uint16-quantized rings against per-polygon f64 bboxes, scanline even-odd fill at texel centers, row 0 = north matching the provider-mask convention) and OR-composites them over the ocean-only Cesium World Terrain mask, expanding the 1-byte all-land fast path to 256×256 only when a lake actually rasterizes into the tile. Opt-in via `globe.lakeWaterMask` — DEFAULT OFF, byte-identical off; the dataset is fetched lazily on first enable and toggling invalidates tiles. Known dataset limits: Caspian classed as ocean by Natural Earth (absent); ImageBitmap-shaped masks pass through un-augmented. The B631 screen-space coastline AA applies to lake shorelines automatically (same mask isoline). v2 upgrade path: ~30m public-domain SRTMSWBD raster (research lane R-LAKE-SRTMSWBD). Probe: `probe-lake-water-mask.mjs`. (SHIPPED — opt-in; the default-flip is a user decision pending visual review)
- Ocean vertical-datum stack (Core/VerticalDatum.js + Core/GeoidUndulationGrid.js; C6-FFT-OCEAN-TIDE-DATUM slice 1, ruling T2) — backend-agnostic sea-level datum for rendered water. `VerticalDatum` is an `AUTO | ELLIPSOID | GEOID` enum plus the terrain-provider derivation (`EllipsoidTerrainProvider` ⇒ ELLIPSOID structurally, everything else ⇒ GEOID; `instanceof`, never `constructor.name`, because release builds minify class names). `GeoidUndulationGrid` parses and bilinearly samples a bundled coarse EGM2008 undulation grid — `Assets/Geoid/egm2008-0p5deg.i16`, 0.5°, 721×361 node-registered with both longitude seams, int16 centimetres, 520,594 B / 508.4 KiB, lazily fetched only when a geoid datum is actually in force, baked reproducibly by `Tools/build-geoid-undulation-grid.mjs` from the NGA 2.5′ public-domain grid (LICENSE.md "Bundled Engine Assets"). Measured budget against all 37.3 M source nodes: global RMS 0.195 m / max 6.64 m; open water RMS 0.110 m / max 3.60 m. Exposed as `scene.globe.water.oceanVerticalDatum` (default `AUTO`). **This is default-ON correctness work, not a feature toggle** — `OceanSurfacePrimitive` anchored at ellipsoidal 0 over orthometric terrain and floated a measured 101.64 m above Cesium World Terrain's baked sea at the Sri Lanka coast (`probe-ocean-datum.mjs`, Batch 759). (SHIPPED — Batch 763)
- Equilibrium TideModel (Core/TideModel.js; C6-FFT-OCEAN-TIDE-DATUM slice 1, rulings T1/T3/T6) — degree-2 equilibrium ocean tide as `height(time, positionWC)`, pure f64 JS, allocation-free per frame, computed from the in-engine Simon-1994 Moon and Sun rotated to ECEF with a branch-keyed ICRF/TEME memo (the `EclipseState` lesson: a memo built during the XYS-loading window must not freeze under a pinned clock). Uses the tide-gauge form `γ₂ = 1 + k₂ − h₂ = 0.6944` for the TIME-VARYING part (the terrain mesh carries no time-varying solid-Earth tide and is therefore the unmoving crust), and splits the permanent (zero-frequency) part off at `(1 + k₂)` instead — the real crust already carries its permanent deformation while the tide-free EGM2008 geoid has had `(1+k₂)·ζ̄` removed, so the net is an extra `h₂·ζ̄` (+0.060 m equator, −0.120 m poles). `ζ̄` is closed-form in latitude via the addition theorem and evaluates to the classic −0.198 m coefficient at 6371 km. Constants cited in-source (JPL DE421 lunar GM / DE430 solar GM, CGPM standard gravity, IERS 2010 Love numbers, NOAA CO-OPS M2 speed, IAU 2012 au). Wired into the FFT ocean anchor as `h = N + tide × tideExaggeration` then the `scene.verticalExaggeration` map; `scene.globe.water.{tideEnabled, tideExaggeration, tideCallback}` (defaults `true`, `1.0` = true scale per T3, `undefined` = use the engine model — this supersedes WATER_RENDERING_DESIGN OQ5's "null → 0"). **Stylised, not a prediction**: ±0.3 m with zero phase lag and no basin amplification. Acceptance `probe-ocean-tide-datum.mjs` + `ocean-tide-datum.spec.mjs` verify phase-lock to lunar culmination (≤5 min), the mean period against the published NOAA M2 constituent, and the spring/neap envelope against Sun–Moon elongation. (SHIPPED — Batch 763)
- Harmonic tide stack (Core/TidalArguments.js + Core/TidalConstituents.js + Core/HarmonicTideModel.js + Core/TideConstituentGrid.js; C6-FFT-OCEAN-TIDE-DATUM slice 2, rulings T5/T6) — the constituent-based predictor that EOT20 (default) and NOAA CO-OPS both feed, plus the astronomical machinery ruling T6 requires. `TidalArguments` computes the six Doodson fundamental arguments (τ, s, h, p, N′, ps) with their rates from the Meeus mean-element polynomials (ch. 25/47, cited in-source) and bridges time scales explicitly: `JulianDate` stores TAI, so TT = TAI + 32.184 s feeds the five mean longitudes while τ uses UTC via `JulianDate.computeTaiMinusUtc`, with UT1 ≈ UTC (DUT1 = 0, matching this engine's default `EarthOrientationParameters.NONE`) and an optional DUT1 argument for callers holding real EOP. **`JulianDate` is NOON-origin, so τ carries a +180° term** — dropping it leaves every semidiurnal constituent invariant (2 × 180 = 360) while silently INVERTING every diurnal one; measured correlation against the geometric tide falls from 0.998 to −0.375 at 43 °N while staying bit-identical at the equator, which is why the negative control asserts at a non-equatorial site. `TidalConstituents` carries ten constituents (M2 S2 N2 K2 K1 O1 P1 Q1 Mf Mm) with Doodson numbers, speeds DERIVED from the fundamental rates (matching NOAA CO-OPS to ≤ 6.0e-8 °/h), Cartwright–Tayler/Edden equilibrium amplitudes, phase offsets re-derived from the product-to-sum expansion, species latitude factors from the P₂(cos ψ) addition theorem, and Schureman nodal f/u in the node longitude Ω (M2 band 0.9633–1.0379 vs the published 0.963–1.038). `TideConstituentGrid` is the baked-atlas loader seam ("TCG1", Re/Im int16 so bilinear filtering never crosses a phase wrap, −32768 no-data sentinel unreachable as data, duplicated seam column bit-identical so there is no antimeridian tide line). `Tools/build-eot20-constituent-grid.mjs` bakes it from EOT20 (CC BY 4.0, DOI 10.17882/79489) with a self-contained NetCDF-3 reader, dimension-order resolution from the variable's own dimension names (and a REFUSAL rather than a guess when names are unresolvable on a square grid), `scale_factor`/`add_offset` applied with `_FillValue` tested against the raw stored value before unpacking, a mandatory `--imag-sign` for (real, imag) archives cross-checked against any phase variable (a wrong choice time-reverses every tide and looks correct), and a pre-write sanity gate (ocean-coverage bounds, M2 amplitude bounds that catch unit/scale errors, phase circular-resultant, and a node round-trip through the SHIPPED reader sampled by longitude/latitude rather than index). The geometric `TideModel` remains the default and is strictly more accurate — it carries the full equation of centre, evection, variation and parallax that a ten-constituent development truncates (residual ~7% of signal RMS at low/mid latitude) — so the harmonic form exists to give the atlas seam an answer and to cross-check the two. **The EOT20 asset is NOT bundled**; the engine falls back to `TideModel` until a bake lands (tracked in DEFERRED_WORK). Gates: `tidal-harmonics.spec.mjs` 34/34 including known-answer J2000 mean elements, two-bulge moon alignment to 0.000° with 180.000° antipodal separation, a 50.46–50.47 min/day lunar lag from a 120-day least-squares fit over 232 high waters (published 50.472), spring/neap with moon phases derived from the ephemeris, the 18.6-yr nodal cycle, and negative controls proven by re-injecting each defect. (SHIPPED — Batch 767)
...
- WebGPURenderPipelineCache — central LRU cache of GPU render pipelines (SHIPPED). **Key upgraded Batch 825 (`NEW-WEBGPU-PIPELINE-KEY-DEFINE-AXIS-GENERAL`, Edge-verified Batch 828):** `generateCacheKey` folds shader-MODULE IDENTITY (`sh:<vsId>.<vsEntry>/<fsId>.<fsEntry>` via a `WeakMap`-backed `webgpuObjectIdentity`) plus `pl:` layout, `pr:` primitive, `dz:` depth/stencil and `mx:` multisample — i.e. every field `buildPipelineDescriptor` forwards to `createRenderPipeline`. Before that it hashed `descriptor.name` + a few structural fields only, so a define that changed neither the name nor the vertex layout aliased silently; the marker fleet (`, ld=1`, `, noCull`, `, imagery4`, `defines=0x…`) is retained as readable provenance/defense-in-depth, not as the correctness mechanism. `stats.wrongModuleHits` stays as the runtime canary and must read 0. `WebGPUComputePipelineCache` folds `compute.module` the same way (`m:`).
...
- WebGPUSharedImageryRealizations + ImagerySourceIdentity — per-renderer (= per-pooled-GPUDevice) dedup of imagery `GPUTexture` realizations by immutable-source identity (`ImageBitmap` or provider-declared canvas; NO pixel hashing), refcounted with quadtree/imagery lifetime; never-shared realizations retire promptly at zero refs, ever-shared ones enter a HARD byte-budgeted zero-ref pool (over-budget sweeps retire oldest-first regardless of age); the table keeps its own sweep clock and destruction always routes through a call-time live-context `scheduleTextureDestroy`. Imagery mip generation moved out of draw emission into one frame-owned `"ImageryMipPreparation"` encoder submitted immediately before the frame encoder's own submit (`WebGPUContext.enqueueImageryMipGeneration` → `endFrame`; mid-frame private submits that sample imagery call `flushPendingImageryMipJobs`). Imagery cache keys are layer-scoped (`L{layerId}_x_y_level`), closing the cross-layer x/y/level collision. Collapses repeated identical GridImagery realizations to ONE per layer (measured 2 layers → 2 realizations + 62 shares; C9-01 baseline 513 identical realizations + 513 private submits) and closes the direct-upload zero-retirement leak. Output byte-identical (ownership/lifetime only; `probe-terrain-selection-parity` exact hash match) (SHIPPED — C9-12A-IMAGERY-SOURCE-REALIZATION-DEDUP-AND-MIP-PREP, Batches 685-686 incl. 16-agent adversarial-review hardening)
...
- WebGPUModelRenderer — full glTF Model render path with PBR + KHR extensions (SHIPPED; **C10-05 (2026-07-18) landed trilinear material-texture mip sampling** — `ModelPBRComplete.wgsl` was mip-0-locked (`textureSampleLevel(...,0.0)` at every material site because non-uniform discards forbid implicit-derivative sampling), so distant models aliased/shimmered while WebGL trilinear-filtered. Now hoists `dpdx/dpdy` of all material UV sets at `fragmentMain` (+ the 4 pick/velocity alt-pass alpha-test entries) in uniform control flow and samples via `textureSampleGrad` (36 material sites; data/screen-space/IBL-explicit-LOD samples kept at LOD 0). The mip CHAINS already existed on the live stub path (`WebGLStubTexture` allocates+generates per the glTF sampler, mirroring WebGL); only the shader needed to consume them. `createGPUTextureFromReader`'s secondary fallback also upgraded to a real chain via the frame-owned `enqueueImageryMipGeneration` (C9-12A, uncompressed-only). Near-field magnified byte-identical; distant HF-aliasing energy drops to WebGL trilinear parity — `probe-model-mip-shimmer`)
...
- WebGPUPolylineRenderer — Polyline collection with arrow/dash/glow/outline materials (SHIPPED — 3D + 2D + Columbus View + Morph. Phase 3 Slice 4 / Batch 3 landed the scene-mode path 2026-06-13: segment + pick builders encode `SceneTransforms.computeActualEllipsoidPosition` per endpoint (CPU morph lerp), full-frustum log-depth encode, and a `noDepthTest` pipeline variant for the settled 2D/CV co-planar-with-map case — WGSL byte-identical, SCENE3D unchanged. Verified `probe-collections-2dcv-morph.mjs` polyline 2D 0.96 / CV 0.89 / 3D 0.96 vs WebGL. See DEFERRED_WORK MORPH-POLYLINE-COLLECTION-2D for the deferred IDL-split + mid-morph-velocity follow-ups. Current velocity/material residuals (2026-07-14): base Color passes `"Color"` to a velocity guard that checks `"polylineColor"`, so TAA velocity is skipped; Arrow/Dash/Glow/Outline velocity entry points are absent; Image/DiffuseMap retain their public routing but fall back to solid color. These are tracked correctness/completeness follow-ups, not feature removals.)
...
- WebGPUVoxelRenderer — VoxelPrimitive ray-march. NOTE (triage Batch 172, updated 2026-07-02): originally a PLACEHOLDER gradient ray-marcher; Batches 474-497 landed the real ROOT-TILE data path (megatexture-root upload + shape/OBB proxy placement + default-shader gray color parity + the WebGL world→shapeUv sample convention), and VOXEL-CELL-PICK-RELAND landed per-cell pick coordinate parity (`pickVoxelCoordinate` byte-identical to WebGL; gate `probe-voxel-cell-pick.mjs`); VOXEL-OCTREE-LOD (2026-07-02) landed depth-1 octree LOD (9-slot Z-stacked atlas: root + eight async level-1 children w/ per-octant root fallback; per-sample child selection in the parity march; CPU SSE refine test mirroring SpatialNode.computeScreenSpaceError; gate `probe-voxel-octree.mjs`). VOXEL-USER-CUSTOMSHADER (2026-07-02) landed USER native-WGSL customShaders on the ray-march (`WebGPUVoxelCustomShaderCodegen.ts` generated metadata chunk + `VOXEL_USER_CUSTOM_SHADER` bit-29 nested define; GLSL-only + uniform-carrying customShaders warn + default-gray — see DEFERRED_WORK VOXEL-USER-CUSTOMSHADER-RESIDUALS; gate `probe-voxel-user-customshader.mjs`). NEW-VOXEL-OCTREE-DEEP-TRAVERSAL (Batch 521) extended the walk to depth 2 (73-slot atlas, iterative `octreeDescend`); NEW-VOXEL-STREAMING-UPLOAD (Batch 522) made descendant upload demand-driven (SSE-ladder `demandLevel`, far camera = root-only atlas); NEW-VOXEL-ATLAS-LRU-EVICT (2026-07-03) landed the dynamic megatexture slot allocator — when the 73-slot set exceeds capacity (device limit or opt-in `_webgpuVoxelAtlasMaxSlots`), the level-2 region becomes an LRU pool driven by a per-tile SSE+frustum demand mask (`computeVoxelL2DemandMask`), evicted tiles re-request under future demand; under-capacity byte-identical static path (gate `probe-voxel-megatexture.mjs` PARTs 1-3). Still open: octree traversal DEEPER than level 2 (per-level page table through the LRU pool — DEFERRED_WORK PARITY-VOXEL-OCTREE-TRAVERSAL item 1), non-box shapeUv mapping, customShader uniforms/GLSL transpile, camera-inside-volume black frame (NEW-VOXEL-INSIDE-CAMERA-BLACK), and the `scene.pickVoxel` VoxelCell tail (C-R9-VOXEL-CELL-PICK-TAIL — `VoxelPrimitive.update` returns early so `_traversal` is never built). **The upstream PR#13517 default voxel shader (default-shader for common metadata types) is UNREACHABLE on WebGPU** — because `VoxelPrimitive.update()` short-circuits when the VOXEL_PRIMITIVE FR is registered it never reaches the `processVoxelProperties` / `buildVoxelDrawCommands` / `buildVoxelCustomShader` path, the renderer is a hardcoded RGB-density ray-marcher on a 4×4×4 gradient placeholder texture, and there is no WGSL transpilation of the CustomShader GLSL. The PR#13517 default shader is one piece of a feature whose entire WebGPU data path is unimplemented (XL scaffold; tracked §C.5 C-R9-VOXEL-CELL-PICK). (SCAFFOLDED — not real voxel data; PR#13517 default-shader / CustomShader / metadata-typed path unreachable)
- **2026-07-31 voxel status correction (supersedes the contradictory tail of the preceding historical paragraph):** the fork has a real root-tile through depth-2 streamed/LRU WebGPU data path; it is not a 4×4×4 placeholder and the entire feature is not absent. The upstream PR#13517 metadata-typed default-shader path and GLSL custom-shader transpilation are still unreachable because the feature renderer deliberately bypasses the upstream WebGL command-building path. Correct status: **WIP / PARTIAL** — real fork-native data, LOD, pick-coordinate, and native-WGSL paths shipped; deeper traversal, non-box mapping, metadata-typed default shading, GLSL/uniform custom shaders, camera-inside rendering, and public `VoxelCell` completion remain open.
- WebGPUGaussianSplatRenderer — Gaussian splat rendering (3DGS); back-to-front depth sort consumed via a sorted-index storage buffer (the VS reads `splats[sortedIndices[ii]]`) + logarithmic frag_depth producer (Batch 288, NEW-SPLAT-SORT-CONSUME-INDEXES / NEW-LOG-DEPTH-REMAINING-PRODUCERS-POINTCLOUD-SPLAT splat half). WIP boundary: single-pack command occluded by opaque geometry in multi-frustum scenes — NEW-SPLAT-MULTIFRUSTUM-DEPTH-COMPOSE. **PRODUCTION-PRODUCER GAP (C10-04 STOP-AND-BLOCK, 2026-07-18):** the renderer has no production data feed — `GaussianSplatPrimitive.update` returns to the WebGPU FR before the WebGL data-commit runs, and nothing assigns `primitive._splatData`/`_renderResources.splatBuffer` in-tree, so the path draws only under `probe-splat-sort.mjs` (synthetic `_splatData`). Tracked NEW-WEBGPU-SPLAT-DATA-PRODUCER (DEFERRED_WORK); this renderer is effectively SCAFFOLDED-not-SHIPPED for production until that producer lands. (SHIPPED as a rendering path — probe-verified; production producer DEFERRED)
...
- WebGPUDynamicEnvironmentMapManager — per-position dynamic env map probes (SHIPPED; Batch 354 added the atmosphere-derived diffuse-IBL **SH-L2 producer** — a `ProjectRadianceToSH.wgsl` compute pass projecting the radiance cube to 9 SH coefficients into a STORAGE|UNIFORM buffer published as `_webgpuSHBuffer`, closing NEW-WEBGPU-KHR-SPECULAR-IBL-OVERBRIGHT: model diffuse IBL now matches WebGL's `czm_sphericalHarmonics` path, 56%→1.3% probe parity). **Batch 446 (C2-25 `ENV-SCENE-CAPTURE` globe V1)** — opt-in `contextOptions.webgpu.sceneCaptureReflections` + `DynamicEnvironmentMapManager.enableSceneCapture` (default off, byte-identical-off): renders the globe surface from 6 ENU face cameras (`uniformState.updateCamera`) into the env-cube faceViews over the compute sky, feeding the existing mip→prefilter→SH→model tail (`WebGPUDynamicEnvironmentMapCapture.ts` + `CAPTURE_MODE`=1<<17 single-target globe pipeline variant). Nadir hemisphere captures textured terrain, E/W verified vs ground truth. **Batch 447** extends capture to MODEL geometry (glTF + 3D Tiles, both via the shared `WebGPUModelRenderer` producer): a parity-neutral `packCameraUniforms` eye-swap unifies the model camera with the globe, a `CAPTURE_MODE` single-target `ModelPBRComplete` variant + separate model `_capturePipelines`, one-pass-per-face globe-then-models depth compositing, ring-allocator capture camera UB, neutral IBL, opaque-only (BLEND skipped). **C2-25 reflections epic COMPLETE (Batches 446–451):** 3-A scene capture (globe 446 + model/3D-Tiles 447 + tileset verify/demo 448), 3-B `ENV-TEMPORAL` history-cube accumulation (449), 3-C `ENV-CLOUDS`/CLOUD-IBL-FULL per-face cloud march (450), 3-D `ENV-PARALLAX` Lagarde box/sphere proxy (451) — all opt-in, default-off byte-identical, adversarial-audit-clean. WIP/deferred: side-face outward terrain (`ENV-CAPTURE-PER-FACE-LOD`, per-face quadtree re-selection — a tile-subsystem epic; realistic ground-level case already covered).
...
- LTC analytic area lights (C6-LTC-AREA-LIGHTS) — WebGPU-only, opt-in/default-off `RectAreaLight` / `DiskAreaLight` (`Scene/LightTypes.ts`, `LightType.RECT_AREA=3` / `DISK_AREA=4`) shaded with Linearly Transformed Cosines (Heitz et al., SIGGRAPH 2016 — permissive BSD-style, see LICENSE.md). Two fitted 64×64 LUTs embedded fp16 in **`Renderer/WebGPU/WebGPULTCLUTData.ts`** (base64, uploaded once as a 64×64×2 `rgba16float` array texture); the edge-integral + horizon-clip + disk cubic-solve WGSL lives in **`Shaders/WebGPU/chunks/structs/ClusteredLighting.wgsl`** (`ltcEvaluateRect` / `ltcEvaluateDisk` / `evalLTCAreaLights`). Rides beside the clustered punctual path: a parallel 96 B `LTCAreaLight` storage buffer (`WebGPUClusteredLightingDispatcher`, MAX 8, eye-space packed, no clustering v1), count carried in the previously-unused `ClusteredParams.activeLightCount.y`, group-3 effects bindings 23 (LUT) + 25 (areaLights) — **no dedicated sampler** (Model PBR is at the 16-sampler stage limit, so the LUT is read via `textureLoad` + manual bilinear). Consumed by ModelPBRComplete. Analytic soft falloff, no shadow map, no distance attenuation (intensity = emitter radiance). Byte-identical when zero area lights exist (FS gate `activeLightCount.y < 0.5`). `probe-ltc-area-light.mjs` PASS (rect light lights a glTF model +240 mean brightness / 56k px, off-gate 0-pixel-diff, 0 device errors). (SHIPPED — rect+disk v1. Remaining: primitive Mat*Lit + Phong call sites, line lights, textured emitters, area-light clustering, WebGL backend — all in DEFERRED_WORK under C6-LTC-AREA-LIGHTS.)
...
- WebGPUWeatherRenderer — weather particle system orchestrator: GPU-compute rain/snow/fog/hail simulation + instanced camera-relative quad render (SHIPPED, default off via `scene.enableWeather` / `atmosphericConditions.weather.enabled`). **Batch 423 (Atmospheric Phase E wiring)** closed the gap that left it effectively dormant: the env-effects dispatch now builds the renderer's `CesiumWeatherConfig` from the flat `scene.weather*` fields (it had been passing `scene` raw → the `enabled` gate read a nonexistent `scene.enabled` and returned early, so no particles ever rendered), the render half resumes the default canvas pass (env effects run AFTER post-process, Batch 127, which ends the active pass) and the render pipeline now declares a single canvas-format alpha target instead of the 2-target scene-FB MRT layout (the phantom rgba16float slot-1 made it incompatible with the 1-attachment canvas pass). Driven by both the manual `atmosphericConditions.weather` facade and the 417a `effects.precipitation` auto-master (`PrecipitationType` enum + `precipitationTypeToString` in `AtmosphericEffects.ts`). Probe `probe-precip-wiring.mjs`. Data-driven WMO weather-code selection from ingest + ground accumulation deferred.
...
- WebGPUProceduralCloudRenderer — Schneider-style volumetric raymarched clouds (HG dual-lobe phase + Beer-Powder lighting + 3D FBM density + light-ray marching). Phase 6 main render path. (SHIPPED, default off — **driven THROUGH `CloudCollection`** as of the cloud-unification epic (Batches 617–625): set `collection.renderMode = CloudRenderMode.VOLUMETRIC` (or the `enableVolumetric` alias) and configure `collection.volumetric` ({@link CloudVolumetrics}) + `collection.cloudType`. `globe.defaultCloudCollection` is the managed default the `AtmosphericConditions.clouds.*` facade proxies onto; the legacy `globe.showProceduralClouds` / `globe.cloud*` surface was REMOVED in slice 4B. See migration_doc/CLOUD_UNIFICATION_DESIGN.md.) **Campaign-3 Arc-A lighting fidelity (Batches 391-394, `CloudUniforms` 80→96 floats):** W1 (391) uniform-driven dual-lobe HG phase + the keystone **Reinhard HDR tone-map** (`CLOUD_EXPOSURE`) at the composite — the accumulated radiance was clipping every cloud to flat white, hiding all subsequent lighting; tone-mapping restored form + silver lining and unblocked the rest of the arc. W2 (392) per-step sky-ambient gradient + ground bounce (shadow side lifts off black). W3 (393) time-of-day sun color keyed on LOCAL sun elevation (warm dawn/dusk → neutral noon). W4 (394) **aerial-perspective blend** — distant clouds tint toward a time-of-day horizon-haze color (`aerialColor`) by march-midpoint distance (60 km scale), dialable via `globe.cloudAerialStrength` (default 1.0). Probes `probe-cloud-{phase,ambient,tod,aerial}.mjs` (W4 verified distance-grading via a two-layer-altitude A/B: same camera, deck moved 1.5 km→28 km away raises whole-frame haze 0.41→0.57). **Campaign-3 Arc-B perf (Batch 395):** W5 **adaptive coarse→fine raymarch** (empty-space skipping) — a cheap, smooth, CONSERVATIVE low-detail density (`cloudBaseDensity`, no Worley/detail; base ≥ full so it never skips real cloud and never false-snaps in an erosion pocket) drives a coarse skip through empty space, refining to the fixed-march cadence only inside the cloud SHAPE. `tProcessed` floors the edge back-up so the march is monotonic (no stall). Fine grid stays aligned to the old fixed grid, so the image is preserved BY CONSTRUCTION. Verified `probe-cloud-perf.mjs` (true A/B vs the pre-W5 fixed build: **0.00% luma mismatch**, exact 1.000 cloud-cell ratio, **×1.39 faster** in a sparse-sky scene; speedup scales with empty-sky fraction, image identical regardless).
- Per-genus cloud MORPHOLOGY (`C13-16`, cirrus slice) — `Scene/CloudTypeProfile.js` carried five axes
  per WMO genus and two reached no renderer: the FIBROUS/PUFFY `erosion` style and the
  Henyey-Greenstein `phaseG`. A genus therefore differed only in deck, height-gradient shape, density
  scale, and extinction, so CIRRUS rendered as a faint scaled-down CUMULUS (the C13-01 tour's
  `northatlantic-cirrus-fibratus` note). Both axes now reach the shader through one add-only
  `CloudUniforms` row (168-171): `CloudTypeProfile.FIBRE_MORPHOLOGY` drives `genusFibreFactor`
  (a wind-aligned, fallstreak-sheared, anisotropic Worley carve — the fourth link in the existing
  mammatus/species/feature `[0,1]` factor chain, applied identically in the full density and the W5
  `cloudBaseDensity` oracle) plus `genusErosionHeightWeight` (ice erodes uniformly through the deck
  where water erodes base-weighted), and `profile.phaseG - CUMULUS.phaseG` offsets the HG forward
  lobe. Default byte-identical by explicit early return: the default genus is CUMULUS, PUFFY, with a
  zero phase delta. **No new public dial — `cloudType` remains the single selector.** Guarded by
  `Tools/visual-regression/cloud-genus-morphology.spec.mjs` (21 tests; structural correlation-length
...
- WebGPUProceduralCloudRenderer current Campaign-13 reconciliation — `CloudUniforms` is now **172
  ...
  rules. The current path is still **color-only and half-resolution**, and it shades every current
  texel. `C13-09` (front/weighted depth, velocity, moments), `C13-10` (true 1/16-rate current work plus
  full-resolution reconstruction), and `C13-12` (advanced wind/depth/disocclusion/variance/reactive
  rejection) remain WIP. `C13-06` still owns RTE correction/certification for cloud-shadow, mask,
  environment-capture, and atmosphere consumers. `C13-38` suppresses revision-driven environment
  cube/prefilter/SH refreshes only when full cloud IBL is irrelevant, while preserving active,
  deferred-opt-in, and teardown behavior; this is a removed irrelevant-work path, **not a measured-FPS
  claim**. ([Campaign 13](QUEUE_2026-07-23_CAMPAIGN13.md))
- Cloud-unification API (`CloudRenderMode` enum + `CloudVolumetrics` config carrier + `CloudCollection.renderMode`/`.volumetric`/`.cloudType`/`.enableVolumetric` + `CumulusCloud.cloudType` dirty-index 7 + backend-neutral `context.requestVolumetricClouds`/`consumeVolumetricCloudRequest`) — drives the WebGPU volumetric raymarch deck THROUGH the upstream `CloudCollection` instead of a globe-attached system. Exclusive `renderMode` (BILLBOARD default / VOLUMETRIC) is the single activation gate; VOLUMETRIC suppresses that collection's billboards and publishes its `.volumetric` snapshot for the env-effects composite. WebGPU-only expanded flags (genera via `cloudType`, weather, exotic E1/E2/E3) default off + byte-identical when off + documented no-op on WebGL. `globe.showProceduralClouds`/`globe.cloud*` REMOVED (slice 4B); `globe.defaultCloudCollection` is the managed single authority. Off-identity guarded by `probe-cloud-u8-offident.mjs`. (SHIPPED — Batches 617–625, cloud-unification epic; see migration_doc/CLOUD_UNIFICATION_DESIGN.md)
- Weather data ingest (`Scene/Weather/` — `WeatherSource` interface + `Edr`/`WcsCoverages`/`Metar`/`Synthetic` sources, the shared `CoverageJsonParser`, `WeatherProvider` orchestrator with the live/historical/projected time model + LRU slice cache, and `WeatherTexPacker`, which bakes a normalized `WeatherField` into the C2-16 `weatherTex` bytes) — backend-agnostic Scene data; nothing here imports the renderer. Two contract modules own the geometry: `WeatherMapSeam.ts` (C13-07 — the texel-centre ↔ lon/lat convention, the exactly-periodic procedural fBM, and the wrap-aware polar low-pass, shared by the procedural producer in `ProceduralWeatherMap.ts` and the real-data packer) and `WeatherFieldGrid.ts` (C13-08 — the SOURCE-GRID coordinate reference: node-centred/gridline-registered by default with a declarable `"cell"` alternative; regional `WeatherField.bounds` placed on their true rectangle of the global texture, antimeridian-straddling included; and no-data semantics with a typed `WeatherNoDataFill` defaulting to the procedural map, so a gap never renders as an observed clear sky). The texture stays GLOBAL — `weatherTexBounds` is fixed and the regional rectangle is honoured by the packer, because a repeating-U sampler would tile a regional bounds uniform across the planet. Public surface: `WeatherProvider`, the four sources, `packWeatherField`/`packWeatherFieldDetailed`, `GLOBAL_WEATHER_BOUNDS`, `PROCEDURAL_NO_DATA_FILL`, `DEFAULT_WEATHER_GRID_REGISTRATION`, `weatherTexelCenterLonLat`. CPU twins: `weather-map-seam.spec.mjs`, `weather-field-bounds.spec.mjs`. Probes: `probe-weather-ingest/-edr-mock/-wcs/-metar/-channels/-time/-map/-seam-poles.mjs`. (SHIPPED — Batches 410/425/444 + C13-07/C13-08; per-tile bounds/gutters/atlas/LOD is `C13-14` and multi-source priority composition + boundary feathering is `C13-20`. See migration_doc/WEATHER_DATA_INGEST_ROADMAP.md)
...
- Twilight range in the sky-brightness estimator — C12-34 (**IMPLEMENTED Batch 823; ENGINE-leg Edge acceptance PASS Batch 824; star-PIXEL leg still owed**) — `Scene/SkyBrightness.js`'s sun term was `smoothstep(-0.1, 0.4, sunAlt)`, which hit exactly 0 below −5.74° and therefore carried **zero** dynamic range across the whole twilight decade (span −18°→−6° measured at exactly 0.000000). It is now a log-luminance estimate keyed continuously on solar elevation: the published zenith twilight-photometry μ ladder (8.0 at sunset, 14.0 at −6°, 19.7 at −12°, 21.9 at −18°, 4.0 by day), a full-moon term with a `p^3.64` phase-flux law, linear summation in luminance, and a NELM perceptual transfer. Same span is now 0.973697; mid civil twilight (−2°) goes from exactly 0 to 0.004175 (stars return) and full moon overhead 0.018176 → 0.165959 (NELM 2.15 → 4.55 against a published ≈4.5). Both ends stay BYTE-IDENTICAL (≤ −18° exactly 0, ≥ +23.6° exactly 1) and high-sun totality is bit-exact at 0.062810, so the shipped `STAR_MODULATION_*` pair is confirmed rather than moved. `computeCelestialElevationSine` is now the single shared solar-elevation home (`computeStarDayFade` rewired onto it; C15's aurora night-gate is to reuse it). No shader changed — the scalar rides the existing `u_skyBrightness` uniform — so no lockstep row is owed. Node `sky-brightness-twilight.spec.mjs` 27/27 (5 mutation tests); Edge `probe-sky-twilight-range.mjs` reproduces the derivation to six decimals on BOTH backends, bit-for-bit equal. **Owed:** the probe's star-PIXEL leg reports STRUCTURAL, not PASS — the star field drew nothing at the darkest lane in that configuration, so that leg measured an empty sky and is an instrument gap, not a product verdict. ([Campaign 12](QUEUE_2026-07-19_CAMPAIGN12.md) `C12-34`)
- Moon texture request / device-generation lifecycle — C12-35 (**SHIPPED, LANDED Batch 819; L0-L5 gate PASS incl. the schema-v2 real-Edge lane**) — WebGL Moon realization moved out of the shared Material URL loader so Moon textures are Moon-owned; WebGPU candidates carry exact `(GPUDevice, resourceGeneration)` ownership, a bounded lease-pinned `MoonDecodedSourceCache` (deliberately **not** the rejected replay journal), and destroy-cancels-before-native-destroy. Two real GPU leaks closed. Rider recorded at landing: fleet-wide cubemap mip pre-allocation costs ~+33% memory on generator-supported formats. Canonical evidence: [`C12_MOON_TEXTURE_LIFECYCLE_AUDIT_2026-08-02.md`](C12_MOON_TEXTURE_LIFECYCLE_AUDIT_2026-08-02.md).
- Moon mip / explicit-LOD sampling — C12-33 (**IMPLEMENTED, LANDED Batch 819 — ACCEPTANCE OWED, do NOT read as complete**) — neither backend mipmapped Moon textures, so C12-24's 2K albedo minified ~64:1 at the default camera. WebGL now generates mips Moon-locally after direct `Texture` creation with trilinear sampling (WebGL1 NPOT keeps its legal single-level fallback); WebGPU allocates the exact mip count per lifecycle candidate and queues generation through the context's frame-owned shared texture-mip lane (no Moon-private submit). Both shaders compute longitude-unwrapped derivatives BEFORE the fragment-varying miss discard and sample with `textureGrad` / `textureSampleGrad`. **Owed:** focused Jasmine plus five paired real-Edge `normal`/`force-lod0` moving runs with calibrated per-lane thresholds and mandatory seam PNG review — `probe-moon-mip-motion-edge.mjs` deliberately still exits `CALIBRATION_PENDING`.
- Bright-star catalog starfield (WebGPU) — Track V-C, Batch 313 (SHIPPED — NEW-STARS-BRIGHT-CATALOG). Real Yale Bright Star Catalog (BSC5) factual-field subset (`Scene/BrightStarCatalog.js`, **263 entries**, visual magnitude **-1.46 through 5.0**, each `[raJ2000°,decJ2000°,vmag,B−V]`) rendered as instanced HDR point sprites (`Renderer/WebGPU/WebGPUStarFieldRenderer.ts` — TS-converted Batch 314 (NEW-TS-CONVERT-JS-RENDERERS first slice, zero behavior change); `Shaders/WebGPU/Catalog/StarField.wgsl`, `FeatureRendererKey.STAR_FIELD`=51) additively into the scene FB so bloom makes bright stars glow. **Provenance:** BSC5 is freely available, but no public-domain dedication or redistribution licence has been located; do not call it public domain. The embedded RA/Dec/Vmag/B−V values are factual measurements; residual database-rights review is tracked as `C12-09` / DR-02 and documented in `BrightStarCatalog.js`. Magnitude→brightness via the Pogson scale (gamma-compressed `[LO,HI]` band + magnitude-driven size boost); B−V→blackbody-temperature→Planckian-locus RGB. Catalog directions are TEME/J2000-inertial, rotated into the Earth-fixed frame each frame by `Transforms.computeTemeToPseudoFixedMatrix` (same as SkyBox) so constellations land at correct RA/Dec for the scene clock; camera-altitude-gated daytime fade. Backend-agnostic `Scene/StarField.js` (FR-seam) owned by `SkyBox` (toggle `scene.skyBox.starField.show`); `Scene.updateEnvironment` routes the command to `environmentState.starFieldCommand`, injected AFTER `skyBoxCommand` so it augments (draws on top of) the cubemap. Verified by `probe-stars-catalog.mjs` (Sirius-aimed center cluster, RA/Dec correctness, Pogson intensity liveness, 0 errors). **WebGL now draws the SAME physical catalog (Batch 324, NEW-STARS-BRIGHT-CATALOG-WEBGL-FALLBACK SHIPPED):** a GLSL point-sprite `STAR_FIELD` FR (`Renderer/WebGLStarFieldRenderer.js` + `Shaders/StarFieldVS/FS.glsl`) reusing the shared `Scene/StarFieldMath.ts` math (B−V→RGB, Pogson packing, day-fade) extracted from the WebGPU renderer, so both backends place/color/brighten the IDENTICAL stars (bright-block IoU=1.000 in `probe-starfield-webgl-parity.mjs`) — only the draw path differs (GLSL vs WGSL).
- Star-map / sprite-catalogue seam — `SkyBox.Variant.TYCHO_T5_DIFFUSE` — C12-11 / decision record DR-01 (**IMPLEMENTED + LANDED Batch 833, 2026-08-06; Edge acceptance OWED — do NOT read as complete**) — DR-01 assigns one physical owner per celestial signal: the cube map carries **diffuse Milky Way light only** and every *resolved* star comes from the `StarField` sprite catalogue at its real RA/Dec. `SkyBox.defaultVariant` is now `TYCHO_T5_DIFFUSE` (six bundled 2048 q90 4:4:4 faces, **0.362 MB total**, replacing the 4.355 MB un-blurred set as the default). What C12-10 had shipped was the *un-blurred* `TYCHO_T5` faces, a deliberate transitional step — which is why the Batch-815 census measured the whole 2,868-sprite field as worth ~3 px: both sources were painting the same stars. **Two real defects in the bake, whose output had never been consumed:** (1) `.blur()` was chained onto a `sharp.composite()` pipeline and sharp applies `composite` LAST regardless of chain order, so the blur hit the empty create-base and the un-blurred strips were pasted over it — a total no-op; (2) `composite()` returns RGBA even from a `channels:3` base and the caller read it with a 3-byte stride, making the map ~29× too bright. **Measurement trap worth carrying forward: a MEAN is invariant under blur**, so mean conservation cannot distinguish blurred from not-blurred — only a peak/point metric can (a real σ=20 low-pass drops a synthetic field's max 250 → 65; the broken path left it at 250). Faces were regenerated from the hash-pinned NASA SVS source (re-downloaded, SHA-256 matched the pin), and reproducibility is proven decisively: the same run's un-blurred faces came out **byte-identical** to the six already in the tree. Redundancy is resolved by construction and offline: every diffuse face censuses to **0 resolved point sources** (peak luminance 8–28 vs 251–255) while retaining **83.5–94.6%** of its own band structure. `TYCHO_T5` stays bundled as DR-01's un-blurred reversal artifact and `TYCHO_T3` as the historical faint render. Gates: new `skybox-diffuse-seam.spec.mjs` (24 tests, synthetic ground-truth + mutation group, SHA-re-derived manifest); `probe-stars-catalog` check (A) re-expressed as a resolved-POINT CENSUS rather than a brightness count (Sirius sits only ~9° off the galactic plane, where a brightness count stays cubemap-dominated even after the seam lands) plus a new check (G) requiring the cubemap alone to yield no resolved stars. **OWED:** the Edge run of `probe-stars-catalog.mjs`, the G3 diffuse/reversal visual review, and the moving-camera alias/frame-cost lane
...
- Moon 2K NASA albedo + `Moon.Variant` switch — C12-24 (**SHIPPED pending Edge gate, 2026-08-01**) — the upstream 256×128 `moonSmall.jpg` (0.67 texels/px over a ~190 px disc) is replaced as the default by NASA/GSFC SVS 4720 "CGI Moon Kit" 2019 colour map at 2048×1024 (`Assets/Textures/Moon/lroc_color_poles_2k.jpg`, JPEG q90 4:4:4, 563,276 B), derived from the LROC WAC "Hapke normalized" mosaic. New `Moon.Variant` / `Moon.defaultVariant` enum mirrors `SkyBox.Variant`; `moonSmall.jpg` stays bundled and selectable as `Moon.Variant.SMALL`, and an explicit `options.textureUrl` still overrides both. Reproducible hash-pinned bake at `Tools/moon-albedo-bake/` (no transfer correction — SVS states no non-sRGB standard for this product and the nearside median luminance already matches the map it replaces to within 1%, so the swap is resolution-only and NOT a lighting change); `LICENSE.md` → Bundled Engine Assets entry added. **Texture-only — no shader change, no lockstep row.** UV/prime-meridian alignment is pinned COMPUTATIONALLY rather than by eyeball: five named lunar-landmark checks (Mare Crisium ring contrast, Tycho vs its equatorial reflection, Copernicus local contrast, nearside-vs-farside mean, maria-vs-highlands) assert the 0°-centred/east-right/north-top convention that the shared `czm_ellipsoidTextureCoordinates` / `ellipsoidTexCoords` `atan2`/`asin` unwrap requires, and are **adversarially validated** — `shift180`, `mirrorLon`, `mirrorLat`, `mirrorBoth` and `rot180` are all REJECTED, so the check cannot pass vacuously. **Fixed en route: the WebGPU moon albedo was rendering vertically MIRRORED against WebGL** — WebGL reaches the texture through `Material.ImageType` → `Texture` (defaults `flipY:true`), while `WebGPUEnvironmentRenderer._loadRealMoonTexture` used `copyExternalImageToTexture`'s `flipY:false` default, yet both backends share `v = asin(n.z)/π + 0.5`; invisible on the soft 256×128 map, obvious at 2K. Fixed at the upload layer so it also holds for user-supplied `moon.textureUrl` and the GLSL/WGSL twins stay identical. Gate: `moon-albedo-asset.spec.mjs` (11 tests — dependency-free JPEG SOF parse for geometry + 4:4:4 chroma, manifest-vs-file SHA-256 binding, LICENSE entry, landmark battery, adversarial rejection, both-backends wiring)
- Moon LOLA terminator relief (tangent-space normal map) — C12-25 (**SHIPPED pending Edge gate, 2026-08-02**) — the moon was a geometrically perfect sphere; it now carries real surface relief. Ships `Assets/Textures/Moon/ldem_normal_1k.png` (1024×512, 8-bit truecolour PNG, 679,782 B), **derived** — NASA publishes displacement, not normals — from SVS 4720's LOLA height field `ldem_16.tif` (5760×2880 float32 km on a 1737.4 km sphere). Paired with `Moon.Variant.LROC_COLOR_2K` and default ON via the new `atmosphericConditions.lighting.enableLunarNormalMap`; `Moon.Variant.SMALL` ships **no** map, so the legacy flat look stays exactly reachable. New `Moon.getVariantNormalMapUrl` + `Moon#normalMapUrl` / `#normalMapStrength` (1.0 = true derived geometry, 0.0 = exactly flat); reproducible hash-pinned bake at `Tools/moon-albedo-bake/bake-lola-normals.mjs`, `LICENSE.md` → Bundled Engine Assets entry added. ⚠ **Two premises corrected:** SVS publishes **no 2K displacement map** (HEAD-verified: `ldem_2k/1k/512.tif` all 404; the family is 4/16/64 px per degree only), and `ldem_4` is COARSER than the output grid so it cannot be the source. **Derivation:** area-average the heights to the shipped grid *then* central-difference — an area filter cannot ring, and ringing next to a crater rim becomes a false slope reversal once differentiated; the longitude stencil widens to `round(1/cos(lat))` texels so the east baseline stays a constant GROUND distance (10,660.6 m, equal to the north baseline at W=2H) instead of shearing toward the poles; rows past a pole wrap ACROSS to the antipodal longitude rather than clamping, which would paint a ring of false slope at both poles. Measured p99 tilt **10.53°** — the asymmetry IS the feature: near the terminator N·L≈0 so a 10° facet flips lit/unlit, while at full phase the cosine is flat and the same facet moves brightness ~1.6%. **Output is 1024×512 deliberately**, not matched to the 2K albedo: neither backend mipmaps the moon (`C12-33`), so at the default ~16 px disc a 2048-wide map is ~64:1 minification off mip 0, and normal aliasing flickers the *lighting* rather than the colour; 1K still leaves ~2.7 texels/px at the ~190 px zoomed disc and costs 664 KB against 2.92 MB (`--width 2048` re-bakes in one flag once `C12-33` lands). **Encoding measured, not assumed:** JPEG q90 4:4:4 costs **1.26° mean / 9.47° max** normal-tilt error against a signal whose own mean tilt is 2.73° (the two informative channels land in the chroma planes after the RGB→YCbCr rotation) — rejected; 8-bit lossless PNG costs 0.173°/0.347°; 16-bit PNG is 1.9× the size to remove an error already 14× below the median signal. **Unlike C12-24 this IS a shader change and carries a `SHADER_PAIRS_LOCKSTEP.md` row:** `EllipsoidFS.glsl`'s `LUNAR_NORMAL_MAP` block ↔ `Moon.wgsl`'s `@binding(3)` + `u.normalStrength` block, both rebuilding the east/north/up basis in MODEL space from the identical expression (`up = normalMC`, `east = normalize(-y, x, 0)` pole-guarded, `north = cross(up, east)`) rather than calling `czm_eastNorthUpToEyeCoordinates`, so the twins are the same expression instead of "equivalent if you work it out"; the perturbed vector is the **lighting** normal, so the relief rides C12-20's Lommel-Seeliger term and the Phong fallback alike. Moon UB 336→352 bytes, add-only at the tail. Strength resolved ONCE backend-agnostically in `Moon.update` and published as `frameState.moonNormalMapStrength`, so the backends cannot disagree about whether relief is on; both reach the EXACT identity at strength 0. Deliberate **wiring** asymmetry (not math): WebGL gates the sampler behind a define because `EllipsoidFS.glsl` is shared by every `EllipsoidPrimitive`, WebGPU keeps binding 3 always bound against a 1×1 flat placeholder because `Moon.wgsl` is moon-only. WebGL texture owned by `Moon.js`, not `Material` (`Material.ImageType` has one image slot and no normal channel). ⚠ **Found en route: the `Moon.wgsl` entry in `SHADER_PAIRS_LOCKSTEP.md` was WRONG** — it listed the file as fork-only and claimed WebGL renders the moon "through a CZML/Entity path"; WebGL renders it through `EllipsoidPrimitive` → `EllipsoidFS.glsl`, which has been `Moon.wgsl`'s twin since C12-20/C12-23. Corrected, and `Moon` removed from that doc's fork-only list. Gate: `moon-normal-map-asset.spec.mjs` (19 tests — dependency-free PNG IHDR parse, manifest-vs-file SHA-256 binding, LICENSE entry, direct pins on the derivation math including a constant east GROUND slope reproduced exactly at lat 0/30/60/80, crater-bowl polarity at Tycho + Copernicus independently signed in red and green, an end-to-end grazing-illumination check at four azimuths, a no-polar-ring assertion (pole bands measure 0.86× / 1.42× the global mean tangential tilt), adversarial rejection of `flipGreen`/`flipRed`/`swapChannels`/`mirrorLat`/`mirrorLon`/`flatten`, and both-backends wiring including the twinned tangent-frame construction)
- Moon phase-dependent earthshine — C12-21 (**SHIPPED pending Edge gate, 2026-08-06**) — earthshine was a CONSTANT with no phase term, which is physically backwards: Earth's phase seen from the Moon is the exact complement of the Moon's phase seen from Earth, so earthshine must PEAK at new moon and vanish at full. Now scaled by `1 - phaseFraction`, resolved once CPU-side in the new pure `Scene/MoonPhaseAppearance.js` and published as `frameState.moonEarthshinePhaseScale`. ⚠ **Found + closed en route: earthshine had NO GLSL implementation at all** — a WebGPU-only celestial term since Phase 1.2, the Principle-5 gap the `C11-176b` row had noted in passing; this ships its first GLSL half (`EARTHSHINE` define + `u_earthshinePhaseScale` in `EllipsoidFS.glsl`), making it a real `SHADER_PAIRS_LOCKSTEP.md` pair. `computeEllipsoidColor` gained a SINGLE exit in the same edit, collapsing the three duplicated `C12-23` surge multiplies to one. Toggle `atmosphericConditions.lighting.enableEarthshinePhase` (default ON); OFF passes scale exactly 1.0 — the historical constant bit-for-bit. `enableMoonPhase = false` is handled explicitly (it forces the 1.0 phase sentinel, and `1 - 1 = 0` would have DELETED earthshine). ⚠ **INERT at engine defaults**: the master toggle `lighting.enableEarthshine` still defaults to **false**, so nothing moves until an application opts in — whether that default should now flip is recorded as an open maintainer question in the C12-21 queue row, not decided. Gate: `moon-phase-terminator.spec.mjs`, with the complement law adversarially validated against the un-complemented scale, a quarter-peaking tent, and the unclamped complement (all three REJECTED)
- Moon soft terminator (finite solar disc) — C12-22 (**SHIPPED pending Edge gate, 2026-08-06**) — the Lommel-Seeliger μ0 used a hard `max(N·L, 0)` horizon clip, which models a point Sun. From the Moon the Sun subtends an angular RADIUS of `asin(SOLAR_RADIUS/d)` = 4.64915e-3 rad = 0.26638°, so the terminator is a penumbra. Replaced by the C1 quadratic wrap `f(c) = (c+w)²/(4w)` inside the band and `max(c,0)` outside — value and slope match at both seams, so `w = 0` is a true identity, and the peak excess is exactly `w/4` at `c = 0`. ⚠ **Two queue-row premises corrected:** "one `smoothstep`" is wrong (a bare `smoothstep(-w, w, N·L)` is a 0..1 gate, not an irradiance — it leaves the dark side at exactly 0 and darkens the lit side, making the edge HARDER; kept as an explicit negative control in the spec), and "±0.0044" is the half-angle of a solar diameter rounded to 0.5° — the unrounded value is 6.6% larger and is now measured per frame from the true Sun→Moon distance (±1.7% annual swing). Applied in the Lommel-Seeliger branch only, on BOTH backends: the WebGL phong fallbacks run inside the shared `czm_private_phong` / `czm_phong` builtins, so both backends deliberately keep the fallback hard-clipped. Toggle `atmosphericConditions.lighting.enableSoftTerminator` (default ON); OFF passes softness exactly 0.0. Lockstep pair `SOFT_TERMINATOR` / `u_terminatorSoftness` ↔ `Moon.wgsl` `softTerminatorMu0` / `u.terminatorSoftness`; the Moon UB grows add-only into the slot C12-25 already opened, so it stays **352 bytes** with no BGL churn. ⚠ **Physically real but SUB-PIXEL**: the softened band spans `2·R·w` screen px — 0.883 px at the ~190 px zoomed disc, 0.074 px at the default ~16 px disc. It removes a hard binary edge; the real Moon's soft-looking terminator is TOPOGRAPHY (C12-25's LOLA relief), not the Sun's angular size. Gate: `moon-phase-terminator.spec.mjs`, which **extracts `softTerminatorMu0` from both shader texts, compiles each body as JavaScript and requires all three implementations to agree to 1e-15**, plus five rejected mutants (bare smoothstep gate, swapped edges, width in DEGREES, mirrored quadratic, no softening)
...
- Per-fragment lunar shadow on globe terrain — C12-29 S5 (**INTEGRATED; LANDED Batches 777/780, 2026-08-01** — the per-fragment lunar-shadow uniforms and GLSL/WGSL globe twins landed with the Codex C11 pass; **targeted browser and moving-route gates PASS; final certification open, 2026-07-26**) — supersedes the older S1/S2 inventory tail that says every S3-S6 slice remains deferred. `Scene/EclipseGlobeShadow.js` converts the active logical `View`'s f64 eclipse geometry into a camera-independent common-ray payload: Sun unit direction + inverse range, Moon-minus-Sun direction differential + inverse range, and fit/composition vectors. Each `View` owns stable S1/S2/S5/S6 state; `Scene.updateFrameState()` publishes only transient active-View aliases before `UniformState.update`, so render, pick, offscreen, and capture work cannot substitute a mutable pass camera as the eclipse observer. Both globe fragment shaders consume direct exaggerated `positionMC`, multiply it by inverse astronomical range before subtraction, and form `s=uS-P*a`, `D=dU+P*(a-b)`, `m=s+D`; this preserves the tiny umbral separation without a camera high/low reconstruction or `acos(dot)` of independently rounded rays. An exact algebraic local support reject precedes inverse trig, analytic circle overlap feeds the fitted limb-darkening law, and an ellipsoid-space ray/limb test uses the rendered inverse radii plus the stable cross-product closest distance for WGS84, custom ellipsoids, elevated terrain, and the antipodal night side. S2-aware absolute/relative gates prevent double dimming, including correction-only gates 3/4. Terrain activation is two-stage: a provider-wide realized-mesh envelope supplies the O(1) ordinary-frame rejection, and only a globally possible shadow scans skirt-inclusive spheres for the selected rendered meshes. Main render, pick, and retained capture refine the same View-owned block. WebGL carries one packed `mat4` through command-local properties and uses a bit-33 inactive/active shader-cache variant, so ordinary programs have no S5 uniform/function IR or fragment composition. WebGPU uses a dedicated 64-byte group-0/binding-2 dynamic UBO: ordinary frames bind one renderer-owned inert slice with no ring allocation/upload; an active logical View reuses one allocation-epoch-memoized slice across tile, imagery, wireframe, pick, and capture commands. `CameraUniforms` remains 232 floats / 928 bytes. Toggle `atmosphericConditions.lighting.enableEclipseGlobeShadow` defaults ON and gates only S5. Evidence: S5 RTE/resource contract **18/18**, visual-source contract **3/3**, protected eclipse/recovery Node set **144/144**, focused Edge/Karma dynamic-environment-manager gate **11/11**, full build and targeted lint PASS, and `probe-eclipse-globe-shadow.mjs` PASS on WebGL/WebGPU with zero-pixel control changes and aligned footprint/core/darkening. The earlier terrain-mesh classification lane passed **4/4**; its requested current-tree rerun was blocked before browser launch by the executor approval cap, not by a test failure. The instrumented multi-altitude route proved the ordinary WebGL stalls are synchronous `LINK_STATUS` waits; the static variant reduced the seven waits from 889.9 to 753.9 ms in the directional run. A six-pair counterbalanced current-build route passes all 12 executions and certifies current renderer parity, but the one-run banked pre-spatial baseline cannot establish a stable before/after speedup. NASA-SVS geospatial comparison, controlled active-eclipse pre/post CPU/GPU measurement, real terrain/exaggeration, custom-ellipsoid/pole/antimeridian, multi-View/capture, and real device-loss lanes remain before this item is called complete.
...
- VolumetricFog.wgsl — froxel-grid density injection + HG light-scattering with sun-shadow-map sampling + front-to-back integration (Phases 5a-5d shipped) (SHIPPED). **Cheap-path cloud-shadow coverage gate re-derived — `CLOUD-LOW-COVERAGE-CUTOFF` fog arm (IMPLEMENTED Batch 832, 2026-08-06; Edge acceptance OWED).** `sampleCloudShadow`'s non-hi-fi path carried the last raw `1.0 - coverage` threshold in the cloud-density family (the 2026-08-01 fix converted `ProceduralClouds.wgsl` ×3 and `ProceduralSkyCubemap.wgsl` ×1 and missed the fog; `cloud-coverage-response.spec.mjs`'s "every gate routes through the shared response" test never read this file). **The obvious one-line fix measures WORSE than doing nothing** — dropping in `cloudEffectiveCoverage` and leaving the sample alone gives 28.2 pp error vs the historical gate's 23.9 pp — because the fog's cheap shadow never samples the baked cloud textures: it builds its own local 3-octave `fbm3d` whose distribution differs from the baked shape channel (baked mean 0.4307 / σ 0.0896; fog mean 0.5000 / σ 0.1206, symmetric by construction and ~35% wider), and a coverage threshold only transfers between fields that share a distribution. Fixed as a **re-derivation, not a rescale** (the cloud arm's acceptance forbids a shifted cutoff): `normalizeFogCheapCloudField` standardises the fog SAMPLE onto the baked field's first two moments, then the shared response applies unmodified — which matches the smoothstep RAMP and not merely its support. Worst support-tracking error **23.9 pp → 1.5 pp**. `CloudDensityDomain.wgsl` is now prepended to the fog compute module by `WebGPUVolumetricFogResources`, so the engine still holds ONE definition of the response. Clouds-off (the default) is byte-identical — the change sits below the `cloudShadowHiFi` / `cloudShadowEnable` / `coverage` early returns. Gate: new `fog-cheap-coverage-gate.spec.mjs` (15 tests, four-way mutation group; the historical gate is pinned as its own mutant so nobody "simplifies" back to it). **OWED:** an Edge run with volumetric clouds ON at coverage 0.15 / 0.35 / 0.55 plus a clouds-OFF byte-identical control
...
- WebGPU SSGI diffuse global illumination (C6-SSGI-DIFFUSE) — SSILVB visibility-bitmask screen-space GI (Therrien et al. 2023, MIT-derived from three.js SSGINode / SSRT3). Opt-in default-off, WebGPU-only. Rides the AmbientOcclusion stage as `algorithm: "ssgi"` (`scene.postProcessStages.ambientOcclusion.uniforms.algorithm = "ssgi"`); one pass produces improved AO (thin-surface fix) + diffuse indirect color-bleed via `SSGIGenerate.wgsl` → `BilateralBlur1D.wgsl` (H/V) → `SSGIComposite.wgsl`, all rgba16float. Planet-scale guards: log-depth reuse, sky/far rejection, per-sample linear thickness, screen-space radius + maxWorldRadius cap, CPU altitude fade (byte-exact orbit no-op), depth sanitize, HDR luminance clamp. Temporal slice rotation keyed to the frame index; downstream TAA denoises. Debug modes 0/1/2/3 (composite/AO/GI/scene). Verified: probe-ssgi.mjs (SHIPPED — Campaign 6; byte-identical off-gate + orbit no-op + GI color-bleed confirmed). Known limitation: grazing-angle banding at low slice count / high giIntensity — mitigate with more slices + TAA + à-trous (see DEFERRED_WORK). No GLSL twin (WebGL deferred, §D.7).
...
- WebGPUMotionBlurEffect — velocity-buffer motion blur (C6-VELOCITY-MOTION-BLUR; WebGPU-only, opt-in `scene.motionBlur`, default off / byte-identical when off). Single fullscreen pass in the pre-tonemap HDR chain, right AFTER TAA. Per-pixel screen-space velocity is derived from the SAME data TAA consumes — the per-pixel model MRT velocity texture (object motion) with a fallback to RTE depth-reprojection (`inverseCurrentVpRte` → eye-relative pos + `cameraDelta` → `previousVpRte`, the exact TAA math) for whole-scene camera motion; color is gathered along the velocity vector (center-weighted, `maxBlurPx` clamp). `Scene.render` feeds the effect the current/previous VP-RTE + camera delta independently of `scene.taaEnabled` (mirrors the TAA block; gated on `scene.motionBlur === true`). Best for fast camera pans / fly-to / CZML playback. WebGL: documented no-op (`_alternateSceneRenderer` null → block short-circuits). Port: realism-effects (MIT). Verified `probe-motion-blur.mjs` (vertical camera motion smears the atmosphere limb band; blurLarge 31.7% vs blurSmall 4% velocity-driven; off-frame byte-identical offStable 0; gate clean). (SHIPPED — Campaign 7)
...
- WebGPUOIT — Order-Independent Transparency; weighted-average via the MRT (accumulation + revealage) composite path only. **CONTAINED OFF by default (FAR-003, ratified 2026-07-16):** `_webgpuOITEnabled` default `false`; default translucency = sorted back-to-front alpha. Toggle via `CesiumDebug.webgpuOIT(t/f)`. **UNREACHABLE for standard translucency (M-OIT verify 2026-07-18):** the MRT accumulation path in `WebGPUSceneRendererTranslucentPass` only engages for a `Pass.TRANSLUCENT` command carrying `_shaderCode`/`_oitPipeline`, but the ONLY `_shaderCode` producers are Gaussian splats (`Pass.GAUSSIAN_SPLATS`) + the opaque globe (`Pass.GLOBE`) — no primitive/model/collection produces one, so `hasOITPipelines` is always false and even `webgpuOIT(true)` reports `active=false` (proven: `probe-oit-transparency.mjs`, `OIT_DEFAULT_FLIP_EVIDENCE_2026-07-18.md`). The Batch-697 `_ensureSceneColorResolved` composite line has never executed. Wiring gap = `NEW-WEBGPU-OIT-TRANSLUCENT-PRIMITIVE-WIRING` (DEFERRED_WORK). **WGF-2: dual-source-blending single-pass OIT is NOT wired** — `WebGPUOIT.ts` carries only the MRT fallback (no `blend_src` / `src1` output, no `dual-source-blending` feature request); the single-pass dual-source path is a docstring aspiration (lines 16-17), confirmed never wired (`PHASE_5_MODERN_WEBGPU_DESIGN.md`). (EXPERIMENTAL — contained default-off; MRT accumulation path unreachable for standard translucent primitives; dual-source NOT wired)
- WebGPUGlobeDepth — packed globe-depth subsystem with copy-readback for picking (SHIPPED)
- WebGPUEdgeFramebuffer — MRT edge target + 16-bit feature-id channel split across rgba8 (SHIPPED; the `EdgeDisplayMode` tri-mode landed in Batch 316 — see §C.3 NEW-EDGE-DISPLAY-MODE-WEBGPU, ✅ SHIPPED Batch 316 `cf7edec7dc`, residuals only)
- WebGPUEdgeVisibilityEmitter — generates per-fragment edge intensity for Model edges (✅ SHIPPED Batch 316, `cf7edec7dc` — now reads `model.edgeDisplayMode`; `EDGES_ONLY` slot-12 `CESIUM_3D_TILE_EDGES_DIRECT` direct pass + `SURFACES_ONLY` default-suppression + explicit `lineStrings` edges are wired. per-edge/per-lineString `materialColor` overrides (+ per-vertex COLOR_0) now SHIPPED Batch 330 (NEW-EDGE-MATERIALCOLOR-OVERRIDE-WEBGPU — `location(7)` 19-float/76-byte edge vertex carries the per-edge color, WebGL `a_edgeColor` equivalent). Authored-`silhouetteNormals` signed-byte path SHIPPED (EDGE-AUTHORED-SILHOUETTE-NORMALS, 2026-07-02 — WebGL-identical decode + sequential pair indexing; derived fallback byte-identical when the accessor is absent) and PR#13421 degenerate-tri parity confirmed (`probe-edge-degenerate.mjs` repaired + GREEN) — no residuals; see §C.3 NEW-EDGE-DISPLAY-MODE-WEBGPU)
...
- GLOBE-UNDERGROUND-COLOR — `globe.undergroundColor` + `undergroundColorAlphaByDistance` tint (GlobeFS UNDERGROUND_COLOR parity): camera-UB tail (+12 floats incl. CPU-computed `showUndergroundColor` gate + eyeHeight), `@builtin(front_facing)` back-facing alpha-blend in GlobeTerrain.wgsl. Includes the Bug 487 underground-rendering fixes: no-cull pipeline central-cache NAME de-aliasing (`, noCull` label), fog disabled when `cameraUnderground` (WebGL parity), skirt suppression via `indexCountWithoutSkirts` truncation. Probe: `probe-globe-underground.mjs` (2026-07-02) (SHIPPED)
...
- Per-object cache device-loss recovery walk (Batch 197, corrected 2026-07-31) — the original walk clears literal `_webgpuCache` owners reached through scene/ground primitives plus shadow-map/post-process owners, but does not prove nested tileset-model, clipping, label-cache, or replacement-device lifetime coverage. Point/Label normal teardown is now covered and landed (Batch 778, 2026-08-01). (WIP / PARTIAL; `C11-20`)
...
- WebGPUCollectionRendererBase — shared base for the collection feature renderers; folds the resident-instance-manager per-frame flow (lazy create + capture→sync→consume ordering), scene-format-gen pipeline invalidation, settled-2D/CV coplanar-depth flag/key, per-device shader-module-cache accessor, the re-entry/null-target/overflow sentinels, and grow-on-demand pick buffer mgmt. Billboard + Point migrated (Batch 302); Cloud + Polyline migrated (Batch 307 — folded the shader-module-cache accessor + re-entry sentinel for both, plus `computeNoDepthTest` for Polyline; each keeps its unique non-resident/bucket-shaped pack + pipeline logic); Label migrated (Batch 332 — re-entry sentinel via `_updateWebGPULabelsInner` split + `validateInstanceSyncResult`, completing 5/5). All five collection renderers now fold onto the base. (SHIPPED, 5/5 complete Batch 332, NEW-COLLECTION-RENDERER-BASE / NEW-COLLECTION-RENDERER-BASE-LABEL)
- Collection error sentinels — the three mandated permanent sentinels in the collection base: re-entry/infinite-loop guard (sane-limit depth, synchronous-bracketed), null-target guard (null buffer for non-zero visible count + `validateDrawTargets`/`validateInstanceSyncResult`), size-validation/overflow guard (pick writeBuffer clamp + `validateInstancedDrawBuffer` clamp). Billboard + Point covered Batch 302; **extended to ALL FIVE collections (Label, Cloud, Polyline) in Batch 331, completing 5/5**. Unit-tested 16/16 (`Tools/collection-sentinels-check.mjs` + `WebGPUCollectionSentinelsSpec.js`). **Batch 334 (`dab9dfda04`, AUDIT-P1-SENTINEL-331)** repaired the dead destroyed-buffer branch: the null-target guard now keys off `WebGPUBuffer.isDestroyed` (previously the branch was unreachable, and its tests were de-fictionalized). (SHIPPED, 5/5 complete Batch 331, NEW-COLLECTIONS-ERROR-SENTINELS)
...
- UP144-VECTOR-LAYER-WGSL: upstream v1.144 clamped vector-tile polylines (VectorProvider/VectorPipeline + HAS_VECTOR_LAYER in GlobeFS). **WGSL twin IMPLEMENTED 2026-08-06 (C11-213) — browser acceptance OWED, so the row stays here rather than moving to §B.** `GlobeTerrain.wgsl::vectorPolylineRender` composites draped polylines between the underground tint and the translucency ramp, matching `GlobeFS.glsl`'s block order. The row's "bind the five `u_vector*` textures" shape was NOT buildable: group 2 already charges 5 of the 12 non-imagery fragment sampled textures and the C11-208 reduced low-limit shape lands on exactly the 16-texture WebGPU spec floor, so five more (21) would fail globe pipeline creation on default-limit adapters. GLSL's `texelFetch` there is WebGL2's buffer-read stand-in, so the WGSL reads ONE read-only storage buffer at `@group(2) @binding(11)` packed by the new `WebGPUVectorTileResources.ts`; `GLOBE_NON_IMAGERY_FRAGMENT_TEXTURES` is unchanged at 12. Per-tile gating is a runtime header word (no globe pipeline variant forks); WebGL's `0x400000000` shader-set bit stays WebGL-only. Bake routing goes through the `GLOBE_SURFACE` feature renderer's new `prepareVectorTileData` hook, so `Core/VectorPipeline.js` still has no backend test. Node: `vector-layer-draping.spec.mjs` 21/21 (GLSL-oracle ↔ WGSL-reader equivalence over a real bake, 5 mutation tests, naga). **Browser acceptance PARTIALLY discharged (2026-08-06):** `probe-vector-draping.mjs` (Batch 830, 6 gates) ran twice on Edge. Run 1 (Batch 831) VERIFIED the core claim — draping renders on WebGPU, centroid agrees to 0.4 px, count ratio 1.014, both colour classes with the width-driven ratio in band, the grazing-view Jacobian holds, and pan-churn produces 0 console/destroyed-buffer errors — and caught a real WebGPU-only defect on the bbox leg (faint horizontal streaks a count- or centroid-based check would have passed). Root cause (Batch 834): `vectorInverse2x2` answered a **singular** UV Jacobian with the ZERO matrix, collapsing `length(screenFromUv * offsetUv) < lineWidth` to always-true for the cell's first segment; every terrain SKIRT quad is singular because `HeightmapTessellator` derives skirt UV from the unmoved edge lat/lon. WebGL only looked clean because GLSL's `inverse()` divides by zero and NaN comparisons are false — undefined behaviour — so **both** shaders now carry an explicit determinant guard and agree by construction (WebGL pixels unchanged; spec grew to 25/25). Run 2 (Batch 835) verified the fix at pixels: WebGPU nadir frame clean, RED changed-pixel count **exactly** equal on both backends, centroid delta 0.0/0.0, oblique bboxes identical. **Still owed, which is why this row stays in §C:** the bbox gate now fails on a PRE-EXISTING WebGL-side extent (`NEW-WEBGL-VECTOR-DRAPING-RESIDUAL-EXTENT`, LOW) and the non-regression gate is STRUCTURAL pending a cross-build pre-change baseline. (DEFERRED_WORK UP144-VECTOR-LAYER-WGSL)
...
- NEW-EDGE-DISPLAY-MODE-WEBGPU — ✅ SHIPPED (Batch 316, `cf7edec7dc`); now mostly-§B (residuals only). The tri-mode WebGPU model-edge path landed: `SURFACES_ONLY` default-suppression (the emitter now DOES read `model.edgeDisplayMode`), the `EDGES_ONLY` slot-12 `CESIUM_3D_TILE_EDGES_DIRECT` direct pass (now wired into the WebGPU frustum loop), and explicit `lineStrings` edges. **RESIDUALS RESOLVED (EDGE-AUTHORED-SILHOUETTE-NORMALS, 2026-07-02):** the authored-`silhouetteNormals` signed-byte path now ships — `extractEdgeGeometry` decodes the accessor exactly like WebGL `generateEdgeFaceNormals` (map `2*((v+128)/255)-1`, normalize, (0,0,1) fallback) with sequential dedupe-order pair indexing; zero normals for out-of-range pairs; adjacency derivation is skipped when the accessor is present and byte-identical when absent (probe `probe-edge-authored-silhouette.mjs`). The PR#13421 degenerate-triangle confirmation is discharged too: `probe-edge-degenerate.mjs` was repaired (esbuild-bundles the `.ts` emitter; 19-float stride) and is GREEN. (Per-edge / per-lineString `materialColor` overrides — listed in the pre-fix data-path gap (3) below — SHIPPED Batch 330, NEW-EDGE-MATERIALCOLOR-OVERRIDE-WEBGPU.) **Pre-fix gap analysis retained for context** (found by WEBGPU_PARITY_AUDIT_2026-06, post-merge v1.142): the WebGPU model-edge path (`WebGPUEdgeFramebuffer` MRT infra + `WebGPUEdgeVisibilityEmitter`, §B.6) was only PARTIALLY realized vs upstream `EdgeDisplayMode`. Two tri-mode gaps + three data-path gaps: **(tri-mode)** `SURFACES_ONLY` (the DEFAULT) must HIDE all extension edges, but the emitter gated only on `defined(edgeGltfPrimitive?.edgeVisibility)` and never read `model.edgeDisplayMode`, so any edge-bearing glTF emitted edges in the default mode (visible divergence on the most common CAD/BIM config); `EDGES_ONLY` (CAD wireframe) rendered surfaces with NO edges — the inverse of intent — because there was no `CESIUM_3D_TILE_EDGES_DIRECT` pass (slot 12) in the WebGPU frustum loop (commands binned there silently never executed), the emitter hardcoded `Pass.CESIUM_3D_TILE_EDGES`, and the surface command was emitted unconditionally with no EDGES_ONLY suppression. **(data paths)** the WebGPU extractor consumed only the per-triangle 2-bit `edgeVis.visibility` encoding and early-returned otherwise, so it was missing (1) explicit `lineStrings` edges (BENTLEY/styled-gltf-lines assets yielded zero WebGPU edges), (2) the authored `silhouetteNormals` signed-byte accessor (WebGPU re-derives face normals from adjacency → silhouette classification can diverge, and the PR#13421 degenerate-triangle repro can't be confirmed clean since the synthesized normal biases the dot-product differently), and (3) per-edge / per-lineString `materialColor` overrides (WebGPU applies one primitive-level color, no `a_edgeColor` equivalent). Files: `WebGPUModelRenderer.js`, `WebGPUEdgeVisibilityEmitter.ts`, `WebGPUSceneRendererFrustumLoop.ts`, `WebGPUSceneRenderer3DTilePasses.ts`, `WebGPUSceneRenderer.ts`. Tracked in DEFERRED_WORK NEW-EDGE-DISPLAY-MODE-WEBGPU. (NEW-EDGE-DISPLAY-MODE-WEBGPU)
...
- NEW-GROUNDPRIM-CLASSIFIER-RECON-PRECISION: flat textured GroundPrimitive materials render (Batch 185, see §B.3), but the textured classifier degrades toward the far corner — a real eye-space reconstruction-precision artifact (standard-depth `windowToEye` catastrophic cancellation near `storedDepth ≈ 0.9999997`). Stripe (1-D) hides it; Checkerboard (2-D) exposes it. **UNBLOCKED as of Batch 251** — renderer-wide log depth is LIVE (master switch TRUE; the classifier's log consumer-reverse runs, probe-classifier-scenemode at WebGL parity in all 3 modes); re-verify the far-corner case via probe-groundprim-textured-classify.mjs and close. **2026-07-10 (C7-GROUNDPRIM-TEXTURED-CLASSIFY-ZERO): the intervening "textured renders 0 px" reports (B375/B595) were a probe-harness race (settle loops exited on tilesLoaded, which doesn't cover the globe pipeline's ~1-2 s createRenderPipelineAsync — captures ran globe-less); textured classification RENDERS and converges with WebGL at HEAD (Stripe 16.7% / Checkerboard 5.2% / Grid 0.0% / Image 7.3% polygon-ROI mismatch) after 4 parity fixes (classifier PRE_MULTIPLIED blend, wrapped-inner double-dispatch gate, derivative-based Grid lines with analytic st derivatives, Image flipY) + the czm_packDepth(1.0)==vec4(0) no-surface sentinel in the WGSL depth packs.** Canonical detail in `WEBGPU_DEBUGGING_LOG.md`. (NEW-GROUNDPRIM-CLASSIFIER-RECON-PRECISION)
...
- ~~C-R9-VOXEL-CELL-PICK~~: ✅ per-cell pick RESOLVED (VOXEL-CELL-PICK-RELAND, 2026-07-02) — `Picking.pickVoxelCoordinate` decodes WebGL-byte-identical {tileIndex, sampleIndex} on WebGPU via a dedicated `fragmentPickVoxelMain` pipeline routed on `passes.pickVoxel` (WebGL VoxelFS PICKING_VOXEL packing over the VOXEL-SHAPEUV-CONVENTION sample frame); also fixed the `readCenterPixel` vertical mirror. Gate: `probe-voxel-cell-pick.mjs`. Remaining tail: **C-R9-VOXEL-CELL-PICK-TAIL** — `scene.pickVoxel` (the public API) still throws AFTER the coordinate decode because the WebGPU path never builds `_traversal` (`findKeyframeNode` TypeError), so the `VoxelCell` result object can't be constructed yet. Broader (still open): the voxel data path refines to level 1 only (Batches 474-497 + VOXEL-OCTREE-LOD 2026-07-02: real root-tile upload + default-shader gray color parity + shapeUv convention + depth-1 octree LOD atlas); deeper octree traversal / dynamic slot allocation (PARITY-VOXEL-OCTREE-TRAVERSAL), CustomShader WGSL transpilation, and the PR#13517 metadata-typed default shader remain unwired.
...
- UP144-SNAP-WEBGPU: upstream v1.144 experimental `Scene.snap` now has a WebGPU implementation (C11-212, 2026-08-02) — `WebGPUSnapPayload.ts` (single home of the WebGPU payload encoding, now an exact compact RG32Uint via `SNAP_PAYLOAD_FORMAT`; WebGL retains upstream's RGBA32F payload), `WebGPUSnapFramebuffer.ts` (occluder + payload + shared depth, device-generation lifecycle, one-frame-stale sync readback + `endAsync`), `fragmentSnapMain` in `ModelPBRComplete.wgsl`, `WebGPUModelPipelineCache.getSnapPipeline`, the two-phase occluder/payload schedule in `WebGPUSceneRendererPickPass`, and the backend-agnostic `GraphicsContext.createSnapFramebuffer()` factory. **PARTIAL (matches the C11-212 queue row):** the fixed-camera browser acceptance gate ran at Batch 813 (frozen-view surface hit proven on both backends) and the both-backend multi-frustum real-Edge gate is green (forced distinct-slice case, with TAA, clean errors); broader gates remain open — SCENE2D slice depth, genuinely moving camera/cursor across DPR/projection/viewport/edge/RTE, aperture/logical-padding, transient pooling, classification/non-Model producers. The RG32Uint **edge payload** (`UP144-SNAP-WEBGPU-EDGES`) is no longer part of that open list: it was IMPLEMENTED at Batch 821 (2026-08-02) — `WebGPUEdgeVisibilityEmitter.fragmentSnapMain` writes the two-word payload with the edge sign bit always set, the emitter takes the SAME `ensurePickId` per-primitive pick color the surface snap draws use (no parallel ID path), `ensureEdgeEmitterSnapPipeline` adds the RG32Uint variant under the add-only `ShaderSourceId.EDGE_EMITTER`, and `WebGPUSceneRendererPickPass` admits it at the TAIL of the payload phase without loosening the FORK-34 guard; Node `webgpu-snap-edge-payload.spec.mjs` 25/25. **Its own browser gate is still OWED** — an Edge snap at a model silhouette must return `isEdge: true` on BOTH backends — so the tier stays here rather than moving to §B. Shipped alongside it: the `Snapping.captureSnapView` half-pixel sample-center fix, which is SHARED scene code and therefore moved WebGL's snap results by half a drawing-buffer pixel too (correct under Principle 5, but not "WebGL byte-identical"). (DEFERRED_WORK UP144-SNAP-WEBGPU / UP144-SNAP-WEBGPU-EDGES)
...
- WGF-4-EXPAND RTE assertions in 5 of 8 camera packers — Cloud's primary march and bounded temporal
  proxy now have Campaign-13 WGS84/RTE corrections, but the shadow/mask/capture/atmosphere consumers
  remain open under `C13-06`; Ellipsoid/Splat/PointCloud/Voxel follow-ups remain pending
  (WGF-4-EXPAND)
...
- Volumetric clouds Phase 6 main render path SHIPPED via `WebGPUProceduralCloudRenderer` (Schneider HG
  dual-lobe + Beer-Powder + 3D FBM); activation is the exclusive
  `CloudCollection.renderMode = VOLUMETRIC` (default `BILLBOARD`) after the cloud-unification epic
  (Batches 617–625) — `atmosphericConditions.clouds.enableVolumetric` (which proxies onto
  `globe.defaultCloudCollection`) defaults FALSE per B15 lock; the legacy
  `globe.showProceduralClouds`/`globe.cloud*` surface was REMOVED (slice 4B). This “SHIPPED” label means
  the main feature exists, not that reconstruction is complete: the current temporal path remains a
  color-only half-resolution proxy that shades every current texel. `C13-09`, `C13-10`, and `C13-12`
  are the live attachment/true-1/16/advanced-rejection owners; `C13-06` owns the remaining
  shadow/mask/capture/atmosphere RTE work. ([Campaign 13](QUEUE_2026-07-23_CAMPAIGN13.md);
  CELESTIAL §4.6)
...
- WebGPUModelRenderer + WebGPUAutoExposure don't route through central pipeline cache (BACKLOG-§Recent). **Partly stale as of 2026-08-06:** `WebGPUComputePipelineCache` DOES exist and `WebGPUAutoExposure` holds one (`_computePipelineCache`, used at `:439`), as do `WebGPUBrdfLutGenerator` and `WebGPUClippingPolygonCollection`; Batch 825 folded `compute.module` identity into its key. What remains open is the render-side model family, which deliberately uses direct `createRenderPipeline` hatches for its snap/pick variants (`WebGPUModelPipelineCache`, `ensureEdgeEmitterSnapPipeline`) — re-scope this row before working it rather than treating the parenthetical as current.
...
- C-R12-PER-OBJECT-CACHES: PARTIAL — Point/Label normal teardown is covered; the device-loss invalidation walk still does not prove nested `model._webgpuCache`, tileset-owned models, clipping caches, label-cache variants, or replacement-device destruction/recreation.
...
- WEBGL-ASYNC-SHADER-COMPILE-LIFECYCLE — **PARTIAL; LANDED (Batch 773, 2026-08-01)** (moved out of §B by the 2026-07-31 doc-truth audit and kept in §C after landing — landing is not completion; `C11-180` is still W1 PARTIAL). **Working:** explicit opt-in only — ordinary `ShaderCache` creation remains lazy, while callers may enqueue an exact final executable or an idle preparation callback. Required compilation and speculative preparation have separate queues; preparation runs only behind the foreground queue. `KHR_parallel_shader_compile` submits without `LINK_STATUS`, polls `COMPLETION_STATUS_KHR`, and finalizes validation/reflection when ready. Unexpected first bind/getter, no extension, or no idle opportunity synchronously completes for exact legacy correctness. Camera-visible final selection follows log depth → HDR → shadow receive. Instrumentation adds real-draw `firstDrawProgram` rather than treating `useProgram` as useful work. **Attribution on the measured route (avoided work, not a certified timing win):** lazy baseline 7 programs/14 shaders with 7 blocking `LINK_STATUS` waits and 7 long tasks; rejected auto-eager scheduling 28/56 with the same 7 required waits and 21 unused programs; accepted policy 8/16, 7 drawn + 1 unused, 4 remaining blocking waits, 4 async completions, 4 long tasks. **GAP:** four structural quantization × texture-cohort first-use stalls remain, and the shadow/HDR/translucent companion families are still synchronous pending their own demand/payoff measurement; renderer-wide completion is not claimed and the counterbalanced timing gate is open. Landing (Batch 773) closed only the "uncommitted" qualifier. (C11-180 / WEBGL-ASYNC-SHADER-COMPILE-LIFECYCLE — DEFERRED_WORK)
- WEBGL bounded globe opposite-FOG companion + globe shader-wrapper ownership — **IMPLEMENTED / VERIFIED / LANDED (Batch 773, 2026-08-01)** (moved out of §B by the 2026-07-31 doc-truth audit and kept in §C after landing; the ownership half is `C11-181`, which the C11/C12 queues record as IMPLEMENTED / VERIFIED / LANDED, not complete). **Working:** prepares only the opposite fog state for zero/one-imagery-texture cohorts, and only off the hot path; exact draw selection still uses per-frame `fog.enabled`, while companion prediction uses persistent `fog.configuredEnabled` so an orbit frame may prepare descent fog. Shadow-active, translucent/OIT, debug/pick/depth, and greater-than-one-texture variants remain excluded and use the synchronous correctness path. Globe replacement is acquire-before-release; release clears the shared wrapper and unschedules its derived tree so a stale culled tile cannot reuse a released program. `globeShaderRequests` exposes camera/fog/cohort/final-variant/queue evidence. Clean r3 route PASS (1,175 frames; CPU p95 7.43 ms; p99 10.152 ms; four long tasks) and the nine-waypoint WebGL/WebGPU/diff set is visually clean — reported measurements, not a before/after win. **GAP:** four structural quantization × texture-cohort first-use stalls remain. The changeset landed as Batch 773 with tsc/`gulp build`/Node contracts green at the tip; the browser-owned and counterbalanced-timing gates above are still open. (C11-180 bounded lane / C11-181 — DEFERRED_WORK `WEBGL-GLOBE-SHADER-VARIANT-EVICTION-REFERENCE`)
...
- Procedural textures for globe (cloud layers, aurora) (BACKLOG-§9) — aurora scope superseded by `EPIC-AURORA-SPACE-WEATHER` / [QUEUE_2026-08-02_CAMPAIGN15.md](QUEUE_2026-08-02_CAMPAIGN15.md) (unlaunched)
- Terrain blend/splat mapping multi-texture close-range (BACKLOG-§9)
- Weather: aurora borealis, sandstorm/dust, lightning, wet surfaces (BACKLOG-§9) — the aurora item is superseded by `EPIC-AURORA-SPACE-WEATHER` / [QUEUE_2026-08-02_CAMPAIGN15.md](QUEUE_2026-08-02_CAMPAIGN15.md) (unlaunched); the rest stays here
...
- ~~Tide simulation — non-goal (offset injection only) (WATER §1)~~ — **NON-GOAL REVISED 2026-07-24 by maintainer ruling T1** (`TIDES_FEASIBILITY_2026-07-24.md` §5a). The in-engine EQUILIBRIUM tide lands in Core (`Core/TideModel.js`, Batch 763 — see §B); REGIONAL PREDICTION (harmonic atlases, station APIs, basin hydrodynamics) remains the non-goal. `scene.globe.water.tideCallback` still lets an application substitute its own source.
