...
- MapMode2D / SceneMode / SceneTransitioner — 2D/Columbus/3D modes + transitions
...
- WebGPUDerivedCommand — derived-command variant management (depth-only, pick, translucent) for WGPU (SHIPPED)
...
- WebGPUBillboardRenderer — Billboard collection on WGPU (SHIPPED)
- WebGPULabelRenderer — SDF text label collection (SHIPPED)
...
- WebGPUGroundAtmosphereRenderer — ground-atmosphere fog applied per primitive (SHIPPED)
...
- WebGPUTAAEffect — TAA with RTE depth-reprojection motion vectors + history validity gate (SHIPPED Slice 1+2; per-model MRT 2c-2e WIP)
...
- Tools/variant-smoke-test.mjs — Playwright-based smoke test for each variant (SHIPPED)
...
- BUG-3 2D / Columbus View modes — ⚠️ STILL OPEN (Columbus View works; SCENE2D blank). REFINED DIAGNOSIS (Batch 214, runtime-instrumented — supersedes the multi-agent workflow's BV theory, which was DISPROVEN):
  - The workflow claimed the cause was the WebGPU globe command builder attaching no 2D bounding volume (Batch 167 set `boundingVolume: non3D ? undefined`). Attaching the WebGL planar BV (`fromRectangleWithHeights2D`) DID fix the frustum binning (numFrustums 9→4, withBV 0→3, matching WebGL) but did NOT fix the blank — because **WebGL also smears tiles across all bands** (15 cmds × 4 bands = 60 inserts) and renders fine. So the BV was a real Batch-167 divergence but NOT the blank's cause. (BV change reverted; re-apply separately if desired.)
  - REAL cause (runtime-confirmed via per-pack mvp dump): the globe camera UB's 2D `modifiedModelViewProjection` is packed with a **corrupted orthographic projection** for some packs — `proj0` balloons ~30× (8.9e-8 → 2.66e-6) and `proj12` shifts to ~-28, mapping tiles to NDC X ≈ -12 (far off-screen left → the observed left-edge sliver). Most tiles pack with the correct ortho (NDC tiling -1.0→1.23, on-screen) yet the final render is still blank, so the corrupted-ortho packs are overwriting/winning at draw time. The 2D OrthographicOffCenterFrustum width tracks camera height, and the per-band camera-Z compress (`WebGPUSceneRendererFrustumLoop.ts:221`) drives the height tiny → the ortho zooms in ~30×. (Note: an explicit post-loop `position.z` restore did NOT change the result — the corruption is within-frame, not cross-frame drift; and the `Matrix4.setDepthRangeType("webgpu")` SCENE2D guard is already present.) NEXT: trace why specific packs see the zoomed ortho (per-band repack ordering vs the shared globe camera UB) and prevent the 2D ortho width from being re-derived from the compressed per-band camera height. probe-2dcv-verify.mjs. (BACKLOG-§1)
