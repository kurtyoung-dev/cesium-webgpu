...
- WebGPUResidentInstanceBuffer — resident CPU instance array + GPU vertex buffer + stable `_index→slot` map with full-vs-partial sync predicate, dirty-range coalescing (gap ≤ 4 slots; >40% changed-fraction whole-buffer fallback), and slot-aligned velocity prev mirror; O(changed) partial `writeBuffer` for sparse-dynamic collections (Batch 229 — NEW-RESIDENT-INSTANCE-BUFFER-MGR; consumed by WebGPUBillboardRenderer, Point/Label wiring tracked as NEW-PARTIAL-WRITE-WIRE-BPL) (SHIPPED)
...
- WebGPULabelRenderer — SDF text label collection (SHIPPED in 3D as of Batch 219 — glyph-atlas/BV fix via `BillboardCollection.prepareForFeatureRenderer`; bug 2 + 2D/CV remain — see DEFERRED_WORK WEBGPU-BILLBOARD-POINT-LABEL-NO-RENDER)
- WebGPUPolylineRenderer — Polyline collection with arrow/dash/glow/outline materials (SHIPPED in 3D; renders on WebGPU. 2D/CV/morph still SceneMode-unaware — see DEFERRED_WORK MORPH-POLYLINE-COLLECTION-2D)
...
- WebGPUOrbitalCatalogRenderer — GPU-resident orbital catalog (Phase 3 of the Large Dynamic Objects roadmap, Batch 230). Element catalog uploads once to a storage buffer; `OrbitalPropagate.wgsl` compute propagates circular orbits per frame into an RTE high/low positions buffer; instanced draw vertex-pulls `positions[instance_index]` — positions never leave the GPU, per-frame CPU upload is one time scalar + camera UB. Scene API: `OrbitalCatalogCollection` (FR key 49, `_consumeDirtyState` discipline). Verified via `probe-orbital-catalog.mjs` (2000 objects render + move, 0 errors); Sandcastle "WebGPU Orbital Catalog". (SHIPPED circular-orbit MVP; J2/SGP4 kernels, GMST Earth rotation, WebGL2 SGP4-WASM fallback, picking, motion vectors tracked in DEFERRED_WORK NEW-ORBITAL-*)
...
- OrbitalPropagate.wgsl + OrbitalCatalogRender.wgsl — circular-orbit catalog propagation (one invocation/object, workgroup 64) + storage-buffer-vertex-pull instanced point render (Batch 230) (SHIPPED — MVP; low part of the RTE output is 0 until the df64 kernel upgrade)
