...
- UP144-SNAP-WEBGPU: upstream v1.144 experimental `Scene.snap` now has a WebGPU implementation (C11-212, 2026-08-02) — `WebGPUSnapPayload.ts` (the RGBA32F encoding's single home), `WebGPUSnapFramebuffer.ts` (occluder + payload + shared depth, device-generation lifecycle, one-frame-stale sync readback + `endAsync`), `fragmentSnapMain` in `ModelPBRComplete.wgsl`, `WebGPUModelPipelineCache.getSnapPipeline`, the two-phase occluder/payload schedule in `WebGPUSceneRendererPickPass`, and the backend-agnostic `GraphicsContext.createSnapFramebuffer()` factory. WebGL snap is byte-identical. **Still WIP:** the browser acceptance gate (snap hit on a model on both backends) is unrun, and WebGPU edges carry no pick ID so every snap hit is `isEdge:false` — `UP144-SNAP-WEBGPU-EDGES`. (DEFERRED_WORK UP144-SNAP-WEBGPU)
