...
- WebGPUCSMRenderer — CSM Slice 1+2a; RTE-precise per-cascade VPs + slope-scaled bias; soft-shadow 3x3 PCF box kernel (Batch 289, `softShadows`/`pcfRadius` config + `effects.csmControl.y` → `sampleOneCascade` in all 25 receivers, matches WebGL `czm_shadowVisibility` USE_SOFT_SHADOWS). Batch-297 retest (post-cast-fix): PCF VERIFIED softening the primitive self-shadow edge (hard-vs-soft A/B diff 0.000%→6.551%, 0 device errors); end-to-end globe-receiver soft-shadow closeout still gated on NEW-CSM-GLOBE-RECEIVE-PROJECTION-MISS (the globe ground does not receive the cast shadow). See NEW-CSM-SOFT-SHADOW-PCF / NEW-CSM-GLOBE-RECEIVE-PROJECTION-MISS (SHIPPED code; PCF verified on self-shadow; globe-receive end-to-end gated)
- WebGPUCSMCastPass — dedicated cast pass for all 7 shadow cast variants (SHIPPED; Batch 296 FIXED the cast no-dispatch: `SceneRenderer.executeShadowMapCastCommands` now populates `passes[].commandList` for BOTH backends before delegating, and appearance-primitive commands carry `rte24` cast metadata → `_castDispatches` > 0, caster self-shadows. Globe-terrain RECEIVE-side projection miss remains: see NEW-CSM-GLOBE-RECEIVE-PROJECTION-MISS) (SHIPPED cast; receive WIP)
