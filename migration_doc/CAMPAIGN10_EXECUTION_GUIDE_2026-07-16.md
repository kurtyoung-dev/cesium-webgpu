...
| **Pick-fleet convert-back (reversed-Z reconciliation, added 2026-07-18 by C10-11 Phase-0)** | The `C10-11` pick-fleet log-depth conversion (currently **BLOCKED** — one producer, the voxel, has no log-depth infrastructure; see `DEFERRED_WORK.md` `NEW-WEBGPU-PICK-FLEET-LOG-DEPTH` Phase-0 correction) will, when it lands, ADD ~24 pick `frag_depth` entries to slice-b's convert-back surface (making it 71 color + ~24 pick). **Per `C11-GT-01`/`C10-13`, the reversed-Z spike GO/NO-GO verdict must be recorded in BOTH `NEW-WEBGPU-PICK-FLEET-LOG-DEPTH` AND this dossier AND `DEFERRED_WORK.md` before the pick-fleet log-depth is treated as PERMANENT.** If reversed-Z goes GO, the pick fleet converts to reversed-Z f32 together with the color surface; the two streams must resolve to one depth convention and never land in the same slice. | — |
