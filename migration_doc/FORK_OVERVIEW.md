...
- **Headline NEW capabilities** — clustered (Forward+) lighting, CSM (cascaded shadow maps), TAA, IBL + spherical-harmonics, **C2-25 dynamic scene-content reflections**, volumetric clouds + weather + atmospheric effects (driven through the `CloudCollection.renderMode = VOLUMETRIC` API since the cloud-unification epic; `globe.showProceduralClouds` removed — see §4), snapshot mode, compute pipeline. See §4.
...
**Post-campaign audit issues (2026-07-03) — ✅ ALL FOUR RESOLVED by Batches 509–530 (reconciled 2026-07-04):**
- **Voxel pick ↔ octree composition (HIGH)** — ✅ **depth-1 RESOLVED (Batch 509).** `fragmentPickVoxelMain` now descends into the level-1 child octant on refined frames (childCoord/childSlots lookup, tileUv rescale, atlas z-offset, `megatextureId = packVoxelIntToVec2(tileSlot)`) and has a `//>>ifdef VOXEL_USER_CUSTOM_SHADER` pick branch, so pick composes with the color march and honors user-shader alpha. **Residual = reach the color march's L3 traversal (B552) + share one routine → queued as Q9.**
- **Below-surface/limb darkening (HIGH)** — ✅ **RESOLVED (Batches 510/512/513).** Root cause was water masks never uploading (`computeEnhancedOcean` ocean-shaded whole land tiles), not a diffuse atmosphere darkening; both probes now PASS under un-loosened limits (underground 1.43/4.28% vs 8; translucency 4.00/0.54% vs 11.9). The surviving thin **far-zoom limb ring** is the only atmosphere-brightness residual → Q23.
- **Model SCENE2D shading (MEDIUM)** — ✅ **RESOLVED (Batch 530, `NEW-MODEL-IBL-AMBIENT-RELAND`).** The olive-vs-blue-gray 2D tint was the model IBL-ambient path (radiance-cube lookup flip + procedural-sky scattering port); `probe-model-scene-modes` 2D 34.42 → 11.85 GATE PASS.
- **PP pre-tonemap ordering (MEDIUM)** — ✅ **RESOLVED (Batch 511, `NEW-PP-LIBRARY-TONEMAP-ORDER`).** Library builtins + user WGSL stages moved to run *post*-tonemap matching WebGL (`tonemap → _stages → fxaa`), plus the plain-HDR tonemap gate + exact Khronos PBR-Neutral WGSL port; the stale header docstring + two wrong comments were corrected. Off-gate byte-identical with no user/library stages + HDR off.

**Known-incomplete (the standing tail):**
- **Limb-ring / atmosphere brightness** — below-surface darkening + limb-width are now ✅ RESOLVED (B510/512/513); the surviving residual is the thin **far-zoom limb ring** (~+10 GPU-brighter, 7.5% of far-view residual) → queued as Q23.
- **f16 post-process device-verify** — the B478 opt-in f16 variants still need an on-device `shader-f16` verification pass.
- **Deeper voxel octree levels** — depth-1 (B501) then depth-3 (B552, static full atlas) SHIPPED; levels ≥4 (flat-slot impractical → needs the upstream node-table port) + a dynamic L3 pool remain (BY-DESIGN, device+demand gated).
