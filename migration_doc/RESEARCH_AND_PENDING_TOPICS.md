...
> Supersedes: `FUTURE_RESEARCH_2026_05_01.md`, `RESEARCH_TAKRAM_GEOSPATIAL_VISUALS.md`, `CLOUD_RENDERING_STRATEGY.md` (the completed cloud-rendering research portion), `OPTION_B_SCENE_IN_WORKER.md`, and the long-tail research recommendations in `archive/audits-2026-04-30/2026-04-30_ARCHITECTURE_PERFORMANCE.md` (archived 2026-09-03; see `ARCHITECTURE_REVIEW_2026-09-02.md` §3.9/§3.13). Cloud *taxonomy* roadmap (`CLOUD_TAXONOMY_ROADMAP.md`) is cross-referenced, not fully folded.
...
- **R-2b — Unified feature-id texture** (source-agnostic per-fragment feature IDs for post-process). **CORE PRIMITIVE SHIPPED (2026-07-05, C4-UNIFIED-FEATURE-ID-TEXTURE)**: the Batch-133 pick pass already rasterizes a per-object ID for EVERY source (globe/3D-tile/model/voxel/splat) into one shared rgba8 target — that IS the unified per-fragment feature-ID texture. It was only ever read back on the CPU; R-2b's enabling gap was that it was never shader-samplable. Now `WebGPUPickFramebuffer.resolveFeatureIdRecolorAsync()` runs `FeatureIdResolve.wgsl` (fullscreen PP pass) over that target — `textureLoad`s the ID, decodes the key on the GPU (same little-endian order as `Color.fromRgba`), and hashes each distinct feature to a color. **CORRECTION (AR-751, 2026-09-04): the "24-bit" stated here and above was WRONG when written.** The key is a uint32 across all four bytes — alpha carries bits 24-31, and `WebGPUPickFramebuffer`'s CPU decoder has folded all four in with `Color.bytesToRgba(r, g, b, a)` all along. The shader shipped reading r, g, b only, so two ids differing only above bit 23 recolored identically and every id that is a multiple of 2^24 decoded to 0, which the shader paints as background. AR-751 widens `FeatureIdResolve.wgsl`, `PickId.normalizedRgba` and `GraphicsContext#_pickColorToKey`'s byte-object branch to the full 32 bits; the record here is amended in place, not deleted. Probe `probe-feature-id-texture.mjs` PASS: a billboard + a point (two distinct source renderers) resolve to two distinct non-black in-shader colors (bb `[177,121,55]`, pt `[98,243,110]`), deterministic, 0 GPU errors. Default-OFF (helper is lazily constructed; nothing calls it in normal flow → byte-identical). **Residual R-2b follow-ups** (not yet done): per-frame wiring of the ID G-buffer into the STANDING PP pipeline (today the pass runs on-demand off the last pick render), a source-tag channel, and the imagery per-texel ID channel — all needed before the R-2a join. *(Note: upstream's feature-ID-texture transform fixes are per-model glTF feature IDs, a different, source-local thing.)*
...
- **Off-thread decode — RESOLVED (2026-07-05, NS-MVT-SYNC-DECODE-OFFTHREAD-WASM).** `MVTDataProvider` now accepts an opt-in `decodeInWorker` option (**default false → byte-identical synchronous path unchanged**). When enabled, `decodeMVT` + `buildVectorGltfFromMVT` run on a `TaskProcessor` web-worker pool (`Workers/decodeMVTTile.js` via `Scene/MVTTileDecoder.js`), moving PBF decode + polygon tessellation off the main thread. The input `ArrayBuffer` is deliberately not transferred so a synchronous main-thread decode remains a robust fallback when the worker pool is saturated (`scheduleTask` → `undefined`), the worker is unavailable, or the task rejects. This is a **pure-JS worker (no WebAssembly)** — the Rust/WASM `mvt-reader` accelerator remains the only still-open R-4 sub-angle (a speculative perf delta, not a correctness gap). Verified by `Tools/visual-regression/probe-mvt-worker-decode.mjs`: with `decodeInWorker:true` a dedicated `decodeMVTTile` worker is spawned on both backends, the tile renders identically (1 feature, `geometryByteLength=108`, 0 errors) and the WebGL↔WebGPU diff stays 0.00%; the default-off path stays at 0.00% via `probe-mvt-datasource-parity.mjs`.
...
From `archive/audits-2026-04-30/2026-04-30_ARCHITECTURE_PERFORMANCE.md` (archived 2026-09-03; corrected re-derivation at `ARCHITECTURE_REVIEW_2026-09-02.md` §3.9/§3.13), the long-tail research / structural items that are research-shaped rather than mechanical:
...
| R-2b | Unified feature-id texture | Plan | **CORE + STANDING-WIRING PRIMITIVE SHIPPED (2026-07-05)** — shader-samplable via `resolveFeatureIdRecolorAsync` + `FeatureIdResolve.wgsl` (B587); residual (a) standing per-frame **record-into-encoder** primitive landed (`WebGPUFeatureIdTexture.record()` + `WebGPUPickFramebuffer.recordFeatureIdResolve()`/`featureIdRecolorView` — records the recolor into a caller's per-frame command encoder, no separate submit/readback, persistent re-sampled output). Remaining residual = (a2) actually invoking it per-frame inside the standing PP pipeline (needs a per-frame pick render / debug-overlay driver) + (b) source-tag channel + (c) imagery per-texel ID channel | probe-feature-id-texture.mjs |
...
| R-4 | Rust/WASM MVT vector basemap | Plan | **SUPERSEDED** (loader); WebGPU parity **CONFIRMED** (0.00% diff, 2026-07-05); off-thread decode **SHIPPED** (opt-in `decodeInWorker`, 2026-07-05) | upstream `MVTDataProvider.js` merged; parity via `probe-mvt-datasource-parity.mjs`; off-thread JS worker (`decodeMVTTile.js`/`MVTTileDecoder.js`) via `probe-mvt-worker-decode.mjs`; only Rust/WASM accelerator + Mapbox-GL-Style paint-property delta remain open |
