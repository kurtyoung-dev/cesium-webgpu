...
## R-2026-08-29-3 - Developer-experience wave: organisation, decomposition, deduplication

**Ruling (maintainer, verbatim):** "yes lets do all of the organization, decomposing, deduplication, and other improvements. Lets queue these up to run next." Given in answer to the seat's census (937 top-level tooling files of which 682 launch their own browser and 91 define their own sha256; eleven engine files over 1,000 lines against the decomposition rule).

**How it binds.** WAVE DX in `QUEUE_2026-08-29_RESEARCH_DISPATCH.md` �6a is queued NEXT after the in-flight Wave 1 rows. The ruling also answers the five `TOOLING_CATALOG.md` questions pending since 2026-08-15, each per the catalog's recommendation: M1 B for the HIGH set and B-after-grep for MED; M2 C; M3 A now / B as the follow-up / C rejected; M4 B with the re-audit folded into the wave-end gate; M5 B. Every DX row is a no-behaviour-change row whose acceptance is byte-identity on both backends via the wave-end gate. Two new gates: M-DX-1 (spec-runner home names) and M-DX-2 (ledger rotation).
