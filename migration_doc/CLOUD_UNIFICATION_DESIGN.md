...
|---|-------|----------------|-----------|
| 1 | **API scaffold** — `CloudVolumetrics` class + `CloudCollection.enableVolumetric` / `.volumetric` / `.cloudType` (stores only) + `CumulusCloud._cloudType` (index 7) + `context.requestVolumetricClouds` no-op base | **None** — pure additive stores | Zero-risk surface; unblocks everything |
