...
4. **`globe.showProceduralClouds` was REMOVED**, not kept as a deprecated alias — the user decided REMOVE-not-alias (slice 4B, 2026-07-05); section 2.5 above (`:109-111`) supersedes the original "deprecate + alias" plan this bullet described. <!-- corrected 2026-09-12, R-2026-09-12-10: this bullet originally read "`globe.showProceduralClouds` keeps working as a deprecated alias routed through the same publish path — existing globe-driven volumetric scenes render identically until users migrate." That inverted a fact: the property was REMOVED, not aliased. -->
