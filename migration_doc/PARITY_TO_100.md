...
> **Residual (tracked `NEW-BUFFERPOLYLINE-2D-EXTRUSION` in DEFERRED_WORK):** ✅ **CLOSED (2026-07-03).** The extrusion-collapse hypothesis was wrong — the SCENE2D blank was a **culling bug** (commands carried the raw ECEF bounding sphere while positions were reprojected; the ortho 2D culling volume rejected it). Fixed by scene-mode-aware command BVs (`computeBufferModeBoundingVolume`: projectTo2D for 2D/CV, union for morph) across all three Buffer\* renderers; `probe-buffer-2dcv-parity` now asserts polyline presence + width in SCENE2D. See DEFERRED_WORK entry for full evidence.
