...
| PARITY-EDGE-DEGENERATE-TRI-VERIFY | Edge visibility: PR#13421 degenerate-tri parity verification (probe-only debt) | S | PARITY-EDGE-AUTHORED-SILHOUETTE-NORMALS | `Tools/visual-regression/probe-edge-degenerate.mjs` | `probe-edge-degenerate.mjs` exit 0; 4 parity checks pass (zero-edge valid, WebGL/WebGPU classify-match, no NaN/Inf, silhouette dot parity) | No clipping/edge in default scene; behavior identical to WebGL when off |
| PARITY-HDR-COLORGRADING-MATH | ColorGrading HDR-aware math: clamp/skip input when tonemap bypassed | S | — | `Shaders/WebGPU/PostProcess/ColorGrading.wgsl` (L71–74 pivot, L114–116 windows), `WebGPU/WebGPUPostProcessPipeline.ts` (L321–327 `_skipSDRStagesForHDR`) | `probe-hdr-colorgrading.mjs` (new) — HDR canvas + ColorGrading vs SDR baseline | No-op passthrough / skip when HDR canvas on (parallels FXAA skip L1378); SDR unaffected |
| PARITY-HDR-FXAA-THRESHOLDS | FXAA HDR-aware edge detection: clamp/skip when tonemap bypassed | S | — | `Shaders/WebGPU/PostProcess/FXAA.wgsl` (L19–21 thresholds, L56–69), `WebGPU/WebGPUPostProcessPipeline.ts` (L750–766, skip L1378) | `probe-hdr-fxaa.mjs` (new) — HDR scene + fine edges, verify no over-blur | Already skips when `_skipSDRStagesForHDR`; polish to re-enable with corrected thresholds OR confirm skip acceptable |
...
| PARITY-EDGE-AUTHORED-SILHOUETTE-NORMALS | Edge: authored `silhouetteNormals` signed-byte accessor path | M | — | `WebGPU/WebGPUEdgeVisibilityEmitter.ts` (re-derives L537–596; guard L576–578), `WebGPU/Model/WebGPUEdgeVisibilityBatch.ts`, `Tools/visual-regression/probe-edge-degenerate.mjs` | `probe-edge-authored-silhouette.mjs` (new) — authored normals, WebGPU/WebGL same edge classification + colors | Falls back to derived normals when accessor absent; edge display opt-in |
