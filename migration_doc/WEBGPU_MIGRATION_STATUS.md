...
Twelve commits over three days (2026-05-26 → 05-28) advancing the ground-primitive / classification arc, plus a 27-agent triage workflow over the whole deferred backlog. Full per-batch detail + the roadblock analysis is in **NEXT_SESSION_HANDOFF.md** (2026-05-28). Headline outcomes:

- **Flat-color ground classification now renders in ALL scene modes.** Batch 167 fixed the WebGPU globe blanking at regional 2D zoom (a 3D-ECEF bounding-volume culled by the 2D projected frustum). Batches 169-170 then removed the over-broad `_needs2DShader` skip and fixed the last 2D blocker: a coordinate-frame mismatch where `position2DHigh/Low` `(projX, projY, height)` was subtracted against `camera.positionWC`'s ENU frame `(altitude, projX, projY)`. Fix = a mode-conditional `.zxy` swizzle in `colorVS`/`vsVelocity` matching WebGL's `czm_translateRelativeToEye(.zxy)` convention. Probe-verified: SCENE2D 20781 px / Columbus View 14574 px vs WebGL 20787 / 15484, 0 device errors.
- **Textured-material classification: 3 of 4 layers built, blocked on the 4th.** Batch 171 landed the dispatch infrastructure (UBO 384→640, `applyMaterial` for Color/Stripe/Checkerboard/Grid, `surfaceUV` planar+spherical). Batch 173 added the per-slice frustum-state UBO + corrected `windowToEye` (Batch 171 wrongly applied `reverseLogDepth` to linear NDC depth). Batch 174 fixed frustum-slice distribution (mode-aware bounding volume so the command lands in the slice containing its surface). Color renders; Stripe/Checkerboard/Grid still flat — blocked on **globe depth precision**: the WebGPU globe writes hyperbolic NDC depth into an RGBA8 texture, so a 350 km surface in a wide frustum slice reconstructs to ±50-90 km eye-z error. The proper fix is **renderer-wide log depth** (foundational, all-or-nothing — every pipeline emits `frag_depth` log + every consumer reverses, since all geometry shares one depth buffer). Tracked as NEW-WEBGPU-GLOBE-CLASSIFY-DEPTH-PRECISION.
- **Triage workflow (Batch 172) — the backlog was mostly doc-drift.** 27 agents triaged the 19 open DEFERRED_WORK items; 6 were already fully implemented (adversarial verify confirmed each producer→consumer chain) and just needed striking. Probe-first earned its keep: C-R9-MODEL-FEATURE-PICK's code-level verify said "resolved," but the probe showed the b3dm tileset doesn't visibly render → re-scoped (infra done, residual b3dm content-render gap), not falsely closed. Total: 8 entries retired/corrected.
- **Cheap independents (Batch 175-176):** split-screen clock sync (VR3-SPLIT-SCREEN-CLOCK-SYNC, Playwright-verified — panes no longer drift); outlines-on-terrain warning closed as upstream parity; NEW-GBUFFER-MRT-INTEGRATION producer side marked shipped (176 shaders emit `@location(1)`).
- **Vector3DTile 2D/CV (Batch 178) — implemented, e2e-UNVERIFIED.** Ported the verified Batch 170 reprojection to `WebGPUVector3DTilePrimitiveRenderer` (CPU-reproject RTC positions to the ENU 2D frame, mode-branch the center, mode-agnostic VS). Builds, 0 device errors across modes, but NOT visually verifiable: the repo has no `.vctr` sample tileset and the internal classes aren't bundle-exported. Modern sample vector tilesets use the glTF-vector `BufferPolygon` path instead — which surfaced an **active compile bug** (`BufferPolygonMaterial.wgsl` `#import` reaches the WGSL compiler unresolved → `#` invalid token; NEW-WEBGPU-BUFFERPOLYGON-WGSL-IMPORT). That bug is the recommended next item (verifiable, on the path real vector tilesets use).

**Roadblocks (now precisely documented):** (1) textured ground materials → renderer-wide log depth; (2) Vector3DTile 2D/CV verification → no `.vctr` test data; (3) the BufferPolygon `#import` compile bug blocks the modern vector path. None are regressions — all are newly-surfaced architectural depth that the 3-day push drove down to.

## Recent Progress (2026-05-13 — Session 65 Batches 37-43: camera/rasterization bug cluster, ground-atmo proper integration, Phase 4/6 wiring, doc accuracy sync)

Eight commits today (Batches 37, 38, 39, 40, 41, 42, 43 + a gitignore cleanup + 4 doc-only follow-ups) closing four deferred entries end-to-end:

- **NEW-VR2-1-LOW-ALT-TERRAIN-LOAD** (Batch 41) — Two stacked root causes shipped together: (1) `computeModifiedModelView` was passed a `GlobeSurfaceTile` whose `.center` property doesn't exist, so the helper's `if (!center) return new Float64Array(view)` fallback fired on every globe tile draw → `v_positionEC = view × position_tile_local` produced a `v_distance > 100 km` even at 100 m camera altitude → `computeFog` saturated to 1.0 at every pixel and replaced imagery with a flat fog color. (2) FOG branch's `nightFogDimming * 0.05` factor + `fogMinimumBrightness` floor was ungated; WebGL's equivalent darken is gated on `enableLighting && DYNAMIC_ATMOSPHERE_LIGHTING` so default-config demos (Bloom, Particle System) saw uniform `24/255` fill. Fix: pass `mesh.center` (matching WebGL `rtc = mesh.center`) and AND-gate the darken on `atmosphereParams.w > 1.5`. Bloom WebGPU went 35.9 % → 73.8 % colored (WebGL 69.6 %); Particle System 4.6 % → 68.1 % (WebGL 71.2 %); 3D Tiles Photogrammetry 65.6 % → 87.0 % (WebGL 86.6 %); Bathymetry 79.4 % → 90.7 % (WebGL 90.7 %).
- **NEW-VR2-3c-DISK-EXTENT-DRIFT** (Batch 40) — `Viewer.createAsync` created the temp `CesiumWidget` inside a `style.display = "none"` container; hidden ancestors zero `clientWidth/clientHeight`, so the synchronous Scene + Camera constructor inside that widget read a 1 × 1 canvas and set `frustum.aspectRatio = 1.0` instead of the real `1.333`. `rectangleCameraPosition3D` then placed the WebGPU camera 25 % closer to Earth than WebGL. Fix: replace `display: none` with `position: absolute; inset: 0; visibility: hidden` so the canvas inside has correct layout dimensions when Camera constructs. All 13 camera / canvas / frustum state probe fields now bit-perfect identical.
- **NEW-VR2-3b-LIMB-HALO-OVERBRIGHT** (downgraded to MOSTLY RESOLVED by Batch 40) — Once VR2-3c shipped, the limb sample points lined up between backends and the apparent over-bright reading collapsed to within noise. Sentinel-2 on-disk delta is now bit-perfect; ~25 px residual disk-edge offset is sub-pixel rasterization detail.
- **NEW-ORBIT-PHASE-4-ATMOSPHERIC-CONDITIONS-FINISH** (Batch 42) — Final missing leaf was `windSpeed`/`windDirection`. Pre-emptively added `windDirectionAndSpeed: vec4<f32>` slot to the SkyAtmosphere UBO (`UNIFORM_BUFFER_SIZE` 256 → 272) so Phase 5/6 advection consumers can bind without a layout rebuild.

Plus three feature wirings (no deferred-entry mapping):

- **Batch 37** — Moon render-bundle MSAA sampleCount mismatch fix. Phase 5 (a-d) MSAA bridge re-enable (Batch 36) had inadvertently caused the Moon pipeline (built at sampleCount 4) to mismatch its bundle encoder (defaulted to 1) → `executeBundles` rejected the entire command buffer → 3D Tiles Photogrammetry regressed to 81.6 % black. Threading `context._msaaSamples` through the bundle descriptor + cache key fixed it.
- **Batch 38** — Ground-atmosphere proper integration. Retired the Batches 30-31 empirical `cap=1.5 × scale=0.15` workaround once the actual cause (FS `viewDir` evaluating to `normalize(v_positionMC)` = surface outward normal instead of camera-to-surface direction; broke the Henyey-Greenstein phase function on sunlit fragments) was located. Disk-bleed deltas dropped from `(-56, -59, -57)` to `(-12, -11, -17)` at the center sample.
- **Batch 39** — Auto-exposure altitude gate (pairs with bloom altitude gate from Batch 22). Blends the adaptive `1/avgLuminance` multiplier toward 1.0 (neutral) above `altitudeGateMinMeters` so the bright limb at orbit doesn't crush daylight terrain. Mirrors Frostbite/Karis convention.
- **Batch 43** — `atmosphericConditions.clouds.enableVolumetric` toggle wired. Pre-Batch-43 the flag was a plain field that did nothing; now aliases `globe.showProceduralClouds` so the canonical AtmosphericConditions API toggles the existing Schneider-style volumetric cloud raymarcher. (The renderer itself, despite its "procedural" name, has been a full volumetric raymarcher with HG dual-lobe + Beer-Powder + 3D FBM + light-marching since it landed.)

**Doc accuracy sync (commits `47ddffb4e3` + this commit):** `FEATURE_INVENTORY.md` had `WebGPUVolumetricFogRenderer`, `WebGPUProceduralCloudRenderer`, `VolumetricFog.wgsl`, `VolumetricFogComposite.wgsl` all tagged `SCAFFOLDED` despite shipping months ago. `CELESTIAL_ATMOSPHERE_DESIGN.md` Phase status table marked Phase 5/6 as "Not yet started." Both corrected to reflect actual shipped state. Without this sync, future sessions would either re-derive completed work or treat shipped features as risky/unverified.

**Remaining Phase 6 work** (estimated 1-2 sessions total, down from the original "3-5 sessions" estimate which was based on the stale inventory):

- **6c — Cloud shadows in volumetric fog froxel** (~150 LOC WGSL + ~50 LOC TS, ~1 session). Inject cloud-density extinction into the existing `VolumetricFog.wgsl::sampleSunShadow` term so volumetric fog god rays see cloud-cast shadows. Requires copying `cloudDensity` + `fbmNoise` from `ProceduralClouds.wgsl` into `VolumetricFog.wgsl` and plumbing 6-8 cloud uniforms from `frameState` into the fog UBO per-frame.
- **6d — Quality dial** (~0.5 session). Expose `atmosphericConditions.clouds.volumetricQuality` as `"low" | "medium" | "high" | "auto"`; renderer maps to `maxSteps` / `lightSteps`.
- **6b — High-altitude 2D fast path** — re-evaluate after 6d ships. The original design called for a separate cheap 2D shader at ≥100 km; the existing renderer's `maxSteps` is runtime-tunable so 6d's "auto" mode can drop sample counts at high altitude without a second shader.
