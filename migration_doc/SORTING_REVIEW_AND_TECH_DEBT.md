...
| **SORT-3** | MaterialSortIdAllocator only runs inside unused RenderScheduler.binCommand | No material batching (materialSortId always 0) | Depends on opaque sort | 🟡 **DEFERRED** — Requires adding opaque command sorting (currently CesiumJS does not sort opaque commands). materialSortId field is ready in comparators. |
