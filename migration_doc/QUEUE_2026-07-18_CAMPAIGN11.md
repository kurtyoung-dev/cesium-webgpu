...
| `C11-157` (OIT translucent-primitive wiring) | **PARTIAL (Slice A: PRIMITIVE family DONE, 2026-07-18)** | §1.23 / G1/G3 | **TOP of W1**; absorbs `C11-91` body-wash; Batch-700 fallout. **Slice A**: translucent PRIMITIVES (flat single-`@location` PrimitiveBasicColor path + LIT `FragOutput`-struct PrimitivePhongColor path via new `injectOITOutput` struct branch) now REACH MRT-OIT accumulation — `_webgpuOITActiveThisFrame`=TRUE, 0 validation errors, WebGPU OIT-on now **1.33%** from WebGL OIT-on (was 10.33% @ Batch-700). Evidence: `probe-oit-primitive-reachable.mjs` (lit+flat PASS), updated `probe-oit-transparency.mjs` oracle-c PASS, regression net green (ellipsoidprim 0.499 single-blend, globe-translucency 3/3, splat-sort, capture-and-diff globe-default 0.46%). FAR-003 stays DEFAULT-OFF (reachable, not default-on). **Slices B/C REMAIN**: COLLECTION + MODEL translucent OIT wiring. Runs at `msaaSamples=1` (MSAA×OIT accumulation = `NEW-WEBGPU-OIT-MSAA-RESOLVE-ORDERING`). Weight desaturation follow-up = `NEW-WEBGPU-OIT-WEIGHT-LINEAR-DEPTH` |
