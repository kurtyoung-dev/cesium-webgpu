...
| `C11-12`, `C11-14 … C11-19`, `C11-21 … C11-25` (standing-reds) | NOT STARTED | G1 §B (C11-13 → G6 A1) | evidence-pending. **UPDATE 2026-08-09 (handover audit FIX 20):** `C11-11` and `C11-20` were carved OUT of this range — both have progressed and now carry their own rows below. `C11-13` stays in the range but is **coordinated with Campaign 18** — see its own row below. |
...
| `C11-13` (NEW-VOXEL-INSIDE-CAMERA-BLACK) | NOT STARTED — **P0, W1, and the highest user-impact voxel gap on record.** *(C18-coordination stamp added 2026-08-09, handover audit FIX 19.)* ⚠ **CROSS-CAMPAIGN:** [`QUEUE_2026-08-09_CAMPAIGN18.md`](QUEUE_2026-08-09_CAMPAIGN18.md) §5 lists this row as a **coordinated dependency it must not duplicate, re-file or renumber** — C18 explicitly adds nothing here, and "run it earlier" is a **C11 scheduling call**, not a C18 one. Read the C18 §5 table before scheduling. | G1 §B / G6 A1 | evidence-pending |
...
| `C11-133`, `C11-135 … C11-139`, `C11-141 … C11-145`, `C11-147` (test-infra remainder) | NOT STARTED (`C11-137` = EXIT) | G9 §A | evidence-pending. `C11-133` (Karma launcher flakiness) is a **machine-lane** investigation and still gates ~12 downstream launcher gates. |
...
- **The G9 environment prerequisites (hard, W1):** `C11-133` (Karma launcher determinism), `C11-132`
  ...
  **UPDATE 2026-08-07 (batch CO-2):** `C11-132` and `C11-134` are COMPLETE in code (see §3.2), each
  with one Karma round-trip owed to the machine lane. **`C11-133` is the remaining blocker** — it is a
  machine-lane investigation, and the trio is not discharged until it lands, so `C11-136`/`C11-144`
  stay paused.
