...
| `C11-13` | NEW-VOXEL-INSIDE-CAMERA-BLACK | standing-reds | P0 | correctness | M | G1 (walkthrough G6) | **W1 — COMPLETE; IMPLEMENTED / LANDED Batch 1031 (`348063f48b`), PHYSICAL EDGE + KARMA + TEN-PROBE PRESERVATION ACCEPTED 2026-08-12; final acceptance repair `f54d58cdd4`.** |
...
| `C11-192 … C11-211` (post-attribution architecture tail) | **EXECUTING; `C11-192/199/200/201/211` + C11-193 ALLOCATION SLICE IMPLEMENTED AND LANDED (Batches 774-780, 2026-08-01); C11-193A/B/C LANDED at `b20234a16b`; `C11-205` NEXT BLOCKER — C11-193 NOT COMPLETE / NO FPS CLAIM** | §1.29 / G2/G3/G4/G5/G6/G8/G9/G10 | Demand-realized terrain shadow UBs, pending-pipeline ownership, tonemap exposure dirtiness, stable packed-depth view identity, once-per-animation joint updates, and borrowed tileset environment managers are implemented with source/type/build coverage; focused browser/visual gates remain open. C11-205 blocks causal resident timing. C11-193A/B/C now retain transactional manager-local outputs, borrow the exact Scene encoder with submit-owned unique arena leases, and collect primitive-phase manager ticks for final same-frame DEMANDED/UNKNOWN-before-PROVEN_NONE drain. Node priority plus drain/lifecycle suites pass **56/56**, focused Edge **31/31**, and C11-193B/C real-browser gates **19/19** and **29/29**. Moving/FPS attribution, broader recovery/HQ reuse, residual allocation cleanup, one-device/browser limits, and GPU-completion certification remain open; no performance percentage is earned. C11-195 remains high-value measured default-path work; C11-208/209/210 preserve features while closing low-limit binding, startup-submit, and dormant compute-command defects. |
