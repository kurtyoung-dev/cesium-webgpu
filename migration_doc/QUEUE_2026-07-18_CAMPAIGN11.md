...
| `C11-212` | WEBGPU-SCENE-SNAP-PARITY [implement upstream v1.144's experimental `Scene.snap` snap-to-geometry picking on WebGPU: an RGBA32F snap target + snap pipeline variant in the WebGPU pick-pass family, honoring `passes.snap`, `command.snapId`, `GlobeDepth.snapping`, and the depth-only fallback for snapless commands; the merge left it a safe structural no-op behind the alternate-renderer early-returns] | picking / derived-commands | P1 | parity | M | G7 | **W7 — IMPLEMENTED 2026-08-02, MACHINE VERIFICATION PENDING.** Render side + readback complete: `WebGPUSnapPayload.ts` (one home for the RGBA32F encoding), `WebGPUSnapFramebuffer.ts` (occluder + payload + shared depth, device-generation lifecycle, one-frame-stale sync readback + `endAsync`), `fragmentSnapMain` in `ModelPBRComplete.wgsl`, `WebGPUModelPipelineCache.getSnapPipeline` (axis-marked name, direct create hatch), the two-phase pick/payload schedule in `WebGPUSceneRendererPickPass`, the `snapVariant` axis on `selectCommandVariant`, and `GraphicsContext.createSnapFramebuffer()` so `Snapping.js` stays backend-agnostic. Upstream's depth-only fallback for snapless commands is realized by the occluder phase (the unmodified pick fleet writes the depth the payload phase loads). Contract: `webgpu-snap-payload.spec.mjs` 40/40. Open riders: `UP144-SNAP-WEBGPU-EDGES` (WebGPU edges carry no pick ID, so every hit is `isEdge:false`), `UP144-SNAP-WEBGPU-NON-MODEL` (tracking only — upstream is Model-pipeline-only too).  **MACHINE-VERIFIED 2026-08-02 (Batch 813, `probe-scene-snap.mjs`): all four gates PASS on both backends** — functional hit at the model with IDENTICAL 24.8 m hit distance WebGL vs WebGPU (payload depth reconstruction agrees to the meter; WebGPU 5 calls per the one-frame-stale contract, WebGL 1), corner miss returns no hit, `_snapEnabled` latch false->true only after first use, 0 console + 0 validation errors. |
| `C11-213` | WEBGPU-VECTOR-LAYER-DRAPING-PARITY [WGSL twin for upstream v1.144's terrain-draped vector polylines: port `VectorCommon.glsl`'s HAS_VECTOR_LAYER sampling into `GlobeTerrain.wgsl`, bind the five `u_vector*` tile textures through the globe surface layouts (mind the reduced low-limit layout from C11-208), and route `VectorTileData` texture creation through the backend-appropriate path; the shader-set flag bit is already assigned at `0x400000000` (upstream's `0x200000000` collides with the fork's eclipse flag); SHADER_PAIRS_LOCKSTEP row required at landing] | globe / imagery | P1 | parity | M | G2 | **W7 — NOT STARTED (seeded 2026-08-01; merge 65a194d24e)** |
...
| `C11-212` (Scene.snap WebGPU parity) | **IMPLEMENTED 2026-08-02 — MACHINE VERIFICATION PENDING** | §1 v1.144 parity rows / G7 | RGBA32F snap target (`WebGPUSnapFramebuffer`), payload encoding home (`WebGPUSnapPayload`), model snap pipeline variant + `fragmentSnapMain`, and the two-phase occluder/payload schedule in the WebGPU pick-pass family. Upstream's depth-only fallback for snapless commands is realized by the occluder phase, because WebGPU validates pipeline color-target formats against the render pass at draw time and an RGBA8 pick pipeline in an RGBA32F pass invalidates the whole command buffer (FORK-34). `Scene.snap` routes through the new `GraphicsContext.createSnapFramebuffer()` factory, so `Snapping.js` gains no backend branch and WebGL snap is byte-identical. Contract `webgpu-snap-payload.spec.mjs` 40/40; both tsc projects clean; fleet 956/956. Gate owed: `snap()` returning a hit on a model on both backends for the upstream multifrustum snapping Sandcastle (WebGPU snap is one-frame stale — call twice at the same position). Riders: `DEFERRED_WORK.md` `UP144-SNAP-WEBGPU`, `UP144-SNAP-WEBGPU-EDGES`, `UP144-SNAP-WEBGPU-NON-MODEL`.  **MACHINE-VERIFIED 2026-08-02 (Batch 813 landed spec 41/41, fleet 998/998; `probe-scene-snap.mjs`): all four gates PASS on both backends** — functional hit at the model with IDENTICAL 24.8 m hit distance WebGL vs WebGPU (payload depth reconstruction agrees to the meter; WebGPU 5 calls per the one-frame-stale contract, WebGL 1), corner miss returns no hit, `_snapEnabled` latch false->true only after first use, 0 console + 0 validation errors. Gate owed above is CLOSED. |
| `C11-213` (vector-layer draping WGSL twin) | **NOT STARTED — seeded 2026-08-01 by maintainer ruling (v1.144 merge follow-up)** | §1 v1.144 parity rows / G2 | Port HAS_VECTOR_LAYER sampling into `GlobeTerrain.wgsl` + bind the five `u_vector*` tile textures (flag bit already at `0x400000000`); mind the C11-208 reduced low-limit layout. `DEFERRED_WORK.md` `UP144-VECTOR-LAYER-WGSL`. |
