...
`C11-181` is **IMPLEMENTED / VERIFIED / LANDED (Batch 773, 2026-08-01)**, not
complete. `C11-192` (Batch 775), `C11-199`/`C11-211` (Batch 774), `C11-200`
...
open gate below.
`C11-208` is implemented and verified (Batch 777). `C11-193/194/195/202/205`,
`C11-60`,
...
| `C11-13` | NEW-VOXEL-INSIDE-CAMERA-BLACK | standing-reds | P0 | correctness | M | G1 (walkthrough G6) | **W1 — IMPLEMENTED / LANDED Batch 1031 (`348063f48b`); PHYSICAL EDGE + KARMA ACCEPTANCE OPEN — NOT COMPLETE.** |
...
| `C11-181` | WEBGL-GLOBE-SHADER-VARIANT-EVICTION-REFERENCE [balanced replacement references; stale-wrapper poisoning] | terrain-imagery | P1 | correctness/resource-lifetime | S | G2 | **W1 — IMPLEMENTED / VERIFIED / LANDED (Batch 773, 2026-08-01)** |
...
| `C11-202` | MODEL-BACKEND-NEUTRAL-PRIMITIVE-DESCRIPTORS [shared immutable descriptor once; WebGL realizes GLSL/VA/DrawCommand and WebGPU realizes only native equivalents; remove remaining legacy CPU-object tax without losing metadata/style/clipping/classification/silhouette/edge/pick/shadow/custom-shader behaviour] | model-frontend / models | P0 | perf/architecture | XL | G4/G5/G10 | **W1 — PARTIAL (LANDED Batches 774/780 + independently reviewed 2026-08-02 pick/edge-tax slice, LANDED Batch 819):** final WebGL program/VA/DrawCommand realization is bypassed for WebGPU and native async readiness exists. Native descriptor construction now retains all shared geometry/material/feature/metadata/lighting/alpha/statistics stages and the Scene edge-MRT demand signal, but skips the legacy-only `PickingPipelineStage`, `EdgeVisibilityPipelineStage`, and `EdgeDetectionPipelineStage`; WebGL's default path is unchanged. Focused contracts are 3/3 plus Jasmine source coverage. **OPEN:** measure the allocation/timing delta on the moving route; audit any remaining legacy-only stages/objects; close native edge-emitter RTE; route future selected-feature post-process IDs through renderer-owned native IDs; and complete browser/fallback/device-recovery coverage. Do not mark complete. |
...
- `C11-193` also owns the selected-consumer demand registry. Its narrow
  ...
  audit FIX 22 — the bare id read like a missing document)*. Measured
  queue/submission credit remains
...
| `C11-01 … C11-10` (pick) | NOT STARTED | G1 §A/§0 | evidence-pending |
...
| `C11-12`, `C11-14 … C11-19`, `C11-21 … C11-25` (standing-reds) | NOT STARTED | G1 §B (C11-13 → G6 A1) | evidence-pending. **UPDATE 2026-08-12:** `C11-11`, `C11-13`, and `C11-20` are carved OUT of this range because each has progressed and carries its own row below. |
...
| `C11-13` (NEW-VOXEL-INSIDE-CAMERA-BLACK) | **IMPLEMENTED / LANDED Batch 1031 (`348063f48b`), 2026-08-12; PHYSICAL EDGE + KARMA ACCEPTANCE OPEN — NOT COMPLETE.** The proxy keeps the historical outside 36-index prefix byte-identical and appends its exact reversed winding; a finite effective-model containment test selects `firstIndex=36` only while the camera is inside. Odd-reflection parity is XOR-correct, all four color/object-pick/cell-pick/velocity commands remain explicit `uint16`/36-index draws, and lazy command materialization is refreshed every frame. Independent policy, strict-slice, and mutant gates are **6/6**; the adjacent static/lifecycle fleet is **84/84**. Canonical package TypeScript remains blocked before source checking by the unrelated malformed generated `packages/engine/index.d.ts` baseline. **Still owed:** the new Edge waypoint probe, outside byte identity, focused Karma execution, and the ten-probe voxel preservation battery. ⚠ **CROSS-CAMPAIGN:** [`QUEUE_2026-08-09_CAMPAIGN18.md`](QUEUE_2026-08-09_CAMPAIGN18.md) §5 lists this row as a coordinated dependency it must not duplicate, re-file or renumber. | G1 §B / G6 A1 | Batch 1031 implementation evidence; browser/Karma evidence pending |
...
| `C11-100` (PARITY-VOXEL-OCTREE-TRAVERSAL) | NOT STARTED — P1, W7, **XL, sliced (A2-slice-0 triage first)**. *(C18-coordination stamp added 2026-08-09, handover audit FIX 19.)* ⚠ **CROSS-CAMPAIGN, AND ONE DANGLING CLAIM CORRECTED:** [`QUEUE_2026-08-09_CAMPAIGN18.md`](QUEUE_2026-08-09_CAMPAIGN18.md) §5 states that the audit's **brickmap / per-level page-table candidate is "`C11-100`'s implementation VEHICLE, recorded there, not a new ID here"** — **it was NOT recorded here; this stamp is where it becomes recorded.** Treat brickmap / per-level page tables as a candidate implementation vehicle for this row, to be chosen on evidence at A2-slice-0 triage, not as a decided design. **`C18-A2`** (voxel empty-space skip via per-slot min/max occupancy) and **`C18-A6`** (ray-guided residency feedback for voxel streaming) both consume this row's output and are marked "PENDING — coordinated with `C11-100`" in the C18 ledger; scheduling this row without reading them will strand two C18 rows. | G6 §A | evidence-pending |
...
- **Perf-claim prerequisite tooling (early):** `C11-140` (GPU-timestamp unique-sample cert), `C11-146`
  ...
  (G9 Q7).
  **UPDATE 2026-08-07 (batch CO-2):** both landed with `node --test` guards (see §3.2). `C11-146` is
  COMPLETE and **binding on the boot/TTFF cluster**. `C11-140`'s accounting is fixed and unit-certified,
  but its **adapter certification artifact does not exist until the machine lane runs
  `probe-gpu-timestamp-profiler.mjs`** — until then, no C11 perf item may cite a GPU-lane number.
