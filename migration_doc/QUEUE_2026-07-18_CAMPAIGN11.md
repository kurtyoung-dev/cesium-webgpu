...
| `C11-170` (perf regression guard) | **PARTIAL — GATE AUTHORED, BROWSER-FREE HALF GREEN, NOT YET RUN IN ACQUIRE MODE / CERTIFIES NOTHING** (Batch 1150) | §1.25 / G10 | `probe-perf-regression-gate.mjs` spawns the four C11-169 diagnostics **sequentially** (one Edge at a time, `cwd` = repo root, `PROBE_BASE` pinned, `probe-cpu-sampling-profile` forced to `--frames 120` because the banked visibility floor came from a 120-frame profile), proves each report fresh by **sha256 change AND in-file timestamp ≥ gate start**, then adjudicates seven signals whose bars are derived from those probes' own banked post-fix artifacts: A resource-write self-time share < `0.193`, B/C Rule-of-Three `3/n` with `n` read from the report, D zero live WebGL contexts in the `webgpu-solo` getContext census (fenced by a live-WebGPU check and by agreement with the producer's `Q1`), E same-run CPU wall-clock ratio ≤ `2.8737075`, F canonical status passthrough of the C11-169 frame gate, G positive-only `RE-UPLOAD STORM` sentinel scan. Missing report / absent scalar / unproved freshness is **STRUCTURAL, never PASS**; `--adjudicate-only` is **capped at STRUCTURAL** so a stale green is impossible. Browser-free half `c11-170-perf-regression-gate.spec.mjs` is **70/70, 0 skipped** and kills all twelve reviewer mutants (bar-widen a/g/h/i, absence-reads-PASS b, D-fence c, adjudicate-only-PASS d, parallel/import acquisition e/e2, precedence-inert f, D-bar j, freshness-dropped k). **The four C11-169 probes were consumed unmodified — lane discipline, not a row requirement; the row text mandates no such thing.** Banked `--adjudicate-only` artifact (`output/performance/c11-170-perf-regression-gate.json`, sha `fa3cc392…`): STRUCTURAL / exit 3, A-webgpu…F all PASS, G NOT-PROVEN. **Still open:** a real acquire-mode run against a live dev server, and Signal G's collection blind spot, filed as `NEW-C11-170-SENTINEL-UNOBSERVABLE-TO-GATE` in `DEFERRED_WORK.md` (the engine sentinel matches neither `WEBGPU_FAULT_RE` nor survives the backend-isolation six-entry console cap; must be sequenced **after** the `reuploadWatch` over-reporting hardening recorded at `NEW-WEBGPU-UPLOADIMAGESOURCE-CACHE-CONTRACT-TRAP`). Row stays OPEN. |
