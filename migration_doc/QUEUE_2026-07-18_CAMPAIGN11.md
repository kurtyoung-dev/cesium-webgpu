...
complete or imply a generic one-frame-stale contract. The unstaged 2026-08-02
continuation fixes the previously audited **copy old texture → render new snap
...
#### 2026-08-02 rider — `UP144-SNAP-WEBGPU-EDGES` IMPLEMENTED, browser gate owed
...
`screenPosition` half a drawing-buffer pixel up-left on BOTH backends.
...
declaring the emitter under `NO_CENTRAL_CACHE`; package tsc 0 non-TS2307 errors;
...
- `C11-194`'s immutable defaults/layouts are device-generation shared, and the
  unstaged 2026-08-02 recovery slice now rejects/disposes stale model and
