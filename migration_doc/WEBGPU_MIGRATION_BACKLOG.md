...
| **BUG-11** | **Imagery layer textures bind but don't render** | HIGH | **Re-classified as "globe geometry never rasterizes" (2026-04-29)** | **Hypotheses A/B/C are all wrong.** Probe session 2026-04-29 (`Tools/visual-regression/bug-11-imagery-probe.mjs` + `canvas-black-narrow.mjs` + `webgl-vs-webgpu-pixel-check.mjs`) found: (1) imagery uploads correctly — 8 GPUTextures created at 256×256 from `imagery.image` via `_uploadImageSource`; (2) the reprojection branch is correctly skipped because `useWebMercatorT=true` makes `needGeographicProjection=false` (the FS samples Web Mercator directly via `czm_lookUpTextureFromWebMercatorT`) — so `_webgpuReprojectedTexture` staying undefined is BY DESIGN, not a bug; (3) the per-tile probe label `hasWebGPUTex=false` is misleading — it only reports the reprojected slot, not the live-uploaded texture; (4) the production post-process chain renders 0/50 non-black pixels (canvas BLACK), `forceIdentity` blit (sceneFB→canvas, no tonemap) is also 0/50 (sceneFB color is empty), patching the globe FS to `return vec4(1,0,0.5,1)` UNCONDITIONALLY is also 0/50 (FS isn't reaching output), but `debugShowDepthAsColor` produces 50/50 non-black — and the avg color (127,0,127) maps to the depth-overlay's "depth >= 0.99999 (never drawn)" magenta-tint sentinel. Conclusion: the depth buffer is uniformly 1.0 — **no globe primitive ever rasterizes a fragment** in WebGPU mode. Pipelines build, bind groups are valid, no validation errors, 11 commands queued per frame, but everything is silently clipped/discarded before the FS. WebGL same setup renders 50/50 non-black with avg(86,114,123) ocean blue. **Next investigation:** vertex-shader output positions, `czm_translateRelativeToEye` math, tile bounding-volume culling at the WebGPU command level, or pipeline color-write-mask defaulting to 0. This is now MERGED with the "canvas-black-screen" item — they are the same bug. Multi-day investigation; tools landed in the probes above. |
