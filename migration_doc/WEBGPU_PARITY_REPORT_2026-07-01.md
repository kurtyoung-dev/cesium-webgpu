...
- **Post-process (73.3 %) was an artifact of the stub cluster**: 7 of its 30
  rows were the orphaned `PostProcessStageLibrary` built-ins (BlackAndWhite,
  Brightness, NightVision, Silhouette, EdgeDetection, LensFlare, DepthView) —
  pre-translated WGSL assets that nothing imported. **CLOSED 2026-07-02 by
  WIRE-PP-LIBRARY-BUILTINS** (named-stage interception → WGSL twins; see gap
  row 9). Every *live* WebGL PP feature (Bloom, AO, DoF, FXAA, tonemap ×5,
  auto-exposure, OIT, EDL, f16 variants) is full, and 12 rows exceed WebGL
  outright.
...
| --- | --- | --- |
| Voxels (one XL stub) | Real root-tile megatexture upload (B474), shape-OBB world placement (B475, IoU 0.994), WebGL color parity (B476), primitive pick, depth-1 octree LOD w/ 9-slot atlas + SSE refine (VOXEL-OCTREE-LOD, 2026-07-02) | deeper-than-level-1 octree traversal **missing**, user CustomShader→WGSL **stub**, per-cell pick **stub** (B477 blocker doc), non-box shapes **partial** |
...
| EXT_structural_metadata fine grain | display + pick shipped; multicomponent attrs (§6.5 #13 CLOSED); TEXTURE/implicit-sourced tables (§6.5 #15 CLOSED 2026-07-02) | UINT16/32 packing (§6.5 #14), INSTANCE-sourced tables (PARITY-METADATA-TABLE-INSTANCE-SOURCE) |
...
| --- | --- | --- | --- |
| Voxel per-cell pick (`pickVoxelCoordinate`) | **✅ RESOLVED (VOXEL-CELL-PICK-RELAND, 2026-07-02)** | 3D Tiles, Picking | Reverted pick wiring re-landed on the fixed shapeUv convention: `fragmentPickVoxelMain` + dedicated pickVoxel pipeline/command routed via `passes.pickVoxel`; ALSO fixed `WebGPUPickFramebuffer.readCenterPixel`'s vertical mirror (GL bottom-origin rect row used as top-down texture row — hidden by center-screen probes). `probe-voxel-cell-pick.mjs`: decoded {tileIndex, sampleIndex} byte-identical to WebGL at 4 filled + 2 empty + 1 off-box pixels. Remaining tail: `scene.pickVoxel`'s VoxelCell construction still throws on WebGPU (`_traversal` undefined) — C-R9-VOXEL-CELL-PICK-TAIL. |
...
| # | Item | Status | Subsystem(s) | What's missing |
| --- | --- | --- | --- | --- |
| 1 | Voxel octree / multi-tile LOD traversal | **partial — depth-1 shipped 2026-07-02 (VOXEL-OCTREE-LOD)** | 3D Tiles | Honest increment landed: when the provider advertises `availableLevels >= 2` (BOX shapes), the real-data texture becomes a 9-slot Z-stacked 3D atlas (slot 0 root, slots 1..8 level-1 children, async uploads with per-octant root fallback = Octree.glsl `PACKED_LEAF_FROM_PARENT` at depth 1), the parity march does per-sample child selection (`floor(shapeUv·2)` → child tileUv rescale → atlas slab addressing), and a CPU SSE refine test mirroring `SpatialNode.computeScreenSpaceError` picks level 0/1 per frame against `primitive.screenSpaceError`. Probe `probe-voxel-octree.mjs`: two-level procedural provider — close view shows the FINE level-1 diagonal on both backends (64/64 cells incl. 8/8 empty-at-fine/filled-at-root discriminators black on WebGPU), far view drops to root (targetLevel 0, crop diff 0.02); single-tile off-gate: probe-voxel-parity (colorL1 0) + cell-pick + megatexture green. **Remaining (PARITY-VOXEL-OCTREE-TRAVERSAL, still XL):** arbitrary-depth traversal (internal-node texture port), dynamic megatexture slot allocator w/ eviction, non-box shapes, pickVoxel against refined tiles (pick marches the root slab), SAMPLE_COUNT>1 level blending. |
| 2 | Model rendering in 2D / Columbus View / Morph | **CLOSED 2026-07-02 (MODEL-SCENE-MODES)** | glTF Models | Mode-aware matrix/BV pick in `updateWebGPUModel`: when `frameState.mode !== SCENE3D` the single `modelMatrix` local switches to the scene graph's `_computedModelMatrix2D` (the same `Transforms.basisTo2D` product WebGL's `ModelMatrixUpdateStage` consumes — Scene-side `Model.js updateSceneMode` already recomputes it on every mode flip before the FR runs) and the emitted commands carry `_boundingSphere2D` (an ECEF BV under the projected-frame culling volume culled the model outright). All downstream consumers (RTE camera pack, per-node matrices, shadow-cast UB, edge MVP, TAA prev) derive from that one local, so 2D + CV + MORPHING (WebGL uses the identical `mode !== SCENE3D` condition) come along for free with the WGSL untouched. Probe: `probe-model-scene-modes.mjs` — pre-change WebGPU rendered **0 model pixels** in 2D/CV; post-change mask mismatch 0.39%/0.58%, centroid delta 0.1 px vs WebGL in both modes; 3D leg unchanged (mode-gated: in SCENE3D both picks short-circuit to the pre-change expressions by reference). Residuals (deferred, matching WebGL's opt-in surface): `projectTo2D:true` accurate-2D vertex reprojection (MORPH-MODEL-PROJECT2D, still open) and the SCENE2D IDL-crossing duplicate command (`ModelDrawCommand.derive2DCommand`). |
...
| 5 | Globe undergroundColor + undergroundColorAlphaByDistance | **CLOSED 2026-07-02 (GLOBE-UNDERGROUND-COLOR)** | Globe | Camera-UB tail (+12 floats: undergroundColor, undergroundColorAlphaByDistance NearFarScalar, undergroundControl w/ CPU-computed `showUndergroundColor` flag + `max(eyeHeight,0)`); `@builtin(front_facing)` on `fragmentMain`; GlobeFS-parity back-facing alpha-blend after fog/atmosphere. Reaching parity required fixing THREE pre-existing underground bugs (Bug 487): central pipeline-cache NAME aliasing that swallowed the no-cull `_NC` variant (underside faces never rasterized — only skirts), fog not disabled when `cameraUnderground` (WebGL parity gate), and skirts not suppressed underground (`indexCountWithoutSkirts` truncation). Probe: `probe-globe-underground.mjs` (underground-red 6.6% / underground-def 14.3% vs the 22.4% standing above-ground residual; off-gate: fixed-clock above-ground WebGPU pre/post-change screenshots MD5-identical). Follow-up: `BUG-GLOBE-PIPELINE-NAME-AXES` (same aliasing class on other structural axes). |
| 6 | Model.color / colorBlendMode / colorBlendAmount | **CLOSED 2026-07-01 (WIRE-MODEL-COLOR)** | glTF Models | `MODEL_HAS_COLOR` (bit 27, keySalt-folded) ifdef `applyModelColor()` in `ModelPBRComplete.wgsl` (main lit path after customShader + unlit early-out), exact `ModelColorStageFS.glsl` math; uniforms ride material-UB reserved lanes 184-187 (`_pad_reserved8` = RGBA) + 175 (`motionFlags.w` = getColorBlend scalar) via `maybeUpdateForModelColor`. Probe: `probe-model-color.mjs` (HIGHLIGHT/REPLACE/MIX mean-channel deltas ≤ 9/255 vs WebGL, mask diff 0.13%; untouched off-gate untinted + preprocess(0) code-identical). Remaining sliver: color.alpha<1 translucent-pass routing + alpha==0 hide → DEFERRED_WORK `WIRE-MODEL-COLOR-ALPHA-SEMANTICS`. |
| 7 | Model silhouette (silhouetteColor/Size) | **CLOSED 2026-07-01 (WIRE-MODEL-SILHOUETTE)** | glTF Models | `MODEL_SILHOUETTE` (bit 28, keySalt-folded) — the formerly-orphaned `ModelSilhouetteStage.wgsl` rewritten to WebGL parity math and prepended to the module; ifdef VS inflate + FS colour early-out in `ModelPBRComplete.wgsl` gate on a per-command pass flag. Stencil two-pass matches WebGL's `deriveSilhouetteModelCommand`/`deriveSilhouetteColorCommand`: base draw swaps to a stencil-write (ALWAYS/REPLACE, ref `_silhouetteId % 255` via `setStencilReference`) pipeline variant; a derived colour command (second material UB, pass flag 1, stencil NOT-EQUAL, cull off, TRANSLUCENT pass when base is BLEND or alpha<1) draws the inflated rim, pushed after all of the model's base commands. Uniform lanes: material-UB pads 105-107 (expandX/Y pre-folded `proj00/11·size·pixelRatio/vpWidth`, pass flag) + 112-115 (`_pad_cc0` = silhouetteColor RGBA) via `maybeUpdateForSilhouette`. Probe: `probe-model-silhouette.mjs` (RED/size-4 rim 3905 vs 4036 px, 100% dilated-mask overlap; size-0 off-gate at load noise 0.03%). Residual: `silhouetteColor.alpha<1` diverges — WebGL body-washes (upstream OIT-stencil artifact), WebGPU keeps the documented rim-only cutout → DEFERRED_WORK `WIRE-MODEL-SILHOUETTE-TRANSLUCENT-DIVERGENCE` (match-vs-document decision). |
| 8 | Model splitter (splitDirection) | **CLOSED 2026-07-01 (WIRE-MODEL-SPLITTER)** | glTF Models | `MODEL_SPLIT_ENABLED` (bit 26, keySalt-folded) ifdef FS discard in `ModelPBRComplete.wgsl` (fragmentMain + fragmentPickMain), driven by material-UB pad lanes 38/39 via `maybeUpdateForSplit` on the per-model pipeline cache. Probe: `probe-model-splitter.mjs` (LEFT parity 0.16% mask diff; NONE off-gate 0.05% + preprocess(0) byte-identical). |
| 9 | PostProcessStageLibrary built-ins ×7 (BlackAndWhite, Brightness, NightVision, Silhouette, EdgeDetection, LensFlare, DepthView) | **CLOSED 2026-07-02 (WIRE-PP-LIBRARY-BUILTINS)** | Post-process | One named-stage interception mechanism: `WebGPULibraryPostProcessStage.ts` (registry keyed by the well-known `czm_*` names, prefix-match for `czm_edge_detection_<guid>`) substitutes the WGSL twin when the configure pass scans `collection._stages`; enabled + uniforms synced live per frame (`syncInterceptedLibraryStages`). Parity rewrites: EdgeDetection.wgsl → depth-Sobel (3/10/3 kernel, `len > length` threshold — was a color-luminance Sobel that never matched), NightVision.wgsl → frame-seeded `rand` noise ×green (exact GLSL math), DepthView.wgsl → raw depth grayscale, Silhouette.wgsl → full-vec4 mix, Brightness.wgsl → `target` WGSL-reserved-word fix, LensFlare.wgsl → full port of the space/sun gates + isInEarth ghost mask + 4-ghost chromatic chain + halo (czm_* state CPU-fed). Silhouette runs as a two-pass composite (edge → intermediate, composite reads scene + edge). Also fixed: upstream's `removeStages` compaction never ran on WebGPU (FR early-return), so `postProcessStages.remove()` leaked BOTH user WGSL and library stages until the next add. Probe: `probe-pp-library-builtins.mjs` (all 7 on both backends: visible transform + cross-backend mean-delta within per-stage tolerance; remove-all → BYTE-IDENTICAL; 0 errors). Residuals → DEFERRED_WORK `WIRE-PP-LIBRARY-BUILTINS-RESIDUALS` (LensFlare dirt/star textures, czm_selected masking, DepthView encoding). |
...
| 12 | Globe HDR gamma-encode | **CLOSED 2026-07-02 (GLOBE-HDR-GAMMA)** | Globe | `czm_gammaCorrect` now mirrors WebGL's `#ifdef HDR` sRGB→linear decode at runtime via an `hdrControl` vec4 tail on the globe camera UB (x = gate, y = `uniformState.gamma`; offsets 192-195), packed by `WebGPUGlobeSurfaceCameraUB` from `frameState.useHDR && context.hdrCanvasOutput` — the same effective gate as the pipeline's `setHDROutputMode` (tonemap bypassed, chain works in linear HDR). The imagery default-gamma branch now routes through `czm_gammaCorrect` like GLSL's `#if !defined(APPLY_GAMMA)` arm, so under the B479 HDR canvas path the globe emits linear like models/PP already did. Probe: `probe-globe-hdr-gamma.mjs` (HDR two-tone imagery = single decode of the SDR tone within 0.35/255, distinct from missing/double correction; SDR pass BYTE-IDENTICAL off-gate). |
| 13 | Metadata multicomponent property attributes (VEC2/3/4) | **CLOSED 2026-07-02 (METADATA-MULTICOMPONENT)** | 3D Tiles, glTF Models, Picking | Slot-9 transport widened f32→`vec4<f32>` (`float32x4`, stride 16): up to four components per vertex (SCALAR zero-pads, VEC2/3/4 + MAT2 transport fully; MAT3/4 first-four only). Codegen (`MetadataWGSLPipelineStage.constructFromTransport`) swizzles the transport into the property's real WGSL type with component-wise offset/scale; new generated `metadataDebugColor` paints VEC2/3/4 raw components as RGB (scalar path keeps the exact `vec4(s,0,1-s,1)` gradient). Pick packing now reads real components. New asset `Specs/Data/Models/glTF-2.0/BoxVec3PropertyAttributes` (per-face-constant VEC3). Probe: `probe-metadata-multicomponent.mjs` (3 authored face colors round-trip within ~2/255; debug-off gate; 0 errors) + dp46b/c/d/e regressions green. |
...
| 15 | Metadata TEXTURE / instance / implicit-sourced property tables | **CLOSED 2026-07-02 (METADATA-TABLE-SOURCES)** — instance-sourced residual deferred | 3D Tiles, glTF Models, Picking | `findPropertyTableForPrimitive` now classifies the referencing feature-ID set: TEXTURE-sourced tables sample the binding-26 feature-ID texture inside the generated `initializeMetadata` (module-scope forward reference to `featureIdTexture`/`unpackFeatureId`; gated on the set being the model's SELECTED set + a live feature table — the conditions under which binding 26 carries that set's data); IMPLICIT ranges ride the Batch-188 synthesized `featureId0` vertex slot. Three parity fixes shipped alongside: feature-ID GPU textures now reuse the WebGLStub-uploaded texture (Session-65 pattern — previously every glTF feature-ID texture fell back to the white placeholder), are sampled NEAREST (upstream forces `Sampler.NEAREST`; the linear default fabricated fractional IDs), and the regular scene.pick readback converts the GL bottom-origin rect to the top-down texture row (the C-R9-VOXEL-CELL-PICK flip applied to `end`/`endAsync`/`_startReadback` — off-vertical-center picks read the MIRRORED pixel). Late-resolving metadata (featureTableId dirty-flag landing after the first primitive build) triggers a per-primitive cache rebuild keyed on the generated metadata class hash (the WebGPU analogue of `resetDrawCommands`). New assets `Specs/Data/Models/glTF-2.0/PropertyTableFeatureIdSources` (texture + implicit variants). Probe: `probe-metadata-table-texture.mjs` (4-region debug display + per-region ModelFeature.getProperty pick parity on both backends + off-gate; dp46d/e + multicomponent + uint16 + billboard-pick regressions green). Residuals: INSTANCE-sourced feature IDs (no per-instance feature-ID data path in WebGPUModelInstancing) → DEFERRED_WORK `PARITY-METADATA-TABLE-INSTANCE-SOURCE`; `scene.pickMetadata` for table properties returns undefined on BOTH backends (upstream `getMetadataProperty` is texture-only — CesiumGS/cesium#12225). |
...
| 19 | Edge authored silhouetteNormals accessor | ~~partial~~ **CLOSED (EDGE-AUTHORED-SILHOUETTE-NORMALS, 2026-07-02)** | 3D Tiles, glTF Models | `extractEdgeGeometry` now consumes the authored signed-byte accessor with WebGL-identical decode + sequential dedupe-order pair indexing; adjacency-derived normals remain the fallback only when the accessor is absent (byte-identical off-gate). Probe: `probe-edge-authored-silhouette.mjs`. |
