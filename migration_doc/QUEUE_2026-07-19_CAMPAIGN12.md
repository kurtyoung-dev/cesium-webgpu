...
- `73221e8ec1` + `1d22e2c737` — **Batches 1087 and 1100:** the Moon-mip shader-contract repair and pre-registration custody hash landed. The frozen producer ships the four-cell `sign-test-v1` design, while R-24 ruled a sixteen-cell ratio design with a pre-registered ratio. The discrepancy is filed in source and reports. C12-33's 2.5-hour Edge ten-run set had never executed and was **BLOCKED** pending a maintainer design ruling *as of this refresh*. Both conditions were discharged afterwards: the design ruling landed 2026-08-21 as `R-2026-08-21-15`, and twenty Edge runs executed 2026-08-24. See the `C12-33` row under OPEN.
...
- ⛔ **G3 (asset upgrade) — RED at HEAD; the decision is RULED, the work is not done.** First Edge run **executed at Batch 934** (tip `4f6d3c9751`), hitting its pre-registration on 4 of 5; the four asset/format/fidelity predicates are red **identically on both backends** (the shipped 2048/face `TYCHO_T5_DIFFUSE` scores 2.637 arcmin/px, median chroma 0.000, dust-lane IQR 0.585× t3). ~~**`R-2026-08-10-4` orders Option A — re-bake the star cubemap at 4096/face and re-run G3 — NOT YET EXECUTED at HEAD.**~~ **CORRECTED 2026-09-19 (dated in place, nothing removed): that clause contradicted the very next bullet, which records the re-bake and the re-run EXECUTED in Edge tranche 3d on 2026-08-29. The live authority on G3's red is `R-2026-09-02-7`, under which the chroma and dust criteria are unreachable by any bundled variant, so G3's red is RED BY CONSTRUCTION; the 4096 tier ships as an opt-in externally-fetched asset gated on a licence determination AND a fetch path, neither of which exists. G3 stays RED and stays in this gate; this correction discharges nothing.** Coupled to `C12-12`'s 4096 tier (see OPEN). Residual instrument debt on this lane: the `probe-stars-catalog.mjs` Edge run.
- **G3 re-run EXECUTED on the 4096 tier (Edge tranche 3d, 2026-08-29; R-2026-08-10-4 Option A under R-2026-08-28-13):** FAIL, identical on both backends, 2 of 5 red (was 4 of 5 at Batch 934). Resolution bars GREEN (4096 px/face, 1.3184 arcmin/px); chroma 0.000 and dust IQR 0.5851x t3 RED - both source-asset properties no bundled variant reaches at any resolution. DR-01: aliasTwinkle still triggers (2.4477 vs 1.2) with a certifying-resolution asset, control valid - the asset variable is ELIMINATED and DR-01 is now the clean single-variable question the OWED text below anticipated. The tier itself is staged uncommitted pending the maintainer's protocol decision (land as opt-in at 19.5 MB / external asset via the policy seam / drop). Evidence `Tools/visual-regression/output/edge-tranche3d-2026-08-29/`.
...
- `C12-29` **NEW-ECLIPSE-OCCLUSION-EFFECTS — the campaign's exit gate is MAXIMAL by ruling `R-2026-08-10-1`: C12 stays open until every slice lands.** Slice state: **S1/S2 landed**; **S6 landed**; **S3 REOPENED — its canonical owner `C13-41` was reopened by `R-2026-08-14-1`, and the machine-readable state was vacated `closed` → `reopened` by `R-2026-08-17-7`**; **S4 COMPLETE / EDGE VERIFIED**; **S5** integrated with targeted gates passing, **final seven-lane certification matrix still open** (NASA-SVS geospatial comparison, isolated active/inactive lanes). Acceptance = the S5 matrix plus every slice's own recorded criterion. **2026-08-16 (B1048): the S5 NASA/SVS v5 tuple (probe + gate lib + spec) is LANDED and REVIEW-VERIFIED - 57/57 incl. the previously unfinished mutants, schema pin v4->v5 stamped, dense-cost 40/40; see HANDOFF_2026-08-14_CODEX_PAUSE.md §10. The seven-lane certification matrix's NASA-SVS lane now has its instrument; the browser lanes remain owed.**
  **S3 CLOSURE VACATED (recorded 2026-08-25).** The 2026-08-12 landing-equivalent run `b5e3f63c-94c6-4204-8706-dd30eabd2eaf` was a genuine PASS / **30/30** — exact report SHA-256 `DD9BA7DCAC0434B91C0F88EFFA4804F3F1CB99747D888B5449B2A483236E70EE`, and its exact selective packet landed as `9c043987a5` — but it passed a gate that has since been superseded: `R-2026-08-14-1` restored `shadowContrastInvariant` and `refreshCostMeasured` as GATES, so a PASS obtained while `shadowContrastInvariant` was non-gating no longer certifies this row, and the redesigned-control discharge it recorded does not survive that restore. Nothing is erased: [FINDING_DISPOSITIONS_2026-08-13.json](FINDING_DISPOSITIONS_2026-08-13.json) carries `owners["C13-41"]` at `state: "reopened"`, `reopenedBy: "R-2026-08-14-1"`, `reopenedRecordedBy: "R-2026-08-17-7"`, `priorState: "closed"`, with the closure run annotated rather than deleted. CLT-B3's separate terminator browser gate is outside S3 closure and remains open.
...
- `C12-33` **Moon mip/LOD + moving-seam — ACCEPTANCE EXECUTED AND CERTIFIED; two debts remain.** Batch 1087 (`73221e8ec1`) repaired the shader-pair contract and Batch 1100 (`1d22e2c737`) landed custody over the four-cell `sign-test-v1` producer; `R-2026-08-21-15` then blessed BOTH designs, with `sign-test-v1` as design of record. Twenty Edge runs executed 2026-08-24 across blocks `20260824a`/`20260824b` with `hardFailures 0`, and `Tools/visual-regression/output/c12-33/certification-20260824b.json` reads status PASS / exitCode 0 / certificationEligible true / designId `sign-test-v1` (sha256 `c2a0bcf9…fb9ed`). **Open:** (i) the artifact cannot be banked — the certification schema carries no `runId` while `assertFinalArtifact` requires `artifact.runId === --run-id`; (ii) the maintainer countersign is owed — `reviewer.identity` is a self-asserted automated station-3 claim. The sixteen-cell ratio design stays deferred per `R-2026-08-24-11`.
...
- `C12-G1F2` **instrument repair LANDED and the re-measure EXECUTED — this row is not owed.** Batch 904, tip `c810dbace2`, Edge 151.0.4129.72, `sourceDirty:false`; the run is recorded in this file's own CO-3 overlay ("2026-08-07 CO-3 gate-lane overlay") and its numbers sit in `Tools/visual-regression/output/celestial-g1.json`, on disk since 2026-08-07. On the re-scoped Lane A's certifying `default` mode: `m2aRatio` 1.0000000369138204 with both attribution factors reported — `meanLumRatio` 1 and `stddevRatio` 1.0000000369138204 — `default_m2a_in_band` true, and all ten Lane-A criteria true. The original 1.045-vs-1.488 comparison is struck (four independent axes): the 1.488 does not reproduce and there is no RMS-contrast divergence left to attribute. Residual: the reading is 396 commits stale, so a freshness re-read at HEAD is a nice-to-have, not this row's subject.
- `C12-31` **natural solar aureole — sky-shell fix landed B785 (`e748181065`), first browser probes green 2026-08-01; the FULL acceptance sweep is still owed**, and `C12-31-FOLLOWUP-A/B/C` (`DEFERRED_WORK.md`) need an explicit in/out-of-gate call from the maintainer (see ASKS). **2026-08-16 (B1048): the C12-31 L1-L4 gate tuple (probe-sky-aureole-anchor + lib/c12-31-aureole-gate + spec) LANDED at 25/25 by fixing, never de-scoring; findings #4 and #6 of the 2026-08-14 review remain OPEN and are recorded in-source - the row is NOT freeze-ready and a green run is silent on both (handoff §10).** **APPEND 2026-09-19 (lane C12-31-FINDINGS): findings #4 and #6 are now CLOSED in-source, and #7's watchdog residual closed with #6.** #4 — `beginC1231AureoleEvidence` (`Tools/visual-regression/probe-sky-aureole-anchor.mjs`) now requires the prior `.latest.json` to be byte-identical to its own write-once `<prefix>.<runId>.json` archive, the comparison the retained-first-red path already made; a rewritten or fabricated predecessor that still folds clean is refused. #6 — every browser acquisition/teardown step runs under its own ceiling, the observed completions travel in `lifecycle.browserClosure`, `Tools/visual-regression/lib/c12-31-aureole-gate.mjs` scores that list as a structural predicate, and the watchdog releases this run's own lock before it exits. Four behaviour cases in `Tools/visual-regression/c12-31-aureole-gate.spec.mjs` ("a fabricated predecessor that folds clean is refused against its archive", "an unsettled browser step fails as a named bounded timeout", "the watchdog hands its own lock back before it exits", "the bounded runner's own observations are what the gate scores"), each red under an inertness or ordering mutant; the spec is 29/29 and is now homed under the ratified `test-s5` runner (`R-2026-09-02-16`), which it had never had. **What this does NOT change: the FULL acceptance sweep is still owed, and the row is still not freeze-ready on that ground alone.**
- **§5 exit tail — EXIT-3 DISCHARGED at Batch 1144 (see §5 close-condition 3 for the four residuals it closed and the EXIT-2 ordering caveat); EXIT-2's audit LANDED Batch 1082 (see §5 close-condition 2 for its dormancy tripwires) — neither is a gate lane:** **EXIT-2** = the class audit asserting that for every celestial uniform gate (`enableStarBrightnessModulation`, cloud-cover occlusion, `enableMoonPhase`, `enableEarthshine`, `enableNightSkyDimming`) either a GLSL consumer exists **or** the default is off — the gate closes the CLASS, not the instance. **EXIT-3** = `FEATURE_INVENTORY.md` updated (celestial WIP §C → §B; airglow added to §D). EXIT-1/4/5 ride the rows above.
...
- **`C12-33`'s two residual debts are maintainer calls, not machine work.** (a) **Banking route:** `certification-20260824b.json` (PASS / exit 0 / certificationEligible true, sha256 `c2a0bcf9…fb9ed`) carries no `runId`, and `assertFinalArtifact` requires `artifact.runId === --run-id`, so no publication route accepts it as an authoritative artifact today. (b) **Countersign:** the review attestation (sha256 `430de639…e2518`) records `reviewer.identity` as a self-asserted automated station-3 label, and each of its three findings states that the maintainer countersignature is OWED.

**IN FLIGHT:** nothing dispatched on C12 at this batch.

**BLOCKED before machine lane:** `C12-11` — RETURNED TO HELD with ten architecture blockers plus the B1048 hermeticity red. *(`C12-33` left this list on 2026-08-24: its design ruling landed as `R-2026-08-21-15` and its acceptance ran. Its two residual debts are maintainer ASKS, not machine-lane blocks — see BLOCKED / ASKS above.)*

**OWED (machine lane, in order):** (1) `probe-celestial-gates.mjs --g3` re-run only after a candidate can address the ratified `>= 2700 px/face` / `<= 2.0 arcmin/px`, chroma, and dust bars — 4096 is a non-certifying preference, not a precondition; if twinkle still triggers after a certifying asset candidate, the asset variable is eliminated and the **DR-01 revisit becomes a clean single-variable question — DR-01 is NOT decided until then**; (2) the `C12-31` full acceptance sweep; (3) *(freshness only — the answer is already on disk; see the `C12-G1F2` row)* a Lane-A `m2aRatio` re-read at HEAD; (4) `C12-12` default-sky identity run.
...
### 2026-08-28 CLOSE-OUT SPRINT INTAKE (lane C12X, armed by `R-2026-08-28-10`)

_Derived at `a3aef4c2dd` (Batch 1243) in the `cesium-lane-c12close` clone. `main` is one
commit ahead at `e0b92c26e7` (Batch 1244), whose entire diff is the BlackMarble level-3
JPEG pyramid plus `MAINTAINER_RULINGS_2026-08-28.md` and `QUEUE_2026-08-28_VISUAL_WAVE.md`
— nothing in C12's surface — so every reading below carries to `main` unchanged. No
browser ran for this stamp; every claim here is either a node gate re-executed at this tip
or a re-derivation from shipped source. Nothing here closes a browser-owed row._

**THE CLOSE-OUT SET, with ownership. `R-2026-08-21-16` and `R-2026-08-21-14` already ruled
four items out of the gate; what remains divides three ways and only one lane is machine
work that needs no browser.**

| # | Item | True blocker | Owner |
|---|---|---|---|
| 1 | `C12-29` S3 (canonical owner `C13-41`) — exit condition 2 | a MECHANISM for the shadow-contrast excess, not a re-run | ENGINE/SPEC → **advanced below**, one Edge leg pre-registered |
| 2 | `C12-29` S3 — exit condition 1 (SOL-4) | one refused segment: the pre-segment GPU readback drain does not close, so `refreshCostMeasured` stays false | EDGE |
| 3 | `C12-29` S5 | the seven-lane certification matrix has never run in a browser on three of its lanes | EDGE |
| 4 | G3 + `C12-12` 4096 tier | the re-bake `R-2026-08-10-4` orders, batched with the HDR-hardware check by `R-7` | **MAINTAINER SESSION** (`R-2026-08-28-5`) |
| 5 | `C12-31` acceptance sweep | the full sweep. *(2026-09-19: findings #4 and #6 are CLOSED in-source with four behaviour cases and their mutants — see the `C12-31` bullet above; the sweep itself is untouched and still owed.)* | EDGE |
| 6 | `C12-33` | banking route + countersign | MAINTAINER ASK |
| 7 | §5 exit tail EXIT-2 | **DISCHARGED — see below** | — |
| 8 | G1 | RED, ruled acceptable at close by `R-2026-08-21-14`, carried to proposed C17 as `CLT-D10` | not a blocker |

**FINDING 1 — EXIT-2 IS GREEN AT A CLEAN HEAD, AND ITS ORDERING CAVEAT IS DISCHARGED.**
`Tools/visual-regression/celestial-gate-class-audit.spec.mjs` runs **6 pass / 0 fail** at
`a3aef4c2dd` on a clean tree. The caveat under §5 close-condition 3 held EXIT-2 open until
the unlanded lane-F line `WebGPUPointCloudEyeDomeLighting.ts:780 command.enabled !== false`
was classified "when the lane-F package lands". **It landed** — Batch 1167 (`d9e1e04e5d`),
which carried the classification into the audit in the same commit; the line now sits at
`:811` and the spec lists its file at `celestial-gate-class-audit.spec.mjs:1266`. The
condition the caveat named is met, and the audit's own subtest "the masked `!== false`
discovery has no unclassified leak" passes. EXIT-2 closes.

**FINDING 2 — THE SUSPECTED S5 SOURCE-BOUNDARY RED DOES NOT REPRODUCE.** A late-2026-08-28
run reported `c12-29-s5-svs-footprint-gate` test 31 ("semantic source boundary is complete
and raw shaders are map-excluded") failing 47/1. Re-executed in isolation at this tip it is
**green**, and the surrounding S5 node gates are green with it: `terrain-selection` 25/25,
`solar-glare-star-washout` 41/41 (`C12-13`/`C12-14`), `skybox-resolution-policy` 27/27
(`C12-12`), `sky-brightness-twilight` 27/27 (`C12-36`). The earlier red was clone state, not
a HEAD state. **This does not discharge any S5 browser lane** — the node gates were never
the owed half.

**FINDING 3 — `C13-41` EXIT CONDITION 2 HAS A MECHANISM, DERIVED FROM SHIPPED SOURCE.** The
row's standing statement was that the class ("an un-shadowable residue dimming more slowly
than the terrain") stands while the visible cloud deck fails to account for the excess — its
implied share 0.4724 exceeds the beer-floor ceiling 0.32266890343992366. **The locus is the
fog / ground-atmosphere term, and the reason it dims slowly is that it is display-encoded
inside the globe fragment shader before it is mixed into the still-linear surface colour.**
Read in order: the shadowable term takes the factor and then the cast shadow and stays
linear (`GlobeTerrain.wgsl:4857`, `:4897`); the fog term takes the same factor
(`:5056`), is then tonemapped and gamma-encoded (`:5112-5113`), and only then mixed in
(`:5120`). WebGL is identical — `GlobeFS.glsl:1025`, `:1039-1040`, `:1043` — so this is
**inherited upstream shape on both backends, not a WebGPU defect, and nothing is proposed
to change in the engine.** A concave encode does not commute with a multiply, so a linear
dim of `F` ≈ 0.4642 arrives at the measured 8-bit band as ≈ 0.64–0.69. At that residue dim
the two-term model needs a share of ≈ 0.09–0.13 — **inside** the ceiling, exactly where the
deck hypothesis was outside it. Two independent numbers agree without either being fitted to
the other: the locus table asks for 0.681427 at share 0.1, and the shipped encode chain
returns ≈ 0.675.

**WHAT THAT DOES AND DOES NOT DO.** The predicted contrast ratio is invariant along the
curve `share × (residueDim − terrainDim) = compositeDim − terrainDim`, so this lane's three
banked numbers cannot separate the pair; the residue dim is supplied from the shipped encode
chain rather than fitted, which is the only reason the hypothesis is falsifiable. No band
moved, no predicate was demoted, and the new reading is registered reported-only and is
absent from the gate list by construction. **THE ONE MEASUREMENT THAT DECIDES IT is
pre-registered in the instrument**: re-run lane B's deepest rung with the fog term removed
and nothing else changed. If the mechanism is the excess, `shadowContrastRatioAtDeepest`
must COLLAPSE from 1.0341102079879674 toward the split model's own ~1.0002–1.0008 and land
inside [0.97, 1.03]; if it stays near 1.034 the hypothesis is REFUTED. It is a collapse
test, not a value match, which is what makes it robust to the scene-wide encode.
`eclipse-cloud-response-gate.spec.mjs` is **176/176** with the eight new mechanism tests,
four of them mutants.

**FINDING 4 — TWO INTERNAL CONTRADICTIONS CORRECTED IN PLACE (both flagged by the
2026-08-25 close-out audit stamp and neither resolved until now).** §5's close sentence read
"C12 closes when all four gates pass on both backends at HEAD" while §0 records
`R-2026-08-21-14` permitting G1 to stay red; and the `C12-G1F2` row read "the re-measure …
is STILL OWED" while §0 records that re-measure as executed at Batch 904 and on disk since
2026-08-07. Both now carry the ruling and the run. A campaign cannot be declared closed
while its own status authority contradicts itself on whether it may close.

**FINDING 5 — INDEPENDENT CORROBORATION OF FINDING 3, FROM A BRIEF THAT NAMED NEITHER FOG
NOR GAMMA.** A read-only worker audit was dispatched with a brief that asked only for an
exhaustive table of every multiply, add, mix, tonemap and encode reaching a down-looking
ground pixel on both backends, and for four questions about which terms the eclipse factor
and the cast shadow reach. It returned, without prompting: *"Fog atmosphere: YES-linear at
`GlobeTerrain.wgsl:5058-5062`, before the SDR tonemap and gamma at `:5111-5112` and before
the mix at `:5119`. Therefore its final SDR dependence on F is indirect/nonlinear."* That is
Finding 3, re-derived from the source by a different reader who was not told the
hypothesis — the third verification tier, which is the only one that catches a false
diagnosis.

**FINDING 6 — THE AUDIT SURFACED A SECOND, STRONGER RESIDUE CLASS: TERMS THAT TAKE NO
ECLIPSE FACTOR AT ALL.** Verified at source rather than taken from the report.
`GlobeTerrain.wgsl:4491-4494` returns early for subsequent imagery passes —
`if (isSubsequentPass) { return makeFragOutput(vec4<f32>(color, alpha), normalEC); }` — which
is BEFORE the eclipse multiply at `:4857`/`:4879`, before the cast shadow at `:4897`, and
before the fog block. The night-lights emission those passes carry is additive
(`return color + emission`, `:2251`). **So in a multi-imagery-layer scene, subsequent-pass
imagery is neither eclipse-dimmed nor cloud-shadowed.** That is a residue with dim exactly
1.0, which the two-term model accommodates at a share of ≈ 0.04 — also inside the ceiling.
The audit lists further un-shadowable, un-eclipsed terms (underground tint, vector polyline
colour, the post-cloud HSB offsets; on WebGL additionally the fill highlight at
`GlobeFS.glsl:943`).

**WHY FINDING 6 MATTERS BEYOND THIS ROW, AND WHY IT IS FILED RATHER THAN FIXED.**
`R-2026-08-28-3`/`-8`/`-9` have just made a night imagery layer and its city lights DEFAULT
ON. If that ships as a second imagery pass, the default globe now carries an
eclipse-invariant, shadow-invariant additive term — which bears on eclipse correctness
generally, not only on this gate's band. It is filed as
`NEW-SUBSEQUENT-PASS-IMAGERY-SKIPS-ECLIPSE-AND-CLOUD-SHADOW`. **It is NOT fixed here**:
whether the probe's fixture uses multiple imagery passes cannot be settled offline, the
correct disposition on both backends is a design question, and nothing about the C12 exit
gate authorises an engine change to the imagery path in a close-out lane.

**FINDING 7 — TWO S5 INSTRUMENT REDS AT HEAD, BOTH REPAIRED, PLUS TWO GATES THAT CANNOT BE
RUN.** These were found by executing the S5 node gates rather than reading their status.

- `c12-29-s5-replacement-device-gate` was **red on a canonicity check that could never pass
  on a CRLF checkout**. Root cause: the fleet's two canonical-source constants are built by
  different mechanisms and the language treats them differently.
  `FUSED_SNAPSHOT_CAPTURE_SOURCE` is a template literal, whose line terminators the
  specification normalises to LF; `C12_29_S5_REPLACEMENT_SAMPLER_SOURCE` is
  `Function.prototype.toString()`, which returns raw source and keeps CRLF. Every caller
  normalises the probe text it reads, so the sampler comparison was permanently red and its
  sibling permanently green — which is exactly why only one of the pair ever failed.
  Repaired at the two comparison sites only; the exported constant and its SHA-256 are left
  byte-identical so the provenance value still describes the bytes on disk.
- `eclipse-cloud-ibl-response` D3 was **red on a stale source-text assertion**. It pinned the
  refresh predicate's `||` layout at the call site, and the predicate was hoisted into
  `isDynamicEnvironmentMapRefreshRequested` with a state object. The shape it matched no
  longer existed anywhere, so the ordering claim it protects — that `eclipseEnvChanged` sits
  with the sky-bake terms and is deliberately not march-gated, which is load-bearing for
  C13-41's refresh accounting — was unguarded. Re-cut onto the helper's own disjunct chain
  AND onto call-site membership, so a right-shaped helper reached with the term omitted is
  now caught too. 18/18.
- `c12-29-s5-svs-footprint-gate` does **not complete in 25 minutes**, and
  `c12-29-s5-dense-cost-gate` needs about 26 minutes (it finishes **40/40** when given
  40; every earlier "red" from it in this lane was a timeout of mine, not a failure of
  its). A gate nobody can run to completion in a
  bounded lane is a gate whose reds get misread as timeouts and whose timeouts get misread
  as reds — which is precisely what produced the phantom 47/1 of Finding 2. Filed.
- Three S5 gates hash `packages/engine/Source/Shaders/GlobeFS.js`, a **gitignored build
  output** (`c12-29-s5-dense-cost-gate.mjs:314-315` names it as the `generated` half of a
  `raw`/`generated` pair). They therefore pass only on a machine that has built and ENOENT on
  a fresh clone. That is by design for a provenance gate, but it is undeclared: it belongs in
  the lane's preflight, not in a stack trace. Filed, not changed — narrowing a provenance
  gate's inputs is not a close-out lane's call.

**NODE GATE STATE AT THIS TIP, after the two repairs above.** `eclipse-cloud-response-gate`
**176/176** (168 + 8 new mechanism tests, 4 of them mutants); `eclipse-cloud-ibl-response`
**18/18**; `celestial-gate-class-audit` **6/6**; `c12-29-s5-terrain-selection-gate` 25/25;
`c12-29-s5-multiview-gate` 44/44; `c12-29-s5-custom-ellipsoid-gate` 201 pass / 0 fail;
`c12-29-s4-orbital-sunrise-gate` 26 pass / 0 fail; `c12-31-aureole-gate` 25/25;
`solar-glare-star-washout` 41/41; `skybox-resolution-policy` 27/27;
`sky-brightness-twilight` 27/27. **None of this is browser certification and none of it
closes a row.**

**⚠ FINDINGS 3 AND 6 ARE CORRECTED BY THE SAME CHECK, AND THE CORRECTION IS THE MORE
USEFUL RESULT. The law stands; the INSTANCE named in Finding 3 is wrong for this fixture,
and Finding 6's candidate is wrong for it too — both are excluded by the probe's own pins.**

Finding 3 left one condition explicitly unsettled — whether lane B takes the fog branch —
and called it a runtime question no offline derivation could answer. That was wrong: the
probe settles it in its own source. Lane B pins the scene with
`{ groundAtmosphere: false, fog: false, sky: false }`
(`probe-eclipse-cloud-response.mjs:734-736`), and `pinScene` answers those by setting
`scene.fog.enabled = false` and `scene.globe.showGroundAtmosphere = false`
(`lib/weather-probe-pinning.mjs:560-563`) and by removing every imagery layer (`:542`).
So in the banked run `tile.fogDensity` is 0 and `groundAtmosphereControl.x` is 0 — **neither
the fog branch nor the far-atmosphere drape executes** — and there are no imagery layers, so
**Finding 6's subsequent-pass imagery term is absent too.**

**Two candidate loci are therefore CLOSED BY CONSTRUCTION, on the fixture's own pins.** That
is worth more than the guess it replaces: no future run has to re-open either, and the
eliminations are executable rather than asserted — `eclipse-cloud-response-gate.spec.mjs`
test M10 reads both pins out of the probe and the pinning library and fails if either is
reversed.

**THE LIVE INSTANCE IS THE TERM THE ROW ALREADY NAMED, NOW MECHANISED.** ProceduralClouds
tonemaps its own contribution INSIDE the cloud shader and then composites the result over
the still-linear globe: `ProceduralClouds.wgsl:2643` `let exposed = weightedColor *
cloud.exposure;`, `:2644` `let toneMapped = exposed / (exposed + vec3<f32>(1.0));`, `:2658`
`var hazed = mix(toneMapped, cloud.aerialColor, aerial);` — and the source says so itself at
`:2651-2652`: *"Both operands are LDR — the post-tonemap color against the packed horizon
tint — so the lerp stays in display space."* Same encode-before-composite asymmetry, with a
Reinhard curve instead of tonemap-then-gamma. The row's phrase "ProceduralClouds'
independently tone-mapped over-composite" is no longer just a name for the confound; it is
the mechanism, with the line that performs the encode.

**AND REINHARD MAKES THE CLAIM SHARPER THAN THE FOG VERSION COULD.** Its dim has a closed
form, `a(F) = F·(L+1)/(F·L+1)` for exposed radiance `L`, rising from `F` toward 1. At the
banked factor the required residue dim of 0.5347 is reached **only above `L` ≈ 0.35**, so the
mechanism makes a refutable demand on a quantity the lane controls rather than accommodating
any value. Over a declared exposure sweep of 0.5/1/2/4 the implied residue shares are
0.221 / 0.129 / 0.084 / 0.062 — all inside the beer-floor ceiling of 0.32266890343992366,
where the visible-deck hypothesis needed 0.4724 and was outside it.

**THE DECIDING MEASUREMENT CHANGES ACCORDINGLY, and this supersedes the fog-off leg Finding 3
pre-registered — a leg this fixture cannot run, because fog is already off.** `cloud.exposure`
is a live dial that moves `L` and therefore moves `a(F)` along a known curve. Re-run lane B's
deepest rung at two separated exposures with nothing else changed. The mechanism predicts
`shadowContrastRatioAtDeepest` RISES with exposure, by the amount `a(F)` prescribes at each
point. A reading that does not move with exposure refutes the cloud-composite locus; a
reading that moves the other way refutes the curve. It stays a direction-and-magnitude test
rather than a value match, so it survives the scene-encode approximation.

**The fog function and its tests are RETAINED, and not because they were written.**
`scene.fog` is enabled by default in ordinary scenes, so the fog instance of this law is live
everywhere except in lanes that pin it off — including, potentially, the C12-31 and G-gate
lanes. It is the same defect class, one branch over.

**What this episode is, stated plainly:** an unverified premise that survived into the
instrument, caught by the check the derivation itself said could not be done offline. It
could have been caught before the first edit by reading the probe's pins — which is Principle
10's whole content. The two eliminations, the corrected locus and test M10 are the residue of
having checked; the reported-only readings never gated anything, so nothing downstream
carried the error.

### 2026-08-25 CLOSE-OUT AUDIT STAMP — three findings nobody had written down

_Record-only, derived at tip `34fb32c71a` from artifacts already on disk. No run was
executed for this stamp and no verdict is granted by it._

- **`C12-G1F2` is NOT owed.** Its re-measure ran at Batch 904 (tip `c810dbace2`, Edge
  151.0.4129.72, `sourceDirty:false`) and has been on disk since 2026-08-07 in
  `Tools/visual-regression/output/celestial-g1.json`: on the re-scoped Lane A's
  certifying `default` mode, `m2aRatio` 1.0000000369138204, `meanLumRatio` 1,
  `stddevRatio` 1.0000000369138204, `default_m2a_in_band` true, and all ten Lane-A
  criteria true. Only freshness is missing — the reading is 396 commits behind HEAD.
- **`C12-36`'s remaining work is the filed diagnostic, not a re-run.** The repaired
  PIXELS leg has already run (Batch 904, exit 3 STRUCTURAL): ENGINE + CONTROL PASS on
  both backends, and the star-pixel leg is structural at the positional reachability
  control. Between `c810dbace2` and HEAD the star chain — `SkyBrightness.js`,
  `StarField.js`, `StarFieldMath.ts`, `SkyBoxFS.glsl`, `StarFieldFS.glsl`,
  `StarField.wgsl`, `CubeMapPanorama.wgsl` — carries **zero** non-comment change (the
  diff is comment rewrites only; no code token moved), so a bare re-run reproduces
  exit 3. The next action is `TWILIGHT-STAR-REACHABILITY-BLACK-BOX`.
- **All 18 banked `c12-33-moon-mip-motion` publications record**
  `result: {status: "NON_CERTIFYING", exitCode: 3, certificationEligible: false}` —
  every one of them, published 2026-08-24T23:59Z-2026-08-25T00:14Z. The lane's raw runs
  are therefore banked as non-certifying while the certification folded from them reads
  PASS / exit 0 / eligible true. Whether that is by design is **not** settled here; it
  bears directly on the C12-33 banking question and should be answered before a banking
  route is chosen.
...
critical path.** ~~Fifteen S5 runs are banked in the external evidence library~~ **Eighteen S5 run
artifacts are banked** and **exactly one is a PASS**:

_("Seven-lane" is this document's historical count. The tree carries **six** `probe-c12-29-s5-*`
probes and six matching gate libraries, and the table below tabulates six; the discrepancy is
unresolved and is recorded rather than silently corrected — naming a seventh lane or retiring the
count is the `S5-ROSTER` row's job, not this correction's.)_
...
| dense-cost | 1 | 0 PASS — STRUCTURAL/exit 3 (`8123136e-a302-41fa-a718-79a295affeb2`, 2026-08-29), `incomplete false`, `legs []`, seven structural reasons |
| multiview | 1 | 0 PASS — ERROR/exit 2 (`5323314a-a9cf-4cb4-b57e-c1a5186a01e8`, 2026-08-29), self-validation failed on **both** backends: *"webgl: offscreen ray pick is invalid; webgpu: offscreen ray pick is invalid"* |
| replacement-device | 1 | 0 PASS — ERROR/exit 2 (`8a1f6a67-ca21-4cec-b37d-49bdb31696b7`, 2026-08-29), thrown from `page.evaluate`: *"replacement-device executed measurementSha256 is not canonical"* |

Totals: ~~**15 banked / 1 PASS / 4 STRUCTURAL / 10 ERROR.**~~ → **18 banked / 1 PASS /
5 STRUCTURAL / 12 ERROR.** Landed gate libraries and
static GO are not browser certification. ~~Three of these lanes have a landed gate and no
run at all.~~ *(struck 2026-09-19, not deleted: each of the three now has a banked 2026-08-29
artifact — see the correction below.)*

_**Corrected 2026-09-19** (dated in place; the struck text is kept so the drift is visible). The
three lanes this table recorded at 0 runs and "never executed in a browser" each have a banked
2026-08-29 artifact under `Tools/visual-regression/output/edge-tranche3e-e-2026-08-29/`, read
directly for this correction — including replacement-device's, which was thrown from
`page.evaluate` and is therefore unambiguously a browser run. They are banked in that gitignored
output tree rather than in the external evidence library, which is why the library-derived total
missed them. **None of the three is a PASS, so the matrix's standing is unchanged in substance: S5
is UNMET, S5 is still in this gate, and this correction changes no status.** The three also carry
**no machine-readable owner** in
[`FINDING_DISPOSITIONS_2026-08-13.json`](FINDING_DISPOSITIONS_2026-08-13.json), which names only
terrain-selection (`closed`), custom-ellipsoid (`active`) and NASA-SVS (`active`); they stay
ownerless and are recorded as ownerless here, so no later act can "close" S5 and leave three lanes
with no disposition at all. A terrain-selection re-run, which dense-cost requires as a prerequisite
publication, would reopen that `closed` entry if it went red — `R-2026-09-19-15` rules that
consequence written down before any such session._

The fleet-contract red recorded here under audit findings S4/S5 — no
terminating watchdogs, browser leaks, and exit semantics living in `lib/*-gate.mjs`
where the contract did not scan — is **REPAIRED**: fix SOL-5 landed at Batch 1042
(`4371c74a63`) and was discharged at Batch 1104 (`fc3f53bb97`). Verified at this tip:
all six S5 probes carry a terminating `PROCESS_WATCHDOG_MS` above their own lane
watchdog (dense-cost uses the child-process `signal: "WATCHDOG"` + `unref` form); all
six `lib/c12-29-s5-*-gate.mjs` exist and declare their status→exit mapping, and
`probe-fleet-contract.spec.mjs` test D8 enumerates them; and the live
`PROHIBITED_READER_ALLOWLIST` carries no S5 entry (the one S5 string sits inside the
frozen, shrink-only `…_SNAPSHOT`). **The harness is repaired; the certification RUN is
what remains owed on the machine lane.**
...
- **G1: exit 1 — and the red is exactly the KNOWN Lane B.** The RE-SCOPED Lane A **passes** (per-mode criteria incl. the DR-01 zero-census assertion and its positive controls) and `cubemapParity` **passes** (differingFraction 6.25e-6 at maxChannelDelta 1 — quantization-level; the 0.0019875 figure belongs to the star-modulation lane's `modulation-on` mode). The only failures are `in-column-star-modulation:modulationEngaged_on_both_backends` + `starEnergyRatio_in_band` — the lane the standing rule forbids attributing until `NEW-WEBGPU-SKYATMOSPHERE-SHELL-EXTENT-ALPHA` is decided (its starEnergyRatio currently measures the shell's alpha, not the modulation term). **The re-scope is therefore VALIDATED at pixels; G1 stays honestly red pending the shell-extent measurement + decision.**
...
| `C12-33` | **IMPLEMENTED / LANDED Batch 819 — ACCEPTANCE OWED, do NOT mark complete. C12-35 PREREQUISITE PASSED — **RULED `R-2026-08-21-15` (2026-08-21): BOTH designs. `sign-test-v1` (four-cell sign test, 1e-9 gate, custody-hashed at Batch 1100) is the design of record NOW and its scripted 2.5-hour Edge ten-run set runs in the machine lane (queued behind the G9 run); the sixteen-cell ratio design is ALSO ordered as a later build, to run only after the maintainer pre-registers its correlation `r` (never post hoc), under its own designId and custody hash.** **TEN-RUN SET EXECUTED 2026-08-21 evening (runs 01:37-01:53Z, ~16 min total, not 2.5 h): 10/10 STRUCTURAL exit 3 — INSTRUMENT, not product. Two independent instrument defects, identical across all ten runs: (1) FRAMING — every lane measured a ~785 px disc against the 132-348 px band because the catalog starfield (`skyBox.starField`, independent of `skyBox.show`) left 6-20 stray lit pixels per frame that inflated the global bounding box; the Moon itself was correct (principal component 45,224 px = 240 px). REPAIRED Batch 1134 (starfield disabled with a read-back witness; largest-connected-component stray count → STRUCTURAL `background is not black` before the framing check; design metric bit-identical over all 104 frames; custody hash unchanged). (2) READINESS — eight failures per run: the 60 s readiness deadline elapsing, WebGPU reporting Moon textures / normal map / pipeline / mip chains not loaded while its captured frames show a fully textured Moon matching WebGL to 0.2%, a reason firing on `actual=missing, expected=missing`, and the WebGL pick shader not compiling with the lunar explicit-gradient defines — under diagnosis; NOT yet repaired. The set is not re-run until (2) is fixed. **(2) READINESS REPAIRED Batch 1138, CONFIRMED ON HARDWARE the same day.** Eight failures per run, one cause: the WebGPU Moon renderer returns from `updateWebGPUMoon` before any texture request when the Moon is behind the camera (`dotForward < -maxRadius`), and readiness was awaited at the launch pose, where the Moon sits 91.3° off the view axis (`dotForward = −9.37e6 m` vs a −1,737,400 m pad); at readiness WebGPU read `albedoRequestSerial 0`, `pipelineReady false`, all four mip fields null — nothing ran — while `finalDiagnostics` after the lanes read 2048×1024 / 12 mips. Repair: both viewers pre-positioned to the `close` lane's sample 0 before a 20 s (was 60 s) bounded readiness wait and before `applyControlMode`; readiness carries per-backend/channel `blockedBy[]` from observed state and a `moonInFrontOfCamera` observation mirroring the renderer's own dot-product predicate; `actual=missing, expected=missing` is unreachable; both statistics shapes publish Moon texture dimensions so expected mip counts derive from published state only; `compileWebGLPick` drives one `passes.pick` update so the pick program compiles (the Moon is unpickable by upstream design — `NEW-MOON-NOT-PICKABLE-ENVIRONMENT-PASS-GATE`); the `maxLod ≡ mipLevelCount − 1` invariant lives in the contract spec, not the run verdict. Contract 33/33, certification 24/24, custody hash `ac3a3cde…ce81` unchanged, ten source mutants caught. **MACHINE LANE, 2026-08-24 17:16-17:41 ET, fresh bundle (`servedMatchesLocal: true` on all five identity entries), twenty runs = the counterbalanced ten-run schedule `C12_33_COUNTERBALANCED_CONTROL_ORDER` executed TWICE (blocks `20260824a`/`20260824b`): hardFailures 0 on all twenty (vs 16 per run on 08-21); readiness `ready: true`, `blockedBy: []`, 6.1-282.5 ms; every lane at its target disc on both backends (240/240/240/16 px), `strayLitPixels` 0; force-lod0 witnesses complete on all ten control runs (`baseLevelOnly` true both backends, sampler identity live, lodMin/Max 0/0, bind group rebuilt) and correctly negative on all ten normal runs.** The four pre-registered sensitivity cells separate in the same direction in all ten pairs (control − normal: `normalizedP95HighPassDelta` WebGL +0.2789 / WebGPU +0.2803; `spatialHighFrequencyCoefficientOfVariation` WebGL +0.02965 / WebGPU +0.02959; gate `> 1e-9`) — measured values, not a certification. Every run reports `measurementStatus: CALIBRATION_PENDING` and `manualInspection: PENDING`: the finalizer consumes only published v2 evidence manifests plus a hash-bound reviewer attestation (seam-centered, seam-at-limb, unresampled parity findings all PASS), neither of which the machine lane produces — those are the remaining steps. Design note for the maintainer: all ten `normal` runs are byte-identical to each other, and all ten `force-lod0` runs likewise (frame sets 6,309,913 B and 7,507,908 B respectively), so the ten-run repetition carries the information of two runs — the renderer is a fixed point at a pinned clock; the sign test's variance estimate is therefore nil, which R-15's sixteen-cell design should take into account. Evidence: `Tools/visual-regression/output/performance/campaign12-c12-33-moon-mip-motion-edge-{normal,force-lod0}-20260824{a,b}-*.json` (+ frames). **CERTIFICATION LANE, 2026-08-24/25 (Opus station-3) — `C12-33-SHIMMER-ENVELOPE-CERTIFICATION` IS ISSUED on block `20260824b`.** Three prerequisites were cleared in order. (1) **Library repaired** under `R-2026-08-24-12`: publication had been refused for EVERY producer (`visual-evidence library is not safe for publication`; `unsupported top-level library entry: c13-16-u2-b1108 / -b1108b / -b1108c`) because three C13-16 U2 directories were hand-copied into the library root on 2026-08-21. Inventoried (sha256, 185 files), rename-moved to a sibling staging folder with inodes preserved (no copy, no delete), verifier green, re-admitted via `import-legacy` as `legacy/c13-16-u2/c13-16-u2-b1108{,b,c}`, verifier green again, all 185 bytes confirmed content-addressed; staging left in place, nothing deleted. Forced deviation, proven not assumed: the tool stamps Git provenance from `--source-root`, so the import ran from a byte-verified scaffold inside the worktree because the sibling folder has no `.git`. (2) **Block `20260824a` was lost to a provenance collision** — Batch 1138 landed mid-sequence, `archive` refused runs 08 and 10 with `repository provenance changed during archival`, and the eight that landed carry three distinct repository identities; since `validateSharedIdentity` demands one identity, `manifest.runId` is pinned to `report.runId`, idempotent retry requires matching provenance, and the library is append-only, block a can never form a valid set. The replicate was deliberately withheld rather than spent into commit traffic. (3) **Block `20260824b` published in a coordinator-granted quiescent window**: ten runs, ONE repository identity (`head 5af3cf73a3`, `statusSha256 4a8c5d21…`, 28 tokens, `stable: true`), 105 files each, sidecars valid, chronological control order equal to `C12_33_COUNTERBALANCED_CONTROL_ORDER`; the baseline was re-checked after the finalizer and every identity field was unchanged. **Seam review PASS on all three findings**, and it transfers to this block by sha256 rather than by assumption — all 104 frames of `20260824a-01`/`-02` are byte-identical to `20260824b-01`/`-02`, and within block b all five normal runs are frame-identical to one another and all five controls likewise while normal differs from control, confirming the fixed-point property on this block. `seam-centered`: no center-line discontinuity, blur, halo or crawling line; HF flat 0.0268–0.0269, luma monotone 167.5→167.6, disc 240 px, stray 0 across all thirteen samples. `seam-at-limb`: no limb flash, no seam entering or leaving the silhouette; HF monotone 0.0293→0.0295, luma monotone 150.0→149.3, disc 240 px, stray 0. `webgl-webgpu-unresampled-parity`: IoU exactly 1.000000 every lane both modes, normalized mean abs luma error 0.0010/0.0014/0.0015 on the 240 px lanes, changed-pixel fraction 0.0033–0.0041, HF ratio 0.9939–0.9984. The control discriminates at 2.13–2.20× spatial HF and 4.95–5.04× p95 high-pass delta. Two caveats recorded, not smoothed: cross-backend agreement is TIGHTER under force-lod0 (0.0006 vs 0.0010–0.0015) because base-level-only sampling hits identical texels, so the normal-path residual is attributable to mip filtering itself; and `minified-16px` is the weakest parity lane (0.009 normal / 0.0119 control) because one texel is a large fraction of a 16 px disc. **Finalizer verdict: `status PASS`, `exitCode 0`, `certificationEligible true`, zero structural and zero acceptance failures, 5 calibration pairs, 88 threshold values, policy `five-pair-observed-normal-envelope/v1`, all five pairs `sensitive: true`, zero normal-quality failures.** Artifact `Tools/visual-regression/output/c12-33/certification-20260824b.json`, 706,734 B, sha256 `c2a0bcf9f11cd00075baacd9f57ae9dc5437a47b64755aad94f309ca838fb9ed`, finalized `2026-08-25T00:17:48.888Z`, `designId sign-test-v1`, custody `ac3a3cde…ce81`. **Scope, stated by the artifact itself: `doesNotMeasure: ["observed mip or texture-LOD selection across camera motion"]`** — this certifies the shimmer envelope and seam behaviour, NOT observed mip selection. **The certification artifact could not be banked in the library**: `assertFinalArtifact` requires `artifact.runId` to equal `--run-id` and the certification schema has no `runId` (dry-run: `authoritative artifact runId undefined does not match 20260824b`); `import-legacy` would force `NON_CERTIFYING` and misrepresent a PASS. Left for the maintainer; nothing hand-placed. Library final: `valid: true`, 51 publications, 373 objects, 371 referenced, 2 orphaned (bytes staged by the two refused block-a archives — a warning, not a reason). **MAINTAINER COUNTERSIGN REMAINS OWED**: `reviewer.identity` is a self-asserted automated station-3 claim and no machine step supplies a human signature. `R-2026-08-21-15`'s sixteen-cell ratio design is untouched by this and its `r` stays un-pre-registered per `R-2026-08-24-11`. Recorded Batch 1148. [**RULED `R-2026-08-24-11` (2026-08-24):** the sixteen-cell ratio design's correlation `r` is NOT pre-registered this wave and the sixteen-cell build is NOT dispatched — `sign-test-v1`'s ten-run set reports first, because pre-registering a correlation before seeing the instrument's own dispersion would be pre-registration in form only. `R-2026-08-21-15` stands in full: both designs remain blessed, `sign-test-v1` remains the design of record, and the never-post-hoc rule on `r` is reaffirmed, not relaxed. This ruling sets the ordering and nothing else. ([MAINTAINER_RULINGS_2026-08-24.md](MAINTAINER_RULINGS_2026-08-24.md).)] Evidence: `Tools/visual-regression/output/performance/campaign12-c12-33-moon-mip-motion-edge-normal-2026-08-22T01-*`.** / design corrected 2026-08-02.** Execution began 2026-08-02 in three independently reviewable lanes: Moon-local WebGL mips, the canonical frame-owned WebGPU texture-mip queue and Moon realization, and the lockstep shader/implicit-LOD correction. Neither backend mipmaps Moon textures at the starting point, so the 2K albedo minifies ~64:1 at the default camera. C12-35 moved WebGL Moon realization out of generic Material loading: generate mips Moon-locally after direct `Texture` creation, use trilinear sampling, and retain the WebGL1 NPOT single-level fallback. WebGPU allocates the exact mip count on each lifecycle candidate, queues generation through the context's frame-owned shared texture-mip lane (generalize the existing imagery-named enqueue; no private submit), cancels any same-frame destroyed texture job, and uses a trilinear sampler. Flatten the opaque Moon fragment's front/back selection to one `computeEllipsoidColor` call, then compute longitude-unwrapped `dFdx`/`dFdy`/`dpdx`/`dpdy` BEFORE the fragment-varying miss discard and sample albedo and normal with explicit `textureGrad` / `textureSampleGrad` — the shipped design; see point 2 of the 2026-08-02 corrective overlay above (§"The implementation corrects the earlier design note"), which supersedes this row's earlier implicit-`textureSample` and CPU-single-LOD prescriptions. Sharing the same normalized UV gradients does not couple the two maps' LOD (hardware scales them by each texture's dimensions), while one CPU LOD scalar would oversmooth one map. Pin opacity and test the equirectangular seam/limb/close views. Land mips before the optional 2K normal rebake. Shader change ⇒ lockstep row plus Naga and moving Edge shimmer/parity/seam evidence. | M |
...
| `C12-36` | **`NEW-SKY-BRIGHTNESS-ESTIMATOR-NO-TWILIGHT-RANGE` (filed by C12-29 S6, 2026-07-25 — Principle 9, surfaced rather than routed around; originally numbered `C12-S6F1` in the parked S6 worktree, renumbered on extraction 2026-08-01 because `C12-33` was taken; **renumbered AGAIN to `C12-36` on 2026-08-11** (C12-35 is the moon-lifecycle row) because ruling R-2026-08-10-3 assigned `C12-34` to the sun-bloom mirror before this collision was noticed — CO-37 ledger-hygiene catch).** **LANDED PREMISE (corrected 2026-08-21):** Batch 823 (`f7c617304d`) replaced the legacy sun-term `smoothstep(-0.1, 0.4, sunAlt)`; it is not live at HEAD. The landed estimator is the log-luminance twilight model detailed below: a zenith-photometry ladder, linear sun-plus-moon luminance, a `p^3.64` lunar phase-flux law, and the derived NELM transfer. The engine/control leg is recorded passing, but the star-pixel leg remains **OWED**; this row is not complete. ✅ **IMPLEMENTED (worker, 2026-08-02; LANDED Batch 823) — ENGINE-LEG EDGE ACCEPTANCE **PASSED** at Batch 824, star-PIXEL leg still OWED as an instrument gap; not COMPLETE. See the EDGE ACCEPTANCE paragraph at the end of this row.** The double-smoothstep is replaced by a log-luminance model in `SkyBrightness.js`: the sun term is the published zenith twilight-photometry ladder (μ in V mag/arcsec² — sunset 8.0, end of civil −6° 14.0, end of nautical −12° 19.7, astronomical night −18° 21.9, day 4.0 saturating at the SAME +23.6° the old smoothstep did), the moon term is the published full-moon zenith brightness (18.0, i.e. `10^(0.4·3.9) − 1 = 35.31` night-sky luminances) scaled by a `p^3.64` phase-flux law (`0.5^3.64 = 0.0802`, the published quarter-moon ≈8% of full-moon illuminance) instead of a flat 4% with LINEAR phase, the two sum in **linear** luminance, and the combined log-luminance passes through a perceptual transfer derived from the Crumey/Schaefer NELM relation at 0.5 mag NELM per mag/arcsec². **The curve constants are CONFIRMED, not moved:** the transfer is built so `modulation(B(μ))` IS the NELM chain `10^(−0.4·0.5·(21.9 − μ))` throughout the star window, which pins the pair at the shipped (inflection 0, steepness 23.0) and keeps the S2 totality anchor bit-exact. **MEASURED:** ≤ −18° and ≥ +23.6° are BYTE-IDENTICAL (exactly 0 and exactly 1). Across −18°→−6° the old star-factor span was **exactly 0.000000**; it is now **0.973697** (−15° 1.000000→0.604705, −12° →0.363078, −9° →0.098257, −6° →0.026303). Civil twilight: −3° 0.370549→0.006619, and −2° goes from **exactly 0** to 0.004175 — NELM 0.55, the row's "Venus and one or two first-magnitude stars". Full moon overhead 0.018176→**0.165959** (9.13×, −4.35 mag → −1.95 mag, NELM 2.15 → **4.55** against the published ≈4.5 — the row's headline defect, closed). High-sun totality is unmoved at 0.062810 (−3.00 mag); low-sun totality 0.5246→0.664912 (−0.44 mag). **Second finding, fixed en route:** `computeStarDayFade` and the estimator each carried their OWN normalize-and-dot of the camera position. They are now one exported `computeCelestialElevationSine` — the single home C15's aurora night-gate is to reuse — and the spec fails if a second one reappears. **Third finding, fixed:** the log-luminance model's "no sun" value is the DARK end, so a non-finite input would have inverted the module's documented "misconfigured scene → full bright" policy into a bright starry midnight; one degenerate-input policy is now stated and pinned. **Fourth finding, corrected:** `eclipse-sky-totality.spec.mjs`'s off-anchor test asserted the old numbers through an INLINE COPY of the deleted sun term — it would have stayed green while describing an estimator that no longer exists. It now runs the real `computeSkyBrightness` (51/51). **NO SHADER CHANGED** — the scalar travels on the existing `u_skyBrightness` / `params.w` uniform — so no `SHADER_PAIRS_LOCKSTEP.md` row and no naga run is owed; the spec asserts the four modulation implementations stay byte-identical and that neither backend grew a copy of the photometry. **Gate:** `node --test Tools/visual-regression/sky-brightness-twilight.spec.mjs` **27/27**, executing the real `SkyBrightness.js` + `StarFieldMath.ts` (no re-implementation), every metric banded or pointwise, with five MUTATION tests that feed the checks the exact legacy estimator, a binary day/night gate, a 1e-6 epsilon floor, a linear moon phase and a re-inlined second elevation derivation, and require each check to REJECT them. ⚠ **Edge acceptance still owed, and one known downstream effect to measure:** `FIXTURE_NIGHT_MAX_SUN_ELEV_DEG = −8°` in `lib/eclipse-fixture-constraints.mjs` was derived when the estimator saturated to 0 below −5.74°; the star modulation factor at that fixture instant is now **0.063390 (NELM 3.51), a ~16× dim**, not 1.0. `dayFade` is still exactly 1 so the constraint and threshold stand and `probe-eclipse-sky-totality`'s gated verdicts are unaffected (B4 counts command ownership; lane D's sprite/cubemap deltas are reported diagnostics), but the prose has been corrected and any future lane needing factor === 1 must ask for ≤ −18° explicitly.  **EDGE ACCEPTANCE 2026-08-06 (orchestrator, `probe-sky-twilight-range.mjs`): ENGINE + CONTROL legs PASS.** The SHIPPED SkyBrightness module, imported from the served source tree and driven at clock-SOLVED solar elevations, reproduces the derivation EXACTLY to six decimals on BOTH backends: -20deg 1.000000 (control, byte-identical end), -15deg 0.604705, -9deg 0.098257, -3deg 0.006619, webgl == webgpu bit-for-bit, 0 console/device errors. The probe's star-PIXEL leg reports STRUCTURAL (not FAIL): the star field drew nothing at the darkest lane in that configuration, so the leg measured an empty sky and its verdict is meaningless either way - an owed instrument gap, NOT a product verdict. Three earlier probe rounds were orchestrator instrument defects worth recording because the first presented exactly like a feature that never shipped: an object-literal call (the signature is POSITIONAL) hit the module's documented misconfigured-scene guard and returned a uniform 1.000000 on every lane; then sky BRIGHTNESS was compared against STAR-MODULATION predictions; then the pixel leg used a synthesized sun while the renderer drew with the clock's sun, so the two legs described different scenes. ⛔ **THAT LAST CORRECTION IS ITSELF INVERTED, AND THE STRUCTURAL PIXEL LEG ABOVE WAS AN INSTRUMENT DEFECT — root-caused and repaired 2026-08-07.** The landed probe does the OPPOSITE of what this row and commit `43caae0589` record: the ENGINE leg is clock-SOLVED, and the PIXEL leg rendered at the **WALL CLOCK**. `probe-sky-twilight-range.mjs:77` sets `useDefaultRenderLoop = false`, which kills `CesiumWidget.render()` — the only caller that passes `clock.tick()` into `Scene.render` — and every render in the file was then a bare `s.render()`, which `Scene.js` answers with `JulianDate.now()`; `SceneUtilities` stamps that onto `frameState.time` and `UniformStateComputations` derives `sunDirectionWC` from it, so `v.clock.currentTime = jd` reached no pixel. The commit landed at 15:31 UTC = ~08:31 local solar at the probe's site (lon −105 / lat 40), i.e. **full daylight** — which is precisely why "the star field drew nothing at the darkest lane". **The recorded `starPx` numbers from that run are VOID and must not be carried forward.** The file was byte-identical from `43caae0589` to HEAD, so it was unfixed for the whole interval. **REPAIR (2026-08-07):** every render now passes `at()` (= `v.clock.currentTime`, read per call so it tracks the current lane); the capture is same-task (`renderAndGrab` renders, reads `uniformState.sunDirectionWC`, and `drawImage`s with no intervening await, closing the separate rule-2 violation); and a new **RENDER-TIME leg** reads the sun back OUT of the rendered frame and requires each lane's rendered solar elevation to sit within `RENDERED_ELEVATION_TOL_DEG = 1.0°` of the elevation its clock was solved for AND the four lanes to be ≥ `RENDERED_ELEVATION_MIN_SPREAD_DEG = 2.0°` apart — so a wall-clock substitution, which collapses all four lanes onto ONE sun, cannot recur silently. The in-file comment claiming the sun is "synthesized … rather than solved from a clock" (the fingerprint of a clock solve retrofitted without touching the render call) is corrected. **Exit codes changed: 0 PASS \| 1 FAIL \| 3 STRUCTURAL.** The ENGINE-leg acceptance recorded above is UNCHANGED and unweakened — it never depended on the render call — but an exit 0 could not distinguish "both legs certified" from "one leg measured nothing", and this row is the case for the distinction. **The repaired PIXELS leg HAS run** — Batch 904 / tip `c810dbace2`, recorded in this file's CO-3 overlay: exit 3 STRUCTURAL, honestly. The ENGINE leg PASSED on both backends (all four lanes reproduce the shipped derivation to six decimals, control lane exactly 1.0, render-time verified). The star-pixel leg is STRUCTURAL because the positional reachability control found the target box completely black (vmag 2.14 at (370,203), census 0, box peak luma 0.0, both backends, sun −20°). **The next action is the filed diagnostic `TWILIGHT-STAR-REACHABILITY-BLACK-BOX`, not a re-run** — the star chain carries no non-comment change since that run, so a bare re-run reproduces exit 3. The star-pixel leg remains OWED and C12-36 remains incomplete. | M |
| `C12-38` | **NEW-SUN-DISC-CENTRE-DARK-SPOT (maintainer screenshot, 2026-08-24 ~19:00 ET; filed Batch 1148).** The rendered sun shows a small dark, brownish spot at the centre of its disc, inside the white core, mainly at dawn and sunrise when the sun sits low over the terrain. Reproduction from the screenshot: `Apps/CesiumViewer/index.html` (default renderer, WebGPU) at the saved view `view=107.5215780802716,35.05292293726632,1175.3399698570242,…` (the remaining view params are truncated in the screenshot — re-derive the heading/pitch from a low-sun pose at that position), clock `2026-08-24T23:01:41Z`, terrain + default imagery, no ion token; the sun is a few degrees above the horizon with the aureole spanning most of the sky. Candidate loci, all UNVERIFIED: the sun sprite/disc shader's centre texel or limb-darkening term (`C12-15` — limb ≈ 30% of centre; a centre sample that reads the atmosphere-transmittance at the disc's own depth would darken exactly the middle), the bright-pass / `C12-34` sun-bloom mirror compositing a tonemapped core over the disc, or a depth/atmosphere ordering issue at low altitude angles (`C12-31` aureole path). Not yet confirmed on WebGL — the first step is the Principle-8 probe: the saved view above on both backends across a dawn sweep (sun altitude −2°…+10°), disc-centre vs disc-annulus luminance ratio published per sample, with a WebGL twin as the parity control; a FAIL bar is pre-registered from the first WebGL sweep, never from WebGPU. Owned by the C12 sun cluster; not in the current C12 exit gate until triaged (maintainer decision at triage). **RULED 2026-08-25, RECORDED 2026-08-28: the maintainer selected option (A) — verbatim answer "Co-fade alpha with the transmittance (Recommended)" via AskUserQuestion, session eb6dfaec. The ruling was taken on the 25th but never recorded here; that recording gap sent a later lane to refuse an unselected option-(B) implementation before the transcript settled it — the row now carries the ruling so no future reader re-derives it. Option (A) LANDED Batch 1184: co-fade implemented on both backends (shared Scene resolution, GLSL uniform + WebGPU pad conversion at byte 140), interpreter-based fragment spec executing both shaders with ten mutants, 451 celestial tests green. The option-(B) candidate stays banked-unselected in the worker archive. NO VERDICT on the artifact: the 13-sample dawn sweep with sample 7 as the preregistered discriminator remains the acceptance evidence, owed to the Edge executor.** **TRIAGED 2026-08-25 (diagnosis + instrument landed; NO VERDICT CLAIMED, Edge run OWED).** **Mechanism, verified by reading both backends:** the sun billboard blends `ALPHA_BLEND` on BOTH backends — `Sun.js:461` `blending: BlendingState.ALPHA_BLEND`, and `WebGPUEnvironmentRenderer.js:840-852` `src-alpha`/`one-minus-src-alpha`. The disc bake writes its limb-darkened intensity law into ALPHA (`SunTextureFS.glsl:101` `surface = step(radius, u_radiusTS) * limb`, and its CPU twin `WebGPUEnvironmentRenderer.createSunTexture`), so alpha is exactly 1.0 at disc centre (a0+a1+a2 = 0.30+0.93−0.23) and falls to 0.30 at the limb. The atmospheric transmittance is then applied to RGB ONLY and never to alpha — `SunFS.glsl:43` `out_FragColor.rgb *= u_atmosphereExtinction` and its WGSL twin `WebGPUEnvironmentRenderer.js:151` `color.rgb * u.extinction`. Under `ALPHA_BLEND` alpha is an OCCLUSION weight, so at low sun the disc REPLACES the bright aureole with a heavily extincted, reddened colour, most completely at the exact point where alpha peaks: the centre. Evaluating the engine's own `computeAtmosphereExtinction.js` at the reported site with shipped `Atmosphere` defaults gives (R,G,B) = (5.95e-2, 1.87e-2, 1.74e-3) at +5° and (1.64e-1, 7.56e-2, 1.54e-2) at +8.14° — the screenshot instant `2026-08-24T23:01:41Z` is +8.48° at that site — i.e. a dark brown at full alpha. Limb darkening supplies the artifact's SHAPE (alpha maximal at the centre, so the darkness is deepest there and fades toward the limb: a small dark spot inside a bright core), extinction supplies its DARKNESS and its HUE. **The audit lead is HALF-CONFIRMED:** the `Sun.js` comment asserting WebGPU blends additively IS false (three stale comments found: `Sun.js:249-254`, `WebGPUEnvironmentRenderer.js:132` and `:152-157`), but the shaders are authored FOR alpha-blend, not for additive — `SunTextureFS.glsl:140-160` says so explicitly. There is no additive/alpha mismatch in the arithmetic; the defect is the rgb-only extinction against an alpha occlusion weight, which is precisely why the stale comment matters — under a genuinely additive blend the artifact would be impossible. **Alternate loci checked and cleared:** limb darkening alone cannot darken the centre (it is 1.0 there, 0.30 at the limb); the bake's `+0.2` blue term is a limb-warming hue term that saturates to white at the centre; the sun bloom and the screen-space halo run AFTER the disc and the halo is itself extinction-tinted (`SunHaloAppearance.js:313-315`), so neither injects a dark centre. **Parity:** the mechanism is shared scene code plus two twin shaders, so WebGL is expected to show it too — the WebGL leg of the probe is a PARITY control, not a health reference. **Instrument:** `Tools/visual-regression/probe-sun-disc-dawn.mjs` + `lib/sun-disc-dawn-gate.mjs` + `sun-disc-dawn-gate.spec.mjs` (49/49), authored and LEFT UNRUN. It sweeps 13 five-minute samples from `2026-08-24T22:10:00Z` (engine-reported solar altitude −1.89° … +10.17°, spanning the pre-registered band) at the reported site, dual-backend, and publishes per sample the disc-centre vs disc-annulus mean-luminance ratio and the same ratio in blue-over-red, beside the engine's own published `sunAtmosphereExtinction`, `bakeHaloGain`, `discRadiance` and limb coefficients. The FAIL bar is `DERIVED-PENDING` and every bound is null — the gate folds a perfect sweep to STRUCTURAL rather than PASS, so this landing earns no verdict; the bar is derived from the first WebGL sweep only. Backend-independent reference published in the artifact: the shipped law's own black-sky centre/annulus ratio, 1.4398 — a measured ratio below 1 is an inversion of the shipped intensity law. **Engine change on this landing is COMMENT-ONLY** (the three false blend claims corrected; C16 guard clean, `comment-only-diff` classifies the WGSL-in-template-literal edits `code-differs` because the JS grammar cannot see into the shader string — WGSL comments are stripped by the compiler and neither pipeline nor module cache keys on the text). **The rendering fix is HELD as a maintainer decision** because every option changes WebGL's shipped appearance at every sunrise and sunset, and because Principle 8 forbids claiming it without a probe run this lane may not perform. Options recorded: (A) co-fade alpha with the transmittance so an extinguished sun fades into the sky instead of punching a dark hole, the same shape `u_eclipseAlpha` already uses; (B) split the transmittance — apply only its CHROMATIC part (normalised to its maximum channel) to rgb and route the achromatic dimming through alpha, keeping the reddening that is the desirable half; (C) floor the transmittance luminance (a tuning hack, least principled); (D) apply the transmittance only where `discRadiance` carries true radiance and let the tonemapper compress it. Note for whoever rules: the disc's base RGB is already display-referred white, so multiplying it by a raw slant-path transmittance double-counts an attenuation the tonemapped white already represents, which is why the modelled disc ends up darker than the aureole it is supposed to be the source of. | S–M |
| `C12-38b` | **Salvage from `cesium-worker-sundisc` (Ciryon's reconciliation, `migration_doc/branches/SUNDISC_RECONCILIATION_2026-09-02.md`, ruling `R-2026-09-02-33`):** the option-B transmittance split (`solarDiscTransmittanceSplit`: `rgb *= T/max(T)`, `a *= max(T)`) as an alternative to the landed option-A co-fade, which counts the transmittance twice under `ALPHA_BLEND`; a sky-reference third probe region the landed instrument lacks; a WGSL-offset spec; and the only written copy of the pre-fix sweep with the station-3 finding that eight mutations of the landed fix survive all 134 tests (the landed tests are vacuous). QUEUED behind the discriminator sweeps: with the sun hidden and with bloom off, on both backends, to show whether the measured 3.1x is the disc at all or sky plus bloom after the co-fade erases the billboard. Engine freeze stands until those sweeps are read. | —
...
**C12 closes when G2 and G4 pass on both backends at HEAD and the two remaining
gate reds are dispositioned as ruled — G1 RED, explicitly accepted at close by
`R-2026-08-21-14` and carried to proposed C17 as `CLT-D10`; G3 RED until
`R-2026-08-10-4`'s re-bake executes in the maintainer session `R-2026-08-28-5`
schedules — with:**

_(Corrected 2026-08-28, close-out sprint. This line read "when all four gates pass
on both backends at HEAD", which §0 has contradicted since `R-2026-08-21-14`: a
reader who reached §5 first would conclude C12 cannot close, and a reader who
reached §0 first would conclude it can. The ruling governs; the gate reds stay
visible in §0 exactly as recorded.)_
...
3. `FEATURE_INVENTORY.md` updated (celestial WIP §C → §B; airglow added to §D). ✅ **DISCHARGED at Batch 1144 — verified row-by-row against this queue's status ledger, not against prose.** The classification overlay landed at Batch 1110 (`601b521f3c`) as §B.11 (shipped) / §C.10 (acceptance open or blocked) / §D.7 (airglow); Batch 1144 closes the four residuals the verification found, three of which exist because Batch 1110 landed 2 h 19 m BEFORE the ruling packet at Batch 1127 (`e535725479`). (i) The star-appearance cluster `C12-05`..`C12-08` (Batch 748, `e0e20625ff`) and `C12-09` (Batch 804, `6794488107`) had no §B classification although gate G2 — whose headline criteria are exactly their deliverables — passed on both backends at Batch 905 (`ced8c1256f`). (ii) `C12-31`, whose acceptance sweep `R-2026-08-21-16` keeps IN this gate, and `C12-28`, whose manual HDR-hardware check is owed, had no §C classification. (iii) The §D.7 airglow entry still read “maintainer scope call pending” although `R-2026-08-21-16` ruled `C12-26` OUT of this gate and deferred it to proposed C17. (iv) The §C.10 cells for `C12-11` and `C12-33` were refreshed onto `R-2026-08-21-16` and `R-2026-08-21-15`. Every open row this queue records with a feature surface — `C12-11`, `C12-12`, `C12-14`, `C12-28`, `C12-29 S5`, `C12-31`, `C12-33`, `C12-36` — is now in §C and none is claimed shipped in §B; `C12-G1F2` (a metric row), `C12-13` (LICENSE.md, i.e. condition 4's subject) and `C11-79` (ruled to stay in C11) are deliberately outside the inventory. G1's carried red and G3's ordered re-bake are gate state and are untouched by this condition. **⚠ ORDERING CAVEAT — stamping this does NOT close the §5 exit tail.** At this commit EXIT-2's landed audit `Tools/visual-regression/celestial-gate-class-audit.spec.mjs` runs **5 pass / 1 fail** in the working tree: the subtest “the masked `!== false` discovery has no unclassified leak” reports one hit, `packages/engine/Source/Renderer/WebGPU/WebGPUPointCloudEyeDomeLighting.ts:780 command.enabled !== false`, an unlanded lane-F (C18-P) line that `git show HEAD:` proves does not exist at `daaca4fde8`. That red is the dirty-tree honesty the audit declares for itself, not a HEAD-state red. Read the 5/6 exactly: a **second** failure sat masked behind it until this landing group — the anti-vacuity anchor pinned `WebGPUSkyAtmosphereRenderer.js:413`, a line that had drifted off its statement (the real hits are 362 and 867), so EXIT-2 was red at a **clean** HEAD too. That anchor was re-cut onto the discovered line's text rather than a line number in the register/instrument batch that precedes this one, and the spec is 6/6 once the point-cloud hit is classified — proven by running it with the lane-F leak neutralized. EXIT-2 is not closed until that flag is classified when the lane-F package lands, and EXIT-1/4/5 still ride their rows.
