...
- `C12-11` **DR-01 seam LANDED B~873 — one Edge run owed:** `probe-stars-catalog.mjs` (checks A/G/B/C/D/E/F). The other two originally-owed items (diffuse/reversal visual review, moving-camera alias/frame-cost lane) are mechanised inside `probe-celestial-gates.mjs --g3` (CO-24) and were measured in the B934 run. **2026-08-16: its paused repair packet was RETURNED TO HELD with refreshed reasons (ten architecture-level blockers unchanged + a new hermeticity red: 63/66 outside the paused tree because the spec reads a run-output PNG); frozen tuple intact; see handoff §10.**
