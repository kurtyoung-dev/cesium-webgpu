...
## 0. RESUME HERE — Batch 1016 (`161d99950b`, 2026-08-09). Supersedes all status prose below; on conflict THIS section wins, then the row, then stamps by BATCH NUMBER (printed dates unreliable).
...
- `C12-29` **NEW-ECLIPSE-OCCLUSION-EFFECTS — the campaign's exit gate is MAXIMAL by ruling `R-2026-08-10-1`: C12 stays open until every slice lands.** Slice state: **S1/S2 landed**; **S6 landed**; **S3 has one canonical owner, `C13-41` (C13 queue) — implementation landed B871, Edge acceptance owed; R-1 elevated it to the C14 critical path**; **S4 remainder** (observability) ACTIVE; **S5** integrated with targeted gates passing, **final seven-lane certification matrix still open** (NASA-SVS geospatial comparison, isolated active/inactive lanes). Acceptance = the S5 matrix plus every slice's own recorded criterion.
- `C12-33` **Moon mip/LOD + moving-seam — LANDED B819, ACCEPTANCE NEVER EXECUTED.** The probe is deliberately `CALIBRATION_PENDING` / exit 2 until at least five paired normal/control repetitions separate and the seam PNGs are reviewed. The 2026-08-02 usage cap prevented the run and **nothing since has executed it** — this is an unexecuted gate, not a waiver. Next action: the calibrated moving Edge lane (close / seam-centred / seam-at-limb / ~16 px moving routes, both backends, `force-lod0` control).
- `C12-11` **DR-01 seam LANDED B~873 — one Edge run owed:** `probe-stars-catalog.mjs` (checks A/G/B/C/D/E/F). The other two originally-owed items (diffuse/reversal visual review, moving-camera alias/frame-cost lane) are mechanised inside `probe-celestial-gates.mjs --g3` (CO-24) and were measured in the B934 run.
...
- `C12-31` **natural solar aureole — sky-shell fix landed B785 (`e748181065`), first browser probes green 2026-08-01; the FULL acceptance sweep is still owed**, and `C12-31-FOLLOWUP-A/B/C` (`DEFERRED_WORK.md`) need an explicit in/out-of-gate call from the maintainer (see ASKS).
- **§5 exit tail — EXIT-2 and EXIT-3 are unclosed and are NOT gate lanes:** **EXIT-2** = the class audit asserting that for every celestial uniform gate (`enableStarBrightnessModulation`, cloud-cover occlusion, `enableMoonPhase`, `enableEarthshine`, `enableNightSkyDimming`) either a GLSL consumer exists **or** the default is off — the gate closes the CLASS, not the instance. **EXIT-3** = `FEATURE_INVENTORY.md` updated (celestial WIP §C → §B; airglow added to §D). EXIT-1/4/5 ride the rows above.
