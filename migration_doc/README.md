...
| [`CAMPAIGN_STATE.md`](CAMPAIGN_STATE.md) | **READ SECOND.** Tracked orientation mirror of campaign state. Pause/resume authority is the ruling series (`R-2026-08-17-0` recorded the resume) — this file mirrors it and is not the source. It also mirrors the campaign map, quiet-hours rule, and branch-transparency state. **Role change arriving this wave (2026-09-02):** `MAINTAINER_RULINGS_2026-09-02.md` directs campaign status to move into this file as the tracked authority, with the CLAUDE.md "Active Remediation Campaign" section becoming a pointer to it — a separate lane is landing that rewrite in this same wave, so re-read this file's own banner once that lands rather than trusting the "mirror, not source" framing above as still current. |
...
| [`QUEUE_2026-09-03_ARCHITECTURE_REVIEW.md`](QUEUE_2026-09-03_ARCHITECTURE_REVIEW.md) | **Architecture-review dispatch queue (2026-09-03, lane Eärendur):** one prioritised row per CONFIRMED/CORRECTED finding of the 2026-09-03 lens re-run (appended to [`ARCHITECTURE_REVIEW_2026-09-02.md`](ARCHITECTURE_REVIEW_2026-09-02.md) §3 as the "2026-09-03 survey" blocks and §3.17), the five §4 reversals, every §3 item marked unowned, the landscape HIGH gaps G1–G8 as pointer rows, the maintainer decisions (§5.3 ids, four ADRs stand/change/retire) and the 34 runtime measurements (Éowyn). Priority by one rule (P0 record-vs-code or default-path correctness; P1 reachable parity gap or named-campaign blocker; P2 debt taxing named work; P3 drift/hygiene/typing). **Dispatch view only** — no status authority changes; the dedup pass has run, so every `owned-by` cell names an exact id, `PARTIAL: <id>` or `NEW`. |
| [`UPSTREAM_SYNC_PLAN_1.145_2026-09-04.md`](UPSTREAM_SYNC_PLAN_1.145_2026-09-04.md) | **Upstream CesiumJS 1.145 sync plan — LIVE. Plan only; nothing has been landed and no merge commit exists.** Built from one dry-run merge executed in an isolated clone and aborted: 358 commits behind `upstream/main` `73c2eeec0c`, merge-base `6d5d8b1f07`, **32 conflicted files / 79 conflict hunks / 164 paths changed**. Carries the conflict census, four cluster analyses with a resolution per hunk, the toolchain + Playwright-bump survey, the post-merge verification plan with its honest coverage gaps, and the landing sequence. **Two findings that outlive the sync:** CLAUDE.md's sync-procedure `--theirs` default is wrong for the 13 conflicted `.js` files the fork has converted to ES6 classes (§3), and a merge-time break spans the conflict boundary — `AutomaticUniforms.js` auto-merges while its backing `UniformState.js` conflicts (§5.4). Status lives in the `UPSTREAM-SYNC-1.145-*` rows of [`QUEUE_2026-08-29_RESEARCH_DISPATCH.md`](QUEUE_2026-08-29_RESEARCH_DISPATCH.md), not here. Lane U 2026-09-04: lead Nolondil, workers Tar-Ardamin / Tar-Vanimelde / Tar-Telperien / Tar-Surion / Hallacar, reviewer Anardil. |
