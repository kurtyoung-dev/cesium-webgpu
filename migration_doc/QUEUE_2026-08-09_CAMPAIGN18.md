...
| `C18-A2` | Voxel empty-space skip via per-slot min/max occupancy | The march fixed-steps the whole volume; per-atlas-slot occupancy metadata lets it skip empty bricks. Lossless — no visual change, so the performance principle is satisfied by construction. The occupancy data is a by-product of the brickmap/page-table generalization, which is **`C11-100`'s implementation vehicle and stays recorded there**. Technique authority: the SIGGRAPH 2026 Advanced Real-Time Volume Graphics course (Apache-2.0 samples; math reference, not port source). | Interleaved-A/B GPU timing (the `C13-39` doctrine — one run, both legs, never across builds) on a sparse asset showing a real march-cost drop, plus byte-identical output on a dense asset where nothing is skippable. | S-M | `C11-100` (coordinated dependency — C18 does not own it) |
...
| `C18-S5` | Sort-free stochastic splatting (Gaussian Point Splatting adaptation) | SIGGRAPH 2026 intake (scout 2026-08-09): eliminate the global sort AND tile blending via stochastic pixel-sized opaque points, per Rijsdijk (TU Delft, TOG 2026) — the license-clean lineage (BSD-3, independent implementation; see the reference additions section). WGSL has no 64-bit atomics, so the adaptation is a two-pass packed-u32 depth-then-resolve — the same shape as the `C18-A5` point rasterizer, and the recorded single-pass non-goal STANDS. Discharges the audit's "StochasticSplats sort-free spike" watch item with a vetted candidate. Opt-in; the sorted path remains the default and the parity baseline. | Intake first upgrades the GPS reference's unaudited vendored `/packages` caveat to verified per the `C18-S0` discipline. Then: stochastic-vs-sorted comparison on both gate assets within a derived accumulation band (STBN/TAA-converged frames, band derivation stated — not tuned); interleaved A/B timing in one run; feature-off byte-identical on both backends. | M-L | `C15-G8` |
| `C18-S6` | Closed-form splat depth + normals (RaDe-GS clean-room) | SIGGRAPH 2026 intake: per-Gaussian closed-form depth/normal inside the rasterizer (Zhang, HKUST, TOG 45) at ~zero overhead — the keystone for splat-vs-globe depth compositing and `pickPosition` on splats, extending the `C15-G7` classification-depth problem space (G7 itself stays G-track-owned; this row is the post-G7 upgrade). **Source is the PAPER ONLY (arXiv 2406.01467) — the code is Inria/MPII research-only (STUDY-ONLY, see reference additions).** | The `L-03(b)` identifier-naming test carried forward as an acceptance condition (the reference implementation's variable set appearing in the landed shader is evidence of copying — the `C18-S2` precedent); depth-composite probe: splats correctly occlude and are occluded by terrain at grazing angles on both backends; `pickPosition` on a splat surface returns within a stated tolerance of the analytic surface; feature-off byte-identical. | M | `C15-G8` |
| `C18-S7` | **RULING at Wave-S activation: re-affirm or reopen the `C15` §6d no-splat-LOD/streaming non-goal** | SIGGRAPH 2026 put the recorded non-goal under genuine pressure: LoD-of-Gaussians (Windisch, TU Graz — out-of-core hierarchical LOD, "3D Tiles for splats") + object-space contrast-sensitivity LOD selection (Yoshii, Inst. of Science Tokyo) together form a credible planet-scale splat streaming stack. Both code releases are Inria-poisoned or absent — clean-room only if reopened. The scout's recommendation: an explicit maintainer ruling at Wave-S activation rather than silent erosion. **This row is a ruling request, not implementation.** | The ruling is recorded (re-affirm → the non-goal stands with the two papers cited as considered-and-declined; reopen → a scoped epic proposal with its own campaign identity is authored — NOT ad-hoc rows here). | Ruling | `C15-G8` (activation point) |
...
| `C18-S5` | S | M-L | PENDING — **GATED post-`C15-G8`** (SIGGRAPH 2026 intake) |
| `C18-S6` | S | M | PENDING — **GATED post-`C15-G8`** (paper-only clean-room) |
| `C18-S7` | S | Ruling | PENDING — fires AT Wave-S activation |
...
### SIGGRAPH 2026 watch items (no rows; revisit triggers stated)

- **Gabor Fields** (Condor, USI Lugano; MIT, clean lineage): kernel-mixture volume LOD/compression; revisit if a cloud-volume streaming campaign is scoped post-C13-close.
- **Lund neural cloud relighting** (HPG 2026): re-check for a code release; per-asset training caps it at hero clouds regardless.
- **HPG warp-level culling for 3DGS blending** (HPG Best Paper): subgroups-portable, output-identical; verify license before any Wave-S intake.
- **Re-sweep ~2026-09**: the SIGGRAPH papers list still carried empty session placeholders at scout time ([`SIGGRAPH_2026_SCOUT_2026-08-09.md`](SIGGRAPH_2026_SCOUT_2026-08-09.md)).
