...
| Metric | Value |
|---|---|
| Files in census | 1309 |
| ACTIVE | 1069 |
| INVESTIGATION | 196 |
| NO @purpose HEADER | 44 |
| Classes | probe 676, spec 356, lib 119, other 113, gate-lib 21, bake-tool 13, runner 7, fixture 4 |
...
| spec-runner-census.mjs | other | NO @purpose HEADER | 2026-09-12 | 15 | — |
| spec-runner-census.spec.mjs | spec | NO @purpose HEADER | 2026-09-12 | 5 | — |
...
| verify-doc-citations.mjs | other | ACTIVE | 2026-09-05 | 3 | Asserts every relative markdown link and every #L line anchor in the tracked LIVE migration_doc set resolves as written, that the anchored range exists, and that a range still contains the symbol its sentence names. |
...
| verify-landing-compliance.mjs | other | ACTIVE | 2026-08-28 | 24 | After-the-fact detector that re-runs the landing rules + C16 marker gate over a landed commit range, making any --no-verify hook bypass visible. |
...
| verify-readme-index.mjs | other | ACTIVE | 2026-09-05 | 5 | Fails when a tracked migration_doc/**/*.md file is named nowhere in migration_doc/README.md (by basename or by the directory-aggregate convention README.md documents at :121-127), or when a README markdown link names a migration_doc path the tree does not track. |
...
| verify-tracked-references.mjs | other | ACTIVE | 2026-08-21 | 17 | Asserts every node launch target in package.json/.mcp.json and every relative import in changed .mjs/.cjs/.js files resolves to a path the tree actually tracks. |
...
| png-decode.mjs | lib | ACTIVE | 2026-09-13 | 12 | Dependency-free PNG decoder (8-bit, non-interlaced, colour type 2 or 6) plus pixel-diff and frame-stats helpers, the decode-side counterpart of Tools/lib/png-rgba.mjs. |
| png-decode.spec.mjs | spec | ACTIVE | 2026-09-13 | 3 | Round-trip, filter-coverage and error-path coverage for Tools/lib/png-decode.mjs. |
| png-rgba.mjs | lib | ACTIVE | 2026-09-02 | 11 | CRC32 + PNG chunk + zero-dependency RGBA PNG encoder shared across the probe fleet, replacing near-duplicate hand-rolled copies one consumer at a time. |
...
| c11-170-perf-regression-gate.spec.mjs | spec | NO @purpose HEADER | 2026-08-28 | 5 | — |
...
| c13-42-godray-fixture.spec.mjs | spec | NO @purpose HEADER | 2026-09-13 | 4 | — |
...
| capture-and-diff.mjs | runner | ACTIVE | 2026-09-12 | 66 | Primary VR runner: drives the split-screen page over scenes.json, captures WebGL+WebGPU canvases, evaluates 3 gates vs reviewed historical baselines. |
...
| cloud-coverage-response.spec.mjs | spec | ACTIVE | 2026-08-16 | 9 | Pins the CLOUD-LOW-COVERAGE-CUTOFF fix: baked base-field support, monotone coverage response on the CPU twin, exact high-anchor preservation. |
| cloud-demo-probe-routing.spec.mjs | spec | ACTIVE | — | 3 | Drives the eight routed Weather-Inspector cloud probes through runProbe against a stub browser, so the routing is known to reach a receipt, take its origin from the runtime and refuse port 8080 before an Edge slot is spent on it. |
...
| cloud-genus-morphology.spec.mjs | spec | ACTIVE | 2026-08-21 | 8 | C13-16 cirrus row: add-only uniform layout, exact CUMULUS byte-neutrality, structural fibre anisotropy/shear metrics, mutation-rejected predicates. |
| cloud-ibl-revision.spec.mjs | spec | ACTIVE | 2026-08-16 | 6 | Source-anchored guard that WebGPUDynamicEnvironmentMapManager and the procedural cloud renderer keep the IBL revision handshake wired (CRLF-safe). |
...
| cloud-march-emission.spec.mjs | spec | ACTIVE | 2026-08-16 | 7 | C13-10: march-emitted reconstruction depth behind one compile-time bit, four sibling pipelines compile verbatim, CPU-twin cross-validation, mutants. |
...
| cloud-observability-counters.spec.mjs | spec | ACTIVE | 2026-08-16 | 6 | C13-02 Gate-A: cloud GPU total is a union not a sum, Sky Fill excluded, per-frame counters reset, pass counts tied to encode sites. |
| cloud-orbital-ladder-contract.spec.mjs | spec | ACTIVE | 2026-09-12 | 5 | Unit-checks the orbital ladder's four statistics against geometry the plan states independently, then drives the REAL descriptor through runProbe with a stubbed browser so the instrument is known to reach a verdict before an Edge slot is spent on it. |
...
| cloud-probe-harness.spec.mjs | spec | ACTIVE | 2026-09-12 | 9 | Guards lib/cloud-probe-harness.mjs + cloud-perf-evidence pass resolution: config round-trip through the collection contract across six cloud probes. |
...
| cloud-reconstruction-attachments.spec.mjs | spec | ACTIVE | 2026-08-21 | 11 | C13-09: attachment table add-only, march shader content-hash pin enforcing the C13-39 static-register constraint, stage default-OFF byte/cost neutral. |
...
| cloud-spectrum.spec.mjs | spec | ACTIVE | 2026-09-12 | 5 | node:test guard for lib/cloud-spectrum.mjs (C13-N04, O5 Structure): |
...
| eclipse-cloud-ibl-response.spec.mjs | spec | ACTIVE | 2026-08-28 | 9 | C13-41 (C12-29 S3 rider): pins seven exact-1.0 non-eclipse identity sites, rejects the S2 scene-factor shadow substitution, both env-bake halves. |
...
| probe-aec-perf.mjs | probe | ACTIVE | 2026-09-12 | 10 | AEC design-model performance probe: one page load per streaming lever, timed to a streaming-tileset-residency readiness criterion (renderReady ALONE is vacuous on this scene's first forced frame; see DM-10), per-pass command counts, one validated pick reused on both backends, served-build preflight and origin-guard refusals, element screenshots, multi-metric receipts. |
| probe-aec-perf.spec.mjs | spec | NO @purpose HEADER | 2026-09-02 | 2 | — |
| probe-aec-residency-e1.mjs | probe | ACTIVE | 2026-09-12 | 8 | E-1 corrected AEC residency measurement: profiles the WebGPU settle window of the demo's own clipped eight-tileset scene with a CPU sampling profile, a wall-clock-cadence pipelineCache created/pending poll, an allocation-site heap profile and an optional streamed heap snapshot, then reports equal-content frame cost and commandList length on one shared validated pick position under a deadline, or refuses the comparison when a backend never became ready. |
| probe-aec-residency-e2.mjs | probe | ACTIVE | 2026-09-03 | 4 | E-2 residency locus measurement: captures a browser-process Chrome trace over the same AEC settle window E-1 measured, records animation-frame delivery independently of the engine's frame loop, censuses the WGSL handed to the device, and runs three single-dimension controls as separate page loads, then attributes each frame gap of a second or more to the GPU-process work that covered it. |
...
| probe-c13-42-reported-demos.mjs | probe | INVESTIGATION | 2026-09-13 | 7 | Reproduces the three reported cloud and GodRay demos plus four offline cloud stations with real UI brackets and byte-bound identities. |
...
| probe-cloud-density-domain.mjs | probe | ACTIVE | 2026-08-16 | 9 | C13-37 baked-density periodicity oracle: same-build legacy/new density-domain x baked/live x midpoint/IGN factorial, failing closed on encoder timing |
...
| probe-cloud-diagonal.mjs | probe | ACTIVE | 2026-08-16 | 4 | Regression for the fullscreen-triangle fix: an overcast deck must fill the top-right quadrant (old triangle rasterized only half the screen) |
| probe-cloud-dials.mjs | probe | ACTIVE | 2026-08-16 | 3 | B407 struct-growth dials gate: puffSize/exposure/msDecay wired end-to-end, defaults byte-identical, reset returns to the default render |
...
| probe-cloud-exotic-flags.mjs | probe | ACTIVE | 2026-08-16 | 3 | U7 gate: exotic E1/E2 dials (mammatus/species/feature) reach the raymarcher via collection.volumetric; off states byte-identical |
| probe-cloud-extinction.mjs | probe | ACTIVE | 2026-08-16 | 3 | B408 V11 per-genus optical extinction gate: cumulus byte-identical default, cumulonimbus more opaque than cirrus, deck neither vanished nor blown out |
...
| probe-cloud-genus.mjs | probe | ACTIVE | 2026-08-16 | 4 | B408 V11 per-genus vertical-profile wiring: cirrus thinner, cumulonimbus denser, stratus distinct; cumulus/unset byte-identical to the pre-V11 default |
...
| probe-cloud-mammatus.mjs | probe | ACTIVE | 2026-08-16 | 4 | B555 mammatus gate: underside pouch carve visibly thins the deck, OFF byte-identical under a frozen clock, strength=0 restores the baseline |
...
| probe-cloud-species.mjs | probe | ACTIVE | 2026-08-16 | 3 | E1 acceptance: lenticularis/fibratus/uncinus density shaping changes the deck as specified; species-unset path byte-identical. |
| probe-cloud-stbn-lod.mjs | probe | ACTIVE | 2026-08-16 | 3 | Acceptance for cloud march LOD dials (step growth, max ray distance): both reach the march, both true no-ops at defaults. |
...
| probe-descriptor-cells-contract.spec.mjs | spec | ACTIVE | 2026-09-10 | 9 | Drives runProbe over the REAL polyline-TAA-velocity descriptor with a stubbed browser and page, walking cells → receipt → verdicts → summary and asserting the shapes the runtime requires, plus the runtime's own cells-must-be-an-array contract. |
...
| probe-eclipse-cloud-response.mjs | probe | ACTIVE | 2026-09-03 | 19 | C13-41 Edge acceptance: eclipse-driven cloud radiance ratio, shadow contrast, IBL bucket-fill count, and submitted-refresh cost. |
...
| probe-fleet-contract.spec.mjs | spec | ACTIVE | 2026-09-12 | 40 | Authoring-time probe safety contract: every probe must carry watchdog + try/finally close; mutant-tested detectors, shrink-only allowlist. |
...
| probe-q141-pick-readback.mjs | probe | ACTIVE | 2026-09-12 | 6 | Q-141 discriminator: validates one shared pick position asynchronously on both renderers, then attributes every synchronous pick miss to a named pick-framebuffer decline reason rather than reporting an unexplained hit count. |
...
| probe-runtime-lifecycle-adoption.spec.mjs | spec | NO @purpose HEADER | 2026-09-13 | 6 | — |
...
| sandcastle-smoke.mjs | other | ACTIVE | 2026-09-13 | 22 | Standing Sandcastle CI blind-spot smoke: three local-resource WebGPU gallery demos gated on non-black, non-uniform, real device, zero errors. |
...
| sandcastle2-typings-preflight.spec.mjs | spec | ACTIVE | 2026-09-13 | 4 | Pin the refusal, its actionable message, and the HEAD→GET fallback, so a missing `gulp buildTs` can never present as a flaky engine again. |
...
| weather-field-bounds.spec.mjs | spec | ACTIVE | 2026-08-16 | 8 | WeatherField contract: grid registration, regional bounds honoured incl. antimeridian, no-data semantics, global path byte-identical to legacy. |
...
| c13-42-godray-fixture.mjs | lib | NO @purpose HEADER | 2026-09-13 | 7 | — |
...
| cloud-photometry.mjs | lib | ACTIVE | 2026-09-12 | 6 | Recover linear pre-Reinhard cloud radiance from an 8-bit capture, mask the sun disc, and refuse where the recovery is not defined. |
...
| cloud-spectrum.mjs | lib | ACTIVE | 2026-09-12 | 8 | Radially averaged power-spectrum slope fit and area-perimeter fractal |
| cloud-tour-fixtures.mjs | lib | ACTIVE | 2026-09-12 | 12 | Deterministic C13-01 tour definitions: pinned local-solar clocks, absolute camera stations, replay keys, three-lane wind/time discriminators. |
...
| probe-runtime-governance.mjs | lib | ACTIVE | 2026-09-12 | 4 | C13-N01 stage 1 detectors: read whether a probe resolves its origin from a hard-coded fallback and whether it routes through the three governance modules, and census the fleet from those two facts. |
| probe-runtime.mjs | lib | ACTIVE | 2026-09-12 | 54 | Owns probe argv parsing, the single-Edge-slot lock, Edge launch per run, the served-build preflight, the Sandcastle2 origin rewrite and its refusal, renderReady gating, element-only capture, sha256, receipt assembly and the exit-code table, so a probe is a short script that declares cells. |
| probe-work-registry.mjs | lib | ACTIVE | 2026-09-12 | 5 | Tracks every `scope.run` a descriptor starts as an attempt with a settled outcome, so a run cannot be reported complete while work it started is still live, and such work's failure reaches the incident record whenever an incident is written at all; the browser's close is NOT deferred for it — see the header. |
...
| verdict-exit-gate.mjs | gate-lib | ACTIVE | 2026-08-16 | 53 | Single frozen PASS/FAIL/ERROR/STRUCTURAL exit-code table shared by the S5 gate libraries; fixed a six-copy divergence where STRUCTURAL exited 2. |
...
| visual-gate-policy.mjs | lib | ACTIVE | 2026-08-16 | 19 | Manifest field requirements, stable stringify/sha256 and PASS/FAIL/NON_CERTIFYING policy shared by capture-and-diff and certification gates. |
