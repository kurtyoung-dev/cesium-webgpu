...
| generate-tooling-catalog.mjs | other | ACTIVE | 2026-08-16 | 6 | Regenerates the TOOLING_CATALOG census section from @purpose/@status headers, git freshness and inbound refs; --check fails on drift. |
| generate-tooling-catalog.spec.mjs | spec | ACTIVE | 2026-08-16 | 1 | Self-test for the catalog generator: marker containment, determinism, drift reporting and the no-header row. |
...
| landing-rules.mjs | other | ACTIVE | 2026-08-16 | 7 | Pure landing-discipline predicates (quiet-hours window, Batch-N subject grammar, body, co-author trailer) shared by pre-push hook and detector. |
| landing-rules.spec.mjs | spec | ACTIVE | 2026-08-16 | 5 | Hermetic contract for the landing predicates: control+mutant per rule, DST-straddling quiet-hours pairs, narrow merge exemptions. |
...
| pre-push-guard.mjs | other | ACTIVE | 2026-08-16 | 4 | Git-aware driver behind .husky/pre-push: enforces batch-prefix/body/trailer/quiet-hours on every outgoing agent commit; fail-closed, no bypass flag. |
...
| run-far200-shadow-self-test.mjs | runner | ACTIVE | 2026-08-16 | 1 | Thin bootstrap: esbuild-bundles Tools/far200-shadow-self-test.ts and executes it via a data: URL import. |
...
| verify-landing-compliance.mjs | other | ACTIVE | 2026-08-16 | 5 | After-the-fact detector that re-runs the landing rules + C16 marker gate over a landed commit range, making any --no-verify hook bypass visible. |
...
| batch-117-wrap-returns.mjs | other | INVESTIGATION | 2026-08-16 | 2 | One-shot codemod rewrapping every fragmentMain return in GlobeTerrain.wgsl to makeFragOutput(...) for the G-buffer MRT conversion. |
| batch-121-wrap-lit-shaders.mjs | other | INVESTIGATION | 2026-08-16 | 3 | One-shot codemod converting 19 Mat*Lit + 2 Phong primitive shaders to emit FragOutput so they populate G-buffer slot 1 (normalRoughness). |
| wire-flat-shaders-aerial-lut.mjs | other | INVESTIGATION | 2026-08-16 | 2 | One-time codemod that wired the aerial-perspective LUT (EffectsUniforms + fog blend) into every Flat primitive WGSL shader from a template. |
...
| File | Class | Status | Touched | Refs | Purpose |
|---|---|---|---|---|---|
| comment-marker-guard.mjs | other | ACTIVE | 2026-08-16 | 12 | C16 lint guard scanning engine/widgets Source for banned tracker-marker vocabulary, with a clean-list ratchet; lint-staged + one-shot modes. |
| comment-marker-guard.spec.mjs | spec | ACTIVE | 2026-08-16 | 1 | node:test contract for the C16 marker guard: rules still match (self-test vs broken rule), scope does not overreach, ratchet honest both ways. |
...
| File | Class | Status | Touched | Refs | Purpose |
|---|---|---|---|---|---|
| comment-scanner.mjs | lib | ACTIVE | 2026-08-16 | 4 | The one tokenizer both C16 instruments share (JS/TS/WGSL/GLSL comment vs code vs string), fail-closed, with semantic-comment retention rules. |
| marker-grammar.mjs | lib | ACTIVE | 2026-08-16 | 3 | Machine-decidable half of the fork comment standard: the banned tracker-vocabulary regex rules (add-only ids) driven by the marker guard. |

### Tools/lib/ (2)

| File | Class | Status | Touched | Refs | Purpose |
|---|---|---|---|---|---|
| purpose-header.mjs | lib | ACTIVE | 2026-08-16 | 5 | The one shared @purpose/@status header grammar (parse, locate, byte-exact splice, violations) used by the codemod, the catalog generator and the fleet-contract analyzer. |
| webgpu-error-gate.mjs | gate-lib | ACTIVE | 2026-08-16 | 158 | Shared Playwright gate catching unscoped WebGPU validation/OOM errors (onuncapturederror) and device loss, plus a console-error listener. |
...
| lunar-landmarks.mjs | bake-tool | ACTIVE | 2026-08-16 | 6 | Dependency-free landmark alignment checker pinning the lunar albedo map's orientation (180-shift / lon-mirror / lat-mirror each trip a named check). |
...
| File | Class | Status | Touched | Refs | Purpose |
|---|---|---|---|---|---|
| adversarial.mjs | bake-tool | INVESTIGATION | — | 0 | Scratch adversarial run proving each deliberate mis-orientation of the albedo map trips its named lunar-landmarks check. |
| analyze.mjs | bake-tool | INVESTIGATION | — | 1 | Scratch NCC orientation analysis comparing candidate moon maps under flip/shift transforms. |
| analyze2.mjs | bake-tool | INVESTIGATION | — | 0 | Scratch low-pass NCC restricted to \|lat\|<60 deg, a refinement of analyze.mjs for the same orientation question. |
| ascii.mjs | bake-tool | INVESTIGATION | — | 0 | Scratch coarse ASCII terminal render of both moon maps plus low-pass NCC, for eyeballing orientation during the C12-24 investigation. |
| stats.mjs | bake-tool | INVESTIGATION | — | 0 | Scratch luminance statistics + candidate encode-size comparison between the new LROC map and the legacy moonSmall.jpg. |
...
| c11-13-voxel-inside-camera-harness.mjs | other | ACTIVE | 2026-08-16 | 2 | Browser-side ESM harness driving a voxel octree scene through inside/outside-volume camera waypoints on either backend for the C11-13 probe. |
| c11-13-voxel-inside-camera-probe.spec.mjs | spec | ACTIVE | 2026-08-16 | 2 | Browser-free policy + mutant suite for the physical Edge inside-camera voxel probe: waypoint sequence, pixel/command evidence, watchdog ordering. |
...
| c11-90-primitive-restart-harness.mjs | other | ACTIVE | 2026-08-16 | 2 | Browser-side harness loading primitive-restart strip/fan GLB models per backend, recording pipeline recreation history for the C11-90 probe. |
...
| c12-11-star-catalog-gate.spec.mjs | spec | NO @purpose HEADER | — | 3 | — |
...
| c12-29-s5-custom-ellipsoid-gate.spec.mjs | spec | ACTIVE | 2026-08-16 | 2 | Gate spec for the C12-29 S5 custom-ellipsoid certification: geometry oracles, eclipse bindings, cross-backend derivation, v6 gate fold, mutants. |
...
| c12-29-s5-replacement-device-gate.spec.mjs | spec | ACTIVE | 2026-08-16 | 2 | Certifies the S5 eclipse-shadow replacement-device evidence pipeline: schemas, phases, ledger/provenance validators, gate fold of its probe+lib pair. |
...
| capture-and-diff.mjs | runner | ACTIVE | 2026-08-16 | 35 | Primary VR runner: drives the split-screen page over scenes.json, captures WebGL+WebGPU canvases, evaluates 3 gates vs reviewed historical baselines. |
...
| celestial-capture-harness.spec.mjs | spec | ACTIVE | 2026-08-16 | 0 | Mutation-checked guard for lib/celestial-capture-harness.mjs, the shared page/settle/capture recipe the celestial gate probes import. |
...
| celestial-g3-gate.spec.mjs | spec | ACTIVE | 2026-08-16 | 4 | Guards the G3 Milky-Way cubemap gate: arcmin/px definition pinning, orthogonal structure metrics, T3 adversarial rejection, real-byte format arm. |
...
| cloud-density-domain.spec.mjs | spec | ACTIVE | 2026-08-16 | 7 | Pins the cloud density-domain layout: noise origin/phase/rotation float offsets shared between WebGPUCloudDensityDomain.ts and the WGSL, via exports. |
...
| cloud-genus-morphology.spec.mjs | spec | ACTIVE | 2026-08-16 | 5 | C13-16 cirrus row: add-only uniform layout, exact CUMULUS byte-neutrality, structural fibre anisotropy/shear metrics, mutation-rejected predicates. |
...
| cloud-march-transfer.spec.mjs | spec | ACTIVE | 2026-08-16 | 3 | Permanent home of the R3 march-transfer model: predicts the integrated image against 2026-08-06 tour ground truth; mutation group rebuilt non-vacuous. |
| cloud-morphology-composition.spec.mjs | spec | ACTIVE | — | 1 | C13-16 U2 candidate contract: genus-conditioned variance budget + fibre carve before erosion; WGSL wiring invariants and carve-after-erosion mutants. |
...
| cloud-ray-jitter.spec.mjs | spec | ACTIVE | 2026-08-16 | 3 | Pins the cloud ray-jitter contract across ProceduralClouds.wgsl, CloudDensityDomain.wgsl, the renderer and tier presets via source reads. |
| cloud-reconstruction-attachments.spec.mjs | spec | ACTIVE | 2026-08-16 | 4 | C13-09: attachment table add-only, march shader content-hash pin enforcing the C13-39 static-register constraint, stage default-OFF byte/cost neutral. |
...
| cloud-temporal-rte.spec.mjs | spec | ACTIVE | 2026-08-16 | 4 | Pins WebGPUCloudTemporalHistory reset classification (teleport, morph, deck bounds, scene mode...) and commit semantics vs the engine module. |
...
| cloud-u2-perf-evidence.spec.mjs | spec | ACTIVE | — | 1 | Guards lib/cloud-u2-perf-evidence.mjs manifest assessment for C13-16 U2 perf evidence (no-regression / unchanged pass expectations, lane shapes). |
...
| eclipse-cloud-response-gate.spec.mjs | spec | ACTIVE | 2026-08-16 | 9 | Pure-Node half of C13-41's Edge acceptance: recomputes the pre-registered bands, derives the sweep refresh count, mutant-checks every fold predicate. |
...
| eclipse-sky-totality.spec.mjs | spec | ACTIVE | 2026-08-16 | 9 | C12-29 S6 sky half: obs-1 dynamic-lighting resolution root cause, ruling E3 star-brightness modulation (derived curve, exact off), horizon twilight. |
...
| finding-ownership-audit.spec.mjs | spec | ACTIVE | — | 2 | Scans the migration_doc finding-source files for NEW-* IDs and enforces each has an ownership disposition (alias/placeholder/resolved). |
...
| ground-fog-band.spec.mjs | spec | ACTIVE | 2026-08-16 | 5 | Pins the inscribed-sphere altitude bug that made ground fog arithmetically absent, the Koschmieder-derived fix, four mutants, and byte-neutrality. |
...
| model-lazy-pick-demand.spec.mjs | spec | ACTIVE | — | 2 | Contracts for lazy realization of native Model pick resources across renderer, feature-id, Model, feature table and batch texture sources. |
...
| moon-mip-lod-shader.spec.mjs | spec | ACTIVE | 2026-08-16 | 0 | Structural WGSL contract for Moon derivative/LOD sampling: exactly one ellipsoid hit shaded, explicit gradients, no duplicated color evaluation. |
| moon-mip-motion-certification.spec.mjs | spec | ACTIVE | 2026-08-16 | 1 | Certification-pipeline contracts for moon mip-motion evidence: calibrated thresholds, immutable snapshots, reviewer attestation, finalization. |
| moon-mip-motion-probe-contract.spec.mjs | spec | ACTIVE | 2026-08-16 | 0 | Contract over probe-moon-mip-motion-edge exports: frame analysis, verdict decision, exit codes, evidence paths, paired sensitivity checks. |
...
| moon-webgl-explicit-gradients.spec.mjs | spec | ACTIVE | 2026-08-16 | 0 | Structural GLSL contract: Moon UV derivatives execute before miss discards under LUNAR_EXPLICIT_GRADIENTS on the WebGL path. |
...
| pointcloud-voxel-public-correctness.spec.mjs | spec | ACTIVE | — | 2 | Contracts over WebGPU point-cloud RTE history, shared layouts and the EDL state machinery (slot/stencil/pipeline-key correctness surface). |
| probe-2d-blank-where.mjs | probe | INVESTIGATION | 2026-08-16 | 3 | BUG-3 localizer: screenshots SCENE2D on both backends and maps WHERE non-black pixels are (off-screen vs mis-scaled vs depth-failed vs sky-only). |
| probe-2d-cv-modes.mjs | probe | ACTIVE | 2026-08-16 | 9 | Verifies SCENE2D and Columbus View render the globe on WebGPU after the camera-UB tileRectangle/projected-meters fix, per mode and backend. |
...
| probe-2dcv-verify.mjs | probe | INVESTIGATION | 2026-08-16 | 4 | BUG-3 visual verification: instant-morphs to 2D and CV on both backends, screenshots plus pixel stats behind the WebGPU error gate. |
...
| probe-aerial-froxel.mjs | probe | ACTIVE | 2026-08-16 | 4 | Acceptance for the froxel 3D-LUT aerial-perspective fast path: renders, far-band haze signature, distinct from OFF, analytic parity path intact. |
...
| probe-aerial-perspective.mjs | probe | ACTIVE | 2026-08-16 | 8 | Acceptance for the unified aerial-perspective post-process: renders, contributes, depth-correct far-band haze, no double-darkening, no GPU errors. |
| probe-aerial-runtime-config.mjs | probe | ACTIVE | 2026-08-16 | 2 | Proves aerialPerspectiveConfig intensity/inscatterScale are runtime-mutable per frame on the WebGPU aerial-perspective effect, with OFF-gate. |
...
| probe-atmo-lighting.mjs | probe | ACTIVE | 2026-08-16 | 5 | Acceptance for atmosphere-derived model lighting: derived sun colour + sky irradiance shift model hue warm at low sun, consistent with terrain. |
...
| probe-atmo-moon-438.mjs | probe | ACTIVE | 2026-08-16 | 3 | Finds a real moonlit night via ICRF scan, then asserts dualLightInline ON gives clear moon-glow luminance vs OFF on the inline sky march. |
...
| probe-atmo-resolver-consistency.mjs | probe | ACTIVE | 2026-08-16 | 3 | Gate that the shared WebGPUAtmosphereUniforms resolver feeds dynamic-atmosphere lighting into BOTH sky and model-IBL consumers. |
...
| probe-attachment-demand-registry.mjs | probe | ACTIVE | 2026-08-16 | 5 | Acceptance that the per-frame attachmentDemand record matches actual scene-FB behavior for default frame + each G-buffer consumer, observe-only. |
| probe-b3dm-noglobe.mjs | probe | INVESTIGATION | 2026-08-16 | 3 | Minimal check that the BatchTableHierarchy b3dm tileset renders on WebGPU with the globe removed from the picture. |
| probe-b3dm-render-edge.mjs | probe | ACTIVE | 2026-08-16 | 4 | Re-verifies the stale NEW-BG-CONSOLIDATION claim: b3dm renders non-black on Edge WebGPU and matches WebGL, ellipsoid terrain to rule out occlusion. |
| probe-backend-isolation.mjs | probe | INVESTIGATION | 2026-08-16 | 2 | Answered 2026-07-19 maintainer questions: is a WebGL context still created in webgpu mode, and does split-screen cost what solo costs. |
...
| probe-bb-cv-diag.mjs | probe | ACTIVE | 2026-08-16 | 2 | Reproducer measuring magenta billboard coverage in steady 3D/2D/CV, globe off — bisects the billboard quad-size parity defect vs morph gaps. |
...
| probe-billboard-atlas-vflip.mjs | probe | ACTIVE | 2026-08-16 | 2 | Parity acceptance for billboard atlas UV orientation (asymmetric image + SDF glyph) across scene modes, plus pick coverage of the quad. |
| probe-billboard-partial-write.mjs | probe | ACTIVE | 2026-08-16 | 6 | Acceptance for resident-instance partial writes: 0 uploads settled, ~1-stride write on one move, moved billboard renders at its new position. |
...
| probe-bloom-parity.mjs | probe | ACTIVE | 2026-08-16 | 4 | Bloom uniform-parity gate: default-uniform bloomed-fraction band vs WebGL, glowOnly + brightness response, stripped scene with synthetic sun. |
...
| probe-buffer-integer-position.mjs | probe | ACTIVE | 2026-08-16 | 3 | Render-diff acceptance for integer + positionNormalized decode on WebGPU Buffer*: normalized-SHORT cross coincides with DOUBLE control. |
| probe-buffer-logdepth-zfight.mjs | probe | ACTIVE | 2026-08-16 | 5 | Acceptance that the Buffer* family writes log depth: coverage tracked against a co-located log-depth billboard reference at far nadir. |
| probe-buffer-point-single.mjs | probe | ACTIVE | 2026-08-16 | 3 | Regression gate for the WebGL single-vertex BufferPoint path (count==1 sub-data update): lone point renders, moves, zero errors. |
| probe-buffer-point-update.mjs | probe | ACTIVE | 2026-08-16 | 4 | Acceptance for the upstream #13465 fix: setPosition after first render re-encodes and moves the point on BOTH backends within 2 frames. |
| probe-buffercoll-encode-benchmark.mjs | probe | ACTIVE | 2026-08-16 | 7 | Repack+upload benchmark of BufferPointCollection position encode (batch vs forced-scalar) at 10k/50k/100k, both backends, visual check. |
| probe-buffercoll-wasm-encode.mjs | probe | ACTIVE | 2026-08-16 | 4 | Parity acceptance that the WASM/batch fround RTE encode renders pixel-identical to scalar EncodedCartesian3, plus threshold routing checks. |
...
| probe-c-r9-webgl-vs-webgpu.mjs | probe | INVESTIGATION | 2026-08-16 | 3 | C-R9 control: same-view WebGL-vs-WebGPU b3dm render with explicit ellipsoid terrain, ruling out legitimate terrain occlusion first. |
| probe-c10-02-pixel.mjs | probe | ACTIVE | 2026-08-16 | 5 | Pixel + drillPick oracle for styled b3dm: cropped building-region capture and stats per unstyled/subset/all mode, drillPick feature check. |
| probe-c10-02-style-economics.mjs | probe | INVESTIGATION | 2026-08-16 | 4 | Diagnostic that decided style-command economics: translucentFeaturesLength upkeep, styleCommandsNeeded polarity, per-scenario command counts. |
| probe-c10-07-async-model-pipelines.mjs | probe | ACTIVE | 2026-08-16 | 4 | Wraps createRenderPipeline(Async) pre-device to prove Model PBR pipelines compile async (sync=0, no stall) and the model still renders. |
| probe-c10-09-prev-buffer-upload.mjs | probe | ACTIVE | 2026-08-16 | 6 | Acceptance that TAA prev-instance buffers seed once via GPU copy then skip while unchanged, with one-upload mutation exactness (cloud leg). |
| probe-c10-10-shadow-single-sweep.mjs | probe | ACTIVE | 2026-08-16 | 6 | Verifies the shadow caster-list fold: sublist reference-identical to the old full scan incl. off-camera casters, shadows still render. |
...
| probe-c10-11-ddtd-hitrate.mjs | probe | INVESTIGATION | 2026-08-16 | 4 | Hit-rate A/B of the globe-pick point sequence with the pick-fleet switch FALSE/TRUE, showing the occasional miss is a pre-existing race. |
| probe-c10-11-mixed-coherence.mjs | probe | ACTIVE | 2026-08-16 | 4 | Acceptance that the native pick fleet writes coherent log depth: mixed families occlude/reveal monotonically at 20/500/5000 km. |
...
| probe-c11-196-lazy-pick-demand.mjs | probe | ACTIVE | — | 2 | Discriminator that model pick resources stay cold during color rendering, realize synchronously on first pick demand, then remain stable. |
| probe-c11-202-batchtexture-pick-demand.mjs | probe | ACTIVE | — | 3 | Gate that ordinary model picking does not realize the legacy BatchTexture pick registry/texture first; verifies picked feature properties. |
...
| probe-c11-209-effects-placeholder-startup.mjs | probe | ACTIVE | 2026-08-16 | 5 | Startup acceptance that the effects depth-placeholder init encoder creates exactly the expected textures/views/passes, via native WebGPU API wrappers |
| probe-c11-210-compute-command-list.mjs | probe | ACTIVE | — | 2 | Acceptance that a real WebGPU compute command appended to Scene's commandList executes inside the product frame encoder across normal/pick/2D lanes |
| probe-c11-90-primitive-restart-split.mjs | probe | ACTIVE | 2026-08-16 | 7 | Thin retained entry point running the C11-90 primitive-restart WebGL2-vs-WebGPU harness from lib/c11-90-primitive-restart-probe.mjs |
...
| probe-c12-29-s5-custom-ellipsoid.mjs | probe | ACTIVE | 2026-08-16 | 4 | Runtime certification that a custom oblate ellipsoid renders and derives identically on serial fresh WebGL and WebGPU contexts; write-once evidence |
...
| probe-c12-29-s5-replacement-device.mjs | probe | ACTIVE | 2026-08-16 | 3 | Genuine device-loss recovery certification via Chromium GPU-process termination (never destroy()); 'destroyed' losses archived STRUCTURAL not recovery |
...
| probe-c12-29-s5-terrain-selection.mjs | probe | ACTIVE | 2026-08-16 | 2 | Real-terrain/selection acceptance: fill-to-real transitions, x2 radius law, async picking, env-map capture over local QuantizedMesh, both backends |
| probe-c9-14-ground-atmo-stage.mjs | probe | INVESTIGATION | 2026-08-16 | 2 | Before/after self-diff captures (ground/horizon/orbit) proving the globe ground-atmosphere Nishita march runs in exactly one shader stage |
...
| probe-camera-track.mjs | probe | ACTIVE | 2026-08-16 | 10 | Connected orbit-to-ground camera track capturing + diffing both backends at each waypoint to surface tile-streaming/LOD artifacts along motion |
...
| probe-canvas-vs-screenshot.mjs | probe | INVESTIGATION | 2026-08-16 | 1 | Instrument self-check: compares getImageData bytes vs page.screenshot of the same canvas state to detect Playwright capture transforms |
| probe-celestial-extinction-cache.mjs | probe | ACTIVE | 2026-08-16 | 3 | C9-06 close-out acceptance: cached sun/moon extinction bit-equals uncached physics, dusk warm-keep kills star pop-in, backends agree to exact IEEE-754 |
| probe-celestial-extinction-revision-gate.mjs | probe | ACTIVE | 2026-08-16 | 2 | Proves shared sun/star extinction is cached by exact physical inputs and the daytime star gate exits before feature-renderer/cache/draw work |
| probe-celestial-gates.mjs | probe | ACTIVE | 2026-08-16 | 20 | Campaign 12 celestial gate harness: measured G1-G4 star-field gates on both backends with cubemap/sprite source-split and in-column modulation lanes |
...
| probe-cloud-clockbind.mjs | probe | ACTIVE | 2026-08-16 | 2 | Proves cloud advection is scene-clock-bound: identical frames at a frozen clock across wall time, different frames for clock times 3 h apart |
| probe-cloud-cone-parity.mjs | probe | INVESTIGATION | 2026-08-16 | 1 | B436 zero-drift landing gate: one deterministic cinematic-tier capture, byte-compared across pre/post-436 builds via git stash |
...
| probe-cloud-density-domain.mjs | probe | ACTIVE | 2026-08-16 | 12 | C13-37 baked-density periodicity oracle: same-build legacy/new density-domain x baked/live x midpoint/IGN factorial, failing closed on encoder timing |
...
| probe-cloud-genus-morphology.mjs | probe | ACTIVE | 2026-08-16 | 7 | C13-16 GPU-half morphology acceptance: exact neutral-genus uniforms, zero-pixel default baseline, wind-aligned cirrus anisotropy w/ rotation control |
...
| probe-cloud-halfres.mjs | probe | ACTIVE | 2026-08-16 | 4 | B432 half-res tier quality gate: 0.5x march + bilateral upscale vs full-res at identical camera/time; no blocky pixelation or edge halos |
...
| probe-cloud-lod-hoist-perf.mjs | probe | ACTIVE | 2026-08-16 | 5 | C13-39 GPU-timestamp A/B lanes for the density-LOD/domain hoist; codifies the repo's mandatory interleaved bundle-swap GPU-timing protocol |
...
| probe-cloud-property-edit.mjs | probe | ACTIVE | 2026-08-16 | 5 | Regression: per-cloud property edits re-upload the billboard-cloud instance buffer within 2 frames, then settle with no further rebuilds |
...
| probe-cloud-shadows-flagon.mjs | probe | ACTIVE | 2026-08-16 | 4 | Cloud cast-shadow ON/OFF capture over lit terrain, plus a C13-16 U2 CIRRUS acceptance mode with pinned clock/wind. |
...
| probe-cloud-shadows-polar.mjs | probe | ACTIVE | 2026-08-16 | 6 | C13-06 pixel gate: cloud cast shadows must darken the ground band at 82N; fully pinned (P1-P8+P10) with bracketing control legs. |
...
| probe-cloud-u1-scaffold.mjs | probe | INVESTIGATION | 2026-08-16 | 2 | U1 slice acceptance: CloudVolumetrics/CloudRenderMode API scaffold exists and, unused, leaves BILLBOARD renders byte-identical. |
| probe-cloud-u2-config.mjs | probe | INVESTIGATION | 2026-08-16 | 2 | U2 slice byte-identity: config-indirection refactor must render identical ON-cloud hashes vs a HEAD-reverted build. |
| probe-cloud-u2-perf.mjs | probe | ACTIVE | — | 3 | C13-16 U2 cross-bundle GPU-timestamp no-regression gate: interleaved bundle-swap A/B per cloud pass with drift sentinels. |
| probe-cloud-u3-toggle.mjs | probe | ACTIVE | 2026-08-16 | 2 | Standing guard: VOLUMETRIC CloudCollection publishes one resolved config and suppresses billboards; hidden/BILLBOARD/WebGL publish nothing. |
| probe-cloud-u4a-managed.mjs | probe | ACTIVE | 2026-08-16 | 3 | Standing guard for scene-owned defaultCloudCollection: four config producers re-homed onto .volumetric, defaults byte-identical. |
| probe-cloud-u8-offident.mjs | probe | ACTIVE | 2026-08-16 | 5 | Epic close-out standing guard: full volumetric flag surface is byte-inert in BILLBOARD mode on both backends, graceful WebGL no-op. |
...
| probe-collection-pick.mjs | probe | ACTIVE | 2026-08-16 | 2 | Instrumented 7-link lane for the collection pick chain, naming the first dead link per target, with a Primitive positive control. |
| probe-collections-2dcv-morph.mjs | probe | ACTIVE | 2026-08-16 | 15 | Instant-morph placement audit: marker crops at the expected map location in 3D/2D/CV on WebGPU vs WebGL reference. |
| probe-collections-closeup.mjs | probe | ACTIVE | 2026-08-16 | 3 | Close-camera (600 m) parity check that Billboard/Point/Label render on WebGPU after the cull and clip-z fixes. |
| probe-collections-entity.mjs | probe | INVESTIGATION | 2026-08-16 | 6 | Disambiguation probe: do markers render via the entity API where dynamically-added raw collections did not, per mode and backend. |
| probe-collections-far-camera.mjs | probe | ACTIVE | 2026-08-16 | 10 | Far-camera log-depth gate: markers 1000 m above the globe at 220 km must render with log depth ON; kill-switch leg exercised. |
| probe-collections-morph-blend.mjs | probe | ACTIVE | 2026-08-16 | 4 | Animated-morph gate: markers track smoothly through morphTo2D/CV with no mid-morph vanish or completion pop, binned by morphTime. |
...
| probe-collections-regression.mjs | probe | ACTIVE | 2026-08-16 | 9 | THE consolidated collections gate: five collections render, settled scenes upload nothing, mutations repaint in 2 frames, 0 errors. |
| probe-colorgrading-wired.mjs | probe | ACTIVE | 2026-08-16 | 5 | ColorGrading runtime-wiring acceptance: off by default, enable adds the pass, config swap re-grades, disable returns identical. |
| probe-compute-engine-wired.mjs | probe | ACTIVE | 2026-08-16 | 2 | Verifies WebGPUContext lazily instantiates the compute engine so dispatchCompute runs (visually inert by design; 0 errors). |
| probe-compute-instance-generic.mjs | probe | ACTIVE | 2026-08-16 | 6 | Feature-agnosticism check: a non-orbital Lissajous kernel renders, animates from the time scalar, threads BV + TAA velocity. |
...
| probe-compute-instance-webgl2.mjs | probe | ACTIVE | 2026-08-16 | 7 | WebGL2 cpuKernel fallback: matching WGSL/JS kernel pair renders, moves, and lands centroids within pixels of the WebGPU leg. |
...
| probe-contact-shadows.mjs | probe | ACTIVE | 2026-08-16 | 6 | Contact-shadows acceptance: wall base darkens when enabled, strength=0.5 darkens less than 1.0, all cells error-free. |
...
| probe-csm-cast-dispatch.mjs | probe | ACTIVE | 2026-08-16 | 9 | Shadow-cast reachability: CSM cascade and single-map paths dispatch and produce umbra pixels, vs a WebGL reference cell. |
...
| probe-csm-soft-shadow.mjs | probe | ACTIVE | 2026-08-16 | 11 | CSM PCF softness gate: soft ON widens the penumbra vs hard OFF and lands in a WebGL-relative band; umbra floor is a ratio. |
...
| probe-darkness-quant.mjs | probe | INVESTIGATION | 2026-08-16 | 2 | Quantitative backend-brightness comparison: 32x32 pixel-grid RGB means/ratios classify darkening as uniform vs non-linear. |
...
| probe-depth-plane-horizon-oracle.mjs | probe | ACTIVE | 2026-08-16 | 7 | Horizon-occlusion oracle: points just before/beyond the geometric horizon must render/occlude, proven via GPU readback + pickAsync with a skip A/B. |
...
| probe-diag-demand-gates.mjs | probe | ACTIVE | 2026-08-16 | 2 | Disabled CPU pass profiler allocates no per-frame closures (render unaffected); enabled lane yields complete per-pass buckets via begin/endPass. |
...
| probe-dp46b-metadata.mjs | probe | ACTIVE | 2026-08-16 | 3 | Property-attribute scalar read via generated WGSL metadata codegen (struct Metadata + initializeMetadata): gradient proof, chunk asserts, off-parity. |
| probe-dp46c-metadata.mjs | probe | ACTIVE | 2026-08-16 | 1 | Property-TEXTURE metadata read in the model FS via the generated chunk's bindings + textureSample and the property-textures BGL variant. |
| probe-dp46d-metadata.mjs | probe | ACTIVE | 2026-08-16 | 1 | Property-TABLE metadata read via textureLoad(table, featureId); closes display-side structural-metadata parity (attr+texture+table). |
...
| probe-eclipse-cloud-response.mjs | probe | ACTIVE | 2026-08-16 | 9 | C13-41 Edge acceptance: eclipse-driven cloud radiance ratio, shadow contrast, and IBL refresh count — three pre-registered predictions. |
...
| probe-eclipse-scene-dimming.mjs | probe | ACTIVE | 2026-08-16 | 8 | C12-29 S2: scene-light + atmosphere dimming via eclipseSceneLightFactor, measured as within-step off/on/autoexposure luminance ratios. |
...
| probe-eclipse-sun-fade.mjs | probe | ACTIVE | 2026-08-16 | 5 | C12-29 S1: continuous limb-darkened sun-billboard fade replaces the one-frame cull pop; sunset sweep, 2026 eclipse, toggle-off identity lanes. |
| probe-edge-authored-silhouette.mjs | probe | ACTIVE | 2026-08-16 | 6 | Authored silhouetteNormals accessor parity: WebGPU edge extractor consumes the signed-byte accessor like WebGL (numeric + visual lanes). |
...
| probe-edge-display-mode-tri.mjs | probe | ACTIVE | 2026-08-16 | 3 | EdgeDisplayMode tri-mode signature parity: the three modes pairwise distinct on WebGPU and tracking WebGL's coverage ordering. |
...
| probe-edge-percolor.mjs | probe | ACTIVE | 2026-08-16 | 2 | Per-edge color parity: WebGPU edge WGSL gains a per-edge edgeColor vertex attribute; COLOR_0-blue GLB edges render blue on both backends. |
| probe-ellipsoid-mrt.mjs | probe | ACTIVE | 2026-08-16 | 3 | EllipsoidPrimitive writes real eye-space normals to G-buffer slot 1: AO must diverge between G-buffer and depth-fallback reads. |
...
| probe-ellipsoidprim-logdepth.mjs | probe | ACTIVE | 2026-08-16 | 7 | Ray-marched EllipsoidPrimitive log-depth: kill-switch flips rebuild the pipeline, shell stays visible ON/OFF and at far camera, zero errors. |
| probe-ellipsoidprim-translucent.mjs | probe | ACTIVE | 2026-08-16 | 7 | Translucent EllipsoidPrimitive blends exactly once (cull back; ratio ~0.5x opaque, not 0.75x) + WebGL re-verify of the B269 matrix hoist. |
...
| probe-entity-bulk-billboard-label.mjs | probe | ACTIVE | 2026-08-16 | 3 | Bulk billboard/label fast lane: hundreds of static entities render + pick on both backends, route through the flat-buffer lane, dynamics update. |
| probe-entity-bulk.mjs | probe | ACTIVE | 2026-08-16 | 7 | BulkPointVisualizer fast lane: tens of thousands of static points render on both backends, pick returns the Entity, per-frame cost drops. |
...
| probe-env-aerial-ms.mjs | probe | ACTIVE | 2026-08-16 | 5 | envMapMultiScatter dual mode: flag-off byte-identity dump for the stash gate + flag-on deltas (cube warms toward sun, aerial haze matches sky). |
| probe-env-background-clear.mjs | probe | ACTIVE | 2026-08-16 | 4 | Background-clear response gate: scene.backgroundColor must reach the canvas as env content is progressively removed (deferred-clear drop defect). |
...
| probe-env-pass-matrix.mjs | probe | ACTIVE | 2026-08-16 | 7 | Environment-element independence matrix: any subset of skybox/stars/atmosphere/sun/moon/globe renders exactly that subset, matching WebGL. |
...
| probe-error-pipeline.mjs | probe | ACTIVE | 2026-08-16 | 2 | Flat-magenta fallback for a failed model PBR pipeline: forced-failure hook renders magenta instead of a silent hole; hook off renders normally. |
...
| probe-exaggeration-cv.mjs | probe | ACTIVE | 2026-08-16 | 6 | Columbus-view / morph vertical exaggeration ungated in GlobeTerrain.wgsl: WebGPU CV relief must match WebGL instead of rendering flat. |
| probe-farcam-distortion.mjs | probe | INVESTIGATION | 2026-08-16 | 3 | Reproduces far-camera globe distortion + mesh tear: height/pitch/atmosphere sweep with disc-silhouette ratio and concave-bite tear detection. |
| probe-farcam-isolation.mjs | probe | INVESTIGATION | 2026-08-16 | 4 | Long-fixed-settle retest of far-camera views proving the 'cage/ring' was a partially-materialized capture artifact, not a precision bug. |
...
| probe-fleet-contract.spec.mjs | spec | ACTIVE | 2026-08-16 | 9 | Authoring-time probe safety contract: every probe must carry watchdog + try/finally close; mutant-tested detectors, shrink-only allowlist. |
...
| probe-frustum-count-3d.mjs | probe | ACTIVE | 2026-08-16 | 15 | Records scene.numberOfFrustums + per-frustum ENVIRONMENT/GLOBE bins on both backends at 3 altitudes to gate the ENV-command frustum-binning fix |
...
| probe-gbuffer-visualize.mjs | probe | ACTIVE | 2026-08-16 | 5 | Renders CesiumDebug.showGBufferNormals() and checks the G-buffer normal texture reads as a proper RGB-mapped sphere with magenta sentinels |
...
| probe-globe-bindgroup-cache.mjs | probe | ACTIVE | 2026-08-16 | 6 | Gates the globe per-tile bind-group cache: creations settle to ~0, spike+resettle on pan, globe visibly renders, zero validation errors |
...
| probe-globe-default-limits.mjs | probe | ACTIVE | 2026-08-16 | 6 | Gates globe rendering on a device pinned to WebGPU default limits (16 sampled textures): reduced 4-slot imagery layout + multi-pass blend path |
| probe-globe-effects-handle-toggle.mjs | probe | ACTIVE | 2026-08-16 | 3 | Oracle for the per-frame globe-effects bind-group memo: clipping ON-OFF-ON must carve, restore, and re-carve terrain with zero stale-handle errors |
...
| probe-globe-hdr-gamma.mjs | probe | ACTIVE | 2026-08-16 | 3 | Gates the globe sRGB-to-linear decode under the HDR canvas-output path via known-gray imagery: SDR byte-identical off-gate, HDR single-decode means |
...
| probe-globe-pipeline-readiness.mjs | probe | ACTIVE | 2026-08-16 | 6 | Measures whether the WebGPU globe's async pipeline-miss tile skip produces user-visible holes vs WebGL under a healthy event loop |
...
| probe-globe-rasterizes.mjs | probe | ACTIVE | 2026-08-16 | 2 | Smoke check that the WebGPU globe rasterizes at all: canvas coverage, luminance, color histogram vs WebGL, device-error gate |
...
| probe-gp-pipeline.mjs | probe | INVESTIGATION | 2026-08-16 | 1 | GroundPolyline bring-up deep-dump: UBO floats, interleaved vertices, red-pixel footprint — from when the shadow volume rendered nothing |
...
| probe-gpu-timestamp-profiler.mjs | probe | ACTIVE | 2026-08-16 | 7 | Certification lane for GPU-timestamp unique-sample accounting: drained readbacks, closed sample ledger, coverage+unprofiled ratios reconstruct 1.0 |
...
| probe-ground-atmosphere.mjs | probe | ACTIVE | 2026-08-16 | 7 | Deletion-verify gate: separate ground-atmosphere renderer gone (FR key 29 retired) while the in-shader GlobeTerrain path renders and contributes |
...
| probe-ground-polyline-logdepth.mjs | probe | ACTIVE | 2026-08-16 | 3 | Gates the GroundPolyline classifier's log-depth reversal: magenta draped line at a far camera must match WebGL within 25% with the FR cache built |
| probe-ground-view-env.mjs | probe | ACTIVE | 2026-08-16 | 4 | Numerically gates three ground-level divergences vs WebGL: daytime sky brightness, sun disk vs atmosphere ordering, and globe baseColor consumption |
...
| probe-gsplat-parity.mjs | probe | ACTIVE | 2026-08-16 | 8 | Dual-mode gsplat harness: certifies attributable WebGPU absence pre-G3, scores presence with --expect-webgpu; exit-gate instrument for GSPLAT rows |
...
| probe-hdr-pp-math.mjs | probe | ACTIVE | 2026-08-16 | 5 | Gates HDR-aware ColorGrading+FXAA on the tonemap-bypass path: SDR byte-identical vs baseline, stages run in HDR with tonemap absent, WGSL compiles |
...
| probe-i3dm-instance-jitter.mjs | probe | ACTIVE | 2026-08-16 | 3 | i3dm instance-translation RTE precision: pixel-stable across frames and WebGPU placement matches WebGL after the high/low translation split |
...
| probe-khr-meshopt.mjs | probe | ACTIVE | 2026-08-16 | 3 | v1.143 merge acceptance: KHR_meshopt_compression assets load via the lazy MeshoptDecoder, render on both backends, and stay under the parity threshold |
...
| probe-litmat-mrt.mjs | probe | ACTIVE | 2026-08-16 | 4 | Extruded lit-material polygon through WebGPUPrimitiveCommands: zero device errors and a measurable deferred/AO signal from the polygon's MRT writes |
...
| probe-logdepth-zfight.mjs | probe | ACTIVE | 2026-08-16 | 7 | Mat/PBR/Basic primitive log-depth z-fight gate: box pixels lost to terrain bleed, master switch ON vs OFF, plus below-surface negative control. |
...
| probe-mainthread-encode-ceiling.mjs | probe | INVESTIGATION | 2026-08-16 | 5 | Go/no-go spike: fits main-thread encode+upload ms vs N (10k-250k moved points/frame) for 60/30fps ceilings gating the ECS-on-worker phase. |
...
| probe-metadata-mat.mjs | probe | ACTIVE | 2026-08-16 | 4 | Proves MAT3/MAT4 property attributes transport all 9/16 components to WGSL via the widened slot-9 layout; per-face palette readout, both backends. |
| probe-metadata-multicomponent.mjs | probe | ACTIVE | 2026-08-16 | 4 | Proves VEC2/3/4 property attributes transport every component (vec4 slot-9), with debug paint of authored per-face RGB; debug-off gate. |
| probe-metadata-table-instance.mjs | probe | ACTIVE | 2026-08-16 | 2 | Property tables keyed by instance feature IDs on WebGPU: 4-instance debug-paint palette, WGSL path asserts, off-gate, WebGL reference. |
| probe-metadata-table-texture.mjs | probe | ACTIVE | 2026-08-16 | 5 | Acceptance: property tables keyed by texture/implicit feature IDs on WebGPU + cross-backend feature-pick and pickMetadata-undefined parity. |
| probe-metadata-uint16.mjs | probe | ACTIVE | 2026-08-16 | 4 | Proves UINT16/UINT32 property-texture metadata round-trips the multi-byte little-endian unpack: exact pickMetadata quantization, both backends. |
...
| probe-model-appearance-demo.mjs | probe | ACTIVE | 2026-08-16 | 4 | End-to-end check of the webgpu-model-appearance demo scenario: signature-pixel asserts for color/silhouette/split, off-gate identity, thumbnail. |
...
| probe-model-mip-shimmer.mjs | probe | INVESTIGATION | 2026-08-16 | 2 | C10-05 pre/post evidence: high-frequency energy over a minified model texture per backend — mip-0 aliasing shimmer vs trilinear smoothness. |
| probe-model-mrt.mjs | probe | ACTIVE | 2026-08-16 | 3 | Model G-buffer verification: 4-cell ao-x-deferred matrix on Milk Truck; perturbed normals + roughness in slot 1 widen the AO signal. |
...
| probe-model-scene2d-idl.mjs | probe | ACTIVE | 2026-08-16 | 4 | Acceptance: an antimeridian-straddling model in SCENE2D emits the derived wrapped 2D command on WebGPU (both map edges); off-IDL off-gate. |
...
| probe-moon-atmosphere-appearance.mjs | probe | ACTIVE | 2026-08-16 | 6 | C12 moon-wave acceptance: disc extinction+inscatter compositing, Lommel-Seeliger + opposition surge; 3 derived-epoch lanes, WebGL vs WebGPU. |
| probe-moon-atmosphere.mjs | probe | ACTIVE | 2026-08-16 | 4 | Acceptance: the moon dims + reddens through horizon slant-path extinction at low altitude on both backends; orbit config is the off-gate. |
| probe-moon-globe-depth-occlusion.mjs | probe | ACTIVE | 2026-08-16 | 5 | C12-37 physical-depth oracle: derived moon-near/earth-near/crossing lanes certify moon-vs-globe depth routing; immutable UUID run artifacts. |
...
| probe-moon-mip-motion-edge.mjs | probe | ACTIVE | 2026-08-16 | 7 | C12-33 moving-camera moon mip/seam acceptance in real Edge: 4 routes, canvas-element PNGs; structural failures hard, quality metrics advisory. |
| probe-moon-phase-gate.mjs | probe | ACTIVE | 2026-08-16 | 5 | C11-176b acceptance: Moon.wgsl phaseGate blackout defect — derived-epoch crescent/night-full lanes with projected-disc ROI metrics. |
...
| probe-morph-midframe.mjs | probe | ACTIVE | 2026-08-16 | 5 | Captures mid-morph frames of timed 3D-to-2D/CV transitions: the WebGPU globe must not vanish mid-morph (mesh.center double-count P1). |
| probe-morph-normals.mjs | probe | ACTIVE | 2026-08-16 | 2 | DP-H35 acceptance: morph-target NORMAL deltas re-shade on WebGPU — within-backend luminance ratio isolates the normal morph from BRDF gaps. |
| probe-motion-blur.mjs | probe | ACTIVE | 2026-08-16 | 3 | WebGPU-only velocity motion blur: deterministic previous-pose control proves smear exists and scales with camera velocity; OFF stability gate. |
| probe-mrt-validation.mjs | probe | INVESTIGATION | 2026-08-16 | 5 | Postmortem instrument: hooks uncapturederror/device.lost per ao-x-deferred cell to name the validation error behind the MRT-flip black canvas. |
| probe-ms-lut-azimuth.mjs | probe | INVESTIGATION | 2026-08-16 | 2 | One-off readback: multiple-scattering + sky-view LUT variation across the view-sun azimuth axis, confirming the all-azimuth re-param took. |
...
| probe-npr-outlines.mjs | probe | ACTIVE | 2026-08-16 | 3 | B123 verification: scene.enableNPROutlines A/B diff over canyon terrain — edges on geometry, sky sentinel pixels unchanged. |
| probe-ocean-datum.mjs | probe | ACTIVE | 2026-08-16 | 11 | W0 datum survey gating vertical ocean/tide design: sampleTerrain at 6 open-ocean sites vs EGM2008 + FFT-patch-vs-waterline lanes. |
| probe-ocean-tide-datum.mjs | probe | ACTIVE | 2026-08-16 | 6 | Acceptance for the FFT-ocean datum anchor + equilibrium tide: in-run before/after datum fix, M2-period phase ladder, spring/neap envelope. |
| probe-ocean-wave-lod.mjs | probe | ACTIVE | 2026-08-16 | 4 | C11-172 acceptance: physical-wavelength wave LOD — banded HF-variance gates prove structure near, calm far, animated; renderer hard-checked. |
...
| probe-oit-transparency.mjs | probe | ACTIVE | 2026-08-16 | 8 | OIT coverage + default-flip evidence: WebGL OIT genuinely active; WebGPU opt-in hard-gated active post-Slice-A; splat deferral lane. |
...
| probe-orbital-catalog.mjs | probe | ACTIVE | 2026-08-16 | 6 | Regression gate: orbital demo on generic ComputeInstanceCollection renders ~2000 moving points, pinned clock, zero GPU errors |
...
| probe-orbital-sgp4.mjs | probe | ACTIVE | 2026-08-16 | 6 | Verifies GPU df64 near-earth SGP4 kernel against embedded python-sgp4 vectors (<2 km/day); deep-space TLEs must be flagged and skipped |
...
| probe-perinstance-diffuse.mjs | probe | ACTIVE | 2026-08-16 | 2 | Parity gate: lit PerInstanceColorAppearance luminance on WebGPU within 8% of WebGL (fixed light, top-down, shadows/fog off) |
| probe-phase12-bugbash.mjs | probe | ACTIVE | 2026-08-16 | 5 | Gate for four B304 fixes: imagery-projection single-source, uploadImageSource observability, raySphere precision, billboard updateMode order |
...
| probe-pick-ray-async.mjs | probe | ACTIVE | 2026-08-16 | 6 | Gate: sampleHeight/clampToHeight converge to WebGL-matching heights on WebGPU via scene-depth reuse; pickFromRay warns, never throws |
| probe-pickmodel-instanced.mjs | probe | ACTIVE | 2026-08-16 | 10 | Gate for upstream #13433 port: octDecode arg order + CPU pickModel on instanced models (WebGL readback and WebGPU keepTypedArray paths) |
...
| probe-plain-hdr-tonemap.mjs | probe | ACTIVE | 2026-08-16 | 3 | Acceptance for plain-HDR tails: exposure synced to WebGPU tonemap, non-default operators match czm_* references, entity color parity |
...
| probe-point-label-partial-write.mjs | probe | ACTIVE | 2026-08-16 | 5 | Gate: settled point+label scene uploads nothing; one moved point = exactly one 112-byte partial write; label text change = full rebuild |
| probe-point-pick-webgpu.mjs | probe | ACTIVE | 2026-08-16 | 6 | Gate: scene.pick/pickAsync over a PointPrimitive returns it with id on WebGPU (warmed sync + async control), at parity with a billboard |
| probe-point-sprite-shape.mjs | probe | ACTIVE | 2026-08-16 | 6 | Parity gate: point-sprite shape (squares vs carved circles) across point-cloud and PointPrimitive scenes; gain-normalized ds4 metric |
...
| probe-pointcloud-gpulod-scene-wiring.mjs | probe | ACTIVE | 2026-08-16 | 2 | Verifies PointCloudShading.gpuLOD reaches the WebGPU LOD processor, decoupled-scan compute dispatches end-to-end, off-gate keeps atomic path |
| probe-pointcloud-lod.mjs | probe | ACTIVE | 2026-08-16 | 5 | Verifies the LOD compaction WGSL no longer clobbers shared slot 255 at full workgroup occupancy (output = exact permutation 0..255) |
...
| probe-polyline-appearance-2d.mjs | probe | ACTIVE | 2026-08-16 | 4 | Gate: polyline appearance primitives render in 3D/Columbus/2D/mid-morph on WebGPU via projected-2D plumbing + csm_computePolylinePosition |
| probe-polyline-appearance-logdepth.mjs | probe | ACTIVE | 2026-08-16 | 4 | Gate: appearance/material polylines write log frag_depth matching the globe so surface polylines rest on it at far cameras (no z-fight) |
| probe-polyline-appearance-pick.mjs | probe | ACTIVE | 2026-08-16 | 8 | Gate: scene.pick over a PolylineColorAppearance Primitive returns the primitive on WebGPU (pick pipeline + per-primitive pick command) |
| probe-polyline-appearance-primitive.mjs | probe | ACTIVE | 2026-08-16 | 3 | Gate: Primitive+PolylineColorAppearance renders a screen-space ribbon on WebGPU (dedicated packer/shader; was 0px from collapsed quads) |
| probe-polyline-cloud-consume.mjs | probe | ACTIVE | 2026-08-16 | 5 | Gate: static Polyline/CloudCollections stop re-touching primitives per frame (dirty-consume); tripwire for the cloud scene-FB MSAA mismatch |
| probe-polyline-geodesic.mjs | probe | INVESTIGATION | 2026-08-16 | 4 | Premise-refutation record: proved DP-H7's geodesic-subdivision root cause FALSE — PolylineCollection curves correctly on WebGPU (CPU-side) |
| probe-polyline-image-material.mjs | probe | ACTIVE | 2026-08-16 | 4 | Acceptance: Image-material polyline samples its texture along the line on WebGPU (red->blue gradient split), not a solid color |
| probe-polyline-material-primitive.mjs | probe | ACTIVE | 2026-08-16 | 2 | Gate: Primitive+PolylineMaterialAppearance MATERIAL slice — Dash renders dashed (run-count metric) and Glow renders, at parity with WebGL |
| probe-polyline-multimaterial.mjs | probe | ACTIVE | 2026-08-16 | 5 | Parity gate: one PolylineCollection mixing Solid/Dash/Glow — per-hue run counting proves non-Color groups don't collapse to solid on WebGPU |
...
| probe-postprocess-f16.mjs | probe | ACTIVE | 2026-08-16 | 4 | Gate: f16 post-process variants — ON compiles f16 modules per effect with f32-close output; OFF compiles zero f16 modules (off-gate) |
...
| probe-pp-library-builtins.mjs | probe | ACTIVE | 2026-08-16 | 7 | Gate: the 7 PostProcessStageLibrary builtins get WGSL twins with per-stage cross-backend tolerance, off-gate, and post-tonemap HDR order |
| probe-pp-library-demo.mjs | probe | ACTIVE | 2026-08-16 | 4 | Acceptance for the webgpu-post-process-library demo: asset serves, 7 builtins visibly cycle (stale-pipeline check), None byte-identical |
| probe-pp-silhouette-array.mjs | probe | ACTIVE | 2026-08-16 | 2 | Gate: array form of createSilhouetteStage carries the inner edge stage's custom color/length to the WebGPU twin; single form unchanged |
...
| probe-sampled-position-kernel.mjs | probe | ACTIVE | 2026-08-16 | 4 | Gate: SampledPositionKernel interpolates GPU-resident keyframed positions on BOTH backends, matching pure-JS and SampledPositionProperty refs |
| probe-sampleheight-webgpu.mjs | probe | ACTIVE | 2026-08-16 | 6 | Asserts sampleHeight/clampToHeight work on WebGPU via main-scene-depth reuse (cold-cache converges) with WebGL behavior unchanged. |
...
| probe-scene-capture-cardinal.mjs | probe | INVESTIGATION | 2026-08-16 | 3 | Ground-truth cardinal check for globe env capture: per-face on-screen camera replicas vs captured cube faces, detecting E/W swap or mirror. |
...
| probe-scheduler-octree-demand.mjs | probe | ACTIVE | 2026-08-16 | 5 | C9-08 gate: scheduler/octree do zero per-command work at defaults; consumer registration pixel-identical; octree never admits terrain/tiles. |
...
| probe-sky-ms-azimuth.mjs | probe | ACTIVE | 2026-08-16 | 2 | Verifies the reparameterized MS LUT lifts the twilight sky at ALL azimuths (toward/side/anti sun), directionally, with no wrap seam. |
| probe-sky-ms-directional.mjs | probe | ACTIVE | 2026-08-16 | 2 | Render-level directionality check: MS ON must lift the sun-side half of a horizon-perpendicular view more than the anti-sun half. |
| probe-sky-ms.mjs | probe | ACTIVE | 2026-08-16 | 2 | SKY-MS gate: multipleScattering default-OFF byte-identical across rebuilds; ON measurably brightens the horizon band with sane zenith hue. |
| probe-sky-twilight-range.mjs | probe | ACTIVE | 2026-08-16 | 8 | C12-34 browser acceptance for the twilight sky-brightness curve: shipped-module ENGINE leg plus positional star-contribution PIXELS leg. |
...
| probe-skybox-star-modulation.mjs | probe | ACTIVE | 2026-08-16 | 9 | C11-176 star-modulation parity + opt-in: backends modulate identically; in-column 30 km lane proves the flag moves pixels; exit 3 if unseen. |
...
| probe-splat-globe-occlusion.mjs | probe | ACTIVE | 2026-08-16 | 8 | Gaussian splats compose over the opaque globe and stay hidden behind it; OIT-deferral never-drop seatbelt plus occlusion controls. |
| probe-splat-sort.mjs | probe | ACTIVE | 2026-08-16 | 10 | Splat back-to-front sort consumption + log-depth producer gate; docstring voids pre-2026-08-01 gate-OFF results (pipeline-key aliasing). |
...
| probe-ssr-consumer.mjs | probe | ACTIVE | 2026-08-16 | 3 | Verifies SSR reads the G-buffer regardless of deferredLighting (legacy gate B122 removed): SSR-on legs match with deferred on and off. |
| probe-ssr-tuned.mjs | probe | INVESTIGATION | 2026-08-16 | 3 | ssrMaxDistance parameter sweep over the lake+wall scene to find a distance that engages SSR (diagnosing the B132 near-zero signal). |
| probe-ssr-water.mjs | probe | ACTIVE | 2026-08-16 | 5 | SSR verification scene: lake + wall reflector 4-cell matrix; visible reflection when on, deferredLighting toggle now a no-op. |
...
| probe-starfield-webgl-parity.mjs | probe | ACTIVE | 2026-08-16 | 5 | WebGL bright-star catalog fallback gate: WebGL draws catalog stars and the pattern matches WebGPU (positional IoU at pinned time, cubemap off). |
| probe-stars-catalog.mjs | probe | NO @purpose HEADER | 2026-08-06 | 21 | — |
| probe-stars-hdr-autoexposure-parity.mjs | probe | ACTIVE | 2026-08-16 | 5 | Guards the B364 fix: WebGPU honors the opt-in auto-exposure flag so the HDR night sky is not crushed to black versus WebGL. |
| probe-stars-hdr-verify.mjs | probe | ACTIVE | 2026-08-16 | 3 | Verifies the WebGPU baked-SkyBox sRGB-to-linear decode under HDR (faint star dusting visible like WebGL) with an SDR no-op control. |
| probe-sun-glow-profile.mjs | probe | ACTIVE | 2026-08-16 | 8 | Measures the sun billboard's radial luminance profile (solar radii) per backend; discriminates the WebGPU zero-contribution near the limb. |
| probe-sun-glowfactor.mjs | probe | ACTIVE | 2026-08-16 | 4 | Gates scene.sun.glowFactor driving the WebGPU sun bake + quad size, with glowFactor=1.0 byte-identical to the historical hardcoded bake. |
...
| probe-sun-lens-glare.mjs | probe | ACTIVE | 2026-08-16 | 4 | Locks in the WebGPU sun lens-flare bake (disc + glow + six bursts) present and its halo extent converging with WebGL's SunTextureFS bake. |
| probe-sun-pixel-check.mjs | probe | INVESTIGATION | 2026-08-16 | 7 | Samples canvas-center pixels with the sun centered (Earth behind) to quantify sun rendering and capture all console/validation messages. |
| probe-sun-shadow-gate.mjs | probe | ACTIVE | 2026-08-16 | 6 | Sun-shadow receive-on-globe acceptance: WebGL reference darkening, WebGPU receive ratio band, faded-vs-unfaded darkness asymmetry. |
| probe-sun-stars-extinction.mjs | probe | ACTIVE | 2026-08-16 | 5 | Sun and catalog stars dim/redden through the atmosphere near the horizon on both backends; byte-identical from orbit or atmosphere hidden. |
| probe-taa-disocclusion.mjs | probe | ACTIVE | 2026-08-16 | 2 | Exercises TAA's third disocclusion check (G-buffer normal divergence) under static and orbiting cameras; new binding builds cleanly. |
...
| probe-taa-morph-prevvp.mjs | probe | ACTIVE | 2026-08-16 | 4 | TAA history invalidation across the 2D/3D morph flip: clean render through the transition; motion-vector valid flag drops during MORPHING. |
| probe-taa-resolve.mjs | probe | ACTIVE | 2026-08-16 | 6 | TAA resolve-consumer gate: effect lazily added on first taaEnabled frame; resolve encodes, settled-stable, no smear on rotation. |
| probe-taa-userwarn.mjs | probe | ACTIVE | 2026-08-16 | 3 | Two-part gate: TAA stays pre-tonemap and temporally stable; GLSL-only user post-process stages emit a permanent deduped warn, not silence. |
...
| probe-timedynamic-pointcloud-load.mjs | probe | ACTIVE | 2026-08-16 | 6 | Regression probe for TimeDynamicPointCloud loading zero on WebGPU: animates the 5-frame sample; checks boundingSphere, memory bytes, pixels. |
...
| probe-uniformstate-viewport-371.mjs | probe | ACTIVE | 2026-08-16 | 2 | Gate for UniformState.viewport seeding on WebGPU: viewportTransformation non-identity and matches WebGL; polyline-ribbon pixel check. |
...
| probe-vector-draping.mjs | probe | ACTIVE | 2026-08-16 | 7 | C11-213 acceptance for terrain-draped vector polylines: backend/placement/material/Jacobian/cleanup gates with STRUCTURAL verdicts. |
...
| probe-vertex-lighting.mjs | probe | ACTIVE | 2026-08-16 | 2 | Smoke-verifies the globe VERTEX_LIGHTING path with world-terrain vertex normals; lambertDiffuseMultiplier must visibly move the canvas. |
...
| probe-volumetric-clouds.mjs | probe | ACTIVE | 2026-08-16 | 3 | Scout probe: does the WebGPU volumetric cloud raymarcher render cloud pixels at all — bright-pixel count vs WebGL at a cloud-covered view. |
| probe-voxel-cell-pick.mjs | probe | ACTIVE | 2026-08-16 | 13 | Byte-level cross-backend parity of pickVoxelCoordinate decode: single tile, refined L1 octree, customShader alpha gate, refined L3 (Parts A-D). |
...
| probe-voxel-parity.mjs | probe | ACTIVE | 2026-08-16 | 6 | Acceptance: the WebGPU voxel box renders at the correct world placement/extent (footprint IoU + color structure vs WebGL), not a flat quad. |
...
| probe-voxel-pick.mjs | probe | ACTIVE | 2026-08-16 | 8 | Acceptance: public Scene.pickVoxel end-to-end on both backends — VoxelCell returned without throw, identical color/tile/sample cross-backend. |
...
| probe-weather-channels.mjs | probe | ACTIVE | 2026-08-16 | 15 | Gate-B leg: raymarcher applies weather-map G/B/A channels, 9-longitude sweep with a richA/richB determinism control bracketing scored swaps. |
| probe-weather-edr-mock.mjs | probe | ACTIVE | 2026-08-16 | 6 | Gate-B leg: full EDR ingest chain (fetch, CoverageJSON, packer, weatherTex, clouds) end-to-end against the /mock-edr fixture, offline. |
| probe-weather-ingest.mjs | probe | ACTIVE | 2026-08-16 | 6 | Gate-B leg: ingest MVP — SyntheticWeatherSource through provider/packer to the C2-16 weather map; deck appears at 0.95, clears at 0.0. |
...
| probe-weather-map.mjs | probe | ACTIVE | 2026-08-16 | 5 | Keystone C2-16 seam check: cloudWeatherMap ON makes coverage vary spatially across locations (high cell stddev); OFF stays uniform. |
| probe-weather-metar.mjs | probe | ACTIVE | 2026-08-16 | 8 | Gate-B leg: full-RGBA METAR chain (obs parse, IDW rasterize, packer) against the /mock-metar fixture; spatial + calibrated channel gates. |
...
| probe-weather-seam-poles.mjs | probe | ACTIVE | 2026-08-16 | 4 | C13-07 pixel gate: no dateline luminance wall (frame-relative column-step test), bounded polar-cap variance, no NaN cluster; non-vacuity gate. |
...
| probe-weather-wcs.mjs | probe | ACTIVE | 2026-08-16 | 5 | Gate-B leg: OGC API-Coverages ingest via the shared CoverageJSON parser against the /mock-wcs fixture — same pattern gates as the EDR mock. |
| probe-webgpu-allocation-tax.mjs | probe | ACTIVE | 2026-08-16 | 6 | Instruments raw browser WebGPU/WebGL API boundaries to detect a hidden WebGL context in explicit-WebGPU scenes and classify compat allocations. |
...
| probe-webgpu-tile-popping.mjs | probe | ACTIVE | 2026-08-16 | 1 | Acceptance for the stale tile-buffer-cache fix: counts intra-frame black wedges during a cold LOD zoom; WebGPU must not exceed WebGL. |
...
| probe-wgs84-quick.mjs | probe | INVESTIGATION | 2026-08-16 | 3 | Quick orbit-only WebGL-vs-WebGPU comparison used to verify the Batch-56 alpha=1 force in WebGPUImageryReprojection. |
...
| probe-wireframe-verify.mjs | probe | ACTIVE | 2026-08-16 | 2 | Acceptance for the WebGPU debugShowWireframe fix: wireframe draws an imagery-colored line mesh (not black), settled by frame-signature stability. |
| purpose-header-contract.spec.mjs | spec | ACTIVE | 2026-08-16 | 6 | Contract spec for maintainer ruling M4: every probe and gate library must carry a readable @purpose/@status header. |
| run-performance-campaign.mjs | runner | ACTIVE | 2026-08-16 | 51 | The performance characterization runner: consumes performance-workloads.json, records Scene.render CPU samples + GPU timestamps; never FPS. |
...
| sky-light-direction.spec.mjs | spec | ACTIVE | 2026-08-16 | 5 | C12-31 contract: atmosphere NONE mode uses the astronomical Sun (not per-sample local-up), LEGACY_OVERHEAD reproduces history, twin lockstep. |
...
| vector-layer-draping.spec.mjs | spec | ACTIVE | 2026-08-16 | 8 | Acceptance for WebGPU vector-layer draping: GLSL-derived oracle vs real storage-buffer packer/WGSL indexing, six named mutations. |
...
| visual-evidence-library.mjs | other | ACTIVE | 2026-08-16 | 4 | CLI for the append-only content-addressed visual-evidence library: archive/import-legacy/verify/catalog/upgrade with provenance and run identity. |
...
| weather-probe-headroom.spec.mjs | spec | ACTIVE | 2026-08-16 | 4 | Guard for Gate-B probe repairs: METAR gate-4 partial-coverage discriminator and channels-probe migration onto shared pinning with brackets. |
...
| webgpu-pick-center-identity.spec.mjs | spec | ACTIVE | 2026-08-16 | 2 | Fake-device coverage of WebGPUPickFramebuffer/PickPass readback identity: map/unmap lifecycle, per-identity pixel decode, voxel pick pins. |
...
| webgpu-shadow-receive-contract.spec.mjs | spec | ACTIVE | 2026-08-16 | 2 | Pins the sun-shadow fix: single cast-dispatch site + same-frame wipe guard, receive matrix reproduces the cast texel, naga-validated shaders. |
...
| webgpu-voxel-resource-lifecycle.spec.mjs | spec | ACTIVE | 2026-08-16 | 2 | Drives real WebGPUVoxelResourceLifecycle exports (retain/release, atlas slot publish/retire/LRU, async-failure capture) plus structural pins. |
...
| probe-logdepth-diag.mjs | probe | INVESTIGATION | 2026-08-16 | 2 | Read the classifier dsColorFS 'TEMP DIAG' RGB encoding (storedDepth/eyeDist/encFar) to localize breaks in the log-depth reconstruction chain |
...
| sandcastle-batch-66-runner.mjs | other | INVESTIGATION | 2026-08-16 | 3 | Batch-66 rerun of the Batch-65 Sandcastle sweep adding the rendering-error hard-FAIL marker and the viewer-capture init shim. |
...
| File | Class | Status | Touched | Refs | Purpose |
|---|---|---|---|---|---|
| build-source-identity.mjs | lib | ACTIVE | 2026-08-16 | 23 | Shared provenance helpers: safe git HEAD, build/shader fingerprinting, and the STRUCTURAL build-absent reason evidence shards bind to. |
...
| c11-205-owner-attribution.mjs | lib | ACTIVE | 2026-08-16 | 5 | Frozen config plus fail-closed policy for the resident-SF CPU owner-attribution diagnostic; instrumented timings never recertify the causal artifact. |
| c11-209-effects-placeholder-provenance.mjs | lib | ACTIVE | 2026-08-16 | 4 | Provenance fingerprints binding the effects-depth-placeholder startup token across engine source, bundle, source map, probe, and policy files. |
...
| c12-29-s5-custom-ellipsoid-gate.mjs | gate-lib | ACTIVE | 2026-08-16 | 4 | Frozen v7 contract plus independent f64/stepwise-f32 eclipse oracle for the S5 custom-ellipsoid certification; refuses self-attested evidence. |
...
| c12-29-s5-multiview-gate.mjs | gate-lib | ACTIVE | 2026-08-16 | 3 | Frozen policy for the S5 same-context logical-View A->B->A shard; explicitly must not be cited as proof of engine multi-View scheduling. |
| c12-29-s5-replacement-device-gate.mjs | gate-lib | ACTIVE | 2026-08-16 | 4 | Fail-closed acceptance for S5 replacement-device recovery after genuine Chromium GPU-process termination (v5 schemas, nine-phase ledger). |
| c12-29-s5-svs-footprint-gate.mjs | gate-lib | ACTIVE | 2026-08-16 | 6 | Gate library for the S5 NASA-SVS-5073 eclipse-footprint certification shard, pairing its probe and spec. |
...
| c13-41-deckfree-control.mjs | lib | ACTIVE | 2026-08-16 | 6 | State-isolated ABBA session plan plus pinned lighting/fade constants for C13-41's deck-free eclipse control lane. |
| celestial-capture-harness.mjs | lib | ACTIVE | 2026-08-16 | 6 | Shared Playwright/page half of the celestial fleet: one pinned-clock settle recipe, warm-up-then-same-task capture, lane driver, PNG writer. |
...
| celestial-g3-gate.mjs | gate-lib | ACTIVE | 2026-08-16 | 6 | Pure metrics and verdicts for G3 (star-map cube-face asset quality), criterion 2 re-pointed after DR-01 moved resolved stars to the sprite catalogue. |
...
| cloud-march-transfer-model.mjs | lib | ACTIVE | 2026-08-16 | 4 | Node reconstruction of the camera-to-pixel cloud march (shell geometry, live density chain, saturating transfer) predicting the probe's estimator. |
...
| cloud-probe-harness.mjs | lib | ACTIVE | 2026-08-16 | 38 | Self-contained browser-side helper (addInitScript) configuring defaultCloudCollection.volumetric and verifying every value round-tripped. |
...
| cloud-u2-perf-evidence.mjs | lib | ACTIVE | — | 2 | Manifest policy for the C13-16 U2 cross-bundle GPU-timing gate: comparison, environment-drift rejection, immutable evidence naming. |
...
| eclipse-cloud-response-gate.mjs | gate-lib | ACTIVE | 2026-08-16 | 10 | C13-41 Edge-acceptance predicates with derived-never-fitted bands for deck lighting, cloud-shadow invariance, and the exact IBL refresh count. |
...
| globe-camera-track.mjs | lib | ACTIVE | 2026-08-16 | 36 | Shared orbit-to-ground camera route (plain serializable waypoints) used by both the visual parity probe and the performance campaign. |
...
| moon-mip-motion-certification.mjs | lib | ACTIVE | 2026-08-16 | 1 | Certification contract/finalizer for C12-33 moon mip-motion evidence: lane validation, calibrated thresholds, paired sensitivity, exit tiers. |
...
| owned-resource-transaction.mjs | lib | ACTIVE | 2026-08-16 | 3 | Dependency-free browser-safe helper that swaps an explicitly-owned resource transactionally (rollback on install failure, exactly-once destroy). |
| performance-campaign-utils.mjs | lib | ACTIVE | 2026-08-16 | 11 | Statistics/diff toolkit for the performance campaign: percentiles, counter-label diffs, run quality/stability, pacing and evidence summaries. |
...
| probe-fleet-contract-allowlist.mjs | lib | ACTIVE | 2026-08-16 | 7 | Pinned shrink-only census of pre-contract probes exempt from the fleet authoring contract; the spec fails on any NEW violation or stale row. |
| probe-fleet-contract.mjs | lib | ACTIVE | 2026-08-16 | 9 | Source-text analyzer enforcing the probe authoring contract (watchdog + finally-close); fails closed when it cannot parse a construct. |
...
| purpose-header-allowlist.mjs | lib | ACTIVE | 2026-08-16 | 1 | Frozen shrink-only snapshot of the probes and gate libs that predate the @purpose/@status header rule. |
| representative-performance-content.mjs | lib | ACTIVE | 2026-08-16 | 34 | Builds and validates the local procedural terrain+models+tiles representative scene configuration for offline performance workloads. |
| representative-tileset-request-ledger.mjs | lib | ACTIVE | 2026-08-16 | 5 | Event-sourced ledger of tileset content requests (issue/terminal events, byte totals, hashes) with cross-run comparison for perf evidence. |
| same-task-capture.mjs | lib | ACTIVE | 2026-08-16 | 25 | Canonical capture primitives that keep render+readback in one task (WebGL clears, WebGPU invalidates after present), plus embed-drift validators. |
...
| verdict-exit-gate.mjs | gate-lib | ACTIVE | 2026-08-16 | 9 | Single frozen PASS/FAIL/ERROR/STRUCTURAL exit-code table shared by the S5 gate libraries; fixed a six-copy divergence where STRUCTURAL exited 2. |
...
| visual-gate-policy.mjs | lib | ACTIVE | 2026-08-16 | 11 | Manifest field requirements, stable stringify/sha256 and PASS/FAIL/NON_CERTIFYING policy shared by capture-and-diff and certification gates. |
...
| weather-probe-pinning.mjs | lib | ACTIVE | 2026-08-16 | 20 | Shared determinism pins for the weather probe fleet (offline globe, one clock driver, zero wind), all read back; a failed pin exits STRUCTURAL. |
...
| webgpu-model-preparation-evidence.mjs | lib | ACTIVE | 2026-08-16 | 3 | Accumulates and validates WebGPU model preparation/demand counters as measurement-window evidence for performance workloads. |
...
| File | Class | Status | Touched | Refs | Purpose |
|---|---|---|---|---|---|
| co41-loading-check.mjs | scratch | INVESTIGATION | — | 1 | One-off 6-point browser check that the upstream loading indicator shows/hides on a real first frame on both backends, with a failure control. |
| sunbloom-flip-diag.mjs | scratch | INVESTIGATION | — | 1 | One-off mechanism diagnostic: does flipping scene.sunBloom=false at runtime erase the sun render itself (not just bloom) on either backend? |
| viewer-smoke.mjs | scratch | INVESTIGATION | — | 1 | One-off smoke: bare viewer is upstream chrome with WebGPU default, ?devUi=true restores the toolbar, ?renderer=webgl still selects WebGL. |
