...
| landing-rules.mjs | other | ACTIVE | 2026-09-01 | 24 | Pure landing predicates for commit discipline, push-wide Batch uniqueness, and protected remote-ref updates. |
...
| verify-landing-compliance.mjs | other | ACTIVE | 2026-08-28 | 26 | After-the-fact detector that re-runs the landing rules + C16 marker gate over a landed commit range, making any --no-verify hook bypass visible. |
...
| png-decode.mjs | lib | ACTIVE | 2026-09-13 | 14 | Dependency-free PNG decoder (8-bit, non-interlaced, colour type 2 or 6) plus pixel-diff and frame-stats helpers, the decode-side counterpart of Tools/lib/png-rgba.mjs. |
...
| fog-cheap-coverage-gate.spec.mjs | spec | ACTIVE | 2026-08-16 | 8 | Pins the fog cheap-path cloud-shadow coverage gate: samples standardised onto the baked field's moments, with mutants and byte-neutrality. |
...
| probe-ar002-per-tile-credits.mjs | probe | ACTIVE | 2026-09-04 | 6 | Counts per-tile terrain/imagery credits actually present in the credit-bar DOM on WebGL vs WebGPU at one settled Bing/ion view, so the AR-002 hoist's cross-backend parity is a measurement, not an inference. |
| probe-ar757-derived-origin.mjs | probe | ACTIVE | 2026-09-05 | 5 | Counts, in Edge, how many requests the browser actually sends to a cross-origin derived url carrying the parent resource's request header or its query parameter, so AR-757's acceptance is a network measurement rather than an inspection of the Resource object. |
...
| probe-c13-42-reported-demos.mjs | probe | INVESTIGATION | 2026-09-13 | 8 | Reproduces the three reported cloud and GodRay demos plus four offline cloud stations with real UI brackets and byte-bound identities. |
...
| probe-cloud-extinction.mjs | probe | ACTIVE | 2026-09-13 | 3 | B408 V11 per-genus optical extinction gate: cumulus byte-identical default, cumulonimbus more opaque than cirrus, deck neither vanished nor blown out |
...
| probe-display-conditions-globedepth.mjs | probe | ACTIVE | 2026-09-05 | 6 | Looped acceptance for display-conditions-dev on WebGPU: per-run occurrence counts of the GlobeDepth-DepthCopy destroyed-texture submit fault plus a capture matching WebGL, aggregated into a hit rate over N runs. |
...
| probe-eclipse-cloud-response.mjs | probe | ACTIVE | 2026-09-03 | 20 | C13-41 Edge acceptance: eclipse-driven cloud radiance ratio, shadow contrast, IBL bucket-fill count, and submitted-refresh cost. |
...
| probe-fleet-contract.spec.mjs | spec | ACTIVE | 2026-09-13 | 41 | Authoring-time probe safety contract: every probe must carry watchdog + try/finally close; mutant-tested detectors, shrink-only allowlist. |
...
| probe-pnts-model-attenuation.mjs | probe | ACTIVE | 2026-09-12 | 7 | Measures whether a PNTS tileset's rendered point footprint responds to pointCloudShading.maximumAttenuation on each backend, at three camera distances, and banks the disabled-shading capture hashes so a before/after pair can be compared byte-for-byte. |
...
| probe-primitive-texture-bindgroup.mjs | probe | ACTIVE | 2026-09-04 | 7 | Acceptance for the frustum-dev group-2 texture bind group: zero WebGPU validation errors and a flat translucent-red frustum matching WebGL, with the sibling textured paths still clean. |
...
| sandcastle-smoke.mjs | other | ACTIVE | 2026-09-13 | 24 | Standing Sandcastle CI blind-spot smoke: three local-resource WebGPU gallery demos gated on non-black, non-uniform, real device, zero errors. |
...
| sandcastle2-typings-preflight.spec.mjs | spec | ACTIVE | 2026-09-13 | 5 | Pin the refusal, its actionable message, and the HEAD→GET fallback, so a missing `gulp buildTs` can never present as a flaky engine again. |
...
| cloud-photometry.mjs | lib | ACTIVE | 2026-09-12 | 7 | Recover linear pre-Reinhard cloud radiance from an 8-bit capture, mask the sun disc, and refuse where the recovery is not defined. |
...
| probe-runtime-governance.mjs | lib | ACTIVE | 2026-09-13 | 5 | C13-N01 stage 1 detectors: read whether a probe resolves its origin from a hard-coded fallback and whether it routes through the three governance modules, and census the fleet from those two facts. |
| probe-runtime.mjs | lib | ACTIVE | 2026-09-12 | 55 | Owns probe argv parsing, the single-Edge-slot lock, Edge launch per run, the served-build preflight, the Sandcastle2 origin rewrite and its refusal, renderReady gating, element-only capture, sha256, receipt assembly and the exit-code table, so a probe is a short script that declares cells. |
...
| <a id="ap-tools-visual-regression-probe-c13-42-reported-demos-mjs"></a>[#](#ap-tools-visual-regression-probe-c13-42-reported-demos-mjs) | Tools/visual-regression/probe-c13-42-reported-demos.mjs | INVESTIGATION | 8 | 0 | no | REPOINT-FIRST |
