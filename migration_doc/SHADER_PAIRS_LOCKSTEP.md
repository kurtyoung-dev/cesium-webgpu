...
|---|---|---|---|
| Imagery upload flipY | `flipY: true` (Texture default) on a pre-flipped ImageBitmap → double-flip → texture v=0 = SOUTH (in sampling) | `flipY: false` (default) on the same pre-flipped ImageBitmap; `imageOrientation:"flipY"` only changes presentation metadata, not the underlying pixel buffer that `copyExternalImageToTexture` reads → texture v=0 = NORTH (in sampling) | The two upload chains land OPPOSITE source-V conventions despite consuming the same ImageBitmap. The reproject FS compensates in each language so the downstream globe FS sees a consistent "v=0 = south" reprojected texture (see Batch 67). |
...
| Water/ocean — UV Y-flip | GLSL applies `waterMaskTextureCoordinates.y = 1.0 - .y` after translation/scale. | WGSL omits the flip — `copyExternalImageToTexture` lands the mask row-aligned for direct sampling at `geoUV.y`. | Both reach the correct mask texel; the flip lives in the GLSL FS, the equivalent lives in the WGSL upload path. |
...
| Water/ocean — wave-normal sampling | 2-octave (`czm_getWaterNoise(u_oceanNormalMap, ...)` at high-altitude + low-altitude frequencies, blended over `positionToEyeECLength` ∈ [20km, 60km]). Wave UVs use `czm_ellipsoidTextureCoordinates(normalMC)` for globe-consistent wrapping. | 3-octave (uv × 400 / 200 / 800 with time advection per octave). Wave UVs use the tile UV directly — wave pattern moves with the tile rather than wrapping around the globe. | **WGSL-only enhancement** (not a bug). Different wave appearance in detail; both produce convincing ocean motion. Documented in pair-section header. |
| Water/ocean — specular model | `czm_getSpecular` (Phong-like, exponent=10). | GGX distribution (`distributionGGX(NdotH, 0.08) × NdotL`). | **WGSL-only enhancement.** Different specular falloff curves; both produce sun-glint at the correct angles. |
...
| Sky atmosphere — ray-march steps | 16 with adaptive `rayStepLengthIncrease` curve (AtmosphereCommon.glsl L81). | 64 uniform-stride steps; `NUM_OPTICAL_DEPTH_STEPS = 8` for sun-rays. | Same converged result; WGSL trades adaptivity for more samples. Cost negligible since SkyAtmosphere only renders on thin limb ring. |
...
| Sky atmosphere — translucent globe brightening | `#ifdef GLOBE_TRANSLUCENT` path in computeAtmosphereScattering brightens the inside-globe view when globe-translucency is enabled. | None. Globe-translucency not yet wired through WebGPU pipeline (deferred — part of multi-frustum / translucent classification work). | Tracked in DEFERRED_WORK.md. |
...
- **Tonemapping (unified)** + `Tonemapping_f16.wgsl` — fork-only consolidated tonemapper. WebGL ships 5 separate tonemap shaders (Aces, Filmic, ModifiedReinhard, PbrNeutral, Reinhard); WGSL unifies them into one shader with a runtime `tonemapperType` uniform branch.
