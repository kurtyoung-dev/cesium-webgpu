...
| `DX-06` | Deduplicate the fleet onto the runtime, by family | SONNET-BOUNDED (batches) | S each | QUEUED (after `DX-01`, `DX-02`; the seven reuse pilots already landed as Batches 1347–1353 count toward it) | `DX-01`, `DX-02` | DX |
...
| `DX-16` | PNG/CRC32 helper: `Tools/lib/png-rgba.mjs` (`crc32`, `pngChunk`, `encodeRgbaPng`) with golden-byte identity, two consumers first (27 near-duplicate encoders censused; `capture-and-diff.mjs` and `probe-reproject-baseline.mjs` excluded) | SONNET-BOUNDED | S | DISPATCHED wave 2 (Miriel, Sonnet; two consumers first; Amras's unlanded paths excluded). LANDING NOTE (corrected 2026-09-02 by lane Hirgon's fix round, reviewer Ingold §1): Brandir's lane (LANDED (Batch 1375)) added `Tools/visual-regression/lib/pnglite.mjs` - measured a **decoder plus pixel analysis** (`decodePng`/`readPng`/`diffPixels`/`frameStats`, no `encode` token, no CRC writer), not "another encoder" as this note previously read; it was never one of the 27 censused encoders. Hirgon's DX-16 fix-round patch (unlanded; `_lane-out/LANDING_PACKET_HIRGON.md`) folds it verbatim into `Tools/lib/png-decode.mjs` as `png-rgba.mjs`'s decode-side sibling, retargets its one consumer, and deletes the old path - landing this batch migrates **two of the 27 censused encoders** onto `png-rgba.mjs` (the two consumers this row scoped); the pnglite/png-decode fold is a separate decode-side migration, not a decrement of the 27 | `DX-01` runtime preferred, not required | DX |
