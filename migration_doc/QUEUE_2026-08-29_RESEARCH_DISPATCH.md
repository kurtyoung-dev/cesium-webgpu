...
| `UPSTREAM-SYNC-1.145-00` | The merge commit and the `PORT-INTO-CLASS` resolution pass — all 32 conflicts, one atomic merge commit (§8.0 explains why they cannot be split) | OPUS-JUDGMENT | XL | LANDED (Batch 1408, merge commit `ffb8161c08`; reviews REVIEW_ERADAN_cluster-a.md / REVIEW_HERION_cluster-b.md / REVIEW_TAR-ANDUCAL_cluster-c.md / REVIEW_TAR-FALASSION_cluster-d.md, all LAND; packet LANDING_PACKET_TAR-MINYATUR.md) | — | 1 |
...
- **Acceptance:** the harness is copied into a tracked location (e.g. `Tools/visual-regression/`),
  re-pointed from git stage `:2:` to a named pre-merge commit (`ffb8161c08^1` = `40341305f4`, per
...
as merge commit **`ffb8161c08`, Batch 1408** — parents `40341305f4` (fork, Batch 1407) and
...
parents (`git rev-list --parents -n 1` on `ffb8161c08`). **(2)** All 33 conflicts resolved into that
...
- **Disposition:** VERIFIED — **FIT TO FAST-FORWARD**. Éowyn job 4, 2026-09-04, run against the sync clone (tip `22af08f698`, Batch 1410) with preflight PASS (served md5 == disk md5 on :8094/:8081/:8080, clean tree). Full evidence: `Tools/visual-regression/output/sync-1145-verification-2026-09-04/SUMMARY.md` (+ `STATIC_CHECKS.md`).
...
- **Disposition:** OPEN — **item 1 CLOSED (Éowyn job 6, 2026-09-05).** 1.145 adds GLSL-side features with no WebGPU equivalent. Principle 5 makes the twin an obligation; Principle 9 makes it a named next item rather than a silent gap. Tar-Minyatur's original hand-off (packet §10) numbered six WGSL-twin items; four cluster reviews of the merge itself surfaced six further, related but distinct, findings; Éowyn's `-06` verification surfaced one blocking instrument defect. All are carried here rather than duplicated in `FEATURE_INVENTORY.md` §C `UP144-VECTOR-LAYER-WGSL`, which Penlod's landing keeps current for the sub-items it owns (meters-width branch, pick twin, model-vector-lookup twin, `eyeCartographic` twin, vertical-exaggeration stage) — this card cross-references rather than restates that entry.
  ...
  **Remaining items, one per line, owner and acceptance:**
  ...
  2. **Clipping-algorithm divergence between backends.** WebGL now runs upstream's `vectorClip` edge-texture path (`VectorCommon.glsl:328`); WebGPU still runs its own SDF atlas (`WebGPUClippingPolygonCollection.computePolygonSDF` → `PolygonSignedDistance.wgsl`) — a deliberate D1 resolution (the WebGPU producer does not gate on a WGSL co-land), not a defect, but Principle 5 territory. **Owner:** Eradan (`REVIEW_ERADAN_cluster-a.md` finding 2), confirmed by Herion (`REVIEW_HERION_cluster-b.md` §c.3, which supplies `VectorCommon.glsl:328` and proves reachability at both injection points; `:135-139` for the surviving WebGPU SDF path). **Acceptance:** every capture reports WebGL and WebGPU clipping numbers separately, never averaged, until the algorithms are unified or the divergence is ruled acceptable as-is.
  3. **Unused WebGL-compat clipping textures on WebGPU (both reviews).** `ClippingPolygonCollection.requestRectangleData` calls `packPolygonTextures` unconditionally, unlike `VectorProvider.requestDataForRectangle`'s explicit backend claim — WebGPU allocates three WebGL `Texture` objects per model per rebake that nothing samples (not a correctness break; freed on rebake/destroy). **Owner:** Eradan (`REVIEW_ERADAN_cluster-a.md` finding 3) and Tar-Anducal (`REVIEW_TAR-ANDUCAL_cluster-c.md` F-1, independently re-derived). **Acceptance:** `requestRectangleData` gains the same `packPrimitiveTextures`-style backend branch `VectorProvider` already has; measured multi-metric (allocation count + GPU memory alongside frame time, per SR-8/Tar-Anducal's recommendation), not on frame time alone.
  4. **The 4-arg clip detector gap.** `Snapping._nearestSurfaceHit`'s 4-argument border-clip form has no unit-level detector — proved by mutation, not asserted: making the clip unreachable (`if (false && …)`) leaves all 5 `SnappingSpec` `nearestSurfaceHit` assertions green because they are all 2-arg. **Owner:** Tar-Falassion (`REVIEW_TAR-FALASSION_cluster-d.md` F1). **Acceptance:** a node-runnable behaviour spec over the 4-arg form (the re-based metric — a fragment adjacent to the edge hit beats one adjacent to the cursor; the `|d|=4` kept / `|d|=5` clipped boundary; the `(12,12)` worst case) with an inertness mutant that makes the clip unreachable and requires the group to fail; runner home `test-scene-node`.
  ...
  12. **Packet §10 "Also opened", three related items, not yet tracked anywhere.** **Owner:** `LANDING_PACKET_TAR-MINYATUR.md` §10. **Tier:** SONNET-BOUNDED for (a)/(b), OPUS-EDGE-EXECUTOR for (c). **Acceptance**, one per sub-item: (a) retire WebGL clipping buckets A/B only once item 2's `vectorClip` WGSL twin ships — a dated decision, not an accident; closes when that item's landing packet says so explicitly, not implicitly by omission; (b) the `clipping-performance-dev` Sandcastle demo now trips the `debugShowDistanceTexture` deprecation warning (D2) — closes when the demo is updated to use the non-deprecated accessor or the warning is accepted and documented in the demo's own comment; (c) a two-renderer (WebGL/WebGPU) capture for Gaussian-splat classification depth, to confirm the merge did not disturb the shared depth target cluster (c) and cluster (a) both touch — closes on a `capture-and-diff` run with a named splat-classification scene added to `scenes.json` if one does not already exist.

  13. **`NEW-WEBGPU-MODEL-VECTOR-LOOKUP` — the WGSL twin of the new model vector-lookup stage.** `ModelSceneGraph.js` registers `ModelVectorLookupPipelineStage` on the GLSL path when `model.hasDrapedVectors()` (Tar-Anducal's producer/consumer table, `-03` review); WebGPU has no equivalent stage. **Note, checked against the tracked tree:** this item is named as a "deferred row" in `UPSTREAM_SYNC_PLAN_1.145_2026-09-04.md:691`/`:711`, but a grep of `FEATURE_INVENTORY.md` and `DEFERRED_WORK.md` for the id finds no filed row — the plan named it, nothing opened it. This card is that opening. **Owner:** unassigned (surfaced by Tar-Minyatur packet §10 item 3). **Tier:** OPUS-JUDGMENT (shader, parity). **Acceptance:** a WGSL `ModelVectorLookupPipelineStage` equivalent, producing matching output vs GLSL on the capture set; `FEATURE_INVENTORY.md` gains the row this plan reference pointed at but never created.
  14. **`czm_eyeCartographic`/`czm_eyeToEnu`/`eyeToCartographicDelta` in WGSL.** The JS-level `UniformState` getters landed inside `-00`/`-04` and are verified numerically (Tar-Falassion), but the WGSL `CameraUniforms` struct itself is untouched by this merge (Tar-Falassion (c)4: zero `struct CameraUniforms` declarations touched anywhere in the repo) — **checked against the tracked tree: `FEATURE_INVENTORY.md`/`DEFERRED_WORK.md` carry zero occurrences of `eyeCartographic` or `eyeToCartographicDelta`,** so no WGSL consumer exists and nothing files the gap today. The new members go ahead of `previousViewProjection`, never after (CLAUDE.md pins that field's position). **Owner:** unassigned (Tar-Minyatur packet §10 item 4). **Tier:** OPUS-JUDGMENT (shader, parity). **Acceptance:** the WGSL `CameraUniforms` carries the ENU basis and cartographic eye; a probe shows a WGSL shader consuming them producing matching output vs the GLSL builtin on both backends.
  15. **WGSL model vertical-exaggeration stage.** A pre-existing gap this sync widens (Tar-Minyatur packet §10 item 6) — **checked against the tracked tree: no `FEATURE_INVENTORY.md` entry names a WGSL vertical-exaggeration stage,** so this is unscoped, not merely unfinished. **Owner:** unassigned. **Tier:** OPUS-JUDGMENT (shader, parity) for the scoping pass. **Acceptance:** a scoping note in `FEATURE_INVENTORY.md` §C naming the gap's actual size and the GLSL stage it twins, before a size estimate is trusted.
