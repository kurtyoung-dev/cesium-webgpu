...
- **Disposition:** OPEN, **and it is a gate-trust defect, not a convenience one.** Found by the P0-2 fix's adversarial verifier (Fortinbras, 2026-09-13) while certifying the `sample-height-from-3d-tiles` repair. At `SANDCASTLE_SETTLE_MS=8000` the **known-BROKEN control tree scored a PASS** on that demo — recorded as "1 vacuous PASS @frameNumber 14, 2 HANG" — because the demo's throw had not happened yet when the settle window closed, and the runner's verdict is "no console error was seen", not "the demo reached the state it is being certified for". The fixed tree at the same settle passed 3/3 at frameNumber 18/22/18; at settle 25000 the control scored 0/5. **A gate that a broken tree can pass is the class of defect that voids every count taken under it** — the same mechanism that voided seven bisect verdicts a day earlier.
