...
### `DX-72` — the tooling-catalog gate certifies the candidate index, so a regeneration cannot green it and the report said "regenerate" anyway

- **Disposition:** CLOSED for the misdiagnosis (lane Radagast, 2026-09-11; landed from `_lane-out/radagast2.patch`, rebased onto Batch 1464). Filed from the seat incident of 2026-09-10: `verify-tooling-catalog` → `generate-tooling-catalog` → `verify-tooling-catalog` reds twice with a byte-identical drift report, because `--check` reads the committed catalog from the candidate **index** (`Tools/generate-tooling-catalog.mjs:1951`) while the write path rewrites the **working tree** (`:1976`, `:840`). The index sourcing is deliberate and stays; what was wrong is that the remedy line said "Regenerate … and commit" in the one state where regenerating changes nothing. The same condition reached `test-tooling-catalog` through `assertCensusCurrent` and read as independent `A1*` regressions (`A1a3`, `A1a4`, `A1a5`, `A1i`, `A1m`, `A1n`, `A1o`, `A1s` — eight `ERR_ASSERTION` preconditions; the ninth seat red, `A1g3`, is a separate 30 000 ms `testTimeoutFailure` at 41.98 s, whose bound this lane raises to 120 000 ms to match the file's other launcher-spawning tests); a clean clone at Batch 1461 is 104/104 with `verify` exit 0, and the committed catalog is byte-identical to a fresh regeneration. Root cause, evidence and the fix: `WEBGPU_DEBUGGING_LOG.md` "Bug DX-72".
- **Tier / Size / Backends:** TIER-3 · XS · tooling (no engine, no shader, no backend). **Depends on:** none. **Ruling touched:** none. **Gate:** `npm run test-tooling-catalog` (`A1i2` is the behaviour spec, both legs, with an inertness mutant), `npm run verify-tooling-catalog`.
...
### `DX-82` — The provisioner leaves a clone's tracked governance files modified

- **Disposition:** **LANDING — Batch 1490 (2026-09-16), lane Aldarion; verdict LAND, no blocking fixes.** Gate **7/7/0**; the acceptance grep goes **2 → 0**; an independent mutant disabling the *content-difference* guard — the dangerous inverse, clobbering a modified governance file — is caught (`ASTRA_WORK_AUDIT_2026-09-16.md` §1.1). <!-- corrected 2026-09-16, R-2026-09-12-10 --> *[status stamped 2026-09-16 by record round 5; the line read "**Disposition:** OPEN."]* Prior text, retained: **Section written 2026-09-13 by the record lane; the row was allocated 2026-09-12 by `CAMPAIGN_13_V2_SOLO_ROWS_2026-09-12.md` §3.8 and Batch 1482 declared this section owed** (`DEFERRED_WORK.md:2113`, `QUEUE_2026-07-23_CAMPAIGN13.md:271`). All four `PROVISION` destinations of `Tools/provision-worker-clone.mjs` are tracked — `CLAUDE.md` since Batch 1434 (`33416b6036`), `AGENTS.md` since Batch 1058 (`54ead62ee5`) — so copying the seat's worktree copy over one leaves `git status` reporting ` M` while `git diff --numstat` prints nothing: the seat's copy is LF where a Windows checkout is CRLF. Measured across four live lane clones for `migration_doc/MAINTAINER_RULINGS_2026-08-17.md`, and again in this lane at Batch 1482 — every lane packet of the wave had to list the path as an exclusion and the seat restored it by hand.
- **Tier / Size / Backends:** TIER-3 · M · tooling. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-landing-rules`.
- **Acceptance:** as filed in `DEFERRED_WORK.md` `### DX-82` — `Tools/provision-worker-clone-junctions.spec.mjs` 3/3 → 6/6 with three injected-`execFileSync` tests (identical → restored with a recorded `checkout`; differing → stays modified, no checkout; untracked → left as copied); `grep -ic "gitignored (.gitignore:6)"` → 0; the not-a-git-clone path still exits 2; `node --check` clean; prettier and eslint exit 0.
- **Binds:** none. **Source:** `CAMPAIGN_13_V2_SOLO_ROWS_2026-09-12.md` §3.8; `DEFERRED_WORK.md:2111-2125`; `SOLO_WORKER_HANDOFF_2026-09-13.md:170`.

### `DX-83` — Three `PROGRESS_THRAIN.md` citations name a path no reader can open

- **Disposition:** OPEN. **Section written 2026-09-13 by the record lane** (owed by Batch 1482; `DEFERRED_WORK.md:2129`). The three bare `PROGRESS_THRAIN.md:…` citations in this file (`:1940`, `:1943` inside `DX-78`; `:1947` inside `DX-79`) name a file that `git ls-files | grep -i THRAIN` does not return — it is not in the repository and never was. The archived copy is 706 lines / 53,408 bytes with every cited line on the topic its citation claims.
- **Tier / Size / Backends:** TIER-3 · S · documentation. **Depends on:** none. **Ruling touched:** none. **Gate:** the three Node doc verifiers (`verify-no-doc-shred`, `verify-readme-index`, `verify-tracked-references`).
- **Acceptance:** all three citations grow the prefix `cesium-webgpu-worker-archive/lanes-2026-09-11/cesium-lane-hundar-20260905/_lane-out/`, matching the bare, drive-letter-free form already used fifteen lines below at `:1958`/`:1961` and at `DEFERRED_WORK.md:498`; `grep -c "PROGRESS_THRAIN"` stays 3; no added line introduces a drive letter and the file's 19 pre-existing `F:/Dev/GH` paths are untouched; `git diff --numstat` is one path at 3 insertions / 3 deletions; `verify-doc-citations` unchanged at its `51 dead / 241 advisory` baseline.
- **Binds:** none. **Source:** `CAMPAIGN_13_V2_SOLO_ROWS_2026-09-12.md` §3.9; `DEFERRED_WORK.md:2127-2141`; `SOLO_WORKER_HANDOFF_2026-09-13.md:171` (the only Gemini-assigned row in the solo set, with a named Sonnet fallback).

### `DX-84` — No preflight reports parent-dead browser process trees before a sweep

- **Disposition:** **LANDING — Batch 1490 (2026-09-16), lane Aldarion; verdict LAND, no blocking fixes.** Gate **12/12/0**; `test-tools-lib` **115/115/0**, exactly the roster's re-measured target; report-only confirmed by grep (no `taskkill`, no `process.kill`, no `Stop-Process`). Seat acts at landing: stage the two new files and regenerate `TOOLING_CATALOG.md` (drift **+2/−0/~0**) (`ASTRA_WORK_AUDIT_2026-09-16.md` §1.1). <!-- corrected 2026-09-16, R-2026-09-12-10 --> *[status stamped 2026-09-16 by record round 5; the line read "**Disposition:** OPEN, **SOLO-AFTER-TEMPLATE** (seat ruling `R-HANDOFF-10`, 2026-09-13)." — and `R-HANDOFF-10` is itself superseded by `R-2026-09-13-8`, under which this row is `[SOLO-A]`.]* Prior text, retained: **Section written 2026-09-13 by the record lane** (owed by Batch 1482; `DEFERRED_WORK.md:2145`). Measured 2026-09-12: six orphaned Playwright/Edge renderers whose parents had been SIGKILLed between 09:00 and 10:26 held **14.5 GB**, leaving 3.9 GB free of 31.9 GB; killing the parent-dead ones recovered free memory to 18.4 GB. Every morning measurement that day — the job-13b legs, the `rendererGate` flake rates, both bisect poles — ran on a loaded box and none of them said so. `git grep -l -i "freemem\|ParentProcessId\|tasklist" -- Tools` returns zero files.
- **Tier / Size / Backends:** TIER-3 · M · tooling. **Depends on:** none. **Ruling touched:** `R-HANDOFF-7`/`R-HANDOFF-10` (worker Sonnet; the template is the seat's own 2026-09-12 orphan-kill procedure). **Gate:** `test-tools-lib`.
- **Acceptance:** as filed in `DEFERRED_WORK.md` `### DX-84` — `Tools/lib/browser-orphan-preflight.mjs`, **report-only** (no kill flag, no `taskkill`, no `process.kill`); a 12-test `node --test` spec that never spawns PowerShell, over a pinned nine-row fixture with an inertness mutant; `test-tools-lib` 96/96 → 108/108 via one add-only `package.json:170` append; exit codes 0/1/2 per `Tools/visual-regression/lib/verdict-exit-gate.mjs:29-34`.
- **Binds:** `DX-88` (the runner-side half: a runner that takes its browser down with it removes most of what this row reports). **Landing hazard, named not solved:** `test-sandcastle` sits at `package.json:167`, three lines above `test-tools-lib` at `:170`, and a live lane clone holds an uncommitted edit to `:167`, so a default three-line-context hunk carries that line as context — the seat sequences the two landings. **Source:** `CAMPAIGN_13_V2_SOLO_ROWS_2026-09-12.md` §3.12; `DEFERRED_WORK.md:2143-2161`; `SOLO_WORKER_HANDOFF_2026-09-13.md:174`, `:199`.

### `DX-85` — REFUTED: `provision-worker-clone.mjs` does **not** exit 0 on a missing path; the reading came from a pipeline's exit status

- **Disposition:** **REFUTED at the tree 2026-09-13** (record lane, Batch 1482 `77789c120b`), with one residual kept. The claim entered the wave record on 2026-09-12 as "`provision-worker-clone.mjs <path>` prints `not a git clone` and **EXITS 0** on a non-existent path". Re-measured three ways in a Batch-1482 clone: missing path → **exit 2**; existing non-clone directory → **exit 2**; **the same invocation piped into `tail` → exit 0**, because `$?` after `cmd | tail` is `tail`'s status, not the command's. The tool exits 2 at `Tools/provision-worker-clone.mjs:141-142`, and `DEFERRED_WORK.md` `### DX-82`'s own acceptance already asserted "the not-a-git-clone path still exits 2". **The generalisable defect is the measurement, not the tool:** an exit code read through a pipe is the pipe's. **Residual kept:** the provisioner *provisions* an existing clone and never creates one, and its `not a git clone` message does not say so — a one-line message change (or a `--clone` flag) would have made the recipe self-evident.
- **Tier / Size / Backends:** TIER-3 · XS · tooling / process. **Depends on:** none. **Ruling touched:** none. **Gate:** none for the refutation; `test-landing-rules` if the residual message change is taken.
- **Acceptance:** the refutation is the deliverable and is complete. If the residual is taken: the `not a git clone` path names the two-step recipe (`git clone --no-hardlinks …` then provision) in its message, still exits 2, and `Tools/provision-worker-clone-junctions.spec.mjs` pins the message.
- **Binds:** `DX-82` (same tool, different defect; one owner takes both). **Source:** lane-record entry of 2026-09-12 (Durin), re-measured by the record lane 2026-09-13 in `F:/Dev/GH/cesium-lane-tolman-20260913`; `Tools/provision-worker-clone.mjs:141-142`; `DEFERRED_WORK.md:2117`.

### `DX-86` — The `agy` Gemini worker dies on a transport error; the fallback is ad-hoc

- **Disposition:** OPEN. Measured 2026-09-12 during the C13 v2 fix round: the `agy` Gemini 3.8 Flash High worker died **twice on the same transport error** ("subscriber fell behind updates, stalled for 5s") — once after 80 s and once after 11 minutes — with the worktree **untouched both times** (diff md5 `c730dd037204733f30269a41bdbc2142` before and after each death). The round was reassigned to a Sonnet tier-3 (Ulbar) and completed. No rule, script or roster field encodes any of this, so the next dispatcher rediscovers it.
- **Tier / Size / Backends:** TIER-3 · S · process / tooling. **Depends on:** none. **Ruling touched:** none — this row proposes one. **Gate:** none (process rule); `test-landing-rules` if the roster field is made machine-checked.
- **Acceptance:** the dispatch rule is written where a dispatcher reads it (`SOLO_WORKER_HANDOFF_2026-09-13.md` §3 already carries the profile; `WORKER_ISOLATION_AND_BRANCH_HANDOFF.md` §8 is the convention home): **Gemini takes only short bounded docs tasks under the measured 30-minute cap, and after ONE transport death the lane falls back to a named Sonnet tier-3**; every Gemini row in a roster carries a named fallback lane before dispatch; `run-gemini-worker.sh` keeps its post-run git-write and dirty-files check.
- **Binds:** none. **Source:** seat measurement 2026-09-12 (two deaths, 10:55 and 11:08 ET, worktree md5 unchanged); `SOLO_WORKER_HANDOFF_2026-09-13.md:123`, `:171` (the fallback column exists for exactly one row today); seat memory `reference_antigravity_gemini_worker.md`.

### `DX-87` — `sandcastle-smoke.mjs` has no typings preflight, so a missing `index.d.ts` presents as anything but "typings missing"

- **Disposition:** **OWNED AND DISCHARGED BY BATCH 1483** (`ea651de6d8`, 2026-09-13 13:28:13 −0400) — the `sample-height-from-3d-tiles` fix lane took the sweep's typings preflight in the same batch, "because missing typings presented as this same demo hanging" (its own commit message), and `Tools/visual-regression/sandcastle2-typings-preflight.spec.mjs` is in that batch's landing gate at **9/9**. **The row is recorded here rather than omitted**, because the defect cost seven void bisect verdicts and the reason it is closed belongs in the DX queue's own home; a reviewer should confirm the preflight asserts resolution **on the served origin** before marking it CLOSED outright. Originally filed OPEN from the measurement below. Measured across the 2026-09-11/12 job-13b sweep and the abandoned bisect: with `packages/engine/index.d.ts` / `packages/widgets/index.d.ts` ungenerated (`gulp buildTs` not run), the sweep fails **intermittently and non-locally** — a hang, or a `rendererGate` FAIL, or a pass-with-one-404 — and **never** as "typings missing"; it is not reproducible in a direct page load. Seven bisect verdicts were VOIDED by exactly this, because the bisect clone failed the demo on WebGL too. This is the documented 2026-09-03 Sandcastle2 blocker wearing a different face.
- **Tier / Size / Backends:** TIER-3 · XS · tooling. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-sandcastle`, or the spec home the owner picks.
- **Acceptance:** `sandcastle-smoke.mjs` refuses to start unless `/packages/engine/index.d.ts` and `/packages/widgets/index.d.ts` resolve **on the served origin it was given** (not on disk), with a message naming `npx gulp buildTs`; a spec drives the refusal over a stub origin that 404s one of them. **Note for the owner:** `Tools/visual-regression/sandcastle2-typings-preflight.spec.mjs` already exists in the P0-2 fix lane's landing gate, so check whether that lane lands this row's assertion first — if it does, this row closes by citation rather than by code.
- **Binds:** `DX-90` (same file, same startup path — one owner takes both). **Source:** `Tools/visual-regression/output/wave-end/wave-p0-2-2026-09-11/README.md` ("Runner DX rows", item 3) and `BISECT_sample-height.md` §1 (the void verdicts).

### `DX-88` — A sweep or probe runner does not take its browser down with it

- **Disposition:** OPEN. The runner-side half of `DX-84`. Measured 2026-09-12: six Edge renderer trees survived their SIGKILLed parents and held 14.5 GB for hours; nothing in `Tools/visual-regression/` binds a launched browser's lifetime to the launcher's. Every rate, timing and flake number measured in that window was taken on a box whose free memory nobody stated.
- **Tier / Size / Backends:** TIER-3 · M · tooling. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-tools-lib` (helper) plus one live run.
- **Acceptance:** a runner that launches a browser registers it with a parent-exit watchdog (a Windows job object, or a poll on `process.ppid` that closes the browser when the parent is gone), so a SIGKILLed launcher leaves no renderer tree; and every Edge job's preflight prints "no orphaned `msedge`/`playwright` trees" and **states free memory**, which is banked beside each rate it produces. Report-only enumeration stays `DX-84`'s; this row is the teardown.
- **Binds:** `DX-84`. **Source:** seat measurement 2026-09-12 (Oromë lane, 14.5 GB held / 3.9 GB free of 31.9 GB / 18.4 GB after); `DEFERRED_WORK.md` `### DX-84`.

### `DX-89` — `test-cloud-c13` is a bare `node --test` list while two of its specs hard-require build output

- **Disposition:** **LANDING — Batch 1490 (2026-09-16), lane Aldarion; verdict LAND-WITH-FIXES, and the fix is a RECORD, not a code change.** The built pair measures **29/29/0, zero skips**. **The consequence to carry in the batch message:** after this row, `test-cloud-c13` **exit 0 no longer implies its cloud assertions ran** on an unbuilt tree — verified at **1 pass / 1 skip, exit 0**. The follow-up is filed: assert `skipped === 0` on a built tree, or take this row's ranked option 2 (`ASTRA_WORK_AUDIT_2026-09-16.md` §1.1). <!-- corrected 2026-09-16, R-2026-09-12-10 --> *[status stamped 2026-09-16 by record round 5; the line read "**Disposition:** OPEN."]* Prior text, retained: Measured 2026-09-12: the runner reported 84 tests / 53 pass / **31 fail** on its original eight-spec list, and Frór's triage attributed **28 of the 31 to tree state, not to any commit** — all 24 `webgpu-cloud-godray-current-mask-order` and all 4 `cloud-primary-ray` assertions fail with `Could not resolve …/Shaders/WebGPU/Environment/ProceduralClouds.js`, a generated shader-string module that is gitignored build output (`packages/engine/.gitignore:5`) and absent on any unbuilt tree. Batch 1468's commit body already said "65/65 after a build (36/64 on an unbuilt tree)" — a sentence in a commit message is the only place the dependency is documented, which is why a lead and the seat both read it as a one-day-old landing regression.
- **Tier / Size / Backends:** TIER-3 · S · tooling. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-cloud-c13` itself.
- **Acceptance:** ranked, either is sufficient — **(1)** the `test-cloud-c13` script preflights the generated module and refuses with "run `npx gulp build`" (or the two specs skip-with-reason, which keeps the runner honest on an unbuilt tree); **(2)** the harness generates the shader-string module from the `.wgsl` source so the dependency disappears. Either way the runner's output distinguishes "not built" from "failed".
- **Binds:** `C13-N58` (the mask-order **measurement** owed on a built tree — it is that row's precondition, and this row is what stops the confusion recurring). **Source:** `cesium-webgpu-worker-archive/lanes-2026-09-12/cesium-lane-telchar-20260912/_lane-out/TRIAGE_TEST_CLOUD_C13_FROR.md`; Batch 1468 commit body; `packages/engine/.gitignore:5`.

### `DX-90` — `sandcastle-smoke.mjs` has no per-demo watchdog, so one hung demo stalls a whole leg silently

- **Disposition:** **LANDING — Batch 1490 (2026-09-16), lane Aldarion, second of the five ordered units; verdict LAND, no blocking fixes.** Seven controls; a **live Edge leg**, `dx90-timeout-aldarion-20260914-03`, TIMEOUTs both demos (`lastFrameNumber` **450** and **367**) and the sweep continues; and the inert-deadline mutant makes the spec **hang forever** — bounded at **0/2** — which is the strongest inertness result in the packet set (`ASTRA_WORK_AUDIT_2026-09-16.md` §1.1). <!-- corrected 2026-09-16, R-2026-09-12-10 --> *[status stamped 2026-09-16 by record round 5; the line read "**Disposition:** OPEN."]* Prior text, retained: Measured 2026-09-12: the `webgpu-h2` leg hung on `sample-height-from-3d-tiles` for **5 h 42 m** with empty stderr, the process alive and no diagnostic. `NAV_TIMEOUT_MS=60000`, `EVALUATE_TIMEOUT_MS=20000` and `RETRY_ATTEMPTS=3` did **not** bound it — none of them covers a demo that settles the navigation and then never finishes.
- **Tier / Size / Backends:** TIER-3 · S · tooling. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-sandcastle`, or the spec home the owner picks.
- **Acceptance:** a per-demo wall bound (default sized from a measured run, overridable by env) that, when it fires, records the demo id, the bound and the last frame number reached, marks that demo FAILED-TIMEOUT and lets the leg continue; a spec drives the bound over a deliberately non-settling stub demo.
- **Binds:** `DX-87` (same startup path), `DX-95` (the same verdict function decides both). **Source:** `Tools/visual-regression/output/wave-end/wave-p0-2-2026-09-11/README.md` ("Runner DX rows", item 1).

### `DX-91` — Split-leg sweeps overwrite their own report

- **Disposition:** **LANDING — Batch 1490 (2026-09-16), lane Aldarion, HEAD of the five ordered units; verdict LAND, no blocking fixes.** Four split tests plus a full-gallery compatibility control; the mutant `const split = false && …` takes the spec **4 → 2 passing** (`ASTRA_WORK_AUDIT_2026-09-16.md` §1.1). The order of the five is forced by exported `package.json` context, not preference. <!-- corrected 2026-09-16, R-2026-09-12-10 --> *[status stamped 2026-09-16 by record round 5; the line read "**Disposition:** OPEN."]* Prior text, retained: Measured 2026-09-12: `sandcastle-smoke.mjs` writes exactly one `report-<renderer>.json` per renderer, so when a renderer leg is pre-split into halves — as job 13b's were, 172 / 171, each half its own process with its own failure guard — **the second half overwrites the first**. `webgpu-h1`'s report was harvested in time; `webgl-h1`'s was lost and its per-demo evidence survives only in a stdout log.
- **Tier / Size / Backends:** TIER-3 · XS · tooling. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-sandcastle`, or the spec home the owner picks.
- **Acceptance:** the report filename carries the leg's own identity (renderer plus the id range or an explicit `--leg-label`), so two halves of one renderer cannot collide; a spec runs two legs into one output directory and asserts two distinct files survive. Per-half harvesting under distinct names is the accepted fix and was already named as such in the gate's own report.
- **Binds:** none. **Source:** `Tools/visual-regression/output/wave-end/wave-p0-2-2026-09-11/README.md` ("Runner DX rows", item 2).

### `DX-92` — Re-point the three Batch-1471 anchors in `godray-sun-usability-uniform-ranges.spec.mjs`

- **Disposition:** OPEN. **Tracked also as `GODRAY-SUN-USABILITY-RANGE-CONTRACT-RED` in `QUEUE_2026-07-23_CAMPAIGN13.md` §1 (row added 2026-09-12 by lane L8) — one defect, one owner; this is the dispatch-queue view of that row**, filed here because the repair is tools class and Sonnet-sized. Measured: Batch 1471 (`3a9d6b80f0`, the god-ray energy law) added the `emitter` uniform range and made `sunRadiance` real, which staled a spec-side ownership mirror and an injection anchor; the three failing assertions are B4, C2 and C3. Frór's triage classifies all three as **DRIFT, not regression** — every behaviour they name is present and better enforced at HEAD. After the seat's own re-measurement of 2026-09-13 **on a built tree** these three are the **last red** in `test-cloud-c13` (540 tests / 537 pass / 3 fail), the 24 mask-order and 5 primary-ray assertions having gone green once the generated module existed. *(Basis stated because it matters: the record lane that wrote this section is build-forbidden and did not reproduce that run — see `DX-89`.)*
- **Tier / Size / Backends:** TIER-3 · S–M · tooling / spec repair. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-cloud-c13`.
- **Acceptance:** each of B4, C2 and C3 is re-pointed at the Batch-1471 shape it is meant to pin, **each with its own inertness mutant** that goes red when the behaviour is neutered (not when the source text changes); `test-cloud-c13` reaches zero failures on a **built** tree (see `DX-89`); no assertion is deleted to make the runner green.
- **Binds:** `DX-89` (the runner must be measurable before this row's green means anything). **Source:** `TRIAGE_TEST_CLOUD_C13_FROR.md` (archive `lanes-2026-09-12/cesium-lane-telchar-20260912/_lane-out/`); `QUEUE_2026-07-23_CAMPAIGN13.md:261`; Batch 1471 `3a9d6b80f0`.

### `DX-93` — The landing gate runs only the lane's own runner, though the census can already map paths to runners

- **Disposition:** OPEN. A runner green at one landing and red at the next, with nobody looking, happened twice on 2026-09-12. The mapping that would catch it already exists and is unused by any gate: `runCensus({files})` in `Tools/spec-runner-census.mjs` maps a path set to the runners those paths are homed in — demonstrated on Batch 1471's own changed-path set, which returns `test-cloud-c13`, the runner that batch did not run.
- **Tier / Size / Backends:** TIER-3 · M · tooling / process. **Depends on:** none. **Ruling touched:** proposed as `RR-2026-09-13-D`. **Gate:** `test-landing-rules`.
- **Acceptance:** the landing procedure computes `runCensus({files: <the batch's changed paths>})` and **runs every runner it names**, not only the lane's own; the wave-end gate runs every cloud runner; a spec drives the mapping over a fixture path set and asserts the runner list, with an inertness mutant that makes the mapping return empty and turns the assertion red. Quarantine runners stay excluded by `RR-2026-09-13-A`'s convention.
- **Binds:** `DX-94` (same census, the other blind spot). **Source:** critic pass Arantar, 2026-09-13 (`CAMPAIGN_13_V2_SOLO_ROWS_2026-09-12.md` §6 lineage); `Tools/spec-runner-census.mjs`; `TRIAGE_TEST_CLOUD_C13_FROR.md`.

### `DX-94` — The fork-wide spec-home hole is unguarded, and `spec-census` is in no aggregate

- **Disposition:** OPEN. Measured 2026-09-13 across the whole repository: **375 specs on disk, 166 homed, 209 orphaned, 7 quarantined.** `C13-N02` closed the *cloud* slice of this (0 homeless cloud specs) and `C13-N01` stage 1 added the ratchet for probes, but nothing guards the other 209, and `Tools/spec-runner-census.spec.mjs` — which exists and is green — **drives only the census's fixtures**; the census itself is in no aggregate runner, so a new orphaned spec lands unremarked.
- **Tier / Size / Backends:** TIER-3 · M · tooling. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-landing-rules`.
- **Acceptance:** the census runs as a gate, not only as a report: a shrink-only ratchet over the orphan count (the `probe-fleet-contract` allowlist pattern) wired into an aggregate so the number cannot grow silently, with the quarantined count reported separately from the homed count per `RR-2026-09-13-A`, and an inertness mutant that adds one orphaned spec and turns the ratchet red. The 209 are **not** required to be homed by this row — the row makes the hole visible and bounded.
- **Binds:** `DX-93`. **Source:** critic pass Arantar, 2026-09-13, **re-measured by the record lane at Batch 1482** — `node Tools/spec-runner-census.mjs` prints `Summary: total specs 375, homed 166 (7 quarantined), orphaned 209`; `Tools/spec-runner-census.mjs`, `Tools/spec-runner-census.spec.mjs`; `DEFERRED_WORK.md:789` (the cloud slice, for contrast).

### `DX-95` — A Sandcastle per-demo PASS is the absence of an error, so a broken tree can score a vacuous PASS

- **Disposition:** **LANDING — Batch 1490 (2026-09-16), lane Aldarion, third of the five ordered units; verdict LAND-WITH-FIXES, and the fix is a RECORD.** `wave-end-gate.spec.mjs` stays **36/36/0** and the reverting mutant reds both named adversarial settle values. **The consequence to record:** **no gallery demo declares `__sandcastleSmokeReady`** (`git grep` returns **0 files**), so every quiet demo becomes `INCONCLUSIVE` → `FAIL`. The sweep is **already red at HEAD**, so this widens an existing red rather than breaking a green gate — and the demo-adoption row is filed alongside (`ASTRA_WORK_AUDIT_2026-09-16.md` §1.1). <!-- corrected 2026-09-16, R-2026-09-12-10 --> *[status stamped 2026-09-16 by record round 5; the line read "**Disposition:** OPEN, **and it is a gate-trust defect, not a convenience one.**"]* It remains a gate-trust defect, not a convenience one. Prior text, retained: Found by the P0-2 fix's adversarial verifier (Fortinbras, 2026-09-13) while certifying the `sample-height-from-3d-tiles` repair. At `SANDCASTLE_SETTLE_MS=8000` the **known-BROKEN control tree scored a PASS** on that demo — recorded as "1 vacuous PASS @frameNumber 14, 2 HANG" — because the demo's throw had not happened yet when the settle window closed, and the runner's verdict is "no console error was seen", not "the demo reached the state it is being certified for". The fixed tree at the same settle passed 3/3 at frameNumber 18/22/18; at settle 25000 the control scored 0/5. **A gate that a broken tree can pass is the class of defect that voids every count taken under it** — the same mechanism that voided seven bisect verdicts a day earlier.
- **Tier / Size / Backends:** TIER-3 · M · tooling / gate trust. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-sandcastle`, or the spec home the owner picks.
- **Acceptance:** a per-demo PASS requires **positive evidence** that the demo reached its certified state — a demo-declared readiness predicate (for this demo: the clamp resolved with no `undefined` entry), or at minimum a settle-completion assertion that the demo's own frame loop advanced past the point where its failure mode fires — and a verdict of "no error seen within the window" is reported as **INCONCLUSIVE**, never PASS. Acceptance is adversarial by construction: the spec asserts that a stub demo which throws *after* the settle window scores INCONCLUSIVE or FAIL, never PASS, under at least two settle values.
- **Binds:** `DX-90` (a timed-out demo and an inconclusive demo are decided by the same verdict function). **Source:** `Tools/visual-regression/output/sample-height-webgpu/verify-fortinbras/README.md` (`runs/` leg table: `ctl-short-settle`, `fx-short-settle`, `ctl-1`/`-2`/`-3`), companion `VERIFY_FORTINBRAS.md` in the fix lane's `_lane-out/`; seat dispatch note 2026-09-13.

### `DX-96` — `runProbe` crashes instead of refusing when argv parsing fails

- **Disposition:** OPEN. **Section written 2026-09-16 by record round 5**, which was asked to verify that `DX-96` and `DX-97` exist and found them filed in `DEFERRED_WORK.md` (`### DX-96`, `### DX-97`) and **named nowhere in this file** — the same split-across-two-files defect that caused the `DX-86`/`DX-87` id collision on 2026-09-13 (reviewer Bereg, finding F3). Filed by `C13-N01` stage 2 batch 1, lane W2-T2 (Huan), Batch 1486 (`0e4b898129`). Measured 2026-09-13 at `ea651de6d8` with a two-line descriptor and no probe of that lane involved: `runProbe({name:'x',…}, {argv:['--port','8080']})` and the same call with `{argv:['--nope']}` both raise `TypeError: Cannot read properties of null (reading 'servedBuild')`. **Root cause:** `parseProbeArgs` raises the port-8080 `ProbeRefusal` (and a `TypeError` for a bad flag) **before `options` is assigned** (`lib/probe-runtime.mjs:888`); the catch records the refusal and then `buildRuntimeReceipt` reads `options.servedBuild` off the `null` the runtime passed for it (`lib/probe-runtime.mjs:665` — **re-derived at seat HEAD `c325f858c3` by this record round; the line is `servedBuildAssertion: options.servedBuild ? "enforced" : "waived",`**), outside the try. `runProbe` therefore REJECTS instead of returning `PROBE_EXIT_CODES.REFUSAL` (3) or `ERROR` (2), and a probe whose entry point is `process.exitCode = await runProbe(descriptor)` leaves on an unhandled rejection. Port 8080 is the exact case the origin-governance work exists to make loud.
- **Tier / Size / Backends:** TIER-3 · XS · tooling. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-visual-regression-node`.
- **Acceptance:** one guard at `:665`, or `options` assigned before the refusal can be raised, plus a spec leg over `parseProbeArgs` failure; `runProbe` returns the refusal/error exit code instead of rejecting, with an inertness mutant that restores the crash and turns the leg red. `lib/probe-runtime.mjs` is `DX-01`'s file, so the owner is whoever owns the runtime, not the routing lane.
- **Binds:** `DX-01` (same file). **Source:** `DEFERRED_WORK.md` `### DX-96`; Batch 1486 `0e4b898129`; `cloud-demo-probe-routing.spec.mjs` B2, which asserts what must hold either way (no browser launched, no page opened, no receipt written) and accepts refusal-or-rejection deliberately, so fixing the defect does not turn the spec red.
- **Second, independent sighting — added 2026-09-16 (record round 5, fix round 1), carrying withdrawn `DX-98`:** lane **Aldarion's `BRANCH` unit** met the same defect from the other end while building an unrelated spec and **confirmed it real at HEAD**, which is why the seat's follow-up list names it as a row of its own. It was briefly filed as `DX-98`; that id is **withdrawn as a duplicate** and this row is the sole owner. The second sighting adds no new mechanism — it adds **corroboration from a lane that was not looking for it**, which is the strongest evidence class this row has. **Source:** `ASTRA_WORK_AUDIT_2026-09-16.md` §1.1 (`BRANCH` row: "newly reported defect `lib/probe-runtime.mjs:665` (argv-null deref) confirmed real at HEAD") and §5.

### `DX-97` — `probe-cloud-extinction`'s "default deck not blown out" clause reads the CUMULUS capture

- **Disposition:** OPEN. **Section written 2026-09-16 by record round 5** for the same reason as `DX-96` above. Filed by `C13-N01` stage 2 batch 1, lane W2-T2 (Huan), Batch 1486. The clause's claim text says *default deck not blown-out white* and its boolean reads `cu.m.cloudMeanL` — the **cumulus** capture's mean luminance — not `mDef.cloudMeanL`. Pre-existing since the probe was written; the routed file preserves it verbatim and the pre-routing source has the same pairing. It became visible only because the routing extracted the inline `checks` array into a named pure function.
- **Tier / Size / Backends:** TIER-3 · XS · tooling / cloud measurement. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-visual-regression-node`, or the cloud lane's own runner.
- **Acceptance:** either the claim or the reading is corrected — **deciding which is a cloud-measurement question for the C13 cloud lanes, not a routing decision** — and the probe's reported verdict changes accordingly, with the chosen pairing pinned by a spec. Changing what a probe measures was deliberately out of scope for the routing lane.
- **Binds:** `C13-N01` stage 2 (the routing that surfaced it). **Source:** `DEFERRED_WORK.md` `### DX-97`; `probe-cloud-extinction.mjs` `evaluateExtinction`, clause `default-deck-not-blown-out`; Batch 1486 `0e4b898129`.

### `DX-98` — WITHDRAWN

- **Disposition:** **WITHDRAWN 2026-09-16 as a duplicate of `DX-96`; the follow-up is recorded there.** The id is allocated and stays allocated — add-only, never reused — so that a reader who meets `DX-98` in a brief, a packet or a lane note is routed rather than left to re-derive it. The row was filed the same day by record round 5 for lane Aldarion's `BRANCH`-unit sighting of the argv-null dereference at `Tools/visual-regression/lib/probe-runtime.mjs:665`, which is the **same defect** `DX-96` already records from the routing lane's end. One defect, one owner: **`DX-96` is the owner.** Withdrawn at review (Loni, fix round 1) rather than kept as a second sighting, because two open ids for one mechanism is exactly the condition that produced the `DX-86`/`DX-87` collision.
- **Tier / Size / Backends:** — (withdrawn). **Depends on:** — . **Ruling touched:** none. **Gate:** — .
- **Acceptance:** none — a withdrawn row has no acceptance. `DX-96`'s acceptance governs the defect.
- **Binds:** `DX-96` (the surviving id). **Source:** `ASTRA_WORK_AUDIT_2026-09-16.md` §1.1 and §5; withdrawal recorded 2026-09-16 by record round 5, fix round 1.

### `DX-99` — `sandcastleTemplate.spec.mjs` leaks fifteen temp directories per run

- **Disposition:** OPEN, filed 2026-09-16 by record round 5 from lane Aldarion's follow-ups (`ASTRA_WORK_AUDIT_2026-09-16.md` §5). The spec is `packages/sandcastle/Specs/sandcastleTemplate.spec.mjs` (path confirmed in this lane at seat HEAD `c325f858c3`); the reported leak is **fifteen temp directories per run**, left behind rather than removed. It is a hygiene defect, not a correctness one, and it runs against the temp-hygiene rule of Batch 1464 (`WORKER_ISOLATION_AND_BRANCH_HANDOFF.md` §8i: one sweepable root per lane, taken through `Tools/lib/lane-tmp.mjs`).
- **Tier / Size / Backends:** TIER-3 · XS · tooling / hygiene. **Depends on:** none. **Ruling touched:** none. **Gate:** `test-sandcastle`.
- **Acceptance:** the spec removes every directory it creates — `after`/`afterEach` cleanup, or one lane-temp root per run — and a test asserts the temp-root child count is unchanged across a full run of the spec, with an inertness mutant that disables the cleanup and turns it red. The count (**15**) is the reported figure and is re-measured by whoever takes the row rather than trusted from here.
- **Binds:** none. **Source:** `ASTRA_WORK_AUDIT_2026-09-16.md` §5 ("`sandcastleTemplate.spec.mjs` 15-temp-dir leak per run"); temp-hygiene rule `WORKER_ISOLATION_AND_BRANCH_HANDOFF.md` §8i.

### `DX-100` — No gallery demo declares `__sandcastleSmokeReady`, so `DX-95`'s verdict gate scores every quiet demo INCONCLUSIVE

- **Disposition:** OPEN, filed 2026-09-16 by record round 5 as the **named consequence of `DX-95` landing** (`R-2026-09-16-10`; `ASTRA_WORK_AUDIT_2026-09-16.md` §1.1, `DX-95` row: "Record the consequence … File the demo-adoption row"). Measured by the auditor: `git grep __sandcastleSmokeReady` over the gallery returns **0 files**. `DX-95` replaces "no console error was seen" with a demand for **positive evidence** that a demo reached its certified state; with no demo declaring the predicate, every quiet demo becomes `INCONCLUSIVE` → `FAIL`. **This widens an existing red rather than breaking a green gate** — the Sandcastle sweep is already red at HEAD — which is why `DX-95` was not held for it.
- **Tier / Size / Backends:** TIER-3 · M (a per-demo sweep, batched by family) · tooling / demos. **Depends on:** `DX-95` (landing in Batch 1490). **Ruling touched:** `R-2026-09-16-10` (the record that accompanies `DX-95`). **Gate:** `test-sandcastle`.
...
- **Status 2026-09-18 (lane KIT-A, Goold under Bunce; station 3 Fallohide):** LANDED. `Tools/visual-regression/rigs/` holds **39** records — 10 `wave-end`, 15 `cloud`, 4 `orbital-ladder`, 3 `saved-view`, 3 `sandcastle`, 4 `aurora`; `lib/rig-registry.mjs` loads/validates them and `scenes.schema.json` now exists, so `scenes.json:2`'s `$schema` resolves — and the reviewer additionally compiled that schema and confirmed all ten scenes validate against it. `capture-and-diff.mjs` unmodified; `rig-registry.spec.mjs` (5 tests, `test-visual-regression-node`) asserts byte-identical regeneration of `scenes.json`, unique ids, a frozen tag vocabulary, and a `replayKey` stable across runs and key order and sensitive to a determinism-relevant field. Mutants: the uniqueness assertion made unreachable reds the duplicate-id case (`39 !== 40`); the reviewer's independent ordering and replay-key-insensitivity mutants red tests 5 and 4. **Named:** the four aurora rigs' camera coordinates are the lane's placeholders (§5 gives geography in prose, no numbers) and carry `page: null`; `C15-04` replaces both. **Recorded, not fixed:** `generateScenesJson`'s closing CRLF normalisation is correct only under this repo's `core.autocrlf=true`; on an LF checkout the byte-identity assertion would red. Detail: `DEFERRED_WORK.md`, 2026-09-18 section.
...
- **Status 2026-09-18 (lane KIT-A, Greenhand under Bunce; station 3 Fallohide):** LANDED. `lib/image-diff.mjs` `diffImages` returns `{mismatchPct, changedPx, bbox, diffRgba}` with per-channel tolerance and an optional mask; `diffRgba` is asserted byte-identical to `visual-gate-policy.mjs`'s `compareCaptures` rendering, and no second gate policy is forked. One caller migrated output-identically (`probe-model-ktx2-ibl.mjs`), proven against the verbatim pre-move body — and re-proven by the reviewer over **400 randomised fixtures** including dimension mismatches and the degenerate `modelPx === 0` pair, at **0 output differences**. Spec `image-diff.spec.mjs` (6 tests, `test-visual-regression-node`); mutants: an always-zero diff reds the known-different pair, and the reviewer's tolerance-boundary, mask-polarity and dropped-rounding mutants red cases 3/5, 4 and 6. **Row correction:** this row and §3.2 describe the default tolerance as "16/255"; the tree's constant is an **absolute 16 on 0–255 channel values** (`lib/visual-gate-policy.mjs:247`, tested at `:275`) — read as a fraction it is ~16× tighter. The module states the absolute form and a spec case pins it (16 unchanged, 17 changed); the reviewer re-derived the same reading at the base.
...
- **Status 2026-09-18 (lane KIT-A, Headstrong + Hayward under Bunce; station 3 Fallohide):** LANDED, both halves, after one required review fix. Extraction: `lib/metrics/{masks,luminance,saturation,spectral-slope,region-means}.mjs` moved verbatim out of the three cloud modules, which are now delegating barrels; **no banked number moved** — the cloud specs stayed green UNMODIFIED, and `metrics-cloud-extraction.spec.mjs` compares the moved code against **what the pre-move modules actually returned**: a checked-in golden fixture, `Tools/visual-regression/fixtures/metrics-cloud-extraction.golden.json`, captured once at `1a2baeaa4a` over 36 cases and compared at bit-for-bit equality, with a `+1e-12` mutant still reding it. **The review caught a real defect here:** the first draft read the before side from `HEAD:` and imported it as a `data:` URL, so the instant this batch landed `HEAD:` would have returned the delegating barrel, whose relative specifiers cannot resolve from a `data:` URL — the spec would have stopped loading and taken `test-visual-regression-node` red on its first post-landing run. Pinning the hash would have fixed that but broken every shallow or depth-limited checkout, so the fix removes git from the spec entirely; numbers survive JSON via `Number.prototype.toString()` strings with a `-0` token and base64 typed arrays, and a fourth assertion pins the codec lossless. Left behind deliberately: the orbital model's rig/pin data and every `evaluateO*` verdict. New metrics: `lib/metrics/{connected-components,structure-similarity}.mjs`, `metrics-structure.spec.mjs` (7 tests), whose load-bearing case asserts in ONE test that two fields with an identical band mean have different component counts — `C15-08`'s "never a band mean for a faint sparse additive signal", made checkable. Mutants: an unreachable union/merge collapses the component count; a constant `mssim: 1` reds the SSIM cases; the reviewer's connectivity and covariance-sign mutants red tests 2 and 5. Dedup candidate recorded, not taken: `lib/metrics/spectral-slope.mjs` carries a private `labelConnectedComponents` beside the new module.
...
- **Status 2026-09-18 (lane KIT-B, Rudigar; tier-3 Hildibrand + Hildigard; station 3 Rollo LAND-WITH-FIXES, adversarial verifier Mosco REFUTED; v2 answers both):** DONE, Node half; the Edge leg is owed. `Tools/visual-regression/lib/capture.mjs` composes the runtime's pieces — `launchEdge`, `captureElement`, the three deciders, `assembleReceipt`, `withEdgeSlot` — and never calls `runProbe`, whose own `http://localhost:${options.port}` default (`probe-runtime.mjs:905`) is the thing this row retires. `capture(rig, {BEFORE, AFTER}, options, dependencies)` takes one rig or an array, opens ONE browser for the whole BEFORE × AFTER × both-renderers run, and measures everything in Node afterwards from the PNG bytes (`Tools/lib/png-decode.mjs` postdates Principle 8's "no Node PNG dep" note, so no spec needs a browser). **v2 correction, and it is the substantive one:** v1 opened the browser BY HAND inside the lifecycle body and closed it in a `finally`. That closes on the ordinary path and on the throw path and on neither of the paths the lifecycle exists for — the verifier drove the real `withProbeLifecycle` with a wedged cell and got `browser.close() calls: 0` at `exit(2)`. The slot is now taken through `scope.withEdgeSlot` and the browser through `scope.withResource({kind: "browser"})`, with `scope.checkpoint()` between cells and the per-cell browser context registered as a child resource; the same reproduction now closes the browser at the orderly deadline and only then hard-stops. A failing close is AGGREGATED with the cell failure instead of replacing it. **Row correction (unchanged from v1):** the title's singular `origin` is shorthand — plural origins win; and "no `localhost:8080` literal" cannot be met by passing a rig's `url` through, because 10 of the registry's 39 rigs declare exactly that absolute url, so `resolveRigPage` strips the declared origin and re-bases the path onto the caller's. Port 8080 is deliberately NOT refused — only ABSENCE and unusability are. Spec `capture-seam.spec.mjs`, 31 tests, `test-visual-regression-node`; every assertion reads a returned value, a refusal's `reason`, the bytes handed to the injected writer, or the browser double's observed open/close counts. Mutants: a hand-rolled close-on-success reds the throw path with the happy path GREEN; the hand-rolled `finally` (the v1 shape) reds ONLY the watchdog and close-masking cases, which is what proves the two are independent. **Recorded, not fixed:** the file is ~1,720 lines against CLAUDE.md's ~1,000 guidance; the two extractions are named in `DEFERRED_WORK.md` as a DX row for the seat rather than taken unilaterally. **v3 (Edge leg run by Odo — capture PASS, sheet PASS, banking PASS — plus Mosco's second pass):** readiness is now a POLL, not one post-settle sample. The recipe as written was NOT executable: the seed rig's `settleFrames: 30` (~0.5 s) plus a single read refused every WebGPU cell at ~4.8 s while reporting a 180 s budget it never spent, where the leg measured `renderReady` first true at 6,985 ms and `tilesLoaded` at 10,325 ms on a served built tree. `pollReadiness` re-reads the predicates every 250 ms until they hold — bounded by the per-cell timeout AND by a poll count derived from it, so a frozen clock still terminates — then the settle runs and they are re-read once more; a scene that never becomes ready is the refusal `capture-cell-not-ready` naming the unmet predicate, the last observed values, the polls and the elapsed budget, never a silently UNMEASURED cell. **No rig file changed:** the 39 registry rigs declare SETTLES, not readiness gates (36 `settleFrames`, 3 `settleMs`), and a spec case pins that census. The hard stop now writes the phase, the outstanding cell, the elapsed time and the open resources to stderr and to a durable `capture-incident.json` (through the runtime's own `buildIncidentRecord`) BEFORE the lifecycle's own line — the leg's run 2 exited 2 with the cell's diagnosis lost — and the per-resource close budget is the capture's own 45 s against a measured 3.3 s healthy close, not the fleet's 10 s. The manifest path predicate is now the shared `lib/relative-path.mjs`, which also refuses a drive letter (this seam accepted `C:/ESCAPED/x.png` while the page's reader refused it) and a percent-encoded traversal. Spec 41 tests. **v4 (Mosco F1 + Odo’s minors; LAND on this row both passes):** every timing option is validated with a NAMED refusal before a browser can launch. Only `closeDeadlineMs` was checked, so `readinessTimeoutMs: Infinity`, `: NaN` and `readinessPollIntervalMs: NaN` each reached `pollReadiness` and spun it — the poll’s own bound is arithmetic on the two numbers nobody validated — and only the hard stop ended such a run, because the slot loop checkpoints BETWEEN cells. `requireTimingMs` refuses a non-finite, non-integer, zero, negative, wrong-typed or absurd budget as `capture-timing-option-invalid` for all six budgets, at `buildCaptureDescriptor` and again inside `pollReadiness`; the sane maximum is 24 h and the poll count carries the STATED bound `MAX_READINESS_POLLS` beside the derived one. Readiness is now observable on the SUCCESS path too — `{polls, elapsedMsAfterFinalPoll, budgetMs, phase, lastUnmet}` per cell in the RECEIPT, never the manifest, because the manifest’s bytes are the sheet’s identity — which is what the Edge executor had to wrap a clock recorder to infer. The one-poll overshoot of the budget is BY DESIGN (it is tested after a sample returns) and the JSDoc and the field name now say so. Valid default options behave exactly as they did: same manifest bytes, same 45,000 / 51,001 / 1,020,001 budgets, same refusal message. Spec 48 tests.
...
- **Status 2026-09-18 (lane KIT-B, Hildibrand under Rudigar; station 3 Rollo; verifier Mosco; v2):** DONE. `lib/contact-sheet-page.mjs` is a dependency-free pure module (`sheetModel` → `renderSheetHtml` → `sheetIndexEntry`) and `contact-sheet.mjs` is a thin CLI whose `exitCode` reads no measured value; rigs sort by `id`, never by `mismatchPct`. **v2 corrections, all from the verifier's surviving mutants:** (1) the no-verdict guard was a twelve-word TEXT allowlist, so a judgement spelled as a CSS class (`pair-bad` / `pair-good`, keyed to the mismatch) rendered with `findVerdictTokens` still empty and every spec green — the page now declares a CLOSED class vocabulary (`PAGE_CLASS_NAMES` + `findUnknownClassNames`) and the spec additionally asserts that a MEASURED pair's markup is BYTE-IDENTICAL across a ten-value mismatch range once the numeral is normalised, which catches a value-conditional attribute of any spelling; (2) the exit-code acceptance was a five-point sample, so a threshold in the open band (5, 11) survived — it is now swept over 101 evenly spaced values plus the band and `null`; (3) `isRelativeImagePath` had no `..` clause and `receipt` was checked only for non-emptiness, so a traversing manifest made the CLI copy six images OUTSIDE `--out-dir` at exit 0 — both readers now refuse it and the CLI writes nothing; (4) `localeCompare` made the rig order (and therefore the banked md5) depend on the runtime's collation — the sort is code-unit; (5) an I/O failure mid-write surfaced as an unhandled rejection rather than the documented `{exitCode: 1, error}`. Spec `contact-sheet.spec.mjs`, 42 tests, `test-visual-regression-node`. **Integration found two defects inside this item** (v1, both fixed and asserted): a template literal nested inside a substitution made the file unanalysable to `lib/prohibited-reader-rule.mjs`'s tokenizer, which reports an unanalysable file as a C14 OFFENDER; and `posixPath.dirname` of a backslash `--manifest` is `"."`, so every image copy resolved against the working directory. **Contract correction:** `KIT-B-OWNERSHIP.md` §2 forbids DX-105 importing `lib/capture.mjs` while §4a has it importing `validateManifest` from there; §2 is the more specific rule and the page keeps its own fail-closed reader, with the two readers' agreement asserted in `capture-seam.spec.mjs` group H. **v3 (Mosco's second pass + Odo's leg):** the verdict guard is STRUCTURAL now. `normaliseMismatchNumerals` was `/mismatch [^<]*/`, which blanks the whole text node rather than the numeral, so `mismatch 11.07% degraded` normalised to the same bytes as `mismatch 0.42% clean` and both word-list guards reported clean; the normaliser matches the NUMERAL, and `findForeignFigureTokens(html, model)` requires every text node and attribute value inside a figure to be one the page MODEL produced, so ANY extra token — word, emoji, `title`, `data-` attribute, inline style — is a violation by construction. Its boundary is stated in its JSDoc and asserted by a mutant: a judgement written INTO the caption model is outside it. **That v3 sentence went on to say the case was covered by the other two guards, and v4 corrects it — see the v4 status below; it was true for `mismatchPct` and false for the five other numbers the page prints.** The exit code is independent STRUCTURALLY — `decideContactSheetExit({pageWritten, ioError})` has no measured value to read, asserted with a spy and with throwing getters — beside a 2,010-value sweep (0 to 100 in steps of 0.05). The sheet's date is the LOCAL calendar date (the leg banked a 23:32 EDT sheet under tomorrow), `--date` overrides and an explicit `--generated-at` keeps its UTC derivation. Path relativity is the shared `lib/relative-path.mjs`, which also refuses percent-encoded traversals — `node:fs` does not decode them but a browser does. Spec 56 tests. **v4 (Mosco's third pass, REFUTED on this row alone):** the invariance sweep varied `mismatchPct` and held `changedPx`, `tolerance`, `mssim`, `rawByteLumaMean` and `nonBlackFraction` FIXED, so a judgement keyed to any of those five never moved the page and survived 56 of 56 — and the closed set read `<figure>` blocks only, so a judgement in the page HEADER passed all three guards too. Both are closed: the verifier's `FIX-A` is applied verbatim (every measured value swept INDEPENDENTLY across its range plus `null`, and all six numeral labels normalised), and `findForeignPageTokens(html, model)` applies the closed-set rule to the WHOLE document — every text node and attribute value must come from the page model or from the exported `PAGE_CHROME_TEXTS` / `PAGE_CHROME_ATTRIBUTES`, with the doctype pinned and the `<style>` block required to be the module's own stylesheet byte for byte. The figure guard is KEPT as the POSITIONAL one. His four surviving mutants (`M-MSSIM`, `M-CHANGEDPX`, `M-LUMA2`, `M-HEADER`) are in this lane's harness and all four are RED. Spec 61 tests.
...
- **Status 2026-09-18 (lane KIT-B, Hildigard under Rudigar; station 3 Rollo; verifier Mosco; v2):** DONE, additive only. `Tools/wave-end-gate-receipt.mjs` gains `validateContactSheetEntry`, `buildContactSheetTable`, `collectContactSheets` and `projectContactSheetEntry`, an optional `contactSheets` on `buildReceipt` and a conditional `## Contact sheets` section appended after every existing section. An absent or empty list OMITS the key rather than writing `[]`, which is what lets the no-sheets path be proven byte-identical against a golden captured from the unmodified function; both existing consumer specs stay green unmodified. The collector RECOMPUTES each sheet's md5 from the page on disk rather than trusting the banked entry. **v2 corrections:** a banked entry's `path` was rejected only for a backslash, so an absolute path or a `..` segment made the collector read — and publish the md5 of — any file on the machine; and `{...entry}` carried unknown keys into the receipt, so a `verdict: "PASS"` on a hand-edited `sheet-index.json` landed in the banked artefact. The entry is now refused for a non-repo-relative path AND for any unknown key, and is PROJECTED onto its thirteen fields rather than spread. `collectContactSheets` also takes an optional `repositoryRoot`, so an entry's repo-relative `path` resolves without depending on the caller's working directory — the gap v1 recorded as underspecified. Spec `Tools/wave-end-contact-sheet-index.spec.mjs`, 17 tests, `test-landing-rules`, including two permanent MUTATION-fold cases. **Shape note for whoever wires this in:** a receipt WITH sheets has fourteen top-level keys, the thirteen existing ones plus `contactSheets` last; that is now pinned by this spec. `Tools/wave-end-gate-contracts.spec.mjs:518` builds its own receipt with no sheets and therefore stays green untouched. **Row gap still open:** wiring `collectContactSheets` into `Tools/wave-end-gate.mjs`'s CLI is outside this row's file ownership and remains to be done — `npm run wave-end-gate` banks no sheet today, and `DEBUGGING_GUIDE.md` now says so. **v3 (Mosco's second pass):** the unknown-key refusal was only half the door. `renderers`, `slots` and `rigIds` were typed by nothing, so an entry carrying `rigIds: ["globe-default — REGRESSION vs baseline"]`, `renderers: [{verdict: "PASS", gate: {...}}]` and `slots: [{note: "❌ AFTER is worse"}]` validated clean and reached both `receipt.json` and the markdown summary row. All three are closed vocabularies now — `RENDERER_IDS`/`SLOT_IDS` imported from `contact-sheet-page.mjs` rather than re-declared, `rigIds` against `RIG_ID_PATTERN` pinned to the real 39-rig registry by a spec case — with a recursive verdict scan over the rest of the entry as the backstop, reusing `VERDICT_TOKENS` so there is one vocabulary and not two. `entry.path` and `entry.manifest` both read the shared `lib/relative-path.mjs`. **Premise corrected:** `rig-registry.mjs`'s `validateRig` carries no id grammar to reuse, and that file is not this lane's to change. Spec 22 tests.
...
- **Status 2026-09-18 (lane KIT-A, Bunce; station 3 Fallohide):** LANDED, and this row owns the behaviour-selection half — wave 5 of the Gemini fix plan attaches. `lib/probe-fleet-contract.mjs` gains `launchesBrowserByBehaviour` / `fleetExemption` / `selectFleetByBehaviour`, and `probe-fleet-contract.spec.mjs` gains rules B1–B7 over that set. **Census re-derived at `1a2baeaa4a`: 676 selected, 630 inside the `probe-*.mjs` glob, 46 outside it** — the row's figure, reproduced, and reproduced again independently by the reviewer with its own directory walk (676 / 630 / 46 / 45, allowlist exactly the 45). `capture-and-diff.mjs:816` and `variant-smoke-test.mjs:455` are in the set and B1 asserts both that neither contains the literal `chromium.launch(` and that behaviour selection finds them, so the acceptance cannot be met by the old rule. **45 of the 46 violate** (45 no watchdog, 43 also closing outside a `finally`, 1 never closing); `Tools/readme-screenshots/capture-readme-screenshots.mjs` is the one compliant escapee and is absent from the list. The 45 are NOT repaired here — that is `DX-108`, one lane per family — and sit in the dated `lib/probe-fleet-behaviour-allowlist.mjs` (constructs named, add date from `git log --diff-filter=A`, expiry 2026-12-31). The expiry is RECORDED, not clock-enforced; B3 pins the CEILING (`BEHAVIOUR_ALLOWLIST_BASELINE = 45`, shrink-only) plus no-stale/no-repaired — and the reviewer confirmed that shrinking the list WITHOUT repairing the file reds B2, so the ratchet cannot be escaped by deletion. `@status ARCHIVED-CANDIDATE` is now REACHABLE as a named exemption from the live fleet, wherever the file sits (B4); the 16 files already in `archive/` still read `INVESTIGATION` and are held out by a second, separately named exemption until the first retirement cohort flips them. Mutants: an inert behaviour predicate reds 6 of the 7 B rules; the reviewer's genuine filename-selection mutant (call-form regex narrowed to the literal) reds B1/B3/B5/B6/B7, and a grown allowlist reds B3. **Review fix applied:** B7 claimed to "restore filename selection" but its guard tests the source text against a path pattern, so it is an inert-predicate control, not a filename one — renamed and re-commented to say what it does. **Scope note:** `requiresPurposeHeader` (`:1032`) is UNCHANGED — re-read at HEAD it selects the `@purpose` header population, not the watchdog/`finally` population this row's acceptance is about (that is `probe-fleet-contract.spec.mjs:74`), a reading the reviewer re-derived independently; widening the header rule would demand headers from 46 files in one batch, which is the harvest's work. **Recorded, not fixed:** `FLEET_EXEMPTIONS.LAUNCHER_LIBRARY` is granted by the path shape `…/lib/<file>`, broader than its JSDoc describes; one file takes it today and B6 prints the census.
