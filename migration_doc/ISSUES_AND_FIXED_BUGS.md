...
| **A.4 (GroundPrimitive morph leaf)** ⇄ `MORPH-EXAG-SKIRTS` | PARTIAL | **Exaggeration-in-CV/morph skirt-wall issue is ✅ FIXED (Batch 362; re-verified live 2026-07-05 via `probe-exaggeration-cv.mjs`, EXAG=10 Himalayas CV matches WebGL with zero walls, off-gate EXAG=1 CV normal).** The fix exaggerates the terrain vertex HEIGHT ATTRIBUTE in the planar leg (`GlobeTerrain.wgsl`), mirroring `GlobeVS.glsl:245-258`, so the taller skirt quad stays occluded exactly as in WebGL. SCENE2D + COLUMBUS_VIEW GroundPrimitive work post-Batch 156. **Residual (separate item):** a ~1s GroundPrimitive disappearance transient during the MORPHING animation — tracked under MORPH-REVIEW-GAPS (COMPLETION-POP), not the exaggeration/skirt path. | `MORPH-EXAG-SKIRTS` ✅ SHIPPED; residual morph transient OPEN under MORPH-REVIEW-GAPS. |
...
| MORPH-EXAG-SKIRTS | P1 | ✅ FIXED (Batch 362; re-verified 2026-07-05). Exaggeration in CV/morph no longer turns terrain skirts into walls — WebGL-faithful attribute-based skirt handling in the planar leg (`GlobeTerrain.wgsl`, exaggerate the height ATTRIBUTE not `length()`). Probe `probe-exaggeration-cv.mjs` (EXAG=10 Himalayas CV) matches WebGL, zero walls; off-gate EXAG=1 CV normal. The residual GroundPrimitive ~1s morph-disappearance transient is a separate item under MORPH-REVIEW-GAPS (COMPLETION-POP). |
