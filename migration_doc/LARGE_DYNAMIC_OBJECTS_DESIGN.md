...
| **P1-T5** | Wire **Point + Label** onto the manager (subsumes Point's `needsRebuild` gate at `WebGPUPointPrimitiveRenderer.js:959`). Label needs an index counter or Set-based dirty-list adapter (`_labelsToUpdate` has none); guard that SDF atlas `textureDirty` still forces a full rebuild. | M | P1-T4 |
