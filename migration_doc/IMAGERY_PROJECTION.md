...
**Residual polar/orbit pixel mismatch — NOT a fixable bug (Batch 64, 2026-05-18):** with the black hole gone, the remaining 12–46% pixel mismatch at polar/orbit views (e.g. northpole-orbit) is **accumulated cross-vendor numerical drift** between the WebGL GLSL and WebGPU WGSL pipelines — texture filtering, mipmap selection, depth-test edge AA, and per-fragment derivative precision — concentrated on high-contrast coastlines and the WGS84 polar tessellation singularity. The north > south asymmetry is scene-content (Arctic coastlines vs uniform Antarctic snow), not a renderer asymmetry. Ruled out: sub-pixel positional drift (optimum at (0,0)), day/night/atmosphere (byte-identical with effects off), tile selection/LOD, and anisotropy (regressed midlat-mid when forced to 16×). Driving it under 2% would mean byte-matching two driver stacks — multi-week, diminishing returns, likely not fully achievable. Established as the documented drift baseline that later batches gate against. See `WEBGPU_DEBUGGING_LOG.md` Batch 64.
...
The polar pixel residual remains, but its driver was settled in the following batches. It is *not* the piecewise approximation. The Batch-66 working hypothesis — **source-texture upload conventions** (WebGL uploads via the `Texture` constructor with `UNPACK_FLIP_Y_WEBGL=true` against an `ImageBitmap` produced by `Resource.fetchImage({ flipY: true })`, while WebGPU uses `copyExternalImageToTexture` with no flipY against the same pre-flipped ImageBitmap) — was investigated and **ledgered in Batch 67**: the two upload paths both land `v=0 = south`, and the single `srcV = mercatorFraction` (GLSL) vs `srcV = 1.0 - mercatorFraction` (WGSL) line in the reproject FS is the entire algorithmic divergence between the pair (the `imageOrientation:"flipY"` metadata does NOT flip WebGPU's underlying pixel buffer — proven by an empirical regression mid-Batch-67). So the upload conventions are NOT the source of the residual either. **Verdict (Batch 64): the residual is cross-vendor numerical drift, not a fixable bug** (see the [residual polar/orbit pixel mismatch](#mercator-imagery-on-geographic-terrain--the-interesting-case) note above and `WEBGPU_DEBUGGING_LOG.md` Batches 64/67). Later batches gate against the `probe-polar-multi-plain` numbers as a stable drift baseline rather than chasing them lower.
