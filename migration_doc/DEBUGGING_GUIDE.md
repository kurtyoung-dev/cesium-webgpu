...
| `probe-ellipsoidprim-logdepth.mjs` | **The EllipsoidPrimitive log-depth gate (Batch 266, NEW-ELLIPSOIDPRIM-LOG-DEPTH).** STRUCTURAL (not pixel) probe: places an EllipsoidPrimitive over a settled globe and asserts the LOG_DEPTH pipeline builds, the flip-rebuild guard toggles `_pipelineLogDepth` across ON→OFF→ON, and 0 console/WebGPU validation errors. It is structural because the WebGPU EllipsoidPrimitive renders NO visible pixels (pre-existing BUG-ELLIPSOIDPRIM-WEBGPU-INVISIBLE — `center` dropped + ray-cast quad emits no coverage; orthogonal to log depth). Once that bug is fixed, re-enable the green-pixel ON~=OFF assertions (scaffolding is in the probe). Run after ANY change to `WebGPUEllipsoidPrimitiveRenderer` LOG_DEPTH gating / camera-UB layout. (The Vector3DTile classifiers — Batch 266 NEW-CLASSIFIER-LOG-DEPTH — have NO probe: pixel-verify is blocked on absent `.vctr` test data; they are verified-by-analysis against the proven GroundPrimitive pattern + build-green + 0 validation errors.) |
