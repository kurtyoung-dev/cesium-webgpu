...
| `eye-carto-frame` | Certificate for the eye cartographic frame — camera UB floats 244-263 (`czm_eyeCartographic` / `czm_eyeToEnu` / `czm_eyeEllipsoidCurvature`). **Pure green** = the ENU basis reached the GPU orthonormal with det 1 to 1e-4 and `csm_eyeToCartographicDelta` round-trips the camera to zero. **Red** = the `mat3x3<f32>` was packed as nine tight floats (GLSL `mat3`'s layout) rather than three vec4 columns, or is otherwise not a rotation. **Blue** = the determinant error. **Magenta** = the tail is all zeros (never written, or the camera has no cartographic position and the packer zeroed it) — a zero matrix saturates red and blue together, and the delta divides by zero curvature and returns NaN. **Black** = the basis is a valid rotation but the delta still failed; suspect `eyeEllipsoidCurvature`. | Confirm the camera UB tail survives the trip to the GPU before debugging any consumer of the geodetic delta (model clipping, model vector lookup) |
...
- **`SURFACES_ONLY` (the default) still draws edges** — the mode gate is in the command emitter, not the WGSL emitter: `ModelDrawCommand.pushEdgeCommands` returns before pushing any color-pass edge command when `mode === EdgeDisplayMode.SURFACES_ONLY` (`Scene/Model/ModelDrawCommand.js:279-282`). Note the **snap pass deliberately bypasses that gate** (`:248-259`) — edge snapping works even when edges are visually suppressed — so seeing an EDGES_DIRECT command during a snap frame is correct, not a leak. If edges appear in a normal color frame on an edge-bearing CAD/BIM asset, check `model.edgeDisplayMode` at the call site first; the suppression itself is shipped.
- **`EDGES_ONLY` renders surfaces normally with NO edges (the inverse of intent)** — the three gaps this bullet used to describe are all closed at HEAD; re-verify before re-diagnosing. (1) The `Pass.CESIUM_3D_TILE_EDGES_DIRECT` slot (**slot 13** in `Renderer/Pass.js:30`, renumbered when `CESIUM_3D_TILE_PLANAR_FILL_ID` took slot 5) **does** have a frustum-loop leg, after `Pass.OPAQUE` at `WebGPUSceneRendererFrustumLoop.ts:463-475`. **Do not trust the number in the comments:** older notes say "slot 12", and so do **seven in-code comments across three files** — `WebGPUEdgeVisibilityEmitter.ts:282`, `WebGPUModelRenderer.ts:8717`, and five lines of the rationale block in `WebGPUSceneRenderer3DTilePasses.ts` (`:415`, `:417`, `:423`, `:425`, `:426`) that `ARCHITECTURE.md` §4a.3 cites for its reasoning. Read that block for *why* the pass is not dispatched inside the tile chain, never for its slot; the comment fix is owed engine work. (The `slot 12` hits in `WebGPUModelFeatureId.js:34` and `WebGPUModelRenderer.ts:2284` are uniform slots, not `Pass` slots — unrelated.) (2) Routing is mode-driven, not hardcoded: `ModelDrawCommand.js:287-290` picks `CESIUM_3D_TILE_EDGES_DIRECT` for EDGES_ONLY and `CESIUM_3D_TILE_EDGES` otherwise, and `WebGPUModelRenderer.ts:8732` does the same on the WebGPU side. (3) The surface command **is** suppressed — `ModelDrawCommand.pushCommands` returns early when EDGES_ONLY and the primitive has edge data (`:194-198`). **The live failure mode is the fragment-entry mismatch, not the pass slot:** the EDGES_DIRECT pass draws into the *scene* framebuffer, which has 2 color attachments under MRT, while the dedicated edge FBO has 3. Running the 3-output `fragmentMain` against the scene-FB pipeline yields an invalid pipeline and a draw that renders nothing — the scene-FB variant at `WebGPUEdgeVisibilityEmitter.ts:281-296` exists for exactly that reason. **Which Pass slot did the edge command land in?** Slot 4 (`CESIUM_3D_TILE_EDGES`) is the with-surfaces MRT edge pass; slot 13 (`CESIUM_3D_TILE_EDGES_DIRECT`) is the EDGES_ONLY direct pass. A command in slot 13 that produces no pixels now points at the pipeline's fragment-output count, not at a missing loop leg.
...
> **Runtime-resident probes don't use `PROBE_BASE` at all (DX-01/DX-06).** A probe carrying `@runtime lib/probe-runtime.mjs` in its header — `probe-globe-cold-start-readiness.mjs`, `probe-oit-primitive-reachable.mjs`, `probe-oit-collection-reachable.mjs`, `probe-oit-model-reachable.mjs` as of this migration — takes `--port` instead (default `8094`; port `8080` is refused outright, since the default dev server there serves a live esbuild of the source tree rather than the build the probe means to measure). These probes also default the served-build assertion ON: the bare command REFUSES (exit 3) unless something is already serving the built tree on that port — `node server.js --port 8094 --serve-built` in a separate terminal first, or `--no-serve-built` to waive the assertion deliberately. A migrated run also drops two extra files beside the probe's own report — `<name>-runtime.json` (the runtime's own record: resolved origin, parsed options, preflight result, Edge slot) and `<name>-summary.md` — into the same output directory.

> **The probe lifecycle is OPT-IN, and the opt-in is one descriptor field (`C13-42a`, 2026-09-12).** A descriptor that declares
>
> ```js
> workBudgetMs: (options) => 600_000, // the milliseconds THIS probe's work needs per run
> ```
>
> is run inside `lib/probe-lifecycle.mjs`, and four things change for it:
>
> 1. **`descriptor.cells` is additionally handed `scope`.** `scope.run(label, (signal) => …, { abort })` registers a unit of work so the runtime can wait for it and report what it did — but note `C13-42a-3` item 8 before you use the third argument at all: a malformed one — a non-function `abort`, or `null` — currently drops the work silently and the run still reports success; `scope.checkpoint()` throws if the lifecycle has begun draining; `scope.race(work, watchdog)` races already-registered work; `scope.signal` is the run's `AbortSignal`. Work started through `scope.run` is **tracked to a settled outcome**: the run cannot report complete until it settles, and whatever it reports reaches the incident record instead of becoming an unhandled rejection with no probe name on it — **whenever an incident is written at all**. Several paths write none (bullet 3 covers the hard stop; `C13-42a-3` item 6 lists the rest), and on those the work's failure goes with them. **What this does NOT do — measured, not assumed — is hold the browser open for it.** `closeBrowserAfter` starts `browser.close()` in the same tick it starts awaiting the registry, so a readback the probe left running is drained *concurrently with* the teardown and will find its page gone. **Await your `scope.run` calls if the work needs the page** (the C13-42 probe awaits all seventeen of its sites (fourteen `await` at the site; three — 842, 934, 1098 — are `return scope.run(...)` inside async functions whose callers await at 1263, 1269 and 1322, so no promise is dropped), which is why this does not bite it). Making the close wait for the drain is filed as `C13-42a-3`.
> 2. **The whole run carries a derived orderly deadline**, composed in `lib/probe-lifecycle-run.mjs` as `PREFLIGHT_BUDGET_MS + runs × (LAUNCH_BUDGET_MS + workBudgetMs(options) + RUN_SETTLEMENT_MARGIN_MS)`. Raising what a probe is allowed to do therefore raises the deadline that pays for it — the coupling a fixed timeout does not have.
> 3. **A run that misses the deadline is STOPPED and writes no receipt.** It writes `<name>-error.json` with `outcome: "errored"` and a `lifecycleFailures` array naming each obligation that failed (`"orderly deadline"`, `"<label> close observation"`, `"Edge-slot release"`, …). **There are TWO stops and only the first one writes anything.** The orderly deadline drains the run and produces that incident. Behind it sits one non-resetting hard-stop grace for the drain itself; if teardown is still not quiescent when that expires — a `browser.close()` that never settles is enough — the process calls `process.exit(2)` from inside the lifecycle, so `runProbe` never returns and **no artifact of any kind is written**. All you get is one stderr line, `"<probe>: lifecycle did not reach quiescence before hard-stop grace"`. Measured, not inferred. That silence is a real gap and is filed as `C13-42a-3`; until it closes, an exit 2 with no `-error.json` beside the probe's output means the hard stop, not a crash.
> 4. **Edge-slot ownership is re-asserted around every run.** The slot is still this fork's exclusive-create lock file; `withEdgeSlot` in `lib/probe-edge-slot.mjs` wraps it so `slot.assertHeld()` re-reads the lock's token, and a slot reclaimed as stale mid-run fails the run instead of letting it measure on a shared GPU.
>
> **A descriptor that declares no `workBudgetMs` is unchanged** — no `scope` key on its context, no deadline, the pre-adoption `acquireEdgeSlot` + one-browser-per-run shape. That is the whole staging: 18 probes on this runtime declare none, and migrating them one family at a time (each with a measured budget) is `C13-42a-2` in [DEFERRED_WORK.md](DEFERRED_WORK.md), not a flag day. Pinned by `Tools/visual-regression/probe-runtime-lifecycle-adoption.spec.mjs` (`npm run test-visual-probe-contracts`).
...
| `probe-eye-cartographic-frame.mjs` | **Eye cartographic frame on the GPU (census `C-20`/`C-21`/`C-22`, `-07` item 14), WebGPU-only.** Four scenes — nadir at 2 km / 500 km / 10,000 km plus a grazing near-horizon view from 1,000 km at 66.5°N. Per scene it turns on `globeFragmentDebug('eye-carto-frame')` and requires ≥ 20,000 certificate-green globe fragments with **zero** red or blue, plus `eyeCartographic.z === eyeHeight` in the live frame and 1e-9 agreement between `scene.context.uniformState`'s three values and an independent in-page re-derivation through `Transforms.eastNorthUpToFixedFrame` / `Ellipsoid.getLocalCurvature`. This is the leg that answers what Node cannot: whether the `mat3x3<f32>` survives WGSL's three-vec4-column layout on a real driver. **Refuses `--renderer webgl`** (the uniforms are WGSL; WebGL has had their GLSL twins since 1.145). **Needs a debug-pragma build** — `Build/CesiumUnminified`, not a release build, or the mode's writer is stripped and every scene comes back all-black. Companion Node spec: `eye-cartographic-uniforms.spec.mjs` (`npm run test-engine-node`). |
...
| Probe | What it covers |
| --- | --- |
| `probe-pick-visibility-matrix.mjs` | **The pick/visibility matrix (AR-837) — the instrument `AR-M01` and `AR-M30` need, and the standing guard for `AR-001`.** Per item (billboard / label / point / polyline) placed 60 km BELOW the ellipsoid under a 400 km nadir camera on `EllipsoidTerrainProvider` with imagery removed and `globe.depthTestAgainstTerrain` **true** (at its default the globe's depth is cleared before the primitive and pick passes and nothing occludes the subjects): `visible` (a hue-pixel count in a 21x21 window at the item's projected centre) and `pickAsync` at that same centre (5 attempts, counting the item's own id), over `disableDepthTestDistance` in {0, Infinity} x `scene.logarithmicDepthBuffer` in {true, false} x {webgl, webgpu}. Both are THREE-way — `visible`/`occluded`/`indeterminate` and `hit`/`miss`/`indeterminate` — so a 2-of-5 pick fails every expectation instead of being rounded into one. Runtime-resident (`--port`, default 8094; served-build assertion ON). **`--expect before|after` is REQUIRED and has no default:** `before` demands the `AR-001` difference be PRESENT and be the pick pass alone (WebGL hits, WebGPU misses, WebGPU still VISIBLE), `after` demands no backend difference; the WebGL anchor is asserted in both. An unoccluded GREEN control primitive in the same PointPrimitiveCollection is re-measured in every page leg and must pick — without it a WebGPU miss is indistinguishable from a dead pick path, since `Globe.pickable` is false by default and is WebGPU-only when true. The polyline cell is measured and published but NEVER asserted (held on `AR-D09`) — though its RENDERABILITY is, see below. **Round 2 (2026-09-06), from the instrument's first Edge run (job 10):** every collection carries an off-screen EXTENT KEEPER at +3.0 deg longitude and `controlHeight`, because subjects 60 km below the ellipsoid sit inside the horizon occluder (`Ellipsoid.minimumRadius`) and a collection whose bounding sphere does not clear it is culled — which silently removed job 10's billboard from all sixteen cells and its WebGL polyline anchor from all four. `subject-renderable-*` verdicts assert per subject per backend that the collection's draw command reached execution (read from `scene.debugCommandFilter`, which both backends consult only AFTER frustum and occluder culling), that the subject is present and shown, and that its image is ready where the type has one. Each `disableDepthTestDistance` leg spends discarded warm-up picks before its first measured pick — WebGPU cooks a pick pipeline through `createRenderPipelineAsync` and skips the pick draw meanwhile, so a leg's first sequence read an empty pick buffer — and publishes the cost as `pickWarmup`; `pick-warmup-*` asserts only that it RESOLVED. The `ddtd = 0` cells are now judged EXPECTATION-INDEPENDENTLY as an occlusion-parity row of their own (WebGPU must match WebGL), because job 10 measured WebGPU failing to occlude with `logarithmicDepthBuffer = false` bit-identically on both trees; `--expect` governs the `ddtd = infinity` cells alone. Cell verdict ids are unchanged. Also carries the `AR-M30` leg: a wide-aperture (`--snap-width`, default 45) 81-cursor snap over concentric RINGS scaled to the subject's own measured projected radius (0.45/0.7/0.95/1.2 x 20 steps + centre), on a page pinned to `EllipsoidTerrainProvider` with imagery removed and the model at 2500 m, reporting the `surfacePosition` defined-rate for edge hits more than 2 px from the cursor per backend, under its own `ar-m30-` verdict ids so a red there is attributable to `AR-030` and not to `AR-001`. **The subject must carry `EXT_mesh_primitive_edge_visibility` and must load in an edge-drawing `edgeDisplayMode`** — job 10 returned zero `isEdge` hits over 81 cursors on both backends because its subject (`CesiumMilkTruck.glb`) declares no extensions at all, and `isEdge` is written only by a model's edge pass (`ModelFS.glsl:68,202-203` under `#ifdef HAS_EDGE_VISIBILITY`), which exists only for primitives carrying that extension; separately, WebGPU gates its whole edge emitter, snap variant included, on `edgeDisplayMode !== SURFACES_ONLY` (`WebGPUModelRenderer.ts:8514-8517`) where WebGL's snap pass does not (`ModelDrawCommand.js:250-258`). The leg now uses `Specs/Data/Models/glTF-2.0/EdgeVisibility/glTF-Binary/EdgeVisibility.glb` at `SURFACES_AND_EDGES`, frames it from its own bounding sphere (`rangeFactor` radii out, so the projected radius is `(H/2)/(tan(fovy/2)*rangeFactor)` for any asset), and REFUSES (`snap-edge-mode-unresolved` / `snap-model-never-ready` / `snap-model-not-projected` / `snap-model-not-in-frame`) rather than reporting a rate it did not measure; "in frame" is a `pickAsync` at the model's own projected centre. Section N of the companion spec decodes the asset the config names and reds unless a primitive declares the extension, with job 10's subject as the negative control. Pixels come through the canonical `lib/same-task-capture.mjs` block, never a live-canvas `drawImage`. The in-page half lives in `lib/pick-visibility-matrix-page.mjs` and the decision functions in `lib/pick-visibility-matrix-verdicts.mjs`. Companion behaviour spec: `pick-visibility-matrix-verdicts.spec.mjs` (`npm run test-visual-probe-contracts`). |
...
| `probe-sample-height-webgpu.mjs` | **`sampleHeightMostDetailed` / `clampToHeightMostDetailed` gate (Batch 1482, NEW-PICK-RAY-ASYNC MostDetailed half).** Scores what the API RETURNS -- how many of the demo's 30 points came back `undefined`, whether the promise settled at all, and the clamped positions themselves -- rather than whether `sample-height-from-3d-tiles` happened to throw. That distinction is the point: a run whose clamp never settles adds no entities, throws nothing, and the Sandcastle2 sweep scores it PASS. `--mode=rate --runs=N` for the FAIL/HANG/PASS rate; `--mode=diag` for one instrumented run per renderer (page-side census of every `PickDepth` read with the frustum it was reconstructed against). Needs `--base` / `--sandcastle` on a `--serve-built` origin. Run after touching the offscreen ray-pick path, `PickDepth` readback, or the pick pass's depth publication. |
...
| `sandcastle-smoke.mjs --sandcastle2` | **Sandcastle2 backend sweep.** Same file, second mode: drives the Sandcastle2 app itself (`Apps/Sandcastle2/index.html?id=<id>&renderer=<webgl|webgpu>`, or `standalone.html` with `--standalone`) across every gallery id and asserts, per demo, (i) zero console/page/WebGPU-validation errors, (ii) **every live graphics context reports the renderer that was requested** — read from `Cesium.GraphicsContext.registry` inside the bucket frame — and (iii) the viewer advanced past its first frame. Gate (ii) is the one the three-demo gate above cannot have: a demo whose construction shape the runner fails to rewrite renders a perfectly good picture on WebGL, so every pixel and error check passes. Flags: `--renderer=`, `--ids=a,b`, `--limit=N`, `--standalone`, `--capture-all`, `--dry-run` (enumerates ids + URLs, no browser). Captures on failure to `output/sandcastle2-sweep/`; JSON report per run. Requires a built `Apps/Sandcastle2` (`npm run build-sandcastle`) and the dev server. Opens the app through `lib/sandcastle2-origin-rewrite.mjs` — see the row directly below. |
| `lib/sandcastle2-origin-rewrite.mjs` | **Origin-rewrite + PERSISTENT navigation guard for the built Sandcastle2 app.** The build bakes two literal origins into the app's own JS/HTML — the outer app origin and the run/bucket-mirror origin — and one compiled entry point (`packages/sandcastle/src/main.tsx`) redirects the whole top-level page to the baked outer origin the instant `window.location.origin` disagrees with it. A build produced once stays on disk unchanged, so opening it on any other port silently redirects there — including onto a port someone else is using. `installOriginRewrite(contextOrPage, origins)` intercepts the app's own responses and swaps the baked strings for the origins a run actually wants. The guard is NOT a single post-navigation check: `attachOriginGuard(page, {origins})` installs a `framenavigated` listener that runs for the page's WHOLE life, checking the main frame against `origins.servedOrigin` and every direct child frame of the main frame (by parentage, not by a `bucket.html` url pattern) against `origins.bucketOrigin`, and records the first mismatch it ever sees — a redirect that lands seconds later and several navigations on is caught exactly like an immediate one. `assertNoOriginBreach()` is the synchronous, on-demand read of that recorded state; `gotoWithOriginGuard` / `waitForBucketFrameOriginGuard` call it after a bounded prompt wait, but the listener itself never stops. `createGuardedPage(context, origins)` — THE PAGE FACTORY — wraps `page.close()` so it (1) asserts no recorded breach AND that no guarded navigation is still in flight (a still-pending one refuses on its own with code `NAVIGATION_UNVERIFIED` — closing a page out from under an unfinished check cannot honestly be called "closed clean"), (2) closes the page regardless of what step 1 found, then (3) detaches the listener — in that order, and a refusal found in step 1 always wins as the error `close()` throws even if step 2's own close call also fails. **The unawaited-navigation guarantee is conditional, not automatic: every opener must both create its page with `createGuardedPage(...)` AND navigate with `openSandcastle2Url(...)`; `openSandcastle2Url(...)` alone (called against a plain `context.newPage()`) records later breaches but does not arrange for them to be surfaced when the page or browser is closed** — `browser.close()` in particular never invokes the JavaScript-level `page.close()` wrapper at all, so an opener that only closes the browser gets no fail-closed protection regardless of how the page was created. `sandcastle2-renderer-gate.mjs`'s `openSandcastle2Url` (which installs the rewrite AND guards both the page and the bucket frame — a genuine one-call opener, not a top-level-only wrapper), `sandcastle-smoke.mjs --sandcastle2`, `probe-sandcastle2-webgpu-start.mjs`, and `probe-sandcastle2-ports.mjs` all create their page via `createGuardedPage` and route every exit path through its wrapped `close()`; any new opener of the built app should too, rather than a bare `page.goto` or a page created directly from `context`/`browser`. Contract spec: `sandcastle2-origin-rewrite.spec.mjs` (`node --test`) — pure predicate/rewrite/persistent-guard unit tests (including a deterministic, no-browser proof that a SECOND, later navigation is still caught by the same guard instance), a real-browser proof through `openSandcastle2Url` itself (opened via `createGuardedPage`), a delayed-redirect reproduction, a subprocess proving a fire-and-forget navigation still refuses at page-close (asserted via an explicit close-time sentinel, not an inferred process exit code), and a dead-baked-origin leg with a captured `requestfailed` connection error — each browser test `{skip}`s rather than silently falls back when Edge is unavailable, so a raw pass/skip count says whether the browser legs actually ran. |
...
- `readyGateSkipsThisFrame` — primitive colour commands withheld this frame because the on-screen colour pipeline (`primCache.pipeline`, built via `createRenderPipelineAsync`) had not resolved yet. Exact pick demand may still produce a synchronous pick-only carrier for that primitive, so this counter can be non-zero in the same frame as `pickCommandsEmittedThisFrame`.
- `pickCommandsEmittedThisFrame` — ordinary pick carriers actually built this frame. A resolved colour command owns the carrier as its `derivedCommands.picking.pickCommand`; while colour is pending, the carrier is emitted directly as a native `pickOnly` command and owns any requested snap, precise, or metadata variants.
...
These counters discriminate colour readiness from pick emission while a tileset streams. During exact pick demand, a non-zero `readyGateSkipsThisFrame` with a matching non-zero `pickCommandsEmittedThisFrame` means the colour command was withheld while the synchronous pick carrier remained available. A ready-gate skip with no pick emission still identifies an upstream emission failure or an intentional exclusion such as disabled picking or classification. If ready-gate skips stay near zero while the pick hit rate is low, investigate the pick framebuffer cold-path or stale-readback behaviour instead.
