...
| `probe-scene-snap.mjs` | **C11-212 `Scene.snap` acceptance (Batch 813)** — CesiumMilkTruck at 100 m AGL, camera 300 m out, both backends. Gate A: snap at the model returns a hit within 50 m (measured IDENTICAL 24.8 m WebGL vs WebGPU — payload depth agrees to the meter; WebGPU needs ~5 calls per the one-frame-stale readback contract, WebGL 1). Gate B: corner snap returns no hit (snap is Model-pipeline-only). Gate C: WebGPU `context._snapEnabled` latches false->true only after first use. Gate D: 0 console + 0 validation errors (an attachment-format drift empties every query silently). Run after touching `WebGPUSnapFramebuffer`, `WebGPUSnapPayload`, `fragmentSnapMain`, the pick-pass payload phase, or `Snapping.js`. |
