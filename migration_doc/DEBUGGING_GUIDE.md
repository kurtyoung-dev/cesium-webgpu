...
>   range.
> - **PIXELS** — currently reports **STRUCTURAL, not PASS and not FAIL.** The
>   star field drew nothing at the darkest lane in this configuration, so the leg
>   measured an empty sky: scoring it FAIL would file a phantom defect and
>   scoring it PASS would be a false green. It is recorded as an OWED INSTRUMENT
>   GAP on the C12-34 row. A leg that cannot see its own subject must say so.
>
...
| `probe-moon-lola-relief.mjs` | **C12-25 LOLA terminator-relief acceptance (Batch 813)** — 8 lanes: half/full phase x WebGL/WebGPU x relief ON/OFF (`atmosphericConditions.lighting.enableLunarNormalMap`), pinned clocks, camera 20,000 km short of the moon (~190 px disc), 420 px center-crop diffs. Gates: terminator ON-vs-OFF >= 0.4% both backends (measured 1.30/1.30); full-phase ON-vs-OFF < 3% (measured 1.46 — whole-disc ~1.6% brightness at the diff threshold, per design); cross-backend parity ON and OFF < 15% (measured 0.00/0.00); 0 errors. PNGs `moon-lola-*` show crater-serrated vs smooth terminator. Run after touching the `LUNAR_NORMAL_MAP` block in `EllipsoidFS.glsl`, the binding-3 path in `Moon.wgsl`, `_loadMoonNormalTexture`, or the bake under `Tools/moon-albedo-bake/`. |
...
| `pipeline-key-aliasing.spec.mjs` (`node --test`, no browser) | **The guard for `NEW-WEBGPU-PIPELINE-KEY-DEFINE-AXIS-GENERAL` (59/59 after Batch 825).** Its STRUCTURAL group executes the REAL render + compute caches over every bit of the REAL `ShaderDefine` registry using identically-named, MARKERLESS descriptors and requires distinct keys — enumeration of expected markers was deliberately replaced by execution, because an enumeration test passes for a marker nobody reads. Its MUTATION-FOLD group rewrites a copy of the engine source to REMOVE the module-identity fold and requires the aliasing to come back. Producers that intentionally bypass the central cache declare themselves under `NO_CENTRAL_CACHE`. Run it after touching `WebGPURenderPipelineCache.generateCacheKey`, `WebGPUComputePipelineCache`, or `WebGPUShaderModuleCache`'s key. |
...
| `probe-gsplat-parity.mjs` | **C15-G1 Gaussian-splat harness — the DUAL-MODE gate every later GSPLAT row runs through (2026-08-07).** Loads an in-tree splat tileset (`--asset=sh_unit_cube` default, `tower`, or `both` — served from `Specs/Data/Cesium3DTiles/GaussianSplats/` by the repo-root static mount, no Ion, no network) on BOTH backends at a camera derived from `tileset.boundingSphere` (no hardcoded ECEF), globe hidden, pinned clock, same-task capture. Scored quantity is pixels the tileset ADDED versus the same settled scene with `tileset.show = false` — never "non-black pixels" — with structure floors (edge fraction + luminance σ) so a flood fill cannot satisfy the 2% added-pixel gate, and a two-sided negative control (hidden frame blank, hiding again returns to it, ON beats the control 10×, same metric same reference). **DEFAULT mode** certifies the documented `C15-G0` absence: prints the greppable `WEBGPU-SPLATS-ABSENT (expected until C15-G3)` and stays exit 0, but ONLY when the absence is attributable to a named observed blocker — an unattributed blank canvas exits 3, and PRESENCE in default mode is also exit 3 (stale contract, not a pass). **`--expect-webgpu`** (turned on by `C15-G3`) flips it: absence is exit 1, presence runs the cross-backend pixel-diff parity gate at the `C15-G8` thresholds (tower 3%, cube 1%). The parity leg refuses to emit a number when the WebGPU leg drew nothing — two blank canvases diff to 0.000%. Nine structural preconditions incl. `capture-liveness` (paints the scene a known colour and requires the readback to see it, so the Batch-227 dead-readback trap cannot be filed as a WebGL defect) and `pass-enum-exported` (`indices[undefined] \| 0` is 0, which would MANUFACTURE the absence). Verdict arithmetic lives browser-free in `lib/gsplat-parity-model.mjs`, pinned by `gsplat-harness.spec.mjs` (`node --test`, 50 checks: 22 rules × real + 12 mutants each required to be rejected + CRLF-safe source anchors on the `C15-G0` engine facts, which BREAK LOUDLY when `C15-G2/G3` land). Evidence → `output/gsplat-parity/`. |
| `probe-splat-sort.mjs` / `probe-splat-globe-occlusion.mjs` | WebGPU splat back-to-front sort + globe occlusion, driven by a **synthetic** hand-rolled primitive (`show: true`, `_splatData` pre-filled). They exercise the renderer, NOT the production data path — see `probe-gsplat-parity.mjs` for the production-asset gate. |
