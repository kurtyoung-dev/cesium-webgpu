...
| `probe-voxel-octree.mjs` | **Voxel octree LOD gate (VOXEL-OCTREE-LOD + NEW-VOXEL-OCTREE-L2-ASSET-PROBE).** THREE-level procedural box provider (`availableLevels: 3`, 4³ per tile, Y_UP — asset in `Tools/visual-regression/fixtures/voxel-octree-l3.mjs`): the level-2 truth is the thin 16³ diagonal whose conservative downsample is `y === z` at every level (self-similar), giving L1 discriminators (empty-at-8³ / filled-at-root) AND L2 discriminators (empty-at-16³ / filled-at-8³). Expectations are 5-ray pick-ray CONES per cell (center + sampling-window corners; graze/bleed cells auto-skip; median window pixel defeats star dots). CLOSE view (10R): WebGPU must match LEVEL 1 exactly (≥4 L1 discriminators black; `slotCount 9`, 8 `childSlots`, `lastTargetLevel 1`); WebGL judged per-cell vs L1-or-L2 (mixed per-node refinement is legitimate upstream). CLOSE2 view (5R): WebGL must match LEVEL 2 with all (12/12) L2 discriminators black — proves the asset discriminates; WebGPU at HEAD must match LEVEL 1, and its 0/12 L2 discriminators are the ANNOTATED expected-fail documenting the depth-1 clamp. `EXPECT_L2=1` flips that gate to require full level-2 on WebGPU — the acceptance mode for NEW-VOXEL-OCTREE-DEEP-TRAVERSAL (B17). Far view: `lastTargetLevel 0` + small crop diff vs WebGL. Run after touching `tryUploadChildVoxelTiles`/the atlas allocation (`WebGPUVoxelDataUpload.ts`), the WGSL octree traversal / atlas slab addressing, or `computeVoxelTargetLevel` (`WebGPUVoxelRenderer.ts`). |
