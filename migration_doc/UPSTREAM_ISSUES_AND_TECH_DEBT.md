...
**Last Updated:** March 29, 2026 (Updated: MAT-1 **FIXED** — Material texture binding. `Material._imageSources` retains raw Image/ImageBitmap after async load. `ensureMaterialTextureBindGroup()` creates real GPU textures via `context.createTextureFromImage()`, replacing checkerboard placeholder. All 10 textured material types now display actual textures in WebGPU.)
...
| **FORK-28** | **Material system COMPLETE — 25 of 25 built-in materials mapped.** Primitive materials (18 types × flat+lit = 36 WGSL shaders): Color, Image/DiffuseMap, Checkerboard, Grid, Stripe, Dot, Fade, RimLighting, AlphaMap, EmissionMap, SpecularMap, BumpMap, NormalMap, Water, ElevationContour, ElevationRamp, SlopeRamp, AspectRamp. Polyline materials (4 types, 4 WGSL shaders): PolylineArrow, PolylineDash, PolylineGlow, PolylineOutline — rendered via `WebGPUPolylineRenderer.js` with per-material-type pipeline dispatch, material uniform packing at byte offset 112, and `st` texture coordinates computed from cumulative polyline distances stored in instance data padding slots (`startPosLow.w`/`endPosLow.w`). RTE precision unaffected (`.xyz` access ignores `.w`). `MaterialAppearance.js`, `PerInstanceColorAppearance.js`, `EllipsoidSurfaceAppearance.js`, `PolylineMaterialAppearance.js` converted to ES6 class. | ✅ Done | 48 Primitive WGSL + 4 Polyline WGSL shaders, `WebGPUPrimitiveShaders.js`, `WebGPUPrimitiveCommands.js`, `WebGPUPolylineRenderer.js`, `WebGPUCollectionShaders.js`, 4 Appearance ES6 classes | ✅ Complete. All 25 built-in material types have WebGPU WGSL shader equivalents. |
...
| **FORK-34** | **WebGPU pick render pass — fully wired for all primitive types.** `WebGPUSceneRenderer._executePickPass()` renders to pick FBO. `Primitive.js` pushes WebGPU pick commands during pick-only passes. **All three collection renderers have full pick support:** `WebGPUBillboardRenderer` (`buildPickInstanceData` + `_pushBillboardPickCommand` + `BillboardCollectionPick.wgsl`), `WebGPUPointPrimitiveRenderer` (`buildPickInstanceData` + `_pushPickCommand` + `PointPrimitivePick.wgsl`), `WebGPUPolylineRenderer` (`buildPickSegmentData` + `_pushPolylinePickCommand` + `PolylineCollectionPick.wgsl`). All check `frameState.passes.pick` and push pick commands to `commandList`. **Depth readback:** `WebGPUPickFramebuffer.ts` now has `depth32float` readable depth texture + `readDepthPixelAsync(x, y)`. `PickDepth.js` converted to ES6 class with `getDepthAsync()` for WebGPU. **Remaining:** WGSL depth-to-color blit shader for main scene depth readback (`pickPositionWorldCoordinates` — pick pass depth works, scene depth pending). | ⚡ Improved (near complete) | `WebGPUSceneRenderer.ts`, `Primitive.js`, `WebGPUPickFramebuffer.ts`, `WebGPUBillboardRenderer.js`, `WebGPUPointPrimitiveRenderer.js`, `WebGPUPolylineRenderer.js`, `PickDepth.js` | **Remaining:** WGSL depth-to-color blit shader for scene depth → `pickPositionWorldCoordinates`. ~1-2 days. |
| **FORK-35** | ~~**Duplicate pick ID systems in WebGPU**~~ — `WebGPUPickManager.ts` was removed (FORK-2). `WebGPUContext.ts` now inherits `createPickId()`/`getObjectByPickColor()` from `GraphicsContext` base class. **Context.js** (WebGL) also consolidated: removed its own duplicate `_pickObjects`/`_nextPickColor`/`createPickId`/`getObjectByPickColor` — now inherits from `GraphicsContext`. Single source of truth for pick ID management across all backends. | ✅ Resolved | `Context.js` (cleaned), `GraphicsContext.ts` (canonical), `WebGPUContext.ts` (inherits) | Removed `import PickId` from Context.js, removed shadowed `_pickObjects`/`_nextPickColor` fields, removed duplicate methods. Both backends now use GraphicsContext's concrete implementation. |
| **FORK-36** | ~~**No convenience API for drill-pick-to-earth-center**~~ — Three convenience methods added to `Scene.js`: `scene.pickAll(windowPosition, options)` wraps `drillPick`, `scene.pickRayAll(ray, options)` wraps `drillPickFromRay` with `diameter` param, `scene.pickColumn(position, options)` constructs downward ray from lon/lat toward earth center with configurable diameter. All use options-object pattern per CesiumJS conventions. | ✅ Resolved | `Scene.js` | `pickAll`, `pickRayAll`, `pickColumn` added with full JSDoc. `Ray` import added. |
...
| 🟡 Medium | 23 | 13 (FORK-2, FORK-3, FORK-9 partial, FORK-12, FORK-13, FORK-15, FORK-18, FORK-25, FORK-27, FORK-35) | 10 | WebGLCompatibilityStub (FORK-4), type helpers (FORK-11), test infra (FORK-16-17, FORK-20-23), material system (FORK-28), parity gaps (FORK-31, FORK-32, FORK-33) |
| 🟢 Low | 5 | 3 (FORK-10, FORK-8 acceptable, FORK-36) | 2 | Slang unused (FORK-29), `@webgpu/types` version (FORK-30) |
| ⚠️ **Architectural** | 3 | 3 (FORK-24, FORK-25, FORK-26) | **0** | All architectural items resolved! |
| **Total** | **36** | **21 resolved** | **15 remaining** | — |

### Resolved Fork Tech Debt Items (21 of 36)

FORK-1 ✅, FORK-2 ✅, FORK-3 ✅, FORK-5 ✅ (Phase D 28/28), FORK-6 ✅ (effectively), FORK-7 ✅, FORK-8 ✅ (acceptable), FORK-9 ⚡ (7 of 18 casts fixed), FORK-10 ✅, FORK-12 ✅ (primary), FORK-13 ✅ (confirmed clean), FORK-14 ✅ (drift fixed), FORK-15 ✅ (transitive deps), FORK-18 ✅ (DepthPlane implemented), FORK-24 ✅ (effectively), FORK-25 ✅ (all wired), FORK-26 ✅ (COUNT auto-computed), FORK-27 ✅, FORK-35 ✅ (pick ID consolidated), FORK-36 ✅ (convenience APIs added)
