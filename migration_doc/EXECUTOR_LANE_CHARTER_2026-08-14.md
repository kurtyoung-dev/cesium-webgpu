...
| 2.1 quiet-hours timestamps | Same pre-push hook, using `Intl` with `America/New_York` | Rejects a push whose PUSH INSTANT falls inside the window; the commit-timestamp check inside the pushed range is ordered by `R-2026-08-17-1` and not yet landed — the after-the-fact detector (`npm run verify-landing`) does check commit timestamps | It does not prevent the original commit or other visible GitHub/account activity; hook coverage of in-range commit timestamps lands with the ordered guard fix. |
