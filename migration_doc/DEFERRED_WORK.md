...
**NEXT STEP — confirm the depth-value mismatch directly:** read back the WebGPU depth buffer at the building's screen pixels vs adjacent terrain (copyTextureToBuffer on the depth attachment) with the globe shown; compare the stored NDC z of the building vs the globe. If the globe's z is closer than the building's where the building is geometrically above terrain, that confirms a globe/model projection or convention mismatch — fix by making both passes write depth with the same per-slice projection (and audit the `clearGlobeDepth`/`depthTestAgainstTerrain` path that currently nukes the globe). Then re-verify via `probe-b3dm-noglobe.mjs` (globe-off, baseline) + a new globe-on assertion.
