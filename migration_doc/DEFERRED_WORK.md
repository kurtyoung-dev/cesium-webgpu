...
## NEW-SANDCASTLE2-INDEX-WGSL-IMPORTMAP — Sandcastle2 broken: deep `@cesium/engine/index-wgsl.js` specifier has no browser-loadable import-map target (2026-06-26)
...
## NEW-SANDCASTLE2-C3-DEMOS-NOT-IN-NEW-GALLERY — the C3 WebGPU weather demos live only in the OLD gallery (2026-06-26)

The four C3 WebGPU demos (`WebGPU Weather Inspector`, `WebGPU Fullscreen Sky`, `WebGPU Live Weather (EDR)`, `WebGPU Weather Particles`) are authored as single-file `.html` in the OLD `Apps/Sandcastle/gallery/` (the documented convention) + registered in `gallery-index.js`. The NEW **Sandcastle2 gallery is `packages/sandcastle/gallery/<name>/`** with a DIFFERENT folder format (`index.html` + `main.js` (ESM) + `sandcastle.yaml` + `thumbnail.jpg`), and the dev server serves Sandcastle2 — so the C3 demos do NOT appear in the running gallery. **To make them available:** port each to the new folder format (extract the `//Sandcastle_Begin..End` body into `main.js`, write `sandcastle.yaml` with the title/labels, add a `thumbnail.jpg`), then `npm run build-sandcastle`. **Blocked on** NEW-SANDCASTLE2-INDEX-WGSL-IMPORTMAP (Sandcastle2 must load before a ported demo can be verified). **Trace:** 2026-06-26.
