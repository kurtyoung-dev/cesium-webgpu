...
**Sub-entry — NEW-LOG-DEPTH-REMAINING-PRODUCERS-POINTCLOUD-SPLAT (the two residual producers):** Convert `WebGPUPointCloudRenderer` (PNTS/EDL) + `WebGPUGaussianSplatRenderer` to the log-depth recipe. Point cloud: add `//>>ifdef LOG_DEPTH` to its color/EDL color VS+FS (frag_depth write; EDL also samples neighbor depth — the EDL depth comparison must reverse log first, like the classifier consumers); splats: add the frag_depth write to the splat FS (splats blend translucent with `depthWriteEnabled:false` in the common path, so a producer write only matters where they DO write depth — scope it to the depth-writing splat mode). Each needs a UB log-depth lane + `isWebGPULogDepthActive` gate + flip-rebuild guard + its own far-camera probe. Lower priority than the consumer fill-ins because these are specialized point/splat scenes, not the default globe scene.
