...
- **NEW-GROUND-ATMOSPHERE-DRAPE-LIMB-WIDTH** (S–M, surfaced Batch 327, `a2af8490d9`) — **Open / WIP — live home in the parity backlog** (the full detail lives under the SHIPPED `NEW-VR2-3b-LIMB-HALO-RESIDUAL` heading below; re-listed here so the re-attributed residual isn't buried under a closed entry). Batch 327 re-attributed the reported "WebGPU limb haze ring wider than WebGL" residual: the SkyAtmosphere SHELL is at parity (pixel-identical / marginally narrower in isolation), and the dominant wide blue limb BAND is the WGSL **ground-atmosphere drape** over the lit globe surface (`GlobeTerrain.wgsl` `groundAtmosphereControl`/fade), present with `scene.skyAtmosphere.show=false`. Its limb-width/falloff differs from WebGL's `GlobeFS.glsl` + `AtmosphereCommon.glsl` ground-atmo path — needs a WGSL ground-atmo drape parity pass to match falloff. **Evidence:** probe-limb-skirt re-attribution (Batch 327, `a2af8490d9`); diagnostics `Tools/visual-regression/probe-limb-halo-width.mjs` (+ `probe-limb-skirt`/`azavg`). Lower priority — cosmetic limb-width at full-disc framing; gated on visual review flagging it. See NEW-VR2-3b-LIMB-HALO-RESIDUAL below for the full investigation.
...
- **NEW-BUFFERPOLYLINE-2D-EXTRUSION** (S–M, found by PARITY-BUFFER-2DCV 2026-07-01) — In **SCENE2D only**, a reprojected `BufferPolylineCollection` renders blank while its points/polygons render correctly (Columbus View + 3D are fine). Root cause: `SceneTransforms.computeActualEllipsoidPosition` collapses every 2D vertex onto the map plane (projected **x == 0** for all), and `BufferPolylineMaterial.wgsl`'s screen-space segment-quad extrusion (which projects each vertex via the SEPARATE `modelViewRelativeToEye * projectionMatrix` path, then extrudes perpendicular in screen space using prev/next) does not carry the 2D camera-axis convention that the fused `mvpRelativeToEye` used by the point/polygon paths does — so the extruded quad degenerates. This is the exact Principle-9 shape ("would work if the WGSL planar shader matched Cesium's 2D camera-axis convention, but the convention isn't wired through the polyline's MV×P path"). Columbus View works because its `.zxy` mapping keeps a non-zero depth axis. **Fix candidates:** route the polyline VS through `mvpRelativeToEye` (like point/polygon) for clip position, OR feed a 2D-aware projection so the ortho x==0 plane doesn't collapse the screen-space miter. **Files:** `Shaders/WebGPU/Collections/BufferPolylineMaterial.wgsl`, `WebGPUBufferPolylineRenderer.ts`. **Verify:** extend `probe-buffer-2dcv-parity.mjs` to flip the SCENE2D `expect.magenta` to `true` once fixed.
...
- **NEW-GROUND-ATMOSPHERE-DRAPE-LIMB-WIDTH** (S–M, surfaced Batch 327 — re-attributed from NEW-VR2-3b-LIMB-HALO-RESIDUAL) — At a full-disc view the WebGPU ground-atmosphere drape over the lit globe surface paints a slightly WIDER blue limb band than WebGL (~25 px in the original VR2-3b horizontal scan). Batch 327 PROVED this is NOT the SkyAtmosphere shell (shell halo is pixel-identical / marginally narrower in isolation — see NEW-VR2-3b-LIMB-HALO-RESIDUAL) but the ground-atmosphere drape computed in `GlobeTerrain.wgsl`, which is present with `scene.skyAtmosphere.show=false`. **Fix path:** match the WGSL drape's `groundAtmosphereControl` / fade falloff to `GlobeFS.glsl` + `AtmosphereCommon.glsl`. **Probe:** `Tools/visual-regression/probe-limb-halo-width.mjs` (the wide-band component, isolated from the shell rim). Lower priority — cosmetic limb-width, sub-pixel-to-few-px, only at full-disc framing; gated on visual review flagging it.
