...
- **NEW-VECTOR3DTILE-CLASSIFY-CONTAINMENT** (investigated 2026-07-04, Q15 — depth-sample FS approaches proven insufficient, correct fix is stencil Z-fail). The `WebGPUVector3DTilePrimitiveRenderer` depth-sample classifier `fsMain` discards only `surfaceDepth == 0` (sky), so the classified footprint is the volume's PROJECTED screen extent, inflated `h/(h-1000)` at nadir height h (ROADMAP §4.2). Q15 attempted the FS volume-containment test the brief called for and **verified it is precision- and infrastructure-blocked** — the fix belongs in a stencil rearchitecture, not the FS:
  ...
  - **Correct fix — stencil Z-fail (WebGL parity):** mirror `Vector3DTilePrimitive.getStencilDepthRenderState`: a stencil-mark draw (cullMode "none", colorWrite off, depthCompare less-equal against the BOUND depth buffer, `stencilFront.depthFailOp = decrement-wrap`, `stencilBack.depthFailOp = increment-wrap`, mask `CESIUM_3D_TILE`/`CLASSIFICATION`) followed by a stencil-tested color draw (`compare not-equal 0`, depth test off, reset stencil to zero). This compares per-FRAGMENT volume depth vs per-FRAGMENT globe depth in hardware at full depth-buffer precision, sidestepping every FS-reconstruction wall above. WebGPU supports the single-draw stencil-mark via separate `stencilFront`/`stencilBack` `depthFailOp`. Prereq: confirm the bound depth attachment holds the globe surface during the classifier pass at BOTH msaa levels (the current less-equal color pipeline suggests it does at msaa=4; msaa=1 needs the depth-population check above). M-L effort. Acceptance: `probe-vector3dtile-vctr.mjs` NEAR frame flips to IoU ≥ 0.8 AND FAR stays ≥ 0.8, at the viewer-default msaaSamples. **Trace:** 2026-07-04.
