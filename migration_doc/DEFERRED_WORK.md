...
- **NEW-PERINSTANCE-DIFFUSE-PARITY** (M, surfaced Batch 306 per Principle 9) — a lit `PerInstanceColorAppearance` (`flat:false`) primitive renders its diffuse-shaded body at a noticeably DARKER luminance on WebGPU than WebGL under the same directional sun (measured ~92 vs ~154 on a 120 m extruded polygon's top/side faces at a 30° sun elevation). The gap is **independent of shadows** — reproduced with `v.shadows=false` + `shadowMap.enabled=false` on both backends (WebGPU body lum 92, WebGL 154). Likely a difference in the WebGPU appearance lighting term (the `czm_private_getLambertDiffuse`/half-Lambert equivalent, or the light-direction/normal it feeds) vs the GLSL `PerInstanceFlatColorAppearanceFS`/`PerInstanceColorAppearanceFS` path. **Why it matters:** it silently darkens lit appearance-primitive surfaces and it contaminated the CSM soft-shadow probe's penumbra metric (worked around there by using a `flat:true` caster). **Fix path:** diff the WGSL primitive-lit fragment against `Appearance`'s GLSL diffuse term; align the lambert/clamp constants + light vector. **Reproducer:** capture an extruded `PolygonGeometry` with `PerInstanceColorAppearance({flat:false})`, shadows OFF, top-down, sample the body luminance on both backends.
