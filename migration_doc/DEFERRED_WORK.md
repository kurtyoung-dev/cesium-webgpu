...
1. Add `packages/engine/index-wgsl.js` as a SECOND Unminified ESM build entry so `packages/engine/Build/Unminified/index-wgsl.js` is emitted (exports the WebGPU cluster + WGSL symbols). Find the engine `Build/Unminified/index.js` build call and add the entry.
