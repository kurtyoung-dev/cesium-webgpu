...
## 2026-07-03 — surfaced during NEW-MODEL-SCENE2D-SHADING (attempted; REVERTED after NO-GO audit — task remains OPEN)

- **NEW-MODEL-SCENE2D-SHADING attempt notes (task still OPEN in ROADMAP §4.3)** — the attempt disproved the queue's SCENE2D light-direction/IBL-orientation premise (fixed-clock API-toggle attribution: every SCENE2D lighting input is mode-consistent with WebGL; pure-IBL 2D diff 34.11 ≈ the 34.27 baseline, so the divergence is ENTIRELY the IBL ambient, a mode-INDEPENDENT defect that top-down SCENE2D merely maximizes). Candidate root causes identified in the WebGPU dynamic-env IBL pipeline: (1) `ProjectRadianceToSH.wgsl` samples the radiance cube RAW where WebGL's `ComputeIrradianceFS.glsl:94-95` applies a `(-x, -y, z)` lookup flip — a JS re-projection of the live WebGPU cube showed the FLIPPED lookup reproduces WebGL's measured SH c1 blue 0.151-vs-0.152 while raw gives 0.029; (2) `ProceduralSkyCubemap.wgsl`'s scattering march diverges from `ComputeRadianceMapFS` (111 km rayLength clamp on slant rays, degenerate NONE-mode light dir, uniform 16-step integrator vs WebGL's adaptive scheme, no skyAlpha composite, through-planet down rays, radii semantics). A fix along these lines improved `probe-model-scene-modes` (2D 22.19→17.28) but FAILED the adversarial parity audit and was fully reverted — re-derive the fix from these notes rather than trusting the reverted diff. **Effort:** ~1 session.
- **NEW-PROBES-RED-AT-HEAD-IBL-PAIR** (M, surfaced per Principle 9) — the two standing model-IBL parity probes FAIL at clean HEAD (batch 510, `8b72eb80b8`), verified by revert-and-rebuild baselining (same procedure as `NEW-PROBES-RED-AT-HEAD-482-486-488` above): (1) `probe-model-ibl.mjs` (Batch 354 record: 1.32%/1.31%) now measures **65.0% / 64.5%** mismatch at both camera angles at HEAD (the orbit world-anchoring sub-gate stays green, so the reference-frame rotation is intact and the failure is a large uniform shading/tint shift); (2) `probe-model-pbr-ibl-parity.mjs` (Batch 346 record: ~1%) now measures **lum −13.8% / R −27.0% / G −12.7% / B +8.8%** at HEAD. The at-rest signature is WebGPU strongly BLUER than WebGL with `skyAtmosphere.show=true` + sun on (candidates: the many env/cloud/atmosphere batches 355-510 that touched the sky-cube inputs, or WebGL-side atmosphere changes; neither probe pins `viewer.clock`, so sun-elevation drift may also inflate the numbers — add fixed clocks during triage). **Next step:** dedicated triage batch — pin the clocks, bisect the regression window with the revert-rebuild harness, and re-baseline or root-cause. Until then treat both as known-red for off-gating. **Effort:** ~1 session.
