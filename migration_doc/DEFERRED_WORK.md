...
- **NEW-ENTITY-BULK-FASTPATH-BILLBOARD-LABEL** (M) — Extend the Batch-300 static-lane fast-path from points to BILLBOARD and LABEL entities (the other two homogeneous-catalog graphic types). The shape is the same — detect a static `billboard`/`label` entity (all consumed graphics properties constant, no height-reference clamp, no availability, clustering off), write it once into a flat-buffer `BillboardCollection`/`LabelCollection` with `id = entity`, skip it per-frame. The wrinkle vs points: billboards carry an async image load (`BillboardTexture` ready-state) and labels build a glyph atlas, so "write once" must tolerate the texture/glyph becoming ready a few frames later (the static slot stays but its image binds on ready). A `BulkBillboardVisualizer`/`BulkLabelVisualizer` mirroring `BulkPointVisualizer` (wrap the legacy visualizer for the dynamic lane) is the planned structure. Not built in Batch 300 (scoped to the highest-value point case per the PARTIAL-OK clause).
