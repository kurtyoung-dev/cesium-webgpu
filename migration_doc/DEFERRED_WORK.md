...
**Remaining blocker — NEW-GROUNDPRIM-CLASSIFIER-FRUSTUM-DISTRIBUTION (see below):** Batch 173 RESOLVED the per-slice `invProj` UBO (NEW-GROUNDPRIM-CLASSIFIER-PER-FRUSTUM-UBO) AND corrected `windowToEye` (it had wrongly applied `reverseLogDepth` to linear NDC depth) — but textured materials still render flat because the classification command has NO bounding volume and is distributed to the FARTHEST frustum slice (near 1e8/far 1e10) instead of the near slice containing the surface (near 0.1/far 1e8). Reconstructing depth in the far slice gives eye-z ≈ 1e9 → UV clamps → flat. Probe still shows variance ~50 vs WebGL ~5578. Color path unaffected (it doesn't reconstruct eye-space).
...
**Remaining scope after Batch 173:** (a) ✅ per-slice `invProj` UBO + corrected `windowToEye` (Batch 173). (b) ⬜ Solve NEW-GROUNDPRIM-CLASSIFIER-FRUSTUM-DISTRIBUTION — give the classification command a (mode-aware) bounding volume so it draws in the slice matching its surface; THEN the per-slice `invProj` reconstructs correct UV. (c) ⬜ Add Image material support (texture upload + sampler binding — separate BGL extension following the GroundPolyline pattern). (d) ⬜ Then the 2D/CV variant (WebGL's `appearance2D`) — the Batch 170 `.zxy` swizzle already places the polygon correctly in 2D; the textured FS just needs (b) first.
...
### NEW-GROUNDPRIM-CLASSIFIER-FRUSTUM-DISTRIBUTION — classification command drawn in the wrong (far) frustum slice; textured-material UV reconstructs garbage
