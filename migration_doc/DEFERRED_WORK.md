...
- **NEW-WEBGPU-DEFAULT-LIMIT-GLOBE-LAYOUT** (M, found Batch 242 by the variant smoke under SwiftShader) — the globe terrain pipeline layout requires **31 fragment-stage sampled textures**, so on a default-limit adapter (`maxSampledTexturesPerShaderStage` = 16 — the WebGPU spec minimum; what SwiftShader/CI software adapters expose) `CreatePipelineLayout` fails (`uncaptured GPU error: The number of sampled textures (31) in the Fragment stage exceeds the maximum per-stage limit (16)`) and every globe-terrain pipeline create errors out — the globe never renders on such adapters. `WebGPUDevicePool`'s `ADAPTIVE_LIMIT_CAPS` opt-in (up to 64) can only raise limits the adapter actually OFFERS, so it's a no-op at exactly-spec-default adapters. The Batch 58 comment in `WebGPUGlobeSurfaceLayouts.ts` ("minimum-guaranteed … is 16 so this is safe") missed that the PER-STAGE limit counts the TOTAL across ALL bind groups (16 day-imagery + night/water/ocean-normal/cloud/etc. = 31). Fix options: a low-limit fallback layout variant (fewer imagery slots per draw + multi-pass composite), or degrade max imagery layers per tile when `device.limits.maxSampledTexturesPerShaderStage` is 16, plus a permanent console sentinel naming the limit. Until it lands: the dual + webgpu-only variant smokes FAIL under the harness's `--use-vulkan=swiftshader` flags (webgl-only is the variant gated green in CI; real-GPU adapters are unaffected — Sandcastle/CesiumViewer probes all pass on the local D3D adapter). Reproducer: `node Tools/variant-smoke-test.mjs --variant webgpu-only`.
