...
### BUG-ELLIPSOIDPRIM-WEBGPU-INVISIBLE — the WebGPU EllipsoidPrimitive renderer produces no visible output for a scene-placed ellipsoid (M, found Batch 266)

**What:** A scene-added `EllipsoidPrimitive` (e.g. `new EllipsoidPrimitive({ center, radii, material })`) renders ZERO lit pixels on WebGPU — verified Batch 266 across radii 30 km..200 km, altitudes 0..50 km, camera 400..500 km, with the center supplied via BOTH `center` (with identity `modelMatrix`) AND `modelMatrix = eastNorthUpToFixedFrame(worldPos)`. The renderer DOES run (its `_webgpuCache` is fully populated — pipeline, command, bind groups all present) and emits 0 console / 0 WebGPU validation errors; the draw simply produces no visible coverage. This is INDEPENDENT of log depth (the log-depth-OFF leg is byte-identical to the pre-Batch-266 shader and is equally invisible), so it predates and is orthogonal to NEW-ELLIPSOIDPRIM-LOG-DEPTH.

**Two stacked causes identified (not yet fixed):**
1. **`center` is dropped.** `Scene/EllipsoidPrimitive.js` computes `_computedModelMatrix = modelMatrix * translate(center)` and sets `colorCommand.modelMatrix = this._computedModelMatrix`, but `updateWebGPUEllipsoidPrimitive` (`WebGPUEllipsoidPrimitiveRenderer.ts` ~line 827) reads `primitive.modelMatrix` (the RAW matrix) and `packEllipsoidUniforms` reads `Matrix4.getTranslation(modelMatrix)` — so a `center`-placed ellipsoid lands at the Earth's center. Fix: read `_computedModelMatrix` (or recompute `modelMatrix * translate(center)`).
2. **Even placed via `modelMatrix` translation the ray-cast quad emits no visible coverage** at these views — suggests a deeper issue in the eye-direction reconstruction / FOV-aspect handling / depth interaction in the inline `ELLIPSOID_WGSL` full-screen-quad ray-march. Needs its own probe (capture the quad coverage before the ray-intersect discard) once cause 1 is fixed.

**Impact:** EllipsoidPrimitive is a low-traffic primitive (used for atmosphere/ellipsoid shells, custom bodies). The log-depth plumbing (Batch 266) is in place and verified structurally, so once visibility is fixed the shell will depth-compose with the globe with no further log-depth work.

**Verification when fixed:** extend `probe-ellipsoidprim-logdepth.mjs` from a structural probe to a pixel ON~=OFF probe (the scaffolding — placement, settle, snap — is already there; only the green-pixel assertions need re-enabling).
