...
- **Extra material fields (metalness/occlusion/normalEC) on the WGSL `czm_customModelMaterial` bridge (Q31 slice B).** The bridge struct still exposes only diffuse/specular/roughness/emissive/alpha. Adding metalness/occlusion/normalEC requires plumbing those through the writeback (and, for normalEC, re-deriving lighting from the perturbed normal). Deferred as slice B — now unblocked by the pre-lighting seam this batch landed.
