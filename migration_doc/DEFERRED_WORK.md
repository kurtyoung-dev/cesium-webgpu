...
- **NEW-WEBGPU-TIMEDYNAMIC-POINTCLOUD-CONTENT-LOAD-ZERO** (M, surfaced Batch 377 per Principle 9) — `TimeDynamicPointCloud` (the ONLY consumer of the standalone `PointCloud`/`WebGPUPointCloudRenderer` — PNTS 3D-Tiles route through the Model pipeline instead) **loads ZERO content on WebGPU**: `probe-pointcloud-logdepth.mjs` shows `boundingSphere` never becomes available + `totalMemoryUsageInBytes === 0` after 500 frames on `?renderer=webgpu`, while WebGL loads the first frame at frame 1 (`mem===33000`) with the identical setup (PointCloudTimeDynamic sample 0-4.pnts, clock at interval 0). So the 4D streaming point-cloud content/load path is non-functional on WebGPU — the standalone point-cloud color renderer is effectively never fed instance data. The globe still renders (scene intact, 0 device errors). **This gates the C2-7 log-depth producer from being visually verified** (no points render to sort). **Next step:** trace `TimeDynamicPointCloud._loadFrame` → frame request → `PointCloud` parse → instance upload on the WebGPU path; compare against the WebGL load that succeeds. Likely a content-pipeline/`pnts`-parse or instance-buffer-upload gap specific to WebGPU. **Probe:** `probe-pointcloud-logdepth.mjs` (asserts ready+mem). **Effort:** ~1 session.
