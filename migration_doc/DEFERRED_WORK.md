...
- **NEW-INDEXWGSL-CHURN** (S) — ✅ RESOLVED (Batch 303). Root cause confirmed empirically: `packages/engine/index-wgsl.js` is a TRACKED barrel that `scripts/build.js` regenerates each `gulp build` as a raw single-line-export string (deterministic — pure static concat, `os.EOL` line endings), but at some point the committed copy had been prettier-reformatted (multi-line exports, double quotes, leading-blank stripped). The two forms differed, so every build re-dirtied the file (perpetual status churn; bit Batch 228/229/231, each `git checkout --`'d it first). `.prettierignore` ALREADY excluded the file (so prettier no longer re-diverges it) — the residual was simply that the committed blob was the stale prettier form, not the build-emitted form. Fix (option 2 from the original entry): committed the byte-identical build-emitted raw form once. A subsequent `gulp build` now leaves `git status` clean for this file (verified). One-commit change, no build-script edit needed.
