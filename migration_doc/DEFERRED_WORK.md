...
> - **Bug 2 (OPEN — far-surface depth) — default surface markers z-fight / are occluded by the globe at far camera.** A height-0 billboard/point with NO `disableDepthTestDistance` renders at CLOSE camera but vanishes at far range (~1.5 Mm): `probe-collections-closeup.mjs` (~600 m) → billboard 289, point 548, label 251; `probe-collections-2dcv-morph.mjs` 3D (1.5 Mm) → billboard 0, point 231, label 125. The marker VS writes LINEAR depth (`clipPos = mvpRelativeToEye * positionRTE`) while the globe writes LOGARITHMIC depth, so at distance the marker lands marginally behind the globe and fails `less-equal`. Not OIT, not the BV, not `depthTestAgainstTerrain`. **Fix needed: apply the globe's log-depth transform to the marker clip-z** (see `WebGPULogDepth.ts` + the GlobeTerrain.wgsl LOG_DEPTH path). Shared near-surface limitation across all three collections; `disableDepthTestDistance` is the workaround.
> - **Bug 5 (OPEN — 2D / COLUMBUS_VIEW) — collections render NOTHING on WebGPU in SCENE2D and COLUMBUS_VIEW.** `probe-collections-2dcv-morph.mjs` (1.5 Mm): WebGL 2D = bb 131 / pt 284 / pl 1838 / lbl 497; WebGPU 2D = all 0 (CV likewise). The 3D fixes (BV, atlas, clip-z) do NOT help 2D/CV — this is a projection/camera issue: the WebGPU collection renderers encode the raw 3D `position` (or `_actualPosition`) and a single 3D MVP, but 2D/CV need the projected Columbus/2D coordinate + the orthographic 2D frustum (and, in morph, the `czm_morphTime` blend between position2D/position3D the WebGL vertex path does). This is the original **MORPH-COLLECTIONS-AUDIT** gap, now confirmed as a hard no-render (not just drift). **Next concrete step after Batch 219; tracked under MORPH-COLLECTIONS-AUDIT below.**
