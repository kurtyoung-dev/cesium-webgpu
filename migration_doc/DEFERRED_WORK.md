...
- **NEW-ORBITAL-GPU-PICKING** (M) — *(re-labeled Batch 231: GENERIC-SYSTEM UPGRADE — picking for GPU-resident `ComputeInstanceCollection` instances generally, same for TAA motion vectors and the missing command boundingVolume/per-frustum cull.)* Picking for 1M GPU-resident points (GPU pick pass / coarse BV prefilter) + Sandcastle + parity probe. No CPU position to hit-test for GPU-resident geometry; no upstream pattern. GPU pick pass or coarse CPU bounding-volume prefilter. Plus a Sandcastle demo + visual-regression probe (WebGL WASM vs WebGPU compute parity — diff sensitive to FP32 propagation drift). May defer to a follow-up.
