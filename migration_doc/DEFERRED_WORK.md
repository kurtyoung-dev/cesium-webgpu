...
- **RE-VERIFY FIRST (cheap, before any fix):** in `probe-classifier-textured-materials.mjs` dump `view.frustumCommandsList.length` + each `{near,far}`, and visualize `st` directly (output `vec4(fract(st),0,1)`), to measure the actual `st` range + confirm single vs multi frustum. THEN: if it tracks the log-depth partition, this is fixed by the same epic completion as C-R9 (write log depth for the globe + reverse it in the classifier `windowToEye`, the existing LOG path); the interim View.js `farToNearRatio` gate (C-R9 fix A) would make it multi-frustum and only THEN does the per-slice-invProj wiring matter. Also add a FREQUENCY assertion (band count / FFT, not just variance) so this can't silently regress.
...
**FIX (this is unfinished/deferred log-depth-epic functionality, NOT a regression — Principle 9):**

- **(A) Interim unblock — stop using the log-depth frustum partition while log-write is off.** Make the WebGPU path select `scene.farToNearRatio` (1000 → many tight frustums) instead of `scene.logarithmicDepthFarToNearRatio` until `_logDepthWriteEnabled` flips. Tight frustums give each ~73 m building its own depth resolution, breaking the dead-heat. The naive spot is `View.js:421-424`, BUT a raw `context.isWebGPU` branch there violates backend-agnosticism (Principle 2) — place the gate behind a context-reported "effective log-depth" capability (e.g. have the WebGPU context report `logarithmicDepthBuffer`-effective = false until the epic lands) rather than a backend check in Scene/View. Lowest-risk, no shader changes; will make rendering multi-frustum, which re-exposes the textured-classifier per-frustum-invProj path — see NEW-GROUNDPRIM-CLASSIFIER-RECON-PRECISION "ADDITIONAL FINDING".
- **(B) Real completion — finish the log-depth epic for models, then flip the switch.** In order (do NOT flip before producers are wired): (1) `ModelPBRComplete.wgsl` add the `//>>ifdef LOG_DEPTH` VS varying + FS `@builtin(frag_depth)` write via the `csm_*LogDepth` chunks (mirror `GlobeTerrain.wgsl` + the converted `PrimitivePhong*.wgsl`); (2) `WebGPUModelRenderer.packCameraUniforms` call `packCameraLogDepthLanes` (`WebGPULogDepth.ts:92`); (3) add `LOG_DEPTH` to the model pipeline define set when `isWebGPULogDepthActive()` (mirror `WebGPUGlobeSurfacePipelines.ts:87`); (4) flip `WebGPUContext.ts:399` `_logDepthWriteEnabled=true` LAST, only after globe + ALL model/primitive producers are wired.

**Confirming probe before any code (cheap):** in `verify-model-feature-pick.mjs`, after the nadir render, set `scene.globe.show=false`, render ~10 frames, and assert `scene.pick(center)` returns a defined `Cesium3DTileFeature` (it does, per `probe-b3dm-noglobe.mjs`). Globe-off pick succeeding while globe-on fails is the single assertion proving depth-occlusion over every emission/transform hypothesis.
