...
- **NEW-CLOUD-IMPOSTOR-FS-PARITY** (M, found Batch 253 during the meters-sizing verification) — the WebGPU cloud FS is a simplified 2D-noise impostor (radial smoothstep × noise tint) while WebGL's `CloudCollectionFS` raymarches an ellipsoid volume with `maximumSize`/`slice` semantics: at the same 2000x1300 m scale and 15 km nadir camera, WebGPU draws a 281x183 px noise blob vs WebGL's 59x52 px raymarched core (footprint SCALES correctly since Batch 253, but per-cloud appearance/extent still diverges ~4-5x linearly and `maximumSize`/`slice` are ignored). Fix = port the volumetric FS (czm_ellipsoid raymarch + 3D noise) or derive the billboard alpha mask from the same density integral. Prereq: none (camera-UB lanes + noise texture infra exist). Effort: 1-2 sessions. Impact: visual parity for CloudCollection scenes; `probe-collections-regression` cloud thresholds will need a re-baseline when it lands.
