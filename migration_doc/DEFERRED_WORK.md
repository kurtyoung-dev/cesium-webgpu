...
- **NEW-COLLECTION-RENDERER-BASE** (M) — ⏳ PARTIAL (Batch 302, Phase 11) — Shared base for the collection renderers; fold the resident-instance manager in. Already tracked (audits/2026-06-11_ULTRA_REVIEW_findings.json:231) — ~3000 LOC duplicated across the collection FRs. **SHIPPED this batch:** new `WebGPUCollectionRendererBase.ts` (188 non-comment LOC) extracts the six per-frame sub-flows the renderers re-implemented identically — (1) per-`GPUDevice` shader-module-cache WeakMap accessor (`makeDeviceShaderModuleCacheAccessor`), (2) scene-format-generation pipeline-cache invalidation (`invalidatePipelinesOnSceneFormatChange`), (3) settled-2D/CV coplanar-depth flag + 0x80000000 pipeline-key fold (`computeNoDepthTest` / `pipelineKeyWithDepthFlag`), (4) resident-instance-manager lazy create (`getOrCreateInstanceManager`), (5) the load-bearing capture→sync→consume ORDERING as one structural call (`syncInstancesAndConsume` — the ordering used to be an inline comment-warned hazard), (6) grow-on-demand pick instance buffer + write (`ensurePickInstanceBuffer` / `writePickInstances`). **Billboard + Point migrated** onto it with ZERO behavior change — all collection gates byte-green (collections-regression upload-gate 0/0/0; billboard-partial-write partial=1×176B, point 1×112B unchanged; far-camera / 2dcv-morph / point-label / polyline-cloud all PASS). Removed 131 non-comment lines of duplicated scaffolding from the two renderers; the base now serves 2 of 5 and is ready for the remaining 3. **REMAINING:** migrate Label + Cloud + Polyline onto the base (Label's full-rebuild-on-any-dirty + atlas-guid path and Polyline's segment model differ enough to warrant their own stage). NB: Billboard's async `update` was converted to a synchronous-build + `Promise.resolve()` return so the re-entry sentinel brackets the synchronous command-list build (bracketing across the no-op `await` deferred `endCollectionFrame` to a microtask and false-tripped the guard — caught by probe-collections-far-camera). Closes the P1-T6 fold for Billboard+Point (see NEW-COLLECTIONS-DIRTY-GATE close note). FEATURE_INVENTORY updated.
