...
- **NEW-CSM-GLOBE-RECEIVE-PROJECTION-MISS** (HIGH, surfaced Batch 296 per Principle 9; confirmed ACTIVE + sole closeout blocker for CSM soft shadows in the Batch-297 retest) — with the cast side fixed (NEW-CSM-CAST-NO-DISPATCH-VIEWER above), the WebGPU cast depth map is correctly populated (the caster *self*-shadows), but the **globe-terrain** CSM AND single-shadow-map receivers do NOT darken the ground where the caster's shadow falls. **Batch-297 retest evidence:** `probe-csm-soft-shadow.mjs` output PNGs show the WebGPU globe ground uniformly bright with NO cast shadow vs the WebGL reference's large dark ground shadow; the only WebGPU "shadow" present is the caster wall's own self-shadow (hence the non-zero 6.5% hard-vs-soft A/B diff is the PCF kernel softening the SELF-shadow edge, not a ground shadow). **This bug is now the single gating item between the (already-shipped + self-shadow-verified) PCF kernel and the end-to-end CSM soft-shadow closeout** — fixing it makes `probe-csm-soft-shadow.mjs` go green with NO probe changes. Confirmed NOT a camera-encoding bug (`encodedCameraPositionMC` high+low reconstructs to `cameraWC` exactly), NOT a cascade-footprint bug (cascade-0 sphereRadius ≈ 6226 m, splitFar ≈ 7502 m covers the whole top-down scene), NOT a receive-gate bug (a forced `shadowFactor=0.4` darkens the whole ground 204→82, so the final-color multiply works), and NOT a select-cascade bug (sampling ALL 4 cascades with a generous bias still reports "lit" everywhere). The caster's stored depth is simply absent at the ground's projected cascade UV → a cast-VP vs globe-receive-VP spatial misalignment that is SPECIFIC to the globe receiver (the appearance-primitive receiver, which uses `EncodedCartesian3.fromCartesian(inverseModel × cameraPositionWC)`, self-shadows correctly with the same cast depth). Suspects: (a) the globe's 2^16-split `encodedCameraPositionMCHigh/Low` + its bespoke RTE-assembly (`(center3DHigh - encodedCamHigh) + (center3DLow + pos - encodedCamLow)`) produces a sub-region offset under the cascade VP that everything-else (primitive, model) doesn't; (b) the cascade light-eye sign in `WebGPUCSMRenderer._computeCascadeVPMatrix` (`eye = center - lightDir*2r`, `lightDir = sunDirectionWC` = surface-toward-sun). **Next concrete step:** a cast-depth readback probe that, for a known ground point behind the caster, prints the globe's projected (uv, ndc.z) and the caster's stored depth at that uv — if they disagree by a fixed offset it's the encoding path; if the wall projects to a mirrored uv it's the light-eye sign. **Reproducers:** `probe-csm-soft-shadow.mjs` (FAIL — ground shadow is the missing signal) and `probe-csm-cast-dispatch.mjs` (cast side PASS).
