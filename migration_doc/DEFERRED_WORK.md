...
- **NEW-GROUND-VIEW-ENV-DIVERGENCES** (M, found Batch 240 while stabilizing probe-bloom-parity) — three cross-backend environment divergences observed at a deterministic ground-level view (camera 3-20 km, pinned clock), each strong enough to dominate a pixel-parity metric: (1) the WebGPU daytime sky-atmosphere at ground level renders dramatically brighter than WebGL at the same view/time (bright daytime blue vs dark navy — frame-filling; HSB value ~0.85 vs ~0.35); (2) the WebGPU sun primitive does not render at a ground-level 45°-elevation sun view where WebGL shows the glowing disk (WebGPU sun verified rendering in other probes — likely view/occlusion specific); (3) the no-imagery globe renders `globe.baseColor` (31,38,51) on WebGL but ~(10,10,15) on WebGPU (suspect linear-vs-sRGB handling), and the world-terrain water-mask wave material renders on WebGL only. Reproducers: probe-bloom-parity.mjs iterations v1/v2/v4 (see Batch 240 debugging-log entry + saved PNGs under Tools/visual-regression/output/bloom-parity/). Triage each into its own fix; (1) likely relates to existing SkyAtmosphere intensity/LUT entries.
