...
**Remaining blocker — NEW-WEBGPU-GLOBE-CLASSIFY-DEPTH-PRECISION (see below):** the blocker chain peeled back across three batches: Batch 173 landed the per-slice `invProj` UBO + corrected `windowToEye`; Batch 174 gave the command a bounding volume so it distributes to the correct frustum slice (NEW-GROUNDPRIM-CLASSIFIER-FRUSTUM-DISTRIBUTION, resolved). Textured materials STILL render flat because the deepest layer is depth PRECISION: the WebGPU globe writes hyperbolic NDC depth (not log depth like WebGL) into an RGBA8-packed texture, so at a 350 km surface in a [0.1, 1e8] frustum slice one quantization step ≈ ±50-90 km of reconstructed eye-z — far bigger than the polygon. The UV can't be reconstructed precisely until the WebGPU globe writes log depth (and the classifier reverses it). Color path unaffected (never reconstructs eye-space).
...
**Remaining scope after Batch 174:** (a) ✅ per-slice `invProj` UBO + corrected `windowToEye` (Batch 173). (b) ✅ mode-aware bounding volume → correct frustum-slice distribution (Batch 174, NEW-GROUNDPRIM-CLASSIFIER-FRUSTUM-DISTRIBUTION). (c) ⬜ **NEW-WEBGPU-GLOBE-CLASSIFY-DEPTH-PRECISION** — WebGPU globe must write log depth + classifier must reverse it, so eye-space reconstruction is precise enough for the UV (the current blocker; deep globe-depth-pipeline change). (d) ⬜ Image material support (texture upload + sampler binding — separate BGL extension following the GroundPolyline pattern). (e) ⬜ 2D/CV variant — the Batch 170 `.zxy` swizzle already places the polygon; needs (c) first.
...
**Scope (deep, multi-day):** Make the WebGPU globe write log depth like WebGL — emit `@builtin(frag_depth) = csm_writeLogDepth(...)` from `GlobeTerrain.wgsl`'s `fragmentMain` (the `csm_vertexLogDepth` / `csm_writeLogDepth` chunks already exist and are used by Collections shaders), gated on `frameState.useLogDepth`. Then the classifier `windowToEye` must REVERSE log depth (Batch 173 removed `reverseLogDepth` because the globe wasn't log-encoding — it must come back, correctly paired with the per-slice `far`). RISK: changing the globe depth buffer affects EVERY globe-depth consumer — terrain clamping, `pickPosition`, depth tests, translucent ordering, AO/SSR — each must be re-validated. This is a globe-depth-pipeline change, not a classifier-local one. Alternative: a separate high-precision (R32F / depth32float) globe-depth target for classifiers, avoiding the RGBA8 pack — also non-trivial.

**Why deferred:** Far beyond the textured-material slice it surfaced under; it's a globe-depth-precision architecture change with broad blast radius (every depth consumer). The flat-color classification path — the common case — ships and is unaffected. Textured-material classification (Stripe/Checkerboard/Grid/Image) waits on this.

**Trace:** Batch 174 (root-caused via the per-quantization-step eye-z error analysis); `GlobeTerrain.wgsl` `FragOutput`/`fragmentMain` (no frag_depth); WebGL `GlobeFS` + injected `czm_writeLogDepth` + `czm_windowToEyeCoordinates` `LOG_DEPTH` branch (the reference); `csm_writeLogDepth.wgsl` / `csm_reverseLogDepth.wgsl` (existing chunks); `WebGPUGlobeDepth.ts` (RGBA8 pack).
