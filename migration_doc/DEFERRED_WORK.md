...
- **Root cause = the per-slice projection NOT reaching the color draw (Link 4 unresolved, Batch 184):** the non-log `windowToEye` unprojects the sampled globe depth with `u.invProj` = `inverse(uniformState.projection)` captured ONCE at `createCommands` (scene-update) time. That projection's z-terms (`p22/p23`, near/far-dependent) don't match the per-slice projection the globe used to WRITE the sampled depth, so the recovered `eye.w` — and the `eye.xy` the UV derives from — is uniformly scaled. The per-slice `fstate` (group-2) `invProj` that WOULD fix this exists for the LOG path, but `resolveFrustumStateBindGroup` is **not invoked for the color draw** (`WebGPUGroundPrimitiveRenderer.js:2580` note), so the color draw binds the static slot-0 projection seeded from the stale lazy `uniformState.inverseProjection` cache — which is why Batch 171 fell back to computing `u.invProj` directly.
- **This contradicts the PER-FRUSTUM-UBO entry's "RESOLVED (Batch 173)" claim** that the resolver "binds the active slice's projection" — verified-by-logging in Batch 173, but Batch 184 found the color-draw resolver is not actually wired (Link 4). The frequency mismatch is the runtime proof.
- **Scope to fix:** (1) wire `resolveFrustumStateBindGroup` into the color-draw `bindGroupResolvers` (group 2) so the per-slice `invProj` reaches the FS; (2) supply a FRESH per-slice `inverseProjection` (the lazy cache is stale at the resolver call site — recompute `Matrix4.inverse(perSliceProjection)` like the group-0 path does); (3) switch the non-log `windowToEye` branch from `u.invProj` to `fstate.invProj`; (4) add a FREQUENCY assertion to `probe-classifier-textured-materials.mjs` (count bands / FFT, not just variance) so this can't silently regress again.
- **Why deferred from Batch 198:** out of scope for "add the Image material" (the Image feature is complete + as-faithful as its shipped Stripe/Checkerboard siblings, which share this exact `windowToEye`); the fix is a per-slice-binding architecture change (Link 4) with its own verification surface, best done as a focused follow-up.
...
**Next step (focused investigation):** trace the WebGPU b3dm draw path — why the populated `_webgpuCache.primitives["0_0"]` doesn't produce a visible draw command. Candidate angles: (a) the b3dm `ModelRuntimePrimitive` never builds `renderResources`, so any code gated on it is skipped; (b) the WebGPU model draw-command dispatch for tileset (`Pass.CESIUM_3D_TILE`) models doesn't reach the b3dm primitive; (c) the draw IS emitted but the model transform / shader yields off-screen or zero-coverage output. Compare against a glb-content Model (which renders) to isolate the b3dm-specific divergence.
