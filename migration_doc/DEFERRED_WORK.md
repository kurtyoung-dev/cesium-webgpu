...
- **NEW-MODEL-PBR-DIRECT-LIGHT-IBL-PARITY** (M) — **Open (surfaced by the Batch-329 DP-H35 morph-normal probe, 2026-06-20).** A lit glTF Model with a neutral-gray `baseColorFactor` and no textures renders **gray (R=G=B) on WebGPU but greenish-tinted and brighter on WebGL** at the SAME camera/light/rest-pose (AnimatedMorphCube: WebGPU rest 53.6/53.6/53.6 vs WebGL 53.3/60.3/55.8; morphed 133.4 gray vs 165.5 tinted). The divergence exists with no morph and no animation, so it is a model-PBR direct-light / IBL-ambient / material-color parity gap, not a morph or skinning issue. **Likely cause:** the WebGPU model FS direct-light + IBL/ambient term does not match WebGL's `czm_phong`/PBR ambient + IBL ground-color contribution (the green tint points at an IBL ground/sky ambient or a default-light color the WebGPU path defaults to neutral white). **Needs:** an audit of `ModelPBRComplete.wgsl`'s direct-light BRDF + IBL diffuse/ambient against the WebGL `LightingStageFS` / `ImageBasedLightingStageFS` path, and matching the ambient/IBL ground-color + light-color contribution. **Probe:** extend `probe-morph-normals.mjs`'s rest-pose cross-backend ΔA (currently ~9 — the tint+brightness gap) into a dedicated static-lit-model parity assertion. **Files (investigation start):** `packages/engine/Source/Shaders/WebGPU/Model/ModelPBRComplete.wgsl` (lighting section ~L2200+, IBL ~L1018+), vs `Scene/Model/...LightingStageFS`/`ImageBasedLightingStageFS`. **Trace:** Batch 329 DP-H35 finding above.
