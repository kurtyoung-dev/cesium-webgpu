...
- **NEW-ENTITYCLUSTER-GPU-MERGE** (L) — The Batch-301 GPU path offloads the BINNING half (the O(N log N) → O(N) win) but the representative-selection + neighbour MERGE still reads back the per-cell aggregates and runs on the CPU over the non-empty cells. For the very largest cluster sets (where even the reduced non-empty-cell merge is a CPU cost), move the merge ONTO the GPU: a parallel union-find / connected-components pass over the grid (each non-empty cell links to its densest in-range neighbour, then a pointer-jump compaction emits cluster ids), producing the representative set GPU-resident with no per-cell readback. The output (representative world centroids + member counts) would then drive the cluster label/billboard/point collections directly — the centroid accumulation can stay GPU-side too via an RTE high/low atomic-sum into a per-cluster buffer. Deferred: the current bin/count + CPU-merge split already meets the Phase-10 scale target in the probe; the fully-GPU merge is a perf refinement, not a correctness gap. The dispatcher's grid buffers (`cellCounts`/`cellRep`/`pointCellId`) are the scaffolding this builds on — they stay GPU-resident and are already the right inputs for the union-find.
