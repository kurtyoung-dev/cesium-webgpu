...
- **NEW-BILLBOARD-SIZE-PARITY** — WebGPU renders `BillboardCollection` glyphs at roughly **half the linear dimension (≈¼ the pixel area)** of WebGL, in **every** scene mode (reproduces in steady SCENE3D, SCENE2D, and COLUMBUS_VIEW — NOT morph-specific). Isolated 2026-06-13 while verifying Phase 3 Slice 3: a single 64px magenta billboard at 50 km, globe OFF, top-down 8 Mm camera, gives WebGL ≈4032 magenta px vs WebGPU ≈961-992 in all three modes (consistent ~0.24 area ratio). Initially mis-detected as "billboard vanishes in CV" because a small 16px source billboard scaled below the magenta detection floor on WebGPU. **Not yet root-caused.** Candidate areas: the billboard quad expansion in `BillboardCollection.wgsl` vertexMain (the screen-space size term `compressedAttr`/`imageRect` decode, the `czm_metersPerPixel`/viewport scale, or the eye-space dimension math) vs `BillboardCollectionVS.glsl`'s `computePositionWindowCoordinates` size path; or an atlas-rect / `pixelSize` normalization mismatch. **Reproducer:** `Tools/visual-regression/probe-bb-cv-diag.mjs` (steady 3D/2D/CV magenta counts). P2 — billboards are visible (right place, right color), just undersized; user-visible at parity-sensitive zoom. Distinct from MORPH-COLLECTIONS-AUDIT (a 2D/CV/morph positioning concern, now largely closed for point/label) and from the also-pre-existing **label under-coverage** seen in the same probes (lime ≈150 GPU vs ≈600 GL — likely the same glyph-quad size term).
