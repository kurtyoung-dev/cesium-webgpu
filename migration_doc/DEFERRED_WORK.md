...
- **Phase 6c — SHIPPED 2026-05-13 (Session 65 Batch 44).** Cloud-shadow extinction injected into the existing `VolumetricFog.wgsl::lightScattering` sun-term via a cheap single-sample approximation (1 fbm sample per froxel along the sun direction to the cloud-layer mid-altitude → `exp(-density × absorption × layerThickness)`). UBO grew 72 → 84 floats; CPU plumbing reads from `globe.{cloud*}` + `atmosphericConditions.{clouds, weather}` + `frameState.time`. Default-off path is bit-stable vs pre-Batch-44 (Sentinel-2 disk-bleed unchanged, Bathymetry 90.7 % colored matches WebGL bit-perfect, 0 GPU errors).
