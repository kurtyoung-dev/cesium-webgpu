...
- **NEW-PICK-RAY-ASYNC** — Ray picks (pickFromRay/sampleHeight/clampToHeight) use sync end() -> stale results on WebGPU. Add an async ray-pick path (endAsync + awaited getDepth) plus a oneTimeWarning, mirroring drillPickAsync. **Partial (Batch 254, FQ-5):** the honesty half landed — `getRayIntersection` (PickingRayHelpers.js) now emits a permanent `oneTimeWarning("WebGPU.rayPick.unsupported", …)` once when `!context.supportsSynchronousReadback`, so these three queries no longer fail silently (they still return undefined — the SAFE pre-fix behavior). `sampleHeightSupported`/`clampToHeightSupported` deliberately still report `true` (pickPosition works via Batch 252; down-gating the getter would turn the silent-undefined into a thrown DeveloperError and break the async *MostDetailed variants). **Still open (Phase 6):** the actual async ray-pick depth path (endAsync + awaited getDepth + LOG-depth reconstruction) so the sync sampleHeight/clampToHeight return real heights on WebGPU. Probe: `Tools/visual-regression/probe-sampleheight-webgpu.mjs`.
