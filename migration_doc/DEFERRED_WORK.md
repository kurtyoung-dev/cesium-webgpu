...
- **NEW-COLLECTIONS-ERROR-SENTINELS** — ✅ SHIPPED (Batch 302, Phase 11). The three mandated permanent sentinels (CLAUDE.md "Permanent sentinels to add for new subsystems") now live in `WebGPUCollectionRendererBase.ts` and are wired into the Billboard + Point hot paths: (1) **Re-entry / infinite-loop guard** — `beginCollectionFrame`/`endCollectionFrame` bracket each renderer's `update*`; a per-collection depth past a SANE LIMIT (16, generous vs any legitimate frustum-slice/pass concurrency) `console.error`s (throttled 3 s) — catches a runaway recursive command-list build (BUG-12 "clear loop" family applied to collections). (2) **Null-target guard** — `validateInstanceSyncResult` `console.error`s when the resident manager returns a null buffer for a non-zero visible count (a draw with no vertex buffer is a hard bug) and tells the caller to skip the draw. (3) **Size-validation / overflow guard** — `writePickInstances` `console.error`s + CLAMPS the pick `writeBuffer` to the allocated buffer capacity instead of issuing an out-of-range write that aborts the whole command encoder (BUG-15 family). All three are PERMANENT (no debug pragma) — they indicate broken output that must reach production consoles. Verified by a standalone unit test (`esbuild`-transpile the base + drive the three sentinels): 10/10 assertions — each fires under forced bad input (re-entry depth 17, null buffer with 5 visible, 5-instance payload into a 3-capacity buffer → clamp to 3) and stays silent on good input. The runtime gates confirm 0 console errors on every settled/normal scene (collections-regression, far-camera, billboard/point partial-write).
