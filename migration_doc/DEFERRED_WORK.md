...
- **Producers:** ~~Mat* primitive family (~40 Flat+Lit material shaders); Primitive Basic/PBR~~ — ✅ **SHIPPED (Batch 264 — NEW-PRIMITIVE-MAT-LOG-DEPTH).** All 19 `Mat*Lit`, 19 `Mat*Flat`, 2 `Mat*Basic`, and 2 PBR primitive shaders converted via the codegen-pass recipe (`Tools/apply-logdepth-{matlit,flatbasic,pbr}.mjs`). `Mat*Lit` + PBR are the clip-z/FragOutput-struct family (byte-identical to PrimitivePhongColor). `Mat*Flat` + Basic return a bare `@location(0) vec4` so the `LOG_DEPTH` branch swaps the fragmentMain signature to a `FragOut` struct (color + frag_depth) and wraps every fragment return — `//>>else` keeps the bare-vec4 path byte-identical. **Renderer plumbing:** the FLAT camera UB gained the same 16-byte `logDepth` tail (`FLAT_CAMERA_BYTES` 160→176, packed unconditionally by `writeRTEUniformsFlat`→`writeLogDepthTail` at floats 40-43; the lit/PBR path already had it from Batch 188); the `LOG_DEPTH` define now ORs into BOTH command paths — the Phong/Basic path (`isWebGPULogDepthActive`, dropped the `isLit` restriction) and the material path (`createMaterialPipelineAndCache` now takes `logDepthActive`, threads it to the preprocessor, and tracks `cache.logDepthEnabled` for flip-rebuild). **Drive-by fix:** 18 of 19 `Mat*Flat` shaders wrote/read `eyePosition` without declaring it in `VertexOutput` (a latent Batch-97 FEAT-GAP-09 wiring bug that would fail `createShaderModule` the moment a `flat:true` material primitive rendered) — declaration restored. Verified `probe-logdepth-zfight.mjs` (Mat-pipeline green slab @220 km composes with the log globe, ON/OFF green ratio 0.99, magenta log-reference markers render only with log depth ON), all standing gates green. **Still hyperbolic (this entry's remainder):** **Buffer* family** (BufferPoint/BufferPolyline/BufferPolygon — the WGSL already calls 1-arg `csm_vertexLogDepth`/no-op `csm_writeLogDepth` STUBS injected by `WebGPUBufferPrimitiveRenderer.ts:74-90`; conversion = swap stubs for the canonical 2-arg chunks + UB lanes + define keying); EllipsoidPrimitive (ray-march computed depth — special case); Vector3DTile* primitives; point clouds (EDL path) + Gaussian splats if/where they write scene depth.
