...
### ⚠️ CRITICAL — Billboard / Point / Label collections render NOTHING on WebGPU (Batch 2 audit, 2026-06-07) — DEEP-DIVED: 3-bug chain, 2 fixed (Batch 218), bug 2 (surface depth) remaining

> **Deep-dive resolution (2026-06-07, Batch 218):** the no-render is a CHAIN of three independent bugs in the billboard family. Two are FIXED + verified; the third (the common-case blocker) is root-caused but remains.
> - **Bug 1 (FIXED) — bounding volume never computed on WebGPU → frustum-culled.** `BillboardCollection.update` computed `_boundingVolume` only in the WebGL path (after the `fr` early-return), so the WebGPU draw command carried the default degenerate sphere (center 0,0,0, radius 0 = Earth's centre) and `View.createPotentiallyVisibleSet` culled it (`scene.isVisible` → OUTSIDE). Fix: `computeBoundingVolumeForFeatureRenderer` runs in the `if (fr)` block — reproduces the `_max*`/`_all*` aggregates the WebGL vertex writers compute, transforms `_baseVolume`→WC, and calls `updateBoundingVolume`. Verified: a 100 km-altitude billboard now renders (992 magenta px, was 0). (`packages/engine/Source/Scene/BillboardCollection.js`.)
> - **Bug 3 (FIXED) — `disableDepthTestDistance` rendered BEHIND, not on top.** The `//>>ifdef DISABLE_DEPTH_DISTANCE` block in `BillboardCollection.wgsl` set `clipPos.z = clipPos.w` (NDC z=1 = the FAR plane), which under `less-equal` fails against the closer globe. WebGPU clip-z is [0,1] (near→far), so "always on top" is the NEAR plane (0), not far. Fix: `clipPos.z = 0.0` (3 sites). Verified: a surface billboard with `disableDepthTestDistance = Infinity` now renders (992, was 0) — the standard way to render ground markers now works.
> - **Bug 2 (OPEN — common-case blocker) — default surface billboards z-fight the globe.** A height-0 billboard with NO `disableDepthTestDistance` is occluded by the globe on WebGPU (renders only with `globe.show = false` → 992). The billboard VS writes LINEAR depth (`clipPos = mvpRelativeToEye * positionRTE`, BillboardCollection.wgsl:266) while the globe writes LOGARITHMIC depth, so at the surface the billboard lands marginally behind the globe and fails `less-equal`. WebGL renders these (equal depth passes). Not OIT, not the BV, not `depthTestAgainstTerrain`. **Fix needed: apply the globe's log-depth transform to the billboard/point/label clip-z** (see `WebGPULogDepth.ts` + the GlobeTerrain.wgsl LOG_DEPTH path) so their depth matches the globe's at the surface. Substantial shader work; the common case (default ground pins) stays invisible until done.
> - **Points + Labels:** Labels render via an internal `BillboardCollection`, so bugs 1+3 propagate to them. `PointPrimitiveCollection` is a SEPARATE collection/renderer with the SAME FR-early-return-before-BV pattern — it needs the equivalent bug-1 BV fix (not yet applied) AND shares bug 2. **Next: replicate the BV fix in PointPrimitiveCollection.js; then bug 2 (log-depth) for all three.**
