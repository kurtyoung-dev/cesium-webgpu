...
- **NEW-WEBGPU-PICK-COLD-SYNC-STALENESS** (M, surfaced Batch 323 — needs design) — The FIRST synchronous `scene.pick()` against a freshly-rendered pick FBO returns `undefined` on WebGPU for ANY primitive type (point/billboard/model), because WebGPU readback is async and `WebGPUPickFramebuffer.end()` returns the PREVIOUS frame's pick pixels (one-frame-stale by design); subsequent picks warm up correctly. WebGL's synchronous GL readback resolves the cold pick immediately. This is a cross-backend SYNC-API parity gap affecting ALL WebGPU picking, NOT specific to any graphic type (confirmed in Batch 323 via a billboard control exhibiting the identical cold-null then warm). **Fix (deferred — needs a sync-API parity decision):** either render + read back the CURRENT frame's pick FBO synchronously inside `scene.pick` (a forced sync readback — briefly stalls the GPU, the analogue of WebGL's sync-pick cost), OR formally document `scene.pick` as warm-only on WebGPU and steer callers to `pickAsync`. Files: `WebGPUPickFramebuffer`, `WebGPUSceneRendererPickPass.ts`, the `scene.pick` sync path. Gate: `Tools/visual-regression/probe-point-pick-webgpu.mjs` (its WebGL-vs-WebGPU cold/warm repro section).
