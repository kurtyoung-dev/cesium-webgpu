...
- **NEW-PARTIAL-WRITE-WIRE-BPL** (M) — ✅ SHIPPED (billboard Batch 229; point + label Batch 232). All three renderers sync through the manager with the same load-bearing ordering (dirty list captured BEFORE `_consumeDirtyState()` resets the index).
  ...
  - NOTE: the velocity prev-mirror gate `frameState.scene?.taaEnabled` is DORMANT (see NEW-COLLECTIONS-TAA-GATE-DORMANT below) — the manager's `mirrorPrev` contract is probe-verified but has no live end-to-end consumer until that gate is fixed.
