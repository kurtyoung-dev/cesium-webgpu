...
- **NEW-COLLECTIONS-DIRTY-GATE** — Billboard/Label rebuild + full-upload entire instance buffer every frame. Port the Point renderer needsRebuild gate; keep resident instance array; writeBuffer only dirty sub-ranges. **ATTEMPTED + REVERTED (Batch 226) — subtler than the Point reference implies.** A `needsRebuild` gate (skip `buildInstanceData` + `writeBuffer` unless `_billboardsToUpdateIndex > 0 || lastLength !== length || definesChanged`) DID work for static billboards (verified: 1 build over 30 static frames, renders correctly) — the perf win is real. BUT the dirty-tracking re-arm is entangled with the WebGL vertex-build's reset SEQUENCE: `_updateBillboard` only re-pushes when `!billboard._dirty`, and the WebGL build clears `_dirty` (BillboardCollection.js:806/890) AND `_createVertexArray` (line 786) AND `_billboardsToUpdateIndex`, in order. Replicating only the `_dirty` reset → a *moving* billboard never re-triggered a rebuild (stale render). Adding the `_dirty` re-arm → static billboards rebuilt ~2×/frame (something — the `runSharedSceneLogic` readiness loop calling `_updateBillboard` for `textureDirty`, and/or `_createVertexArray` never being reset on WebGPU so `updateMode` re-runs `recomputeActualPositions` every frame — re-dirties them). **A correct gate must replicate the full WebGL reset sequence** (`_dirty` + `_createVertexArray` + `_billboardsToUpdateIndex`, and stop the readiness loop / `updateMode` from re-pushing settled billboards on WebGPU) and re-verify BOTH static-skips AND moving-updates. Reproducer: a probe toggling a `cache._buildCount` debug counter over static then dynamic frames. NOTE: the Point renderer (the "reference") likely has the SAME latent moving-primitive staleness — verify it too. Reverted to avoid shipping a correctness risk.
  ...
  - **✅ CLOSED FOR BILLBOARDS (Batch 229):** the resident-instance manager shipped and the billboard renderer is wired onto it — see NEW-RESIDENT-INSTANCE-BUFFER-MGR / NEW-PARTIAL-WRITE-WIRE-BPL below. Static collections upload 0 bytes/frame; a single moved billboard uploads exactly 1×176 B. Point + Label wiring remain (tracked in NEW-PARTIAL-WRITE-WIRE-BPL).
...
- **NEW-COLLECTIONS-TAA-GATE-DORMANT** (S, found Batch 229) — `frameState.scene` is UNDEFINED at runtime (FrameState never carries a scene back-pointer), so the Batch-143 velocity gates `frameState.scene?.taaEnabled === true` in `WebGPUBillboardRenderer` (:1014/:1174) and `WebGPULabelRenderer` (:837/:1054) are always false — billboard/label velocity commands have NEVER emitted, and the velocity prev-instance buffer was never allocated (pre-existing; Batch 229 preserved the gate unchanged when wiring the resident-instance manager, whose `mirrorPrev` prev-mirror contract is probe-verified and ready). `frameState.taaEnabled` (the model-renderer spelling at WebGPUModelRenderer:2663) is ALSO undefined in the same scenario — audit who is supposed to publish the TAA flag onto frameState and wire ONE canonical flag for all velocity producers.
...
- **NEW-FORK-MODERNIZATION-REGRESSIONS** (HIGH — fix forward, added 2026-06-11) — the ES6/TS modernization pass shipped behavioral regressions vs upstream: `Resource.contains`, `TimeIntervalCollection.contains`, and an `Animation.js` `childNodes` text-node bug. FIX these in our class/TS form (do NOT revert), with a unit test each that upstream would pass. Highest-value Axis-B item under the fix-forward lens — live defects, not cosmetic drift.
...
- **NEW-DIRTY-CONSUME-CLOUD** (S) — ✅ SHIPPED (Batch 228). `CloudCollection._consumeDirtyState()` (per-cloud `_dirty`, `_cloudsToUpdateIndex=0`, `_createVertexArray=false`, `_cloudsRemoved=false` — redundant-but-harmless, `removeClouds` runs in shared logic before the FR branch — zero `_propertiesChanged[]`), called every frame in `WebGPUCloudRenderer` after the rebuild block (NOT just on rebuild — the count-only gate ignores property dirties; when NEW-CLOUD-REBUILD-DIRTY-GATE is fixed, its gate must read dirty state BEFORE this consume). Verified: static `_updateCloud` 0/frame, queue drained.
...
- **NEW-CLOUD-REBUILD-DIRTY-GATE** (S) — Cloud: WebGPUCloudRenderer count-only rebuild gate misses per-cloud property edits. Separate pre-existing correctness bug (not consume parity, per Principle 9). Rebuild gate at WebGPUCloudRenderer.ts:631 keys only on cloudCount !== lastCloudCount, so per-cloud property edits never re-upload. Do not fold into the consume task; surface and fix independently.
- **NEW-COLLECTION-RETOUCH-PROBE** (S) — Probe asserting settled-scene per-frame re-touch count drops to 0. No harness exists for per-frame re-touch (billboard fix measured ~4/frame by inspection). Add a _buildCount / _updatePointPrimitive counter probe under Tools/visual-regression confirming settled scenes hit 0 re-touch, rather than asserting by inspection.
...
- **NEW-PARTIAL-WRITE-WIRE-BPL** (M) — ⏳ BILLBOARD HALF SHIPPED (Batch 229). `WebGPUBillboardRenderer` now syncs through the manager: `buildInstanceData` extracted into `packBillboardInstance(out, off, bb)` + `isBillboardVisible`; dirty list captured BEFORE `_consumeDirtyState()` (ordering is load-bearing — consume resets `_billboardsToUpdateIndex`); renderer-level `forceFullRebuild` = `_createVertexArray` (read pre-consume) | defines change | sceneMode change | atlas-guid change | MORPHING (morph recomputes every `_actualPosition` without dirtying). Pick path unchanged (full pick build on pick frames, reuses `packBillboardInstance` + pick-color overwrite). **Remaining: Point + Label.** Point subsumes needsRebuild gate (:959); Label needs an index counter / Set-based dirty-list adapter (_labelsToUpdate has none) and must still force full rebuild on SDF atlas textureDirty (Step-0 for Point is done — NEW-DIRTY-CONSUME-POINT, 69c5eac9d7). NOTE for the Point/Label wiring: the velocity prev-mirror gate `frameState.scene?.taaEnabled` is DORMANT (see NEW-COLLECTIONS-TAA-GATE-DORMANT below) — the manager's `mirrorPrev` contract is probe-verified but has no live end-to-end consumer until that gate is fixed.
...
### Phase 3 — GPU-compute orbital propagator + storage-buffer draw (dense-derivable; WebGPU-first headline)
...
- **NEW-ORBITAL-J2-KERNEL** (M) — Simplified secular-J2 mean-element propagator in WGSL (validate pipeline before SGP4). Read element[i]+time -> ECEF -> RTE-encode -> write position[i]. Mirror WeatherParticles.wgsl struct+dispatch. Far simpler than full SGP4/SDP4; validates end-to-end pipeline. Needs a stated FP32 tolerance + CPU FP64 reference to diff against.
- **NEW-ORBITAL-GPU-RESIDENT-RENDERER** (M) — WebGPUOrbitalCatalogRenderer on the weather template (compute -> storage -> instanced draw). Feasibility answered YES: WeatherParticleRender.wgsl:49/81 already vertex-pulls position[instance_index] from a Stage.VERTEX read-only storage buffer (WebGPUWeatherRenderer.ts:419). Upload catalog once, dispatch per frame via WebGPUComputeEngine.executeOnEncoder (:265), reuse PointPrimitiveColor quad/fragment. Respect endCurrentRenderPass-before-beginComputePass on the shared encoder (_executeGBufferProducer pattern :2009-2076).
- **NEW-ORBITAL-CATALOG-COLLECTION** (M) — Scene-level OrbitalCatalogCollection with dirty-state + _consumeDirtyState discipline + FR registration. Owns element catalog, dirty-state for epoch/element edits, _consumeDirtyState equivalent (per the billboard fix — else every settled element re-dirties each frame), FeatureRendererKey registration, per-frame compute scheduling. Could be a PointPrimitiveCollection GPU-resident mode flag instead of a new class.
- **NEW-ORBITAL-SGP4-KERNEL** (L) — Full near-earth SGP4 in WGSL + FP64 mean-element pre-conditioning + reference-vector validation (defer SDP4). Greenfield (zero existing orbital code). Dominant Phase-3 effort. FP32 WGSL drifts vs FP64 reference SGP4 (deep-space SDP4 resonance terms especially) — needs CPU FP64 pre-conditioning + a documented position-accuracy budget; validate vs python-sgp4 / Vallado vectors. Cap v1 at near-earth SGP4, defer SDP4.
- **NEW-ORBITAL-SGP4-WASM-WORKER** (L) — WebGL2 fallback: SGP4-on-worker WASM bridge writing the same SoA RTE position buffer. WebGL2 has no compute. New Rust crate alongside packages/wasm / wasm-naga; writes the same SoA RTE position buffer the WebGL instanced/BufferPoint draw reads (match WebGPUBufferPointRenderer.ts:108-119 so one quad/fragment shader serves both). destroy/free_buffer/version/SIMD/JS-fallback per WASM strategy; threshold-gated; double-buffered with 1-frame staleness tolerance like the cull/sort readback rings.
- **NEW-ORBITAL-GPU-PICKING** (M) — Picking for 1M GPU-resident points (GPU pick pass / coarse BV prefilter) + Sandcastle + parity probe. No CPU position to hit-test for GPU-resident geometry; no upstream pattern. GPU pick pass or coarse CPU bounding-volume prefilter. Plus a Sandcastle demo + visual-regression probe (WebGL WASM vs WebGPU compute parity — diff sensitive to FP32 propagation drift). May defer to a follow-up.
