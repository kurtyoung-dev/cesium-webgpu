...
- **NEW-WASM-BRIDGE-BUNDLE-LOAD** (M) — *Found while shipping Batch 272.* In the BUNDLED build (`Build/CesiumUnminified/index.js` and the IIFE), the dynamic `import(".../ThirdParty/Workers/cesium_wasm.js")` in EVERY `Wasm*Bridge` 404s, so the WASM kernel never loads and every bridge silently uses its (byte-identical) JS fallback. Two distinct failure shapes depending on the specifier: (a) the 5 bridges using `"../../ThirdParty/Workers/cesium_wasm.js"` (RTE, Heightmap, Matrix, PointCloud, QuantizedMesh) keep it EXTERNAL via `webpackIgnore`, and `../../` resolves to `/Build/ThirdParty/...` (wrong dir — glue 404); (b) the 2 bridges using bare `import("../ThirdParty/...")` (Cull, Sort) get INLINED by esbuild, and the inlined glue's `new URL("cesium_wasm_bg.wasm", import.meta.url)` then resolves to the bundle root `/Build/CesiumUnminified/cesium_wasm_bg.wasm` (wrong dir — binary 404). The glue + binary actually live at `/Build/CesiumUnminified/ThirdParty/Workers/`. Fix is bundle-wide: emit an external specifier that resolves to `./ThirdParty/Workers/cesium_wasm.js` relative to the EMITTED bundle (so the glue's sibling-`.wasm` URL is also correct), OR add a build step / `buildModuleUrl`-style resolver. Must keep the standalone node check (`Tools/wasm-subrange-encode-check.mjs`, which drives the real kernel via a loader hook) green. Do this BEFORE NEW-BUFFERCOLL-ENCODE-BENCHMARK — no perf number means anything while the kernel is dark. Rendering is unaffected today (JS fallback is byte-exact), so this is a perf-enablement + correctness-of-intent fix, not a visual bug.
