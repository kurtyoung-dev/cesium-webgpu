...
- **NEW-POLYLINE-APPEARANCE-PRIMITIVE-WEBGPU** (L) — **Open (surfaced by the Batch-328 DP-H7 investigation, 2026-06-20).** A `Primitive` built over `PolylineGeometry` with `PolylineColorAppearance` or `PolylineMaterialAppearance` renders nothing on WebGPU (any `arcType`, including `NONE`). The `PolylineCollection`/`Polyline` runtime path works (renders curves identically to WebGL — see DP-H7), but the appearance-based primitive path is unimplemented: `WebGPUPrimitiveCommands.js` only handles `LINES`/`LINE_STRIP`/`LINE_LOOP` (geometry-outline line-lists) and has no path for the polyline appearances, which expand a TRIANGLES strip in the VS using `PolylineCommon`'s screen-space width math over the `prevPosition`/`nextPosition`/`expandAndWidth` vertex attributes. **Needs:** a WGSL port of `PolylineCommon` + `PolylineColorAppearanceVS` / `PolylineMaterialAppearanceVS` and the matching attribute/uniform wiring in `WebGPUPrimitiveCommands.js` (RTE high/low encoding of the subdivided positions; the polyline geometry already supplies subdivided `arcType` positions CPU-side). **Probe:** `Tools/visual-regression/probe-polyline-geodesic.mjs` records the gap (PolylineCollection path PASSES; the appearance-primitive case is the documented residual). **Files:** new `Shaders/WebGPU/.../Polyline*.wgsl`, `WebGPUPrimitiveCommands.js`, `Scene/PolylineColorAppearance.js`/`PolylineMaterialAppearance.js` (VS plumbing). **Trace:** Batch 328 DP-H7 finding above.
