...
- **NEW-BUFFERPOLYGON-2DCV-REPROJECT** (L, found by WEBGPU_PARITY_AUDIT_2026-06) — **NOTE: distinct from NEW-CLASSIFIER-2D-CV-MORPH (FEATURE_INVENTORY §B.3 :517–518), which covers the `.vctr` `Vector3DTile*` classifiers, NOT the `BufferPolygon` family.** The three Buffer* WebGPU renderers (`WebGPUBufferPolygonRenderer.ts`, `WebGPUBufferPolylineRenderer.ts`, `WebGPUBufferPointRenderer.ts`) encode RTE world-space ECEF positions and project via `modelViewRelativeToEye * projection` with NO 2D/CV reprojected attribute buffer (unlike the Vector3DTile classifiers' CPU-reprojected ENU buffer). Batch 180 verified SCENE3D only; in SCENE2D / COLUMBUS_VIEW the buffer collections (and the GeoJsonPrimitive that rides on them) likely render at wandering points. Fix mirrors the classifier pattern: a CPU-reprojected position attribute (or `SceneTransforms.computeActualEllipsoidPosition` per vertex with CPU morph lerp) + a scene-mode pipeline variant, kept in **lockstep** (CPU pack ↔ GPU `arrayStride`/`format` ↔ WGSL struct). **Files:** `WebGPUBufferPolygonRenderer.ts`, `WebGPUBufferPolylineRenderer.ts`, `WebGPUBufferPointRenderer.ts`, `Collections/BufferPolygonMaterial.js`. **Trace:** WEBGPU_PARITY_AUDIT_2026-06.md (P2 "Vector-tile buffer collections in 2D / Columbus View").
