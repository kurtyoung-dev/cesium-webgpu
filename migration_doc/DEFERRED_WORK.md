...
- **PARITY-METADATA-TABLE-INSTANCE-SOURCE** (M, surfaced per Principle 9) — property tables keyed by INSTANCE-sourced feature IDs (`node.instances.featureIds`, EXT_mesh_gpu_instancing + EXT_instance_features) remain unsupported on WebGPU. METADATA-TABLE-SOURCES closed the TEXTURE + IMPLICIT sources; the instance source is blocked on a missing data path: `WebGPUModelInstancing` carries NO per-instance feature-ID attribute to the shader (grep `featureId` in `WebGPUModelInstancing.js` → zero hits), so there is no WGSL variable the generated `initializeMetadata` could key the table column with. WebGL's `MetadataPipelineStage.mapPropertyTablesToFeatureIdSets` handles instance sets via the instancing attribute pipeline (`featureIds.positionalLabel` → `FeatureIdPipelineStage`'s instanced vertex attribute). **Fix shape:** (1) upload the instance `_FEATURE_ID_n` attribute into the instancing buffer (or a parallel per-instance buffer, step-mode `instance`), (2) thread it through the VS → flat varying (reuse `featureId0`'s slot semantics for the selected instance set), (3) extend `findPropertyTableForPrimitive` (WebGPUModelMetadata.js) to accept instance sets once the varying exists — the classification scaffolding + gating comments are already in place there. Also feeds the batch-styling/pick path for instanced tilesets (i3dm-style), which shares the same missing varying. **Effort:** ~1 session. Test asset: extend `Specs/Data/Models/glTF-2.0/PropertyTableFeatureIdSources` with an EXT_mesh_gpu_instancing variant; probe pattern in `probe-metadata-table-texture.mjs`.
