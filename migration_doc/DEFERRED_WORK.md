...
- **NEW-WEBGPU-PP-STAGE-RESTORE-LEAK** (M, surfaced per Principle 9 during Q7 triage) — adding then removing a `PostProcessStage` via the user-facing `scene.postProcessStages.add()/remove()` collection leaves a DURABLE, deterministic residual on the WebGPU composite: after add+remove of the 6 library builtins, the frame does NOT return byte-identical to the never-touched default (diagnostic on the fully-pinned+damped+settled `probe-pp-library-builtins` scene: default-vs-default2 = **0 bytes** deterministic floor, default-vs-restored = **1,429,998 bytes / max 51 / mean 18.17 DURABLE**, restored-vs-restored2 = **0 bytes** — i.e. the leak is a stable second fixed-point, not noise). This is what keeps `probe-pp-library-builtins` off-gate (4) red at HEAD. The scene-flag path (`scene.colorGradingEnabled = true/false`, `probe-colorgrading-wired` (D)) does NOT leak — it returns byte-identical — so the defect is specific to the `postProcessStages` collection reconfiguration (likely the WebGPU composite staying on the multi-stage ping-pong path with an extra sRGB round-trip after the collection empties, instead of reverting to the direct IdentityBlit). **Next step:** in the WebGPU post-process pipeline, when `postProcessStages` returns to empty, fully revert the composite to the default direct-blit path (or prove the extra copy is byte-neutral). Out of scope for Q7 (engine change in the PP pipeline, not a probe/determinism fix). **Effort:** ~0.5–1 session.
