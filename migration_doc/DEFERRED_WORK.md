...
- **NEW-WEBGPU-POINT-COLLECTION-PICK** (M) — **Re-confirmed Batch 304 as NOT a small map-back fix (it is a pick-readback investigation, deferred per the Phase-12 PARTIAL-OK clause).** Phase 12 considered whether this was the "entity-API returns primitive not Entity" map-back the campaign hypothesized; the Batch-300 debugging-log evidence shows it is actually a "producer pushed, consumer doesn't resolve" pick-pass readback that returns `undefined` for the instanced point quads on WebGPU — it affects ALL WebGPU point picking (raw collection AND entity), not an Entity→primitive remap. The fix is a dedicated WebGPU pick-pass investigation (does the point pick command reach the pick framebuffer; does pick depth/coordinate handling match the working billboard pick path; does the pick-color readback sample the right pixel), not a one-line `id` remap — left tracked, not attempted in the bug-bash. `scene.pick` over a `PointPrimitiveCollection` returns NOTHING on WebGPU (WebGL returns the picked `PointPrimitive` with its `id`). Surfaced + isolated during the Batch-300 entity-bulk work: a raw `PointPrimitiveCollection` added directly to `scene.primitives` (one 50px point) picks fine on WebGL (`id` round-trips) but `scene.pick` over it returns `undefined` on WebGPU — so this is a PRE-EXISTING WebGPU point-pick gap, independent of the bulk fast-path (it affects ALL WebGPU point picking, raw or via entities). The pick INFRASTRUCTURE exists in `WebGPUPointPrimitiveRenderer.js` — `buildPickInstanceData` builds per-instance pick colors, a pick pipeline is built, and a `pickOnly` command IS pushed under `frameState.passes.pick` (line ~1393) — so the producer half is wired but the pick readback returns null for the instanced point quads (the "producer pushed, consumer doesn't resolve" shape). Needs a dedicated WebGPU pick-pass investigation: confirm the point pick command reaches the pick framebuffer, the pick depth/coordinate handling matches the billboard pick path (which DOES work — billboard entities pick on WebGPU), and the pick color readback samples the right pixel. The Batch-300 probe gates pick-returns-Entity on WebGL only and records this gap rather than failing the on-ramp. (Billboard entity pick on WebGPU works, so the Entity-API pick contract is proven on at least one graphic type per backend.)
...
- **NEW-WEBGPU-POINT-COLLECTION-PICK** — re-triaged Batch 304 as a pick-readback investigation (NOT a small map-back), see the expanded entry above; left tracked.
