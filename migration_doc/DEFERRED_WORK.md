...
- **NEW-DIRTY-CONSUME-POLYLINE** (S) — Polyline: bucket-aware PolylineCollection._consumeDirtyState() + call from WebGPUPolylineRenderer:1182. Differs structurally (per-polyline _propertiesChanged + _polylineBuckets at :177). Read WebGL clear at :867-892 and per-bucket path :405-519 before writing. Clear _createVertexArray, _polylinesToUpdate.length=0, collection _propertiesChanged, AND per-polyline _dirty + per-polyline _propertiesChanged. Medium risk. Mostly queue/leak hygiene.
- **NEW-DIRTY-CONSUME-CLOUD** (S) — Cloud: CloudCollection._consumeDirtyState() incl. _cloudsRemoved reset + call from WebGPUCloudRenderer:635. Mirror WebGL clears at CloudCollection.js:855/912/923/938/518: clear per-cloud _dirty, _cloudsToUpdateIndex=0, _createVertexArray=false, _cloudsRemoved=false, zero _propertiesChanged[]. Call after buildInstanceBuffer.
...
- **NEW-RESIDENT-INSTANCE-BUFFER-MGR** (L) — WebGPUResidentInstanceBuffer manager: resident CPU array + stable _index->slot map + full-vs-partial predicate + per-slot writeBuffer (closes NEW-COLLECTIONS-DIRTY-GATE). Owner design verdict (DEFERRED_WORK.md:45). Per-slot primitive exists (WebGPUBuffer.write at :402). Crux: GPU slot = compacted visibleCount NOT bb._index; full rebuild on add/remove/show-toggle/clusterShow/define/length change, partial writeBuffer(buf, slot*stride,...) only on unchanged-slot property edit. API: packInstance(out,off,item) callback + sync(dirtyList,length)->{buffer,visibleCount,dirtyRanges}. The only L task in Phase 1.
- **NEW-PARTIAL-WRITE-COALESCING** (M) — Dirty-range coalescing + changed-fraction whole-buffer fallback. Sort dirty slots, merge adjacent (gap*stride < threshold) into contiguous writeBuffer ranges so M scattered changes become K calls; threshold-gate fallback to one whole-buffer write when changed-fraction exceeds ~30-50%. Mis-tuned threshold can make partial slower than full for clustered changes.
- **NEW-PARTIAL-WRITE-WIRE-BPL** (M) — Wire Billboard/Point/Label renderers onto the resident-instance manager. Replace buildInstanceData()+full writeBuffer (WebGPUBillboardRenderer:1251-1340) with manager.sync(); extract packInstance; keep velocity prev-mirror slot-aligned (else TAA motion vectors corrupt). Point subsumes needsRebuild gate (:959); Label needs an index counter / Set-based dirty-list adapter (_labelsToUpdate has none) and must still force full rebuild on SDF atlas textureDirty. Prereq: Step-0 re-dirty parity for Point+Label.
