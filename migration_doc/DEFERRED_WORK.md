...
### NEW-LOG-DEPTH-REMAINING-PRODUCERS — log depth is LIVE (Batch 251, default TRUE); these writers/consumers still run hyperbolic and need the same gated conversion

**Context:** The renderer-wide log-depth epic shipped Batches 249/250/251 (see NEW-WEBGPU-GLOBE-CLASSIFY-DEPTH-PRECISION + WEBGPU_DEBUGGING_LOG Batches 249-251). Producers now in log space: globe terrain, depth plane, lit Phong primitives, Billboard/Label/Point/Polyline(+materials)/Cloud, ComputeInstance, Model PBR (glTF + 3D Tiles + instanced). The writers below still emit hyperbolic NDC z into the shared scene depth — geometry from these families depth-sorts incorrectly against log-space geometry at FAR range (close range mostly survives because log z < hyperbolic z near 1.0 ⇒ hyperbolic writers lose ties they should win — i.e. they get occluded too eagerly, the same direction as the old bug):
...
**Trace:** Batches 249/250/251 (the flip + evidence); Batch 188 (deferred-producers list this entry supersedes); `WebGPUContext.ts:_logDepthWriteEnabled` (kill switch); `WebGPULogDepth.ts` (gate + lane helper).
