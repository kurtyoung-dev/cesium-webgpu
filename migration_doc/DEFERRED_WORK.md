...
- **Producers:** Mat* primitive family (~40 Flat+Lit material shaders — codegen/template pass recommended; the lit camera UB already carries the Batch-188 `logDepth` tail and `WebGPUPrimitiveCommands.writeLogDepthTail` packs it for them); Primitive Basic/PBR; **Buffer* family** (BufferPoint/BufferPolyline/BufferPolygon — the WGSL already calls 1-arg `csm_vertexLogDepth`/no-op `csm_writeLogDepth` STUBS injected by `WebGPUBufferPrimitiveRenderer.ts:74-90`; conversion = swap stubs for the canonical 2-arg chunks + UB lanes + define keying); EllipsoidPrimitive (ray-march computed depth — special case); Vector3DTile* primitives; point clouds (EDL path) + Gaussian splats if/where they write scene depth.
