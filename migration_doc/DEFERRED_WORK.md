...
- **`CustomShaderMode` (MODIFY_MATERIAL vs REPLACE_MATERIAL) + extra material fields (metalness/occlusion/normalEC).** B473 injects the fragment hook AFTER lighting/tonemap (final display-space color), so it cannot distinguish MODIFY (modify material → THEN light) from REPLACE (skip material stage, user diffuse → output unlit) the way WebGL's pre-lighting `customShaderStage` does, and metalness/occlusion/normalEC have no effect post-lighting. True parity needs the injection point moved to a PRE-lighting seam in the 3500-line hot model shader — a large, risky restructure that jeopardizes the byte-identical-off guarantee. Deferred pending a dedicated pre-lighting-injection batch.
