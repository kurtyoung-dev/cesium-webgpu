...
- **NEW-CSM-CAST-NO-DISPATCH-VIEWER** (HIGH, surfaced Batch 289 per Principle 9) — the WebGPU CSM cast pass renders nothing in CesiumViewer scenes: `csmRenderer._castDispatches` stays 0 across a long warm run with `scene.useCascadedShadowMaps = true`, `viewer.shadows = true`, an in-view `shadowMap` (`outOfView=false`, `enabled=true`, 4 passes), and shadow-casting primitives/models added. `renderCSMCastPass` early-returns only when `castCommands.length === 0`, so the Scene-side shadow-cast command list (`shadowState.shadowMaps[0].passes[].commandList`, populated by `ShadowMap.createCastDerivedCommand` in `Scene.updateDerivedCommands`) is empty when the CSM cast pass runs — even though the SAME scene casts correctly on WebGL. **Both** the CSM path AND the single (non-CSM) WebGPU shadow map showed 0% shadow in the NE cast region for an appearance `Primitive`, so this is a broader WebGPU globe-terrain shadow cast/receive plumbing gap in viewer scenes, not CSM-specific. This blocks visual verification of NEW-CSM-SOFT-SHADOW-PCF (above). Next concrete step: trace whether `command.castShadows` is set on the WebGPU appearance-primitive/model draw commands and whether `createCastDerivedCommand` produces a WebGPU-dispatchable cast command (vs a WebGL `DrawCommand` the WebGPU cast loop skips), in a deterministic top-down probe with a uniform bright base-color receiver. **Estimated effort:** 1 session. Reproducer: `Tools/visual-regression/probe-csm-soft-shadow.mjs` (the A-vs-B masked diff is 0.000% and `_castDispatches` reads 0 in its report JSON).
