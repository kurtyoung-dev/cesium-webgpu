...
- **NEW-ATMOSPHERE-LUT-SUN-RELATIVE** (M, surfaced Batch 247 per Principle 9) — the WebGPU SkyAtmosphere inscatter-LUT fast path is currently disabled for the SKY shader (`ENABLE_SKY_INSCATTER_LUT=false`) because `AtmosphereLUT.wgsl::computeInscatter` bakes the WORLD-space sun direction into a synthetic Y-up sample frame and its 2D `(cosViewZenith, altitude)` parameterization has no view-sun azimuth axis — at ground level it produced a frame-filling over-bright sky regardless of where the sun actually is. To re-enable the fast path, re-parameterize the bake to be sun-relative (e.g. add a sun-azimuth/sun-zenith axis, or bake in a sun-aligned frame and rotate at sample time) so a single texture fetch reproduces the inline `computeScattering` ray-march within tolerance. ALL scaffolding stays in place (bake dispatch, bind group, WGSL `useLut` branch, dual-light moon LUT). The bake itself still runs every frame — the globe/voxel/splat fog drape (`sampleAtmosphereFogLut`) consumes the same inscatter texture and is tuned against it, so unifying the two consumers is part of this work. **Estimated effort:** 1-2 sessions. **Impact:** restores the per-fragment-cost-saving LUT path for the sky shell (currently 16/4-step ray march per limb fragment); correctness already met by the ray-march fallback.
