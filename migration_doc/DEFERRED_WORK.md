...
- **NEW-BUFFERPRIMITIVE-COLOR-ALPHA** (M) — RESOLVED-by `batch-bufferprimitive-parity` (surfaced by WEBGPU_PARITY_AUDIT_2026-06, upstream PR#13384). All three Buffer* WebGPU renderers (`WebGPUBufferPointRenderer.ts`, `WebGPUBufferPolygonRenderer.ts`, `WebGPUBufferPolylineRenderer.ts`) truncate `material.color` to RGB via `encodeRGB8` and force alpha=1.0, so the `if (outColor.a < 0.005) discard` in each WGSL (`Collections/BufferPoint/Polygon/PolylineMaterial.wgsl`) is dead and translucent buffer primitives (and the GeoJsonPrimitive fills/strokes that ride on them) render fully OPAQUE. Fix needs a widened packed lane: point packs `showSizeAndColor` as vec3, polygon `showAndColor` as vec2, polyline `showColorWidthAndTexCoord` as vec4 (saturated → needs a NEW lane) — add a `csm_decodeRGBA8` helper (or a dedicated float alpha lane), emit `material.color.alpha` from each `repack*Dirty`, and keep the **lockstep triple** consistent (CPU pack width/indices ↔ GPU vertex-buffer `arrayStride`/`format`/`shaderLocation` ↔ WGSL struct field) — a stride mismatch is silent GPU corruption. Verify with a 0.5-alpha-fill probe extending `probe-bufferpolygon-vector-tile.mjs` on both backends. **Trace:** WEBGPU_PARITY_AUDIT_2026-06.md (P1, "BufferPrimitiveMaterial color.alpha translucency"); FEATURE_INVENTORY.md §A.4 GeoJsonPrimitive.
- **NEW-BUFFERPRIMITIVE-BLEND-OPTION** (M) — RESOLVED-by `batch-bufferprimitive-parity` (WEBGPU_PARITY_AUDIT_2026-06, PR#13384). None of the three Buffer* renderers read `collection._blendOption`; they hardcode a translucent blend state and always push `Pass.TRANSLUCENT` with `depthWriteEnabled=false`. OPAQUE collections (the common GeoJSON-fill case) then render in the translucent pass → wrong sort/occlusion vs WebGL, and opaque fills don't write depth so stacked/adjacent polygons show order-dependent sort artifacts (this subsumes the audit's separate "GeoJsonPrimitive opaque polygon fills forced through TRANSLUCENT" P2 row, which affects the polyline path identically). Fix: surface `_blendOption` on the shared `WebGPUBufferPrimitiveRenderer.ts` (default `BlendOption.TRANSLUCENT`), build + cache-key a second OPAQUE pipeline variant (blend off, `depthWriteEnabled=true`) in each renderer's pipeline builder, and route to `Pass.OPAQUE` (not the hardcoded `Pass.TRANSLUCENT`) when `_blendOption===BlendOption.OPAQUE`, mirroring WebGL's branch in `renderBufferPointCollection.js` / `renderBufferPolygonCollection.js`. Best landed in the SAME batch as NEW-BUFFERPRIMITIVE-COLOR-ALPHA (both touch pipeline/pass selection). **Trace:** WEBGPU_PARITY_AUDIT_2026-06.md (P1, "blendOption (OPAQUE vs TRANSLUCENT) param" + P2 "GeoJsonPrimitive opaque polygon fills forced through TRANSLUCENT").
...
- **NEW-PANORAMA-CULL-OVERRIDE-WEBGPU** (M, found by WEBGPU_PARITY_AUDIT_2026-06; upstream #13369) — **NOTE: distinct from C-R1-PRIMITIVE-DERIVED above, which is scoped to depth-only/pick renderState forwarding and explicitly EXCLUDES pipeline-cull derivation.** The #13369 `flat:true` lighting fix IS honored on WebGPU (`matImageFlat` selected — tracked SHIPPED as NEW-UPSTREAM-PANORAMA-LIGHTING-13369), but the WebGPU material pipeline in `WebGPUPrimitiveCommands.js` bakes `cullMode` SOLELY from `appearance.closed` and ignores the appearance's `renderState.cull.enabled:false`. So an `EquirectangularPanorama` viewed from INSIDE the sphere shows back faces, and `closed:true` forces `cullMode:'back'` → the panorama is INVISIBLE on WebGPU while WebGL (which honors `cull.enabled:false`) shows it. Fix: derive the pipeline `cullMode` from the appearance's `renderState.cull.{enabled,face}` (→ `'none'` when `enabled===false`) BEFORE falling back to the `closed`-based default, keeping the cache-key in sync. **Files:** `WebGPUPrimitiveCommands.js`. **Trace:** WEBGPU_PARITY_AUDIT_2026-06.md (P2 "EquirectangularPanorama cull-override (renderState.cull.enabled:false)"); upstream #13369; sibling NEW-UPSTREAM-PANORAMA-LIGHTING-13369 (lighting half, SHIPPED Batch 299).
