...
- **BUG-GLOBE-TILE-SEAM-LINES** (surfaced 2026-07-02 during GLOBE-POLAR-STRETCH) — WebGPU-only faint blue/dark dashed tile-seam lines on the globe at orbit/far zoom (visible as a dashed vertical line down tile boundaries in `probe-globe-polar-stretch.mjs` far/extreme PNGs and in the NaturalEarthII geographic control — so it is NOT Mercator/projection-related and NOT fixed by the ReprojectWebMercator.wgsl orientation fix). WebGL shows no seams at the same views. Likely imagery-edge sampling (clamp/alpha-mask edge at `texCoordsRect` boundaries) or skirt shading divergence. Small visual footprint (~1-2% of the remaining far-zoom mismatch) but systematic. Repro: far view (lon −95, lat 40, h 25 Mm), `?renderer=webgpu`, look along tile column boundaries.
