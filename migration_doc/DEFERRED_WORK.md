...
- **NEW-WEBGPU-CLOUD-WORLEY-TEXTURE-PARITY** (S, surfaced Batch 363) — after the volumetric port the WebGPU cloud is ~1.6x linear (2.4x area) larger and denser/less grainy than WebGL: the port substitutes an inline procedural worley FBM for WebGL's baked 3D-packed-2D worley texture (`CloudNoiseFS.glsl` → `voxelToUV`/`sampleNoiseTexture` with `u_noiseDetail=16`, wrapping + `u_noiseOffset` animation). The inline worley erodes edges with the same algorithm but a lower/scale-invariant spatial frequency, so the silhouette stays fuller. Fix = generate the exact baked worley texture on the WebGPU side (one-time render pass porting `CloudNoiseFS`, or CPU worley gen into the same packed layout) and sample it via a WGSL `voxelToUV` port, restoring the texture+sampler bindings 1/2 (already retained). Effort: ~1 session. Impact: pixel-grain + opacity parity for CloudCollection.
