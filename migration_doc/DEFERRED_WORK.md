...
- **NEW-WEBGPU-CLOUD-SIZE-PARITY** (M, surfaced Batch 365) — the WebGPU CloudCollection cloud renders ~1.9x linear larger / denser than WebGL at the same `maximumSize` + camera (2000×1300×900 m cloud, 15 km nadir: WebGPU core ~120 px vs WebGL ~63 px). **Root cause is NOT the worley (now byte-matching WebGL) nor the raymarch/Gardner/TR/color WGSL (all verified line-by-line identical to `CloudCollectionFS.glsl`).** Candidates for the deeper cause: the eye-space-vs-clip-space quad-offset projection (WebGPU adds `scale*offset*P00` in clip space vs WebGL's `positionEC.xy += scale*offset` then project — should be equivalent after the perspective divide but worth a numeric check), the `maximumSize` vs `scale` delivery through the instance buffer, or a per-fragment `v_distance`/depth interaction. **Next step:** instrument the WGSL `coordinate`/`ellipsoidScale`/`ndDot` for a center + edge fragment on both backends at a fixed pixel and diff; OR temporarily render the raw ellipsoid silhouette (pre-erosion) on both to compare the projected extent directly. Effort: ~1 session. Impact: CloudCollection size/density parity. **Probe:** `probe-cloud-volumetric-parity.mjs` (needs a connected-component/core measure to ignore the stray-speck bbox pollution).
