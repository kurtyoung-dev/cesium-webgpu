...
- **NEW-STARS-BRIGHT-CATALOG-WEBGL-FALLBACK** (M, surfaced Batch 313 per Principle 9) — WebGL keeps ONLY the static SkyBox cubemap stars; no `STAR_FIELD` feature renderer is registered for WebGL, so `StarField.update` is an inert no-op there. Reaching WebGL parity means a GLSL point-sprite path (same per-instance buffer + a `BillboardCollection`-style additive draw, or a dedicated tiny VS/FS pair) wired through a WebGL `STAR_FIELD` renderer. **Estimated effort:** 0.5 session (the catalog data + intensity/color math are backend-neutral and already factored into `BrightStarCatalog.js` + `bvToRgb`; only the draw path is WebGPU-specific).
