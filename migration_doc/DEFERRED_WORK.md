...
**Next work (blocks visual usability of ALL froxel volumetric fog, not just ground fog):** rework the scatter integration to an **energy-conserving single-scatter** model (e.g. analytic transmittance-weighted in-scatter `(1 - exp(-σ·d))` per slice, or a proper Beer-Lambert accumulation with a bounded phase-weighted source) so density maps to opacity with graceful dynamic range. Then re-tune `peakDensity` / `bandHeight` for a soft ground mist and flip Phase C to visually-shipped. **Probe:** `Tools/visual-regression/probe-ground-fog.mjs` already drives it (read the ON PNG — it must read as a graded mist, not a whiteout). **Files:** `VolumetricFog.wgsl` (integrate), `WebGPUVolumetricFogRenderer.ts` (tuning). **Trace:** 2026-06-26.
