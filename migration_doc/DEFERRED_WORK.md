...
- **MORPH-MIX-JITTER** — `GlobeTerrain.wgsl:1111` blends the morph endpoints with GPU `mix()`; WebGL deliberately avoids `mix()` via a manual lerp (`czm_columbusViewMorph`, columbusViewMorph.glsl) to dodge endpoint jitter on NVidia 3070 Ti / Intel Arc A750. A byte-identical WGSL port exists at `csm_columbusViewMorph.wgsl:7-11` but is never called. Fix: inline the manual lerp at :1111 (the globe shader is fully inline; don't wire chunk includes). **Open — P2, hardware-specific.**
- **MORPH-WEBMERCATOR-INSTANCEOF** — `WebGPUGlobeSurfaceCameraUB.ts:419-424` detects Web Mercator via `projection.constructor.name === "WebMercatorProjection"` (minification-unsafe); WebGL uses `instanceof` (GlobeSurfaceTileProviderRendering.js:1201). In minified release builds with an explicit `WebMercatorProjection`, the flag flips to 0 → geographic-linear latitude in SCENE2D/CV/morph (vertical tile warping at mid/high latitudes). Masked in dev (unminified) + by the default Geographic projection. Fix: import the class, use `instanceof`. **Open — P2, release-build only.**
