...
- **NEW-CSM-CASCADE-GROUND-FIT** (M, surfaced Batch 298 per Principle 9) — CSM cascade fitting over-includes empty space below the ground for low top-down cameras, ballooning the near cascades' bounding sphere and coarsening shadow-edge resolution. `WebGPUCSMRenderer._computeFrustumCornersWorldSpace` fits each cascade to the camera sub-frustum corners from `splitNear`→`splitFar`; when `splitFar` (7502 m for cascade 0) extends far past the ground the camera is looking at (~1700 m straight down), the far corners sit below the ellipsoid surface and inflate the fit sphere to ~6226 m radius → ~12 m/texel at 1024². WebGL's single shadow map fits tightly to the scene's actual bounding volume. **Fix path (CSM_DESIGN Slice 3):** clamp the cascade frustum-slice far at the ground (ray/ellipsoid intersection of each frustum edge) before `_fitBoundingSphere`, OR fit to the intersection of the frustum slice with the scene's shadow-receiver bounding volume — matching Cesium WebGL's `ShadowMap` scene-volume fit. This sharpens the globe cast-shadow edge to WebGL parity and is the sole remaining sub-check in `probe-csm-soft-shadow.mjs` (the `B.perEdge/C.perEdge` ballpark). **Reproducer:** `probe-csm-soft-shadow.mjs` (5/6 sub-checks PASS; parity ballpark FAIL at ~50×).
