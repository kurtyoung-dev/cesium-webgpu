...
- **(A) Log depth, the WebGL-parity fix — but it is RENDERER-WIDE, not just the globe.** WebGL injects `czm_writeLogDepth` into EVERY shader that writes depth (globe, models, 3D tiles, primitives, collections), so they all share one logarithmic depth space and depth-test correctly against each other + the globe. The WebGPU geometry all renders into the SAME scene-framebuffer depth attachment and depth-tests against each other, so log depth CANNOT be applied to the globe alone — that would put the globe in a different depth space than 3D tiles / models / primitives and break depth testing globally (z-fighting, wrong occlusion). So approach (A) means porting `@builtin(frag_depth) = csm_writeLogDepth(...)` (chunks exist) to ~every WebGPU pipeline + reversing it in every depth CONSUMER (classification reconstruct, Billboard/Label/Model-edge `linearizeWindowDepth`, `pickPosition`, terrain clamp). That is a foundational, multi-week renderer change with broad regression surface — NOT a clean incremental start.
