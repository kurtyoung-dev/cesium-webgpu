...
const BASE = join("packages", "engine", "Source", "Shaders", "WebGPU", "chunks");
...
// Process functions
const funcFiles = readdirSync(FUNCTIONS_DIR).filter(f => f.endsWith(".wgsl")).sort();
let funcCount = 0;
...
// Process structs
const structFiles = readdirSync(STRUCTS_DIR).filter(f => f.endsWith(".wgsl")).sort();
let structCount = 0;
...
let csmBuiltins = "//This file is automatically rebuilt by the Cesium build process.\n";
