#!/usr/bin/env node
/**
 * C13-16 U2 — cross-bundle GPU-timestamp no-regression gate.
 * @purpose C13-16 U2 cross-bundle GPU-timestamp no-regression gate: interleaved bundle-swap A/B per cloud pass with drift sentinels.
 * @status ACTIVE
 *
 * WHAT THIS MEASURES
 * ------------------
 * The U2 acceptance requires per-pass GPU timing for the baked view march, the
 * LIVE escape route, the single and cascaded cloud shadow maps, and the
 * environment/IBL leg. Every cloud pass carries `timestampWrites`
 ...
 *
 * This probe couples timing to two narrow image checks: the untouched CUMULUS
 * control must be pixel-identical across bundles, and every CIRRUS capture must
 * be visibly non-vacuous. Broader morphology remains gated separately.
 *
 ...
 *
 * This is a dedicated descendant of the historical C13-39 probe. It leaves
 * that probe and its evidence contract untouched. Every route runs once as
 * CUMULUS (the U2 untouched-control image) and once as CIRRUS (the U2 subject).
 * U2 is accepted only after two immutable rounds, including both execution
 * orders, whose bundle hashes and replay settings remain stable.
 *
 * Every affected pass is a no-regression gate (<= +2%). Untouched temporal and
 * upscale passes are drift sentinels (absolute motion <= 2%); a sentinel move
 * discards the whole round rather than being interpreted as a product result.
 *
 ...
 *
 * Env:
 *   C13_U2_TAG          pre | post (required)
 *   C13_U2_PAIR_ID      filename-safe campaign identity (required)
 *   C13_U2_ROUND        non-negative integer (required)
 *   C13_U2_ORDER        pre-first | post-first (required)
 *   C13_U2_BUNDLE_PATH  exact Cesium.js served by PROBE_BASE (required)
 *   C13_U2_OUTPUT_DIR   immutable evidence directory (required)
 *   C13_U2_REPEATS      default 5 (median-of-N)
 *   C13_U2_LANES        comma-separated full lane ids (default: all)
...
// Budget: 12 lanes x (up to 300 bounded occupancy frames + 40 warm + 5 x 60
...
const HARD_LIMIT_MS = 2_400_000; // 2400s
const watchdog = setTimeout(() => {
  console.error("[cloud-u2-perf] WATCHDOG FIRED (2400s) — forcing exit");
  process.exit(2);
}, HARD_LIMIT_MS);
if (watchdog.unref) watchdog.unref();

const BASE = process.env.PROBE_BASE || "http://localhost:8080";
const TAG = process.env.C13_U2_TAG;
const PAIR_ID = process.env.C13_U2_PAIR_ID;
const ROUND = Number(process.env.C13_U2_ROUND);
const ORDER = process.env.C13_U2_ORDER;
const REPEATS = Math.max(1, Number(process.env.C13_U2_REPEATS ?? 5));
const LANE_FILTER = (process.env.C13_U2_LANES || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
const OUT = path.resolve(process.env.C13_U2_OUTPUT_DIR || "");
const BUNDLE_PATH = path.resolve(process.env.C13_U2_BUNDLE_PATH || "");
const W = 1024;
const H = 768;
...
// The bright-cell census (pixels with luminance > 150) is calibrated for the
// thick CUMULUS deck and is blind to thin genera: at the acceptance operating
// point CIRRUS paints ~27% of the frame (differential vs clouds-off on the
// black background) yet contributes 0-95 bright cells, with canvas mean
// luminance 4.1-9.6 against an EXACT 0.000 when clouds are off (sky,
// atmosphere and sun are disabled, so every lit pixel is cloud). CIRRUS lanes
// therefore gate on mean luminance instead. Floor derivation: half the
// weakest content observed across both bundles and all six cirrus lanes
// (4.127, r0 first-red evidence, pair b1108-u2, rounded down to 2.0), with the empty scene at
// exactly zero - separation is absolute, not marginal.
const CIRRUS_MIN_MEAN_LUM = 2.0;
...
if (!["pre", "post"].includes(TAG)) {
  throw new Error("C13_U2_TAG must be pre or post");
}
if (!PAIR_ID || !/^[A-Za-z0-9._-]+$/.test(PAIR_ID)) {
  throw new Error("C13_U2_PAIR_ID must be a filename-safe non-empty id");
}
if (!Number.isInteger(ROUND) || ROUND < 0) {
  throw new Error("C13_U2_ROUND must be a non-negative integer");
}
if (!["pre-first", "post-first"].includes(ORDER)) {
  throw new Error("C13_U2_ORDER must be pre-first or post-first");
}
if (!process.env.C13_U2_OUTPUT_DIR) {
  throw new Error("C13_U2_OUTPUT_DIR is required");
}
if (!process.env.C13_U2_BUNDLE_PATH) {
  throw new Error("C13_U2_BUNDLE_PATH is required");
}
 ...
 *
 * `expect` records the U2 policy so the campaign report cannot silently accept
 * a regression:
 *   "unchanged"     — the shader is untouched; require |delta| <= 2%
 *   "no-regression" — the pass may improve; require <= +2%
 */
//
...
const BASE_LANES = [
  ...
  },
  {
    ...
    expect: {
      "ProceduralClouds half-res pass": "no-regression",
  ...
  },
  {
    ...
    expect: {
      "CloudShadow map pass": "no-regression",
  ...
  },
  {
    ...
    expect: {
      "CloudShadow cascade atlas pass": "no-regression",
      "CloudShadow map pass": "no-regression",
      "ProceduralClouds pass": "no-regression",
    },
  },
  {
    ...
    expect: {
      "DynEnvMap Sky Fill": "no-regression",
      "ProceduralClouds pass": "no-regression",
    },
  },
];

// Values are the public CloudType enum values. The expected rows are f32-exact
// because the renderer stores them in Float32Array uniform slots 168..171.
const GENERA = [
  {
    id: "cumulus",
    value: 0,
    expectedUniformRow: [0, 1, 0, 0],
  },
  {
    id: "cirrus",
    value: 1,
...
const LANES = BASE_LANES.flatMap((route) =>
  GENERA.map((genus) => ({
    ...route,
    id: `${genus.id}-${route.id}`,
    routeId: route.id,
    genus,
    occupancyMinMeanLum: genus.id === "cirrus" ? CIRRUS_MIN_MEAN_LUM : null,
    volumetric: { ...route.volumetric, cloudType: genus.value },
  })),
);
...
async function lumStats(png) {
  ...
  return {
    meanLum: +(sum / pixels).toFixed(3),
    cloudCells: cells,
    pixelSha256: createHash("sha256").update(data).digest("hex"),
  };
}
...
const MEASURE = async (cfg) => {
  ...
  // CANONICAL occupancy measurement: render and toDataURL fuse in one task
  // inside captureSnapshot, and the pixels decode from that frozen PNG - a
  // live-canvas drawImage is the retired reader class and can decode
  // all-zero once the task yields. Full resolution (no downsample) so the
  // cell count is directly comparable to the Node-side sharp gate, which
  // uses the same lum > 150 rule on the canvas-element screenshot.
  const fusedOccupancy = window.makeFusedSnapshotCapture(
    { render: (time) => s.render(time) },
    s.canvas,
    () => base,
  );
  const measureCanvas = async () => {
    const { imageData } = await fusedOccupancy.captureSnapshot();
  ...
  const densityRealization = () => {
    ...
    const genusUniformRow = cache?.uniformData
      ? Array.from(cache.uniformData.slice(168, 172))
      : null;
    return {
      flags,
  ...
  if (cfg.awaitReady) {
    ...
    for (; waited < cfg.occupancyMaxFrames; waited++) {
      ...
      const shot = await measureCanvas();
      ...
      const visible =
        cfg.occupancyMinMeanLum != null
          ? shot.meanLum >= cfg.occupancyMinMeanLum
          : shot.cells >= cfg.occupancyMinCells;
      if (densityReady && visible) {
        break;
      }
      await new Promise((resolve) => requestAnimationFrame(resolve));
    }
    ...
    if (!ok) {
      if (!anyReadable) {
        ...
        cfg.occupancyMinMeanLum != null
          ? bestMeanLum < cfg.occupancyMinMeanLum
          : bestCells < cfg.occupancyMinCells
      ) {
    ...
    occupancy = {
      ...
      minMeanLum: cfg.occupancyMinMeanLum,
      bestCells,
...
async function runLane(browser, lane) {
  ...
  // Playwright wraps init-script content in its own scope, so the canonical
  // source's top-level const must be re-exposed on window for page.evaluate.
  await page.addInitScript({
    content:
      FUSED_SNAPSHOT_CAPTURE_SOURCE +
      ";window.makeFusedSnapshotCapture = makeFusedSnapshotCapture;",
  });
  ...
  try {
    ...
    for (let repeat = 0; repeat < REPEATS; repeat++) {
      ...
      repeatRecords.push(
        await page.evaluate(MEASURE, {
          ...
          occupancyMinMeanLum: lane.occupancyMinMeanLum ?? null,
    ...
    const pngPath = path.join(
      OUT,
      `cloud-u2-${PAIR_ID}-r${ROUND}-${ORDER}-${TAG}-${lane.id}.png`,
    );
    if (fs.existsSync(pngPath)) {
      throw new Error(`immutable evidence already exists: ${pngPath}`);
    }
    fs.writeFileSync(pngPath, pngBuffer, { flag: "wx" });
    ...
    const genusUniformRow =
      repeatRecords[repeatRecords.length - 1]?.density?.genusUniformRow ?? null;
    const genusRowMatches =
      Array.isArray(genusUniformRow) &&
      genusUniformRow.length === lane.genus.expectedUniformRow.length &&
      genusUniformRow.every((value, index) =>
        Object.is(value, lane.genus.expectedUniformRow[index]),
      );
    ...
    return {
      ...
      valid:
        ...
        genusRowMatches &&
        errors.length === 0 &&
        (lane.occupancyMinMeanLum != null
          ? fingerprint.meanLum >= lane.occupancyMinMeanLum
...
async function sha256OfServedBundle() {
  const url = new URL("/Build/CesiumUnminified/Cesium.js", BASE);
  const response = await fetch(url);
  if (!response.ok) {
    throw new Error(
      `could not fetch served runtime bundle (${response.status})`,
    );
  }
...
function readCampaignManifests() {
  const prefix = `cloud-u2-${PAIR_ID}-r`;
  return fs
    ...
    .map((entry) =>
      JSON.parse(fs.readFileSync(path.join(OUT, entry.name), "utf8")),
    );
}
...
async function run() {
  ...
  if (!fs.existsSync(BUNDLE_PATH)) {
    throw new Error(`C13_U2_BUNDLE_PATH does not exist: ${BUNDLE_PATH}`);
  }
  ...
  if (fs.existsSync(manifestPath)) {
    throw new Error(`immutable evidence already exists: ${manifestPath}`);
  }
  const existingRound = readCampaignManifests().filter(
    (manifest) => manifest.round === ROUND,
  );
  if (existingRound.some((manifest) => manifest.order !== ORDER)) {
    throw new Error(`round ${ROUND} already exists under a different order`);
  }
  const firstTag = ORDER === "pre-first" ? "pre" : "post";
  const secondTag = firstTag === "pre" ? "post" : "pre";
  if (existingRound.length === 0 && TAG !== firstTag) {
    throw new Error(`round ${ROUND} ${ORDER} must capture ${firstTag} first`);
  }
  if (
    existingRound.length === 1 &&
    (existingRound[0].tag !== firstTag || TAG !== secondTag)
  ) {
    throw new Error(
      `round ${ROUND} ${ORDER} must contain ${firstTag}, then ${secondTag}`,
    );
  }
  if (existingRound.length > 1) {
    throw new Error(`round ${ROUND} already has both immutable legs`);
  }

  const runtimeBundle = fs.readFileSync(BUNDLE_PATH);
  const runtimeBundleSha256 = createHash("sha256")
    .update(runtimeBundle)
    .digest("hex");
  const servedBundle = await sha256OfServedBundle();
  if (
    servedBundle.sha256 !== runtimeBundleSha256 ||
    servedBundle.byteLength !== runtimeBundle.byteLength
  ) {
    throw new Error(
      "C13_U2_BUNDLE_PATH is not the exact Cesium.js served by PROBE_BASE " +
        `(declared ${runtimeBundleSha256}/${runtimeBundle.byteLength}, ` +
        `served ${servedBundle.sha256}/${servedBundle.byteLength})`,
    );
  }
  const source = {
    ...
    runtimeBundle: {
      ...
      sha256: runtimeBundleSha256,
      servedUrl: servedBundle.url,
    },
  };
  ...
  if (selected.length === 0) {
    throw new Error(`C13_U2_LANES matched no lane: ${LANE_FILTER.join(",")}`);
  }
  ...
  try {
    ...
    for (const lane of selected) {
      console.log(
        `[cloud-u2-perf:r${ROUND}:${ORDER}:${TAG}] lane ${lane.id} ...`,
      );
    ...
    const manifest = {
      ...
      measurement: {
        ...
        repeats: REPEATS,
        occupancyMinCells: OCCUPANCY_MIN_CELLS,
        cirrusMinMeanLum: CIRRUS_MIN_MEAN_LUM,
        regressionBandPct: CLOUD_U2_REGRESSION_BAND_PCT,
        controlBandPct: CLOUD_U2_CONTROL_BAND_PCT,
        acceptanceLaneIds: LANES.map((lane) => lane.id).sort(),
        selectedLaneIds: selected.map((lane) => lane.id).sort(),
      },
      lanes,
    };
    ...
    const assessment = assessCloudU2PerfManifests({
      manifests: readCampaignManifests(),
    });
    const summaryPath = path.join(OUT, cloudU2SummaryFilename(PAIR_ID));
    fs.writeFileSync(
      summaryPath,
      JSON.stringify(
        {
    ...
    console.log(`\n=== C13-16 U2 lanes r${ROUND} ${ORDER} (${TAG}) ===`);
    ...
    console.log(`\n=== CAMPAIGN ASSESSMENT: ${assessment.status} ===`);
    for (const round of assessment.rounds) {
      console.log(
        `  r${round.round} ${round.order}: ` +
          `${round.usable ? "USABLE" : round.controlDrifted ? "DRIFT-DISCARDED" : "INCOMPLETE"}`,
      );
      for (const lane of round.lanes) {
        for (const [name, delta] of Object.entries(lane.passes)) {
          console.log(
            `    ${lane.id} / ${name}: ${delta.preMs} -> ${delta.postMs} ms ` +
              `(${delta.deltaPct ?? "n/a"}%) [${delta.outcome}]`,
          );
        }
      }
    }
    for (const failure of assessment.failures) {
      console.log(
        `  [${assessment.structural ? "STRUCTURAL" : "FAIL"}] ${failure}`,
      );
    }
    ...
    const protocolStillCollecting =
      assessment.status === "incomplete-protocol" &&
      readCampaignManifests().length < 4;
    const pass = capturesValid && assessment.passed;
    console.log(
      `RESULT: ${pass ? "GREEN" : protocolStillCollecting ? "INCOMPLETE" : "RED"}`,
    );
    process.exitCode = !capturesValid
      ? 3
      : pass || protocolStillCollecting
        ? 0
