...
p.on("console", (m) => {
  if (m.type() === "error") errs.push("ERR:" + m.text().slice(0, 140));
});
...
const info = await p.evaluate(
  async ({ build }) => {
    ...
    const out = {
      useLogDepth: undefined,
      logDepthWriteEnabled: s.context?._logDepthWriteEnabled,
    };
    v.camera.setView({
    ...
    try {
      const dev = s.context._dbgDev,
        ubuf = s.context._dbgUBuf;
      if (dev && ubuf) {
      ...
      out.uBufFrustum = "err:" + e.message;
    }
    out.frames = frames;
    out.tilesLoaded = s.globe.tilesLoaded;
    out.ready = ground.ready;
    try {
      out.encodeNearFar = Array.from(s.context._logDepthEncodeNearFar || []);
    } catch (e) {
      out.encodeNearFar = "err";
    }
    try {
      out.usEnc = Array.from(
        s.context.uniformState._logDepthEncodeNearFar || [],
      );
    } catch (e) {
      out.usEnc = "err";
    }
    try {
      out.fsUsEnc = Array.from(
        s._frameState?.context?.uniformState?._logDepthEncodeNearFar || [],
      );
    } catch (e) {
      out.fsUsEnc = "err";
    }
    try {
      out.sameUS =
        s.context.uniformState === s._frameState?.context?.uniformState;
    } catch (e) {}
    try {
      const ip = s.context.uniformState.inverseProjection;
      out.invProj = [ip[0], ip[5], ip[10], ip[15]];
    } catch (e) {
      out.invProj = "err:" + e.message;
    }
    try {
      const p = s.context.uniformState.projection;
      out.proj = [p[0], p[5], p[10], p[15]];
    } catch (e) {}
    try {
      out.camFrustumFar = s.camera.frustum.far;
      out.camFrustumNear = s.camera.frustum.near;
    } catch (e) {}
    return out;
  },
  { build: STRIPE },
  ...
  async (durl) =>
    new Promise((res) => {
      ...
      img.onload = () => {
        ...
        for (const [x, y] of globePts) {
          const i = (y * c.width + x) * 4;
          pts.push(["GLOBE", x, y, d[i], d[i + 1], d[i + 2]]);
        }
...
console.log(
  "  -> R/255 =",
  (samples.medRGB[0] / 255).toFixed(3),
  "(storedDepth: ~0.55 LOG / ~0.97+ STD)",
);
console.log(
  "  -> G/255 =",
  (samples.medRGB[1] / 255).toFixed(3),
  "(eyeDist/1e6: ~0.35 OK / ~1.0 garbage)",
);
console.log(
  "  -> B/255 =",
  (samples.medRGB[2] / 255).toFixed(3),
  "(encFar/1e10: ~1.0 OK)",
);
