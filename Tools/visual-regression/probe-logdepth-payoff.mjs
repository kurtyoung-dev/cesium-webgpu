...
const flip = await p.evaluate(async () => {
  ...
  let useLogDepth;
  try {
    useLogDepth = (s._view?.frameState ?? s._frameState ?? s.frameState)
      ?.useLogDepth;
  } catch (e) {
    useLogDepth = "err";
  }
  return {
    before,
    after: !!s.context._logDepthWriteEnabled,
    useLogDepth,
...
console.log(
  "variance far-UR (artifact corner): OFF =",
  vOff.farUR,
  " ON =",
  vOn.farUR,
  "  (higher = checkerboard present; ~0 = solid/artifact)",
);
console.log(
  "variance near-LL (good corner):     OFF =",
  vOff.nearLL,
  " ON =",
  vOn.nearLL,
  "  (sanity: should be high both)",
);
