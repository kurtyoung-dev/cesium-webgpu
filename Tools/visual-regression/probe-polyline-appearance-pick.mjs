...
async function run(renderer) {
  ...
  const res = await page.evaluate(async () => {
    ...
    s.globe.show = false; s.skyBox.show = false; if (s.sun) s.sun.show = false;
    if (s.moon) s.moon.show = false; s.skyAtmosphere.show = false;
    s.backgroundColor = C.Color.BLACK;

    const positions = C.Cartesian3.fromDegreesArray([
      -75.0, 35.0, -74.0, 36.0, -73.0, 35.0, -72.0, 36.0, -71.0, 35.0,
    ]);
    const primitive = s.primitives.add(new C.Primitive({
      geometryInstances: new C.GeometryInstance({
        geometry: new C.PolylineGeometry({
          positions, width: 12.0, arcType: C.ArcType.NONE,
          vertexFormat: C.PolylineColorAppearance.VERTEX_FORMAT,
        }),
        attributes: { color: C.ColorGeometryInstanceAttribute.fromColor(new C.Color(0, 1, 1, 1)) },
        id: "the-polyline",
      }),
    ...
    v.camera.lookAt(center, new C.HeadingPitchRange(0.0, C.Math.toRadians(-90.0), 300000.0));
    ...
    const canvas = s.canvas, w = canvas.width, h = canvas.height;
    const tmp = document.createElement("canvas"); tmp.width = w; tmp.height = h;
    const cx = tmp.getContext("2d"); cx.drawImage(canvas, 0, 0);
    const px = cx.getImageData(0, 0, w, h).data;
    const cyanPts = []; let n = 0;
    ...
    if (n === 0) return { ready: !!primitive.ready, cyan: 0, pickIds: (primitive._pickIds || []).length };
    const sx2css = canvas.clientWidth / w, sy2css = canvas.clientHeight / h;
    ...
    let pickedPrimitive = false, pickedId = false, lastType = "undefined", hits = 0;
    for (let pass = 0; pass < 3; pass++) {
      for (const [pxc, pyc] of samples) {
        const picked = s.pick(new C.Cartesian2(pxc, pyc));
        if (picked) {
          lastType = typeof picked.primitive;
          if (picked.primitive === primitive) { pickedPrimitive = true; hits++; }
    ...
    return {
      ready: !!primitive.ready, cyan: n, pickIds: (primitive._pickIds || []).length,
      allowPicking: primitive.allowPicking, nSamples: samples.length, hits,
      pickCmds: (primitive._pickCommands || []).length,
      hasPickPipeline: !!primitive._webgpuPolylineCache?.pickPipeline,
      pickIdColor: primitive._pickIds?.[0]?.color ? [primitive._pickIds[0].color.red, primitive._pickIds[0].color.green, primitive._pickIds[0].color.blue] : null,
      pickedPrimitive, pickedId, lastType,
    };
  });
  await browser.close();
  return { res, errs: errs.filter((e) => !/AtmosphereLUT|default layout|atmosphereLUT/.test(e)) };
}
...
const checks = [
  ["webgl pick returns the polyline (reference)", wgl.res.pickedPrimitive || wgl.res.pickedId],
  ["webgpu line renders (cyan>200) + has pickIds", wgpu.res.cyan > 200 && wgpu.res.pickIds > 0],
  ["webgpu pick returns the polyline primitive/id", wgpu.res.pickedPrimitive || wgpu.res.pickedId],
