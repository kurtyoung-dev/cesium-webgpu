...
const LON = -75.0, LAT = 40.0, H = 50000.0;

async function run(rendererArg, modeName, modeFn) {
  const browser = await chromium.launch({
    channel: "msedge", headless: true, args: ["--enable-unsafe-webgpu"],
  });
  ...
  const out = await page.evaluate(async ({ modeName, lon, lat, h }) => {
    const C = await import("/Build/CesiumUnminified/index.js");
    const v = window.viewer; const scene = v.scene;
    scene.requestRenderMode = false; scene.globe.show = false;
    ...
    bb.add({
      ...
      image: (() => { const cv = document.createElement("canvas"); cv.width = cv.height = 64;
        const g = cv.getContext("2d"); g.fillStyle = "#ff00ff"; g.fillRect(0,0,64,64); return cv; })(),
    });
    ...
    v.camera.setView({ destination: C.Cartesian3.fromDegrees(lon, lat, 8000000.0) });
    for (let i = 0; i < 90; i++) { scene.render(); await new Promise((r) => requestAnimationFrame(r)); }

    const c = scene.canvas; const off = document.createElement("canvas");
  ...
  await browser.close();
  console.log(`  ${rendererArg}/${modeName} -> magenta=${out.magenta} (mode=${out.mode}) errs=${errors.length}`);
  return out.magenta;
}
