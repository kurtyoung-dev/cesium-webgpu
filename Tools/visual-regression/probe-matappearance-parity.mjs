...
for (const m of MATERIALS) {
  ...
  fs.writeFileSync(
    `Tools/visual-regression/output/matparity-${mat}-webgpu.png`,
    encodePNG(wgpu.decoded),
  );
  fs.writeFileSync(
    `Tools/visual-regression/output/matparity-${mat}-webgl.png`,
    encodePNG(wgl.decoded),
  );
  const sgpu = stats(wgpu.decoded),
    sgl = stats(wgl.decoded);
  const lumPct = sgl.meanLum
    ? +((100 * (sgpu.meanLum - sgl.meanLum)) / sgl.meanLum).toFixed(2)
    : null;
  const gpuTopBot = sgpu.botMean
    ? +(sgpu.topMean / sgpu.botMean).toFixed(3)
    : 0;
  const glTopBot = sgl.botMean ? +(sgl.topMean / sgl.botMean).toFixed(3) : 0;
  const within =
    lumPct !== null &&
    Math.abs(lumPct) < 12 &&
    Math.abs(gpuTopBot - glTopBot) < 0.15 &&
    wgpu.consoleErrors.length === 0;
...
console.log(
  "NOTE: Grid is informational — its grid LINES don't render on WebGPU (separate pre-existing material-pattern bug, not the lighting fix).",
);
console.log(
  pass
    ? "GATE PASS — WebGPU lit MaterialAppearance (flat Color) brightness + directional shading match WebGL"
    : "GATE FAIL — WebGPU lit MaterialAppearance brightness diverges from WebGL",
);
