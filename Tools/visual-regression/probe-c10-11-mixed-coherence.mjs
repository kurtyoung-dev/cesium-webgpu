...
async function runAlt(alt) {
  const result = await page.evaluate(async ({ LON, LAT, altM }) => {
    ...
    if (dev) dev.onuncapturederror = (ev) => errors.push(String(ev?.error?.message).slice(0, 200));

    v.camera.setView({
      destination: C.Cartesian3.fromDegrees(LON, LAT, altM),
      orientation: { heading: 0, pitch: -Math.PI / 2, roll: 0 },
    });
    const center = new C.Cartesian2(Math.floor(scene.canvas.clientWidth / 2), Math.floor(scene.canvas.clientHeight / 2));
    const renderN = async (n) => { for (let i = 0; i < n; i++) { scene.render(); await new Promise((r) => requestAnimationFrame(r)); } };
    const waitTiles = async (maxFrames) => { for (let i = 0; i < maxFrames; i++) { scene.render(); await new Promise((r) => requestAnimationFrame(r)); if (scene.globe.tilesLoaded) return i; } return -1; };
    const idOf = (p) => !C.defined(p) ? null : (typeof p.id === "string" ? p.id : (p.id?.id ?? (p.primitive === scene.globe ? "GLOBE" : (p.primitive?.constructor?.name ?? "?"))));
    const isGlobe = (p) => C.defined(p) && p.primitive === scene.globe;
    const pickStable = async (want) => { let last; for (let i = 0; i < 16; i++) { scene.render(); const r = await scene.pickAsync(center, 3, 3); if (C.defined(r)) { last = r; if (idOf(r) === want) return r; } await new Promise((rr) => requestAnimationFrame(rr)); } return last; };
    ...
    const boxInst = new C.GeometryInstance({
      ...
      modelMatrix: C.Transforms.eastNorthUpToFixedFrame(C.Cartesian3.fromDegrees(LON, LAT, hMid)),
    ...
    const boxPrim = new C.Primitive({
      ...
      appearance: new C.PerInstanceColorAppearance({ closed: true, translucent: false }),
      asynchronous: false,
      allowPicking: true,
    });
    ...
    const frontPt = points.add({ position: C.Cartesian3.fromDegrees(LON, LAT, hFront), pixelSize: 46, color: C.Color.CYAN, id: "mix-point" });
    ...
    window.__c10 = { points, frontPt, boxPrim, pickStable, idOf, isGlobe, renderN };
    return { shippedDefault, gate: ctx._pickLogDepthWriteEnabled, step1, errors, tilesLoadedAt };
  }, { LON, LAT, altM: alt.m });
  ...
  const phase2 = await page.evaluate(async () => {
    ...
    const { points, frontPt, boxPrim, pickStable, idOf, isGlobe, renderN } = window.__c10;
...
console.log("=== C10-11 MIXED-FAMILY pick-fleet log-depth coherence (gate ON) ===");
let pass = true;
for (const alt of ALTS) {
  ...
  if (!ok) pass = false;
  console.log(`  [${k}] default=${r.shippedDefault} gate=${r.gate} | near-point=${r.step1.id}(${s1?"ok":"X"}) mid-box=${r.step2.id}(${s2?"ok":"X"}) globe-revealed=${r.step3.id}(${s3?"ok":"X"}) errors=${r.errors?.length ?? 0} => ${ok?"PASS":"FAIL"}`);
}
if (consoleErrors.length) { pass = false; console.log("console errors:", consoleErrors.slice(0, 6)); }
