...
async function capture(renderer, extruded) {
  ...
  await page.evaluate(async ({ extruded }) => {
    ...
    if (extruded) { geomOpts.extrudedHeight = 500000.0; geomOpts.height = 0.0; }
...
const results = {};
for (const [label, extruded] of [["flat", false], ["extruded", true]]) {
  const wgpu = await capture("webgpu", extruded);
  const wgl = await capture("webgl", extruded);
  if (wgpu.decoded?.b64) fs.writeFileSync(`Tools/visual-regression/output/flatgrid-${label}-webgpu.png`, Buffer.from(wgpu.decoded.b64, "base64"));
  if (wgl.decoded?.b64) fs.writeFileSync(`Tools/visual-regression/output/flatgrid-${label}-webgl.png`, Buffer.from(wgl.decoded.b64, "base64"));
  results[label] = {
...
const noErrs = results.flat.webgpu.errs.length === 0 && results.extruded.webgpu.errs.length === 0;

console.log(`(A) FLAT polygon renders grid pattern on WebGPU (std ${flatGpu} vs WebGL ${flatGl}): ${flatRendersPattern ? "PASS" : "FAIL"}`);
console.log(`(B) EXTRUDED polygon still renders grid pattern (std ${extGpu} vs WebGL ${extGl}): ${extRendersPattern ? "PASS" : "FAIL"}`);
