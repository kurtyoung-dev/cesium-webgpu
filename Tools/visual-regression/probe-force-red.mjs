...
for (const renderer of ["webgl", "webgpu"]) {
  ...
  await page.evaluate(async () => {
    ...
    v.camera.setView({
      destination: C.Cartesian3.fromDegrees(-100, 40, 12_000_000),
    });
    for (let i = 0; i < 600; i++) {
      v.scene.render();
  ...
  const stats = await page.evaluate(async (b64) => {
    ...
    return {
      ...
      meanG: g / n,
      meanB: b / n,
      redPxPct: (100 * redPx) / n,
    };
  }, base64);

  console.log(
    `${renderer.padEnd(8)} center-200x200: R=${stats.meanR.toFixed(1)} G=${stats.meanG.toFixed(1)} B=${stats.meanB.toFixed(1)}  bright-red pixels=${stats.redPxPct.toFixed(1)}%`,
  );
  await browser.close();
}
