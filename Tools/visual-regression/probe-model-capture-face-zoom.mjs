...
(async () => {
  ...
  const dataUrl = await page.evaluate(async ({ MODEL_URL }) => {
    ...
    for (let i = 0; i < 240; i++) { scene.render(); await new Promise((r) => requestAnimationFrame(r)); }
    ...
    for (let i = 0; i < 60; i++) { scene.render(); manager.update(scene._frameState); await new Promise((r) => requestAnimationFrame(r)); }
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  outBuf[idx] = r | (g << 8u) | (b << 16u) | (255u << 24u);
    ...
    for (let f = 0; f < 6; f++) {
      ...
      g2.fillStyle = "#fff"; g2.font = "11px monospace"; g2.fillText(names[f], f * (size + 2) + 4, size + 11);
    }
    return c.toDataURL("image/png");
  }, { MODEL_URL });

  const b64 = dataUrl.replace(/^data:image\/png;base64,/, "");
  fs.writeFileSync(path.join(OUT_DIR, "probe-model-capture-face-zoom.png"), Buffer.from(b64, "base64"));
  await browser.close();
  const errs = messages.filter((m) => m.t === "error" || m.t === "pageerror");
  console.log("[face-zoom] wrote probe-model-capture-face-zoom.png; console errors:", errs.length);
