...
//      globalThis.CesiumWebGPUMetadataDebug = true. The generated codegen
//      now declares the real struct (named after the metadata class's
//      property + class offset/scale baked); the FS override paints
//      vec4(metadataDebugScalar, 0, 1-..., 1). PROOF = a per-face gradient
//      (many distinct colors), matching DP-H46a — now sourced through the
//      generated code. The probe ALSO extracts the generated WGSL chunk
//      from the running renderer (primCache._metadataWGSL) and asserts it
//      declares `struct Metadata` + `fn initializeMetadata` +
//      `fn metadataDebugScalar`.
//   B. meta-debug-off — same model, debug off → normal textured box.
//   C. plain-box-off  — plain BoxTextured (NO metadata) → MODEL_HAS_METADATA
//      never set, no chunk, OFF-parity baseline. Captured for the
//      pixel-identical comparison against a pre-DP-H46b baseline (use
//      --baseline to save, default compares).
//
...
async function capture(label, { modelUrl, metadataDebug, scale }) {
  ...
  const diagnostics = await page.evaluate(
    async ({ view, clockUTC, modelUrl, metadataDebug, scale }) => {
      ...
      // Walk the model caches and read the generated metadata WGSL chunk +
      // class hash + the materialDefines bit.
      const inspect = {
        metadataBitSet: null,
        ...
        wgslHasStruct: null,
        wgslHasInit: null,
        wgslHasDebugAccessor: null,
        wgslStructCount: null,
      };
      try {
        const META_BIT = 1 << 18;
        const models = [];
            ...
            if (pc && typeof pc.materialDefines === "number") {
              ...
              if (bit || inspect.materialDefinesHex === null) {
                ...
                if (typeof pc._metadataWGSL === "string") {
                  ...
                  inspect.wgslHasStruct = /struct\s+Metadata\b/.test(
                    pc._metadataWGSL,
                  );
                  inspect.wgslHasInit = /fn\s+initializeMetadata\b/.test(
                    pc._metadataWGSL,
                  );
                  inspect.wgslHasDebugAccessor =
                    /fn\s+metadataDebugScalar\b/.test(pc._metadataWGSL);
                  inspect.wgslStructCount = (
                    pc._metadataWGSL.match(/struct\s+Metadata\b/g) || []
                  ).length;
                }
              }
              if (bit) break;
  ...
  const out = path.join(OUT_DIR, `dp46b-${label}.png`);
...
(async () => {
  ...
  console.log("[probe-dp46b] DP-H46b metadata codegen proof\n");
  ...
  for (const cell of [A, B, Cp]) {
    ...
    console.log(
      `    inspect: bit=${ins.metadataBitSet} buf=${ins.hasMetadataBuffer} md=${ins.materialDefinesHex} classHash=${ins.classHashHex}`,
    );
    console.log(
      `    wgsl: hasStruct=${ins.wgslHasStruct} hasInit=${ins.wgslHasInit} hasDebugAccessor=${ins.wgslHasDebugAccessor} structCount=${ins.wgslStructCount}`,
    );
    console.log(
  ...
  console.log(
    `\n  [proof] gradient: A(debug-on)=${A.diagnostics.grid.distinctColors} should be HIGH; B/C low. A=${A.diagnostics.grid.distinctColors} B=${B.diagnostics.grid.distinctColors} C=${Cp.diagnostics.grid.distinctColors}`,
  );

  const reportPath = path.join(OUT_DIR, "dp46b-report.json");
