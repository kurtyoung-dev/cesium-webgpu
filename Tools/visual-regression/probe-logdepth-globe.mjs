...
async function setupAndRender(flag) {
  return await page.evaluate(async (logDepthOn) => {
    ...
    const s = v.scene;
    s.skyBox.show = false; s.skyAtmosphere.show = false; s.globe.showGroundAtmosphere = false; s.fog.enabled = false;
    const ctx = s.context;
    const dev = ctx?._device; const de = [];
    if (dev) dev.onuncapturederror = (e) => de.push(String(e?.error?.message ?? "").slice(0, 300));
    // Frame a regional globe view (terrain precision regime).
    v.camera.setView({ destination: C.Cartesian3.fromDegrees(-105.0, 40.0, 350000.0) });
    ...
    return { logDepthEnabled: ctx._logDepthWriteEnabled, deviceErrs: de.length, deviceErrSample: de.slice(0, 4) };
  }, flag);
}
...
const diff = await page.evaluate(async ([a, b]) => {
  async function dec(u) { const img = new Image(); await new Promise((res, rej) => { img.onload = res; img.onerror = rej; img.src = u; }); const cv = document.createElement("canvas"); cv.width = img.width; cv.height = img.height; const cx = cv.getContext("2d"); cx.drawImage(img, 0, 0); return cx.getImageData(0, 0, cv.width, cv.height).data; }
  const A = await dec(a), B = await dec(b);
  let mism = 0, litA = 0, litB = 0, tot = 0;
  for (let i = 0; i < Math.min(A.length, B.length); i += 4) {
    tot++;
    if (A[i] + A[i+1] + A[i+2] > 30) litA++;
    if (B[i] + B[i+1] + B[i+2] > 30) litB++;
    if (Math.abs(A[i]-B[i]) + Math.abs(A[i+1]-B[i+1]) + Math.abs(A[i+2]-B[i+2]) > 40) mism++;
  }
  return { mismatchPct: (mism/tot*100).toFixed(2), litPctOFF: (litA/tot*100).toFixed(1), litPctON: (litB/tot*100).toFixed(1) };
}, [ "data:image/png;base64," + fs.readFileSync(`${OUT}/_logdepth-globe-OFF.png`).toString("base64"), "data:image/png;base64," + fs.readFileSync(`${OUT}/_logdepth-globe-ON.png`).toString("base64") ]);
