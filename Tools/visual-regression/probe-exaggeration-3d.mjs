...
async function capture(rendererArg) {
  ...
  const out = await page.evaluate(async (exagValue) => {
    const v = window.viewer, c = v.camera;
    ...
    // SCENE3D — no morph.
    for (let i = 0; i < 300; i++) { v.scene.initializeFrame(); v.scene.render(); await new Promise((r) => requestAnimationFrame(r)); }
    c.setView({ destination: dest, orientation: { heading: 0.0, pitch: -0.45, roll: 0.0 } });
    for (let i = 0; i < 120; i++) { v.scene.initializeFrame(); v.scene.render(); await new Promise((r) => requestAnimationFrame(r)); }
    const cv = v.scene.canvas;
    return { mode: v.scene.mode, exaggeration: v.scene.verticalExaggeration, dataUrl: cv.toDataURL("image/png") };
  }, EXAG);
  ...
  await browser.close();
  console.log(`${rendererArg}: mode=${out.mode} exag=${out.exaggeration} errors=${errors.length} saved ${file}`);
  return out;
}
await capture("webgl");
await capture("webgpu");
console.log("[probe-exaggeration-3d] done — READ the PNGs: WebGPU 3D relief should match WebGL.");
