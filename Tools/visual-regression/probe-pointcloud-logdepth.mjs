...
async function capture(renderer) {
  ...
  const info = await page.evaluate(async () => {
    ...
    s.skyBox.show = false; s.skyAtmosphere.show = false; if (s.sun) s.sun.show = false;
    ...
    const dates = ["2018-07-19T15:18:00Z", "2018-07-19T15:18:00.5Z", "2018-07-19T15:18:01Z",
      "2018-07-19T15:18:01.5Z", "2018-07-19T15:18:02Z", "2018-07-19T15:18:02.5Z"];
    const uris = [0, 1, 2, 3, 4].map((i) =>
    ...
    const intervals = C.TimeIntervalCollection.fromIso8601DateArray({
      iso8601Dates: dates, dataCallback: (interval, index) => ({ uri: uris[index] }),
    });
    const start = C.JulianDate.fromIso8601(dates[0]);
    v.clock.startTime = start; v.clock.currentTime = start;
    v.clock.stopTime = C.JulianDate.fromIso8601(dates[dates.length - 1]);
    v.clock.clockRange = C.ClockRange.LOOP_STOP; v.clock.shouldAnimate = false;
    const pc = new C.TimeDynamicPointCloud({
      intervals, clock: v.clock,
      style: new C.Cesium3DTileStyle({ pointSize: 8 }),
    });
    s.primitives.add(pc);
    // Render until the first frame loads + boundingSphere is available.
    let bs = null, bsFrame = -1;
    for (let i = 0; i < 500; i++) {
      s.render(); await new Promise((r) => requestAnimationFrame(r));
      if (!bs && pc.boundingSphere) { bs = pc.boundingSphere; bsFrame = i; }
    ...
    if (bs) {
      ...
      v.camera.viewBoundingSphere(bs, new C.HeadingPitchRange(0.4, -0.35, bs.radius * 12.0));
      v.camera.lookAtTransform(C.Matrix4.IDENTITY);
    }
    for (let i = 0; i < 90; i++) { s.render(); await new Promise((r) => requestAnimationFrame(r)); }
    s.canvas.setAttribute("data-pc", "1");
    let mem = -1; try { mem = pc.totalMemoryUsageInBytes; } catch (e) { /* */ }
    return { ready: !!bs, bsFrame, mem, rendererType: s.context.rendererType, radius: bs ? bs.radius : 0 };
  });

  const png = await page.locator('canvas[data-pc="1"]').screenshot({ type: "png" });
...
function diffPct(a, b) {
  ...
  for (let i = 0; i < a.data.length; i += 4) {
    ...
    if (la > 24 || lb > 24) { model++; if (Math.abs(a.data[i] - b.data[i]) > 40 || Math.abs(a.data[i + 1] - b.data[i + 1]) > 40 || Math.abs(a.data[i + 2] - b.data[i + 2]) > 40) mis++; }
  }
...
if (wgpu.decoded) fs.writeFileSync("Tools/visual-regression/output/pointcloud-logdepth-webgpu.png", encodePNG(wgpu.decoded));
if (wgl.decoded) fs.writeFileSync("Tools/visual-regression/output/pointcloud-logdepth-webgl.png", encodePNG(wgl.decoded));
const dp = diffPct(wgpu.decoded, wgl.decoded);
const nonBlack = (img) => { if (!img) return 0; let n = 0; for (let i = 0; i < img.data.length; i += 4) if (img.data[i] + img.data[i + 1] + img.data[i + 2] > 24) n++; return n; };
console.log(JSON.stringify({
  webgpu: { ready: wgpu.info.ready, bsFrame: wgpu.info.bsFrame, mem: wgpu.info.mem, px: nonBlack(wgpu.decoded), errs: wgpu.errs.slice(0, 4) },
  webgl: { ready: wgl.info.ready, bsFrame: wgl.info.bsFrame, mem: wgl.info.mem, px: nonBlack(wgl.decoded) },
  diffPct: dp,
}, null, 2));
const rendered = wgpu.info.ready && nonBlack(wgpu.decoded) > 3000;
const noErrs = wgpu.errs.filter((e) => !/AtmosphereLUT|atmosphereLUT|default layout/.test(e)).length === 0;
const parity = dp !== null && dp < 8;
const pass = rendered && noErrs && parity;
console.log(`(A) WebGPU point cloud renders over globe: ${rendered ? "PASS" : "FAIL"}`);
console.log(`(B) no WebGPU device errors (excl. known AtmosphereLUT): ${noErrs ? "PASS" : "FAIL"}`);
console.log(`(C) WebGPU sorts/matches WebGL < 8% (${dp}): ${parity ? "PASS" : "FAIL"}`);
console.log(pass ? "GATE PASS — point cloud writes log depth + sorts vs globe like WebGL." : "GATE FAIL.");
