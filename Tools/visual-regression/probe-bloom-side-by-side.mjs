...
console.log('Coord                  | WebGL          | WebGPU         | unique?');
console.log('-----------------------|----------------|----------------|--------');
for (let i = 0; i < wgl.length; i++) {
  const a = wgl[i], b = wgpu[i];
  const unique = (a.r !== b.r || a.g !== b.g || a.b !== b.b) ? 'diff' : 'same';
  console.log(`${a.name.padEnd(22)} | (${a.r.toString().padStart(3)},${a.g.toString().padStart(3)},${a.b.toString().padStart(3)})  | (${b.r.toString().padStart(3)},${b.g.toString().padStart(3)},${b.b.toString().padStart(3)})  | ${unique}`);
}
