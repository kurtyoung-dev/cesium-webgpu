...
async function run(renderer) {
  ...
  const res = await page.evaluate(async () => {
    ...
    const halo =
      glowCount > 0
        ? { w: gMaxX - gMinX + 1, h: gMaxY - gMinY + 1, count: glowCount }
        : null;
    return { renderer: s.context.rendererType, canvasWH: [w, h], disc, halo };
  });

  await page.screenshot({
    path: `Tools/visual-regression/output/sun-lens-glare-${renderer}.png`,
  });
  await browser.close();
  return { res, logs };
}
...
(async () => {
  ...
  for (const r of ["webgl", "webgpu"]) {
    ...
    const bad = logs.filter(
      (l) => /error|fail|invalid|validation/i.test(l) && !/favicon/i.test(l),
    );
  ...
  const gl = out.webgl,
    gpu = out.webgpu;
  const fail = [];
  if (!gpu.disc || gpu.disc.count < 20)
    fail.push("WebGPU sun disc missing/too small");
  ...
  if (gpu.disc && gpu.halo && gpu.halo.count < gpu.disc.count * 3)
    fail.push(
      `WebGPU halo footprint too small vs disc (halo ${gpu.halo.count} vs disc ${gpu.disc.count})`,
    );
  ...
  if (gl.halo && gpu.halo) {
    const rw = gpu.halo.w / gl.halo.w,
      rh = gpu.halo.h / gl.halo.h;
    if (rw < 0.6 || rw > 1.6 || rh < 0.6 || rh > 1.6)
      fail.push(
        `Halo extent diverges from WebGL (wRatio ${rw.toFixed(2)}, hRatio ${rh.toFixed(2)})`,
      );
    console.log(
  ...
  if (fail.length === 0) {
    console.log(
      "PASS — WebGPU sun lens-glare (disc + glow halo + flare bursts) present and matches WebGL.",
    );
    process.exit(0);
  } else {
    console.log("FAIL:");
