...
async function run(renderer) {
  ...
  const res = await page.evaluate(async (renderer) => {
    ...
    let data;
    try {
      data = ctx.getImageData(cx - R, cy - R, 2 * R, 2 * R).data;
    } catch (e) {
      return { err: String(e) };
    }
    ...
    return {
      ...
      canvasWH: [w, h],
      centerMaxLum: +maxLum.toFixed(1),
      centerMeanLum: +(sumLum / n).toFixed(2),
      sunBox,
      glowBox,
      pipeInfo,
    };
  ...
  await page.screenshot({
    path: `Tools/visual-regression/output/sun-pixcheck-${renderer}.png`,
  });
  await browser.close();
  return { res, logs };
}
...
(async () => {
  for (const r of ["webgl", "webgpu"]) {
    ...
    const interesting = logs.filter((l) =>
      /error|warn|fail|invalid|pipeline|sun|Sun|validation|GPU/i.test(l),
    );
