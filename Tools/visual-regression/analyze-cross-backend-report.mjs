...
async function main() {
  ...
  for (const r of data.results) {
    ...
    if (!wglOk && !wgpuOk) {
      buckets.bothFail.push({
        demo: r.demo,
        wglErr: r.webgl?.navError,
        wgpuErr: r.webgpu?.navError,
      });
      continue;
    }
    if (wglOk && !wgpuOk) {
  ...
  const p90 = sortedDiffs[Math.floor(sortedDiffs.length * 0.9)] ?? 0;
  ...
  console.log(``);
  console.log(
    `## Canvas-render totals (does the demo render anything at all?)`,
  );
  console.log(
    `- Total demos: ${data.results.length}${data.demoCount ? ` of ${data.demoCount} expected` : ""}`,
  );
  console.log(
    `- Both backends OK: ${buckets.bothOkLowDiff.length + buckets.bothOkMediumDiff.length + buckets.bothOkHighDiff.length + buckets.webgpuPinned.length + buckets.webglPinned.length}`,
  );
  ...
  console.log(
    `- Demos with WebGL pageErrors: ${issueBuckets.webglPageErrors.length}`,
  );
  console.log(
    `- Demos with WebGPU pageErrors: ${issueBuckets.webgpuPageErrors.length}`,
  );
  console.log(
    `- Demos with WebGL console errors/warnings: ${issueBuckets.webglConsoleErrors.length}`,
  );
  console.log(
    `- Demos with WebGPU console errors/warnings: ${issueBuckets.webgpuConsoleErrors.length}`,
  );
  console.log(
    `- **Demos where ONLY WebGPU has issues** (regressions): ${issueBuckets.webgpuOnlyIssues.length}`,
  );
  console.log(``);
  ...
  function listBucket(name, items, includeDiff = true, max = 30) {
    ...
    for (const item of sorted.slice(0, max)) {
      const diff =
        includeDiff && item.diffPct !== undefined
  ...
  console.log(
    `## WebGPU-only issues (regressions vs WebGL — highest priority) (${issueBuckets.webgpuOnlyIssues.length})`,
  );
  if (issueBuckets.webgpuOnlyIssues.length === 0) {
    console.log(`(none)\n`);
  } else {
    for (const item of issueBuckets.webgpuOnlyIssues.slice(0, 50)) {
      const errs = (item.wgpuPageErrors || [])
        .slice(0, 2)
        .map((e) => e.substring(0, 200));
      const msgs = (item.wgpuConsoleMessages || [])
        .slice(0, 3)
        .map((m) => m.substring(0, 200));
      console.log(
        `- ${item.demo} — console=${item.wgpuConsoleCount} pageErrors=${item.wgpuPageErrors.length}`,
      );
  ...
  listBucket(
    "WebGL-pinned demos (both runs WebGL — sync constructor only)",
    buckets.webglPinned,
  );
}
