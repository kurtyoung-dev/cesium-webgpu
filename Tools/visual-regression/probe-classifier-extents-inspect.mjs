...
const info = await p.evaluate(async () => {
  ...
  const material = new C.Material({
    fabric: {
      type: "Stripe",
      uniforms: {
        evenColor: new C.Color(1, 0.05, 0.05, 1),
        oddColor: new C.Color(0.05, 0.05, 1, 1),
        repeat: 10,
        horizontal: true,
      },
    },
  ...
  const out = {
    frames: f,
    ready: ground.ready,
    appearanceMatType: ground.appearance?.material?.type,
  };
  ...
  out.haveBatchTable = !!bt;
  out.haveIndices = !!idx;
  if (idx) {
    out.indexKeys = Object.keys(idx);
    out.planar = {
      southWest_HIGH: idx.southWest_HIGH,
      southWest_LOW: idx.southWest_LOW,
      eastward: idx.eastward,
      northward: idx.northward,
    };
    out.spherical = {
      sphericalExtents: idx.sphericalExtents,
      planar2D_HIGH: idx.planar2D_HIGH,
      planar2D_LOW: idx.planar2D_LOW,
    };
  }
