  ...
  const dump = await page.evaluate(
    async ({ view }) => {
      ...
      for (const t of tiles) {
        ...
        else if (t.y === polarTile.y && t.level > polarTile.level)
          polarTile = t;
      }
      ...
      if (center && viewProjArr) {
        const m = viewProjArr;
        const wx = m[0] * center.x + m[4] * center.y + m[8] * center.z + m[12];
        const wy = m[1] * center.x + m[5] * center.y + m[9] * center.z + m[13];
        const wz = m[2] * center.x + m[6] * center.y + m[10] * center.z + m[14];
        const ww = m[3] * center.x + m[7] * center.y + m[11] * center.z + m[15];
      ...
      return {
        ...
        meshCenter: center ? [center.x, center.y, center.z] : null,
...
(async () => {
  ...
  if (wgl.vertexSample && wgpu.vertexSample) {
    ...
    console.log(
      identical
        ? "  All sampled vertex bytes IDENTICAL"
        : "  Vertex data DIFFERS (see diffs above)",
    );
  }
  console.log();
