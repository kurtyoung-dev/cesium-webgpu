...
async function capture(rendererArg, mode) {
  ...
  const stats = await page.evaluate(
    async ({ modeId, lon, lat }) => {
      ...
      v.entities.add({
        ...
        label: { text: "TEST", fillColor: C.Color.LIME, font: "32px sans-serif" },
      });

      if (modeId === 1) scene.morphToColumbusView(0);
      else if (modeId === 2) scene.morphTo2D(0);
      else scene.morphTo3D(0);
      v.camera.setView({ destination: C.Cartesian3.fromDegrees(lon, lat, 1200000.0) });
      for (let i = 0; i < 150; i++) { scene.render(); await new Promise((r) => requestAnimationFrame(r)); }
      v.camera.setView({ destination: C.Cartesian3.fromDegrees(lon, lat, 1200000.0) });
      ...
      return { mode: scene.mode, magenta, yellow, lime, dataUrl: c.toDataURL("image/png") };
    },
  ...
  await browser.close();
  console.log(`  [${rendererArg}/${mode.name}] bb=${stats.magenta} pt=${stats.yellow} lbl=${stats.lime} errs=${errors.length}`);
...
(async () => {
  ...
  console.log("[probe-collections-entity] done — READ the PNGs (collent-*.png)");
})();
