...
async function measure(rendererArg, debugMode, altitude) {
  ...
  await page.evaluate(async ({ debugMode, altitude }) => {
...
(async () => {
  ...
  for (const altitude of [12_000_000, 20_000_000]) {
    ...
    console.log(`  WebGL  final               : ${wglFinal.meanRgb.toFixed(2)}`);
    const wgpuFinal = await measure("webgpu", null, altitude);
    console.log(`  WebGPU final               : ${wgpuFinal.meanRgb.toFixed(2)}`);
    const wgpuPostComp = await measure("webgpu", "post-composite-color", altitude);
    console.log(`  WebGPU post-composite-color: ${wgpuPostComp.meanRgb.toFixed(2)}`);
    const wgpuSample0 = await measure("webgpu", "sample0", altitude);
    console.log(`  WebGPU sample0 (raw layer 0): ${wgpuSample0.meanRgb.toFixed(2)}`);
  }

  console.log(`\nInterpretation:`);
  console.log(`  - If WebGPU post-comp matches WebGL final, bug is downstream of composite.`);
  console.log(`  - If WebGPU post-comp is dim, bug is in composite.`);
  console.log(`  - If WebGPU sample0 is dim, bug is at upload / sample / mipmap.`);
})();
