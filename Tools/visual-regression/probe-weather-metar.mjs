...
// FEW @ 120. All UNCHANGED from the pre-pinning probe.
...
const PIN = {
  ...
  bandLons: BAND_LONS,
  // The strength-1 sweep covers the band PLUS the clear-station sample, so one
  // determinism control covers every scored strength-1 capture.
  strengthOneLons: [CLEAR_LON, ...BAND_LONS],
  lat: 35.0,
...
const RUN_LANE = async (cfg) => {
  ...
  const allLons = [...new Set([...cfg.strengthOneLons, ...cfg.bandLons])];
  ...
  // Order is deliberate: ch1A -> ch0 -> ch1B, so the ch1A/ch1B determinism
  // control BRACKETS the gated sweep in time and spans BOTH strength changes.
  // A control taken back to back before the gated leg would only prove the
  // instrument was stable across a gap that gate 4 never reads across.
  const ch1A = await sweep("ch1A", cfg.strengthOneLons, [
    cfg.clearLon,
    cfg.cloudyLon,
  ]);
  ...
  await pin.settle(readyTime, cfg.sourceSettleMs);
  const ch0 = await sweep("ch0", cfg.bandLons, [cfg.cloudyLon]);

  const configuredBackOn = configure(1.0);
  await pin.settle(readyTime, cfg.sourceSettleMs);
  const ch1B = await sweep("ch1B", cfg.strengthOneLons, []);
...
async function run() {
  ...
  // Gate 4 reads the BAND only, exactly as before.
  const ch1Fr = BAND_LONS.map((lon) => byLon(ch1A, lon).frac);
  const ch0Fr = BAND_LONS.map((lon) => byLon(ch0, lon).frac);
  ...
  writePng(cloudy, "weather-metar-channels-on.png");
  writePng(byLon(ch0, CLOUDY_LON), "weather-metar-channels-off.png");
  ...
  const checks = [
    ...
    [
      `G/B/A channels reach the shader: turning them ON shifts the deck per ` +
        `location (sum|delta| ${absDeltaSum} >= ${ASSERT.absDeltaSumMin}, mean shift ${meanShift})`,
