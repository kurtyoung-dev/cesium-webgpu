...
test("renderer preserves the WGS84 rows and appends the C13-37 phase layout", () => {
  ...
  // C13-16 appended one 16-byte per-genus morphology row at the tail, and
  // C13-N10 (2026-09-12) appended a second, the tier lighting row at 172-175.
  // The WGS84 rows this test guards keep their offsets; only the total grows.
  assert.match(
    renderer,
