...
async function setup(page, hdr) {
  return await page.evaluate(async ({ hdr }) => {
    ...
    try { if (s.postProcessStages && s.postProcessStages.bloom) s.postProcessStages.bloom.enabled = !!hdr; } catch (e) { /* noop */ }
...
(async () => {
  ...
  await settle(gpu.page, 500); await settle(gl.page, 300);
