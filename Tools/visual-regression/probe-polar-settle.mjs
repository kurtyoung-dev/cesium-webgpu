...
async function capture(renderer, settleFrames) {
  ...
  const stats = await page.evaluate(async ({ settleFrames }) => {
  ...
  const out = path.join(OUT_DIR, `polar-settle-${renderer}-${settleFrames}.png`);
...
(async () => {
  ...
  for (const frames of [120, 600, 2400]) {
    ...
    console.log(`  webgpu @ ${frames} frames: tilesLoadedAt=${wgpu.tilesLoadedAt} tiles=${wgpu.tileCount} loaded=${wgpu.finalTilesLoaded}`);
  }
})();
