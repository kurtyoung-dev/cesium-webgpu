...
async function measure(rendererArg, distance) {
  ...
  const out = path.join(OUT_DIR, `brightness-noatmo-${distance.name}-${rendererArg}.png`);
  ...
  const stats = await page.evaluate(async (b64) => {
    ...
    let rSum = 0, gSum = 0, bSum = 0, n = 0, globePx = 0;
    for (let y = y0; y < y1; y++) {
      for (let x = x0; x < x1; x++) {
        const i = (y * w + x) * 4;
...
(async () => {
  ...
  console.log(`[probe-brightness-no-atmo] WebGL vs WebGPU (showGroundAtmosphere = false)\n`);
  console.log(`${"distance".padEnd(12)} ${"WebGL".padStart(8)} ${"WebGPU".padStart(8)} ${"ratio".padStart(8)}`);
  ...
  for (const d of DISTANCES) {
    ...
    console.log(`  ${d.name.padEnd(12)} ${wgl.meanRgb.toFixed(2).padStart(8)} ${wgpu.meanRgb.toFixed(2).padStart(8)} ${ratio.toFixed(3).padStart(8)}`);
  }
  const avgRatio = rows.reduce((s, r) => s + r.ratio, 0) / rows.length;
  console.log(`\nAverage ratio (atmosphere OFF): ${avgRatio.toFixed(3)}`);
  console.log(`If this is near 1.0, the brightness gap is in the drape branch.`);
