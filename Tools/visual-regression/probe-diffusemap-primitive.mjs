...
function diffPct(a, b) {
  ...
  for (let i = 0; i < a.data.length; i += 4) {
    ...
    if (la > 24 || lb > 24) {
      model++;
      if (
        Math.abs(a.data[i] - b.data[i]) > 32 ||
        Math.abs(a.data[i + 1] - b.data[i + 1]) > 32 ||
        Math.abs(a.data[i + 2] - b.data[i + 2]) > 32
      )
        mis++;
    }
  }
...
if (wgpu.decoded?.b64)
  fs.writeFileSync(
    "Tools/visual-regression/output/diffusemap-webgpu.png",
    Buffer.from(wgpu.decoded.b64, "base64"),
  );
if (wgl.decoded?.b64)
  fs.writeFileSync(
    "Tools/visual-regression/output/diffusemap-webgl.png",
...
console.log(
  `(A) WebGPU DiffuseMap shader renders the geometry (px>8000): ${renders ? "PASS" : "FAIL"}`,
);
console.log(`(B) no WebGPU device errors: ${noErrors ? "PASS" : "FAIL"}`);
console.log(
  `(C) no UBO-corruption garbage (colorful<200, uniform fill): ${noGarbage ? "PASS" : "FAIL"}`,
);
console.log(
  pass
