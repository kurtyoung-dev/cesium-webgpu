...
const radio = await page.locator(`input[type="radio"][value="${radioVal}"]`).first();
...
fs.writeFileSync(`Tools/visual-regression/output/probe-${outName}-${renderer}.png`, await page.screenshot({ type: 'png' }));
...
const matState = await page.evaluate(() => {
  ...
  return {
    ...
    imageHasWebGPU: img && img._webgpuTexture ? !!img._webgpuTexture.view : false,
...
const errs = msgs.filter(m => m.includes('[error]') || m.includes('PAGEERROR') || m.toLowerCase().includes('compilation') || m.toLowerCase().includes('shader '));
const wgpuErrs = msgs.filter(m => m.toLowerCase().includes('webgpu') && (m.includes('error') || m.includes('warning')));
console.log(`msgs=${msgs.length} errs=${errs.length} wgpu-errs/warns=${wgpuErrs.length}`);
console.log('\n=== Errors / Compilation ===');
errs.slice(0, 15).forEach(e => console.log(' ', e.substring(0, 500)));
const allErrs = msgs.filter(m => m.includes('material') || m.includes('shader') || m.includes('Shader') || m.includes('compilation'));
