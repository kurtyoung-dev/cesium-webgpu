  ...
  try {
    ...
    const metric = await page.evaluate(async (roi) => {
      ...
      const center = C.Matrix4.multiplyByPoint(
        model.modelMatrix, new C.Cartesian3(0, 0, 0), new C.Cartesian3());
      ...
      for (let i = 0; i < 24; i++) { scene.render(); await new Promise((r)=>requestAnimationFrame(r)); }
...
const report = {
  mode: MODE,
  webgl: { ready: webgl.ready, coveredPx: webgl.coveredPx, coveredMeanLum: +webgl.coveredMeanLum.toFixed(2), hfEnergy: +webgl.hfEnergy.toFixed(3), faults: webgl.faults },
  webgpu: { ready: webgpu.ready, coveredPx: webgpu.coveredPx, coveredMeanLum: +webgpu.coveredMeanLum.toFixed(2), hfEnergy: +webgpu.hfEnergy.toFixed(3), faults: webgpu.faults },
  webgpuHfVsWebglRatio: +(webgpu.hfEnergy / Math.max(1e-6, webgl.hfEnergy)).toFixed(3),
};
