...
async function capture(rendererArg, target) {
  ...
  const result = await page.evaluate(
    async (call) => {
      ...
      return {
        ...
        samples: samples.filter((_, i) => i % Math.max(1, Math.floor(samples.length / 8)) === 0),
      };
    },
    target.call,
  );
  ...
  if (result.midDataUrl) {
    ...
    fs.writeFileSync(out, Buffer.from(result.midDataUrl.split(",")[1], "base64"));
  } else {
  ...
  const fatal = [...consoleErrors, ...gate.errors, ...(gate.deviceLost ? [gate.deviceLost] : [])];
  ...
  console.log(`  samples(mt:pct): ${result.samples.map((s) => `${s.mt}:${s.pct}`).join("  ")}`);
