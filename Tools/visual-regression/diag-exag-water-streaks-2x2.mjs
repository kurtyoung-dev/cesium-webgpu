...
async function capture(renderer, killAtmosphere) {
  ...
  const res = await page.evaluate(async (killAtmo) => {
    const v = window.viewer, s = v.scene, c = v.camera;
    ...
    const dest = ell.cartographicToCartesian({
      longitude: (86.9 * Math.PI) / 180, latitude: (27.0 * Math.PI) / 180, height: 250000,
    });
    const setIt = () => c.setView({ destination: dest, orientation: { heading: 0, pitch: -0.45, roll: 0 } });
    setIt();
    for (let i = 0; i < 320; i++) { s.initializeFrame(); s.render(); await new Promise((r) => requestAnimationFrame(r)); }
    setIt();
    for (let i = 0; i < 60; i++) { s.initializeFrame(); s.render(); await new Promise((r) => requestAnimationFrame(r)); }
    ...
    return { dataUrl: s.canvas.toDataURL("image/png"), tileCount: levels.length, lvlHist,
      fade: (() => { try { return s.globe._surface.tileProvider.lightingFadeOutDistance; } catch (e) { return null; } })() };
  }, killAtmosphere);
  writeFileSync(join(OUT, `exag2x2-${renderer}-${killAtmosphere ? "off" : "on"}.png`), Buffer.from(res.dataUrl.split(",")[1], "base64"));
  ...
  const stat = await page.evaluate(async (b64) => {
    ...
    for (let i = 0; i < d.length; i += 4) {
      ...
      if (B > 90 && B - R > 25 && B - G > 10) { n++; sr += R; sg += G; sb += B; }
    }
    return { blueStreakPx: n, meanR: n ? +(sr / n).toFixed(1) : 0, meanG: n ? +(sg / n).toFixed(1) : 0, meanB: n ? +(sb / n).toFixed(1) : 0 };
  }, res.dataUrl.split(",")[1]);
  await page.close();
...
for (const r of ["webgl", "webgpu"]) {
  for (const killAtmo of [false, true]) {
    ...
    console.log(`${key}: blueStreakPx=${grid[key].blueStreakPx} meanRGB=(${grid[key].meanR},${grid[key].meanG},${grid[key].meanB}) tiles=${grid[key].tileCount} fade=${grid[key].fade} lvlHist=${JSON.stringify(grid[key].lvlHist)}`);
  }
}
await browser.close();
console.log("\n=== BRANCH ANALYSIS ===");
console.log(`webgl atmo mutes lakes? on=${grid["webgl-on"].blueStreakPx} off=${grid["webgl-off"].blueStreakPx} (delta ${grid["webgl-off"].blueStreakPx - grid["webgl-on"].blueStreakPx})`);
console.log(`webgpu atmo mutes lakes? on=${grid["webgpu-on"].blueStreakPx} off=${grid["webgpu-off"].blueStreakPx}`);
console.log(`cross (atmo OFF both): webgl=${grid["webgl-off"].blueStreakPx} webgpu=${grid["webgpu-off"].blueStreakPx}`);
console.log(`tile counts: webgl=${grid["webgl-on"].tileCount} webgpu=${grid["webgpu-on"].tileCount} (LOD divergence if very different)`);
