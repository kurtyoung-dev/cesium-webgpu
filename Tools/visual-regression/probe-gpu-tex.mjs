...
(async () => {
  ...
  const result = await page.evaluate(async () => {
    ...
    v.camera.setView({ destination: C.Cartesian3.fromDegrees(-75.5, 40.0, 5_000_000) });
    ...
    const fr = ctx.getFeatureRenderer?.(0);  // GLOBE_SURFACE = 0
    let cacheEntries = [];
    let renderer = null;
    ...
    for (const k of probeKeys) {
      if (window[k]) { glob = window[k]; break; }
    }
