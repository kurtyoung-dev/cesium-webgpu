...
async function capture(renderer, logDepthOn) {
  ...
  p.on("console", (m) => {
    if (m.type() === "error") errs.push("ERR:" + m.text().slice(0, 120));
  });
  await p.goto(`${BASE}/Apps/CesiumViewer/index.html?renderer=${renderer}`, {
    ...
    async ({ build, logDepthOn }) => {
      ...
      if (dev)
        dev.onuncapturederror = (e) =>
          de.push(String(e?.error?.message ?? "").slice(0, 160));
      ...
      return {
        frames,
        tilesLoaded: s.globe.tilesLoaded,
        ready: ground.ready,
        deviceErrs: de.length,
        deviceErrSample: de.slice(0, 2),
      };
    },
      ...
      new Promise((res) => {
        ...
        img.onload = () => {
          ...
          for (let y = 250; y < 350; y++)
            for (let x = 320; x < 460; x++) {
              const i = (y * c.width + x) * 4;
              if (d[i] + d[i + 1] + d[i + 2] < 160) continue;
              n++;
              r1 += d[i];
              g1 += d[i + 1];
              b1 += d[i + 2];
