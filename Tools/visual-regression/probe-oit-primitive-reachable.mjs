 ...
 *        node Tools/visual-regression/probe-oit-primitive-reachable.mjs
 *   SCENE=lit|flat selects one scene (default runs both).
 * Out:   Tools/visual-regression/output/oitprim-*.png + oitprim-report.json +
 *        oitprim-runtime.json + oitprim-summary.md
 */
...
async function runScene(page, consoleErrors, sceneKind, outputDirectory) {
  ...
  fs.writeFileSync(
    path.join(outputDirectory, `oitprim-${sceneKind}-off.png`),
  ...
  fs.writeFileSync(
    path.join(outputDirectory, `oitprim-${sceneKind}-on.png`),
...
export const descriptor = {
  name: "oitprim",
  title: "OIT primitive reachability (C11-157 Slice A)",
  outputSubdirectory: "",
  ...
  async cells({ browser, origin, outputDirectory, options }) {
    if (options.renderers.length !== 1 || options.renderers[0] !== "webgpu") {
      throw new ProbeRefusal(
        "renderer-not-webgpu",
        `probe-oit-primitive-reachable only measures webgpu (the scene navigates ?renderer=webgpu unconditionally); got --renderer ${options.renderers.join(",")}`,
    ...
    if (options.runs !== 1) {
      ...
      throw new ProbeRefusal(
        ...
        `probe-oit-primitive-reachable's receipt keys cells by sceneKind, so --runs ${options.runs} would silently drop every run but the last; pass --runs 1 (the default)`,
        { runs: options.runs },
      );
    }
    ...
    // Machine safety: one scene per invocation keeps each Edge session short
    // and bounded. `SCENE=lit|flat` selects one; default runs both.
      ...
      watchdogTimer = setTimeout(
        () =>
          reject(
            new ProbeRefusal(
              "watchdog-timeout",
              `probe-oit-primitive-reachable exceeded its ${WATCHDOG_BUDGET_MS}ms machine-safety budget`,
  ...
  verdicts(cells) {
    return cells.map((cell) => ({
      id: cell.sceneKind,
      claim:
        "C11-157 Slice A — the translucent primitive reaches WebGPU MRT-OIT accumulation",
      pass: cell.pass,
    }));
  },
  summary(receipt) {
    ...
    return [
      "# OIT primitive reachability (C11-157 Slice A)",
      "",
      `Base: \`${receipt.base}\``,
      "",
