...
if (wgpu.decoded?.b64)
  fs.writeFileSync(
    "Tools/visual-regression/output/unlit-vertexcolor-webgpu.png",
    Buffer.from(wgpu.decoded.b64, "base64"),
  );
if (wgl.decoded?.b64)
  fs.writeFileSync(
    "Tools/visual-regression/output/unlit-vertexcolor-webgl.png",
...
const dChan = Math.max(
  Math.abs(mGpu.r - mGl.r),
  Math.abs(mGpu.g - mGl.g),
  Math.abs(mGpu.b - mGl.b),
);
...
console.log(
  `(A) WebGPU unlit NOT black (px>20000 && lum>30): ${notBlack ? "PASS" : "FAIL"}`,
);
console.log(
  `(B) parity maxChannelDelta < 25 (${dChan.toFixed(1)}): ${parity ? "PASS" : "FAIL"}`,
);
console.log(
  pass
