...
async function dumpSourcePixels(renderer) {
  ...
  const result = await page.evaluate(async ({ view }) => {
    ...
    if (!targetImagery) return { error: "no polar tile with source mercator texture" };
    ...
    if (typeof v.scene.context._gl !== "undefined" && targetImagery.textureWebMercator) {
...
(async () => {
  console.log("[source-mercator-compare] dumping the SOURCE mercator texture pixels");
  console.log();
  ...
  if (wgl.webglSource) {
    ...
    console.log(`  size=${wgl.webglSource.width}x${wgl.webglSource.height} read=${wgl.webglSource.readOrigin}`);
    console.log(`  mean RGB=[${wgl.webglSource.pixelMean.map(x => x.toFixed(1)).join(", ")}]`);
  }
  if (wgpu.webgpuSource) {
    console.log("=== WebGPU _webgpuMercatorTexture pixels ===");
    console.log(`  size=${wgpu.webgpuSource.width}x${wgpu.webgpuSource.height} read=${wgpu.webgpuSource.readOrigin}`);
    console.log(`  mean RGB=[${wgpu.webgpuSource.pixelMean.map(x => x.toFixed(1)).join(", ")}]`);
  }
  ...
  if (wgl.webglSource && wgpu.webgpuSource) {
    ...
    console.log("    - WebGPU: pre-flipped ImageBitmap + top-down copyToBuffer");
