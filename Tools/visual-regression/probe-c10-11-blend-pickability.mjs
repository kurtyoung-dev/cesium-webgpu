...
const LON = -75, LAT = 40, CAM_H = 9000;
const browser = await chromium.launch({ channel: "msedge", headless: true, args: ["--enable-unsafe-webgpu"] });
const page = await browser.newPage({ viewport: { width: 1000, height: 700 } });
const errors = [];
page.on("console", (m) => { if (m.type() === "error") errors.push(m.text().slice(0, 200)); });
page.on("pageerror", (e) => errors.push(`PAGEERROR: ${e.message.slice(0, 200)}`));
await page.goto(`${BASE}/Apps/CesiumViewer/index.html?renderer=webgpu`, { waitUntil: "networkidle", timeout: 90000 });
await page.waitForFunction(() => !!window.viewer, { timeout: 90000 });

const out = await page.evaluate(async ({ LON, LAT, CAM_H }) => {
  const C = await import("/Build/CesiumUnminified/index.js");
  const v = window.viewer; const scene = v.scene; const ctx = scene.context;
  scene.globe.show = false;
  if (scene.skyBox) scene.skyBox.show = false;
  if (scene.skyAtmosphere) scene.skyAtmosphere.show = false;
  const errs = [];
  const dev = ctx?._device;
  if (dev) dev.onuncapturederror = (ev) => errs.push(String(ev?.error?.message).slice(0, 200));

  v.camera.setView({ destination: C.Cartesian3.fromDegrees(LON, LAT, CAM_H), orientation: { heading: 0, pitch: -Math.PI / 2, roll: 0 } });
  const renderN = async (n) => { for (let i = 0; i < n; i++) { scene.render(); await new Promise((r) => requestAnimationFrame(r)); } };

  // Opaque box (left).
  const boxInst = new C.GeometryInstance({
    geometry: C.BoxGeometry.fromDimensions({ vertexFormat: C.PerInstanceColorAppearance.VERTEX_FORMAT, dimensions: new C.Cartesian3(800, 800, 800) }),
    modelMatrix: C.Transforms.eastNorthUpToFixedFrame(C.Cartesian3.fromDegrees(LON - 0.02, LAT, 0)),
    attributes: { color: C.ColorGeometryInstanceAttribute.fromColor(C.Color.ORANGE) },
    id: "opaque-box",
  });
  const boxPrim = new C.Primitive({ geometryInstances: boxInst, appearance: new C.PerInstanceColorAppearance({ closed: true, translucent: false }), asynchronous: false, allowPicking: true });
  ...
  try {
    model = await C.Model.fromGltfAsync({
      ...
      modelMatrix: C.Transforms.eastNorthUpToFixedFrame(C.Cartesian3.fromDegrees(LON + 0.02, LAT, 0)),
      scale: 400, id: "translucent-model",
    });
    model.color = C.Color.WHITE.withAlpha(0.4);
    model.colorBlendMode = C.ColorBlendMode.MIX;
    scene.primitives.add(model);
  } catch (e) { return { fatal: `model load: ${e}` }; }
  for (let i = 0; i < 240; i++) { scene.render(); await new Promise((r) => setTimeout(r, 8)); if (model.ready) break; }
  await renderN(40);

  const boxWin = C.SceneTransforms.worldToWindowCoordinates(scene, C.Cartesian3.fromDegrees(LON - 0.02, LAT, 400));
  const mdWin = C.SceneTransforms.worldToWindowCoordinates(scene, C.Cartesian3.fromDegrees(LON + 0.02, LAT, 500));
  const pickConverge = async (win, wantPred) => {
    ...
    for (let i = 0; i < 18; i++) {
      ...
      if (C.defined(p)) { last = { hit: true, ok: wantPred(p), id: (typeof p.id === "string" ? p.id : (p.id?.id ?? null)), isModel: p.primitive === model || p.detail?.model === model }; if (last.ok) return last; }
  ...
  const runGate = async (gate) => {
    ...
    await renderN(24);
    const box = await pickConverge(boxWin, (p) => (typeof p.id === "string" ? p.id : p.id?.id) === "opaque-box");
    const md = await pickConverge(mdWin, (p) => p.primitive === model || p.detail?.model === model);
    return { gate, box, md };
  };
  ...
  await runGate(true); await runGate(false); await runGate(true);
  const off = await runGate(false);
  ctx._pickLogDepthWriteEnabled = true; await renderN(10);
...
console.log("=== C10-11 BLEND pickability (translucent Model + opaque both pickable under switch) ===");
...
if (errTotal) console.log("errors:", errTotal, errors.slice(0, 4), (out.errs || []).slice(0, 4));
console.log(`  blend(model)+opaque pickable: OFF=${offOk} ON=${onOk} (switch-independent blend behavior)`);
