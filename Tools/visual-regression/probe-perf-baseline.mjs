...
const FPS_WINDOW_MS = 6000, SETTLE_MS = 3000, HARD_TIMEOUT_MS = 45000;
...
async function measure(browser, renderer) {
  ...
  const t0 = Date.now();
  await page.goto(`${BASE}?renderer=${renderer}`, { waitUntil: "load", timeout: 30000 });
  const timings = await page.evaluate(async ({ hardTimeout }) => {
    const start = performance.now();
    const v = () => window.viewer;
    const waitFor = (pred, ms) => new Promise((resolve) => {
      const s = performance.now();
      const tick = () => { let ok = false; try { ok = pred(); } catch { ok = false; }
    ...
    if (v() && v().scene) {
      await new Promise((resolve) => {
        const rm = v().scene.postRender.addEventListener(() => { firstFrame = performance.now() - start; rm(); resolve(); });
        setTimeout(resolve, hardTimeout);
      });
    }
    const tilesOk = await waitFor(() => v() && v().scene && v().scene.globe && v().scene.globe.tilesLoaded === true, hardTimeout);
    return { firstFrameMs: firstFrame, tilesLoadedMs: tilesOk ? performance.now() - start : -1,
      backend: (v() && v().scene && v().scene.context && (v().scene.context.isWebGPU ? "webgpu" : "webgl")) || "?" };
  }, { hardTimeout: HARD_TIMEOUT_MS });
  await page.evaluate((settle) => new Promise((r) => setTimeout(r, settle)), SETTLE_MS);
  const fps = await page.evaluate(async ({ windowMs }) => {
    const v = window.viewer;
    if (!v || !v.scene) return { frames: 0, ms: windowMs, fps: 0 };
    v.scene.requestRenderMode = false;
    let frames = 0;
    const spin = v.scene.postRender.addEventListener(() => { frames++; try { v.camera.rotateRight(0.002); } catch { /**/ } });
    const start = performance.now();
  ...
  await page.close();
  return { renderer, ...timings, ...fps, totalProbeMs: Date.now() - t0, errors: errors.slice(0, 3) };
}
...
const g = results.webgl, w = results.webgpu, fmt = (n) => (typeof n === "number" ? n.toFixed(1) : String(n));
console.log("\n=== PERF BASELINE (default globe) ===");
for (const [k, r] of [["WebGL", g], ["WebGPU", w]]) {
...
if (g.fps > 0 && w.fps > 0) {
  ...
  console.log(`Load delta: WebGPU ${fmt(w.tilesLoadedMs - g.tilesLoadedMs)}ms slower to tilesLoaded`);
}
