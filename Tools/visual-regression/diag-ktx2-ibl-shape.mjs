...
const out = await page.evaluate(async ({ modelUrl, ktx2 }) => {
  const C = await import("/Build/CesiumUnminified/index.js");
  const v = window.viewer, s = v.scene;
  s.globe.show = false; s.skyBox.show = false; s.backgroundColor = C.Color.BLACK;
  // Direct loadKTX2 to inspect the raw resolved shape.
  let rawShape = null;
  try {
    const buffers = await C.loadKTX2(ktx2);
    const describe = (x) => x && typeof x === "object"
      ? { keys: Object.keys(x).slice(0, 12), isArray: Array.isArray(x), len: Array.isArray(x) ? x.length : undefined } : { typeof: typeof x };
    rawShape = {
      top: describe(buffers),
      el0: Array.isArray(buffers) ? describe(buffers[0]) : describe(buffers?.positiveX),
      px: Array.isArray(buffers)
        ? describe(buffers[0]?.positiveX)
        : (Array.isArray(buffers?.positiveX) ? describe(buffers.positiveX[0]) : describe(buffers?.positiveX)),
      pxFields: (() => {
        const p = Array.isArray(buffers) ? buffers[0]?.positiveX : (Array.isArray(buffers?.positiveX) ? buffers.positiveX[0] : buffers?.positiveX);
        if (!p) return null;
        return { width: p.width, height: p.height, hasABV: !!p.arrayBufferView, abvCtor: p.arrayBufferView?.constructor?.name, abvLen: p.arrayBufferView?.byteLength, pixelDatatype: p.pixelDatatype, internalFormat: p.internalFormat };
      })(),
    };
  } catch (e) { rawShape = { error: String(e) }; }
  ...
  for (let i = 0; i < 600 && !model.ready; i++) { s.render(); await new Promise((r) => requestAnimationFrame(r)); }
  for (let i = 0; i < 320; i++) { s.render(); await new Promise((r) => requestAnimationFrame(r)); }
