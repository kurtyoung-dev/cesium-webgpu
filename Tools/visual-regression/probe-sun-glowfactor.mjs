...
async function run(renderer, glowFactor) {
  ...
  const res = await page.evaluate(async ({ renderer, glowFactor }) => {
    ...
    s.sun.show = true; s.sun.glowFactor = glowFactor;
    if (s.skyBox) s.skyBox.show = false; s.backgroundColor = C.Color.BLACK;
    ...
    const cv = s.canvas; const w = cv.width, h = cv.height;
    const tmp = document.createElement("canvas"); tmp.width=w; tmp.height=h;
    const ctx = tmp.getContext("2d"); ctx.drawImage(cv, 0, 0);
    let full;
    try { full = ctx.getImageData(0,0,w,h).data; } catch(e){ return {err:String(e)}; }
    const box = (thresh) => {
      ...
      for (let y=0;y<h;y++) for (let x=0;x<w;x++){
        ...
        if (l>=thresh){ count++; if(x<minX)minX=x; if(x>maxX)maxX=x; if(y<minY)minY=y; if(y>maxY)maxY=y; }
      }
      return count>0 ? { w:maxX-minX+1, h:maxY-minY+1, count } : null;
    };
    return { renderer:s.context.rendererType, glowFactor: s.sun.glowFactor,
      sunBox: box(200), glowBox: box(40), useHDR: s.highDynamicRange };
  }, { renderer, glowFactor });

  await page.screenshot({ path: `Tools/visual-regression/output/sun-glow-${renderer}-gf${glowFactor}.png` });
  await browser.close();
  return { res, logs };
}
...
(async () => {
  ...
  for (const r of ["webgl","webgpu"]) {
    for (const gf of [1.0, 4.0]) {
      ...
      const err = logs.filter((l)=>/\[error\]|\[pageerror\]|invalid|validation|fail/i.test(l));
      if (err.length) { console.log("errors:", err.length); err.slice(0,6).forEach((l)=>console.log("  "+l)); }
    }
  }
  // Parity assessment.
  const wgpu1 = results["webgpu-gf1"].glowBox, wgpu4 = results["webgpu-gf4"].glowBox;
  const wgl1 = results["webgl-gf1"].glowBox, wgl4 = results["webgl-gf4"].glowBox;
  console.log("\n=== ASSESSMENT ===");
  console.log("WebGPU glow grows with glowFactor:", wgpu1 && wgpu4 ? wgpu4.w > wgpu1.w : "n/a",
    `(gf1 w=${wgpu1?.w} -> gf4 w=${wgpu4?.w})`);
  console.log("WebGL  glow grows with glowFactor:", wgl1 && wgl4 ? wgl4.w > wgl1.w : "n/a",
