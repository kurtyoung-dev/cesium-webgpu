...
const state = await page.evaluate(() => {
  ...
  return {
    ...
    pp_bloom_enabled: pp.bloom?.enabled,
    pp_ao_enabled: pp.ambientOcclusion?.enabled,
    wg_tonemap_enabled: wgppp?._tonemapStage?.enabled,
    wg_fxaa_enabled: wgppp?._fxaaStage?.enabled,
    wg_colorGrading_enabled: wgppp?._colorGradingStage?.enabled,
    wg_autoExposure_enabled: wgppp?._autoExposure?.enabled,
    wg_taaEffect_enabled: wgppp?._taaEffect?.enabled,
    wg_bloomEffect_enabled: wgppp?._bloomEffect?.enabled,
    wg_godRayEffect_enabled: wgppp?._godRayEffect?.enabled,
    wg_aoEffect_enabled: wgppp?._aoEffect?.enabled,
    hdr: v.scene._hdr,
  };
});
await browser.close();
