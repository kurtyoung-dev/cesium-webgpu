...
export const C12_29_S5_DENSE_CONFIG = Object.freeze({
  ...
  exitCodes: Object.freeze({ PASS: 0, FAIL: 1, ERROR: 2, STRUCTURAL: 3 }),
});
...
function denseSha256(bytes) {
  ...
  // Portable synchronous SHA-256 for pure Node/browser fold verification.
...
export function exitCodeForC1229S5DenseStatus(status) {
  return C12_29_S5_DENSE_CONFIG.exitCodes[status] ?? 3;
}
...
function provenanceSnapshotReasons(snapshot, label, contract) {
  ...
  if (
    ...
    snapshot.identitySha256 !== denseSha256(stableC1229S5DenseJson(identity))
  ) {
...
function foldDenseCostGate(report, contract) {
  ...
  if (
    ...
    denseSha256(stableC1229S5DenseJson(report?.prerequisites))
  ) {
