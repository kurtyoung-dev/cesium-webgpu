/**
 * @purpose Radially averaged power-spectrum slope fit and area-perimeter fractal dimension for scalar fields, with a seeded synthetic fBm generator for validating them against a known answer.
 * @status ACTIVE
 *
 * The 2-D transform here is a hand-rolled separable DFT, not an FFT: O(N^2)
 * per 1-D pass, O(N^3) for a square field. That is sized for the 64-256px
 * fields this is exercised on, and is the wrong tool for a full-resolution
 * capture frame.
 */
