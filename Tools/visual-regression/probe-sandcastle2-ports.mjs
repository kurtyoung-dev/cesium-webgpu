...
try {
  ...
  for (const port of ports) {
    ...
    page.on("console", (m) => {
      const t = m.text();
      if (msgs.length < 40 && (m.type() === "error" || /postMessage|origin|bucket|runComplete|Viewer|Cannot/i.test(t))) {
    ...
    await page.screenshot({ path: `Tools/visual-regression/output/sc2-port-${port}.png` });
...
for (const port of ports) {
  console.log(`\n═══════ PORT ${port} ═══════`);
  console.log("loc:", out[port].state.loc, "| iframe src:", out[port].state.iframeSrc);
