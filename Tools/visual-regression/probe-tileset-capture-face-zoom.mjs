...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  outBuf[idx] = r | (g << 8u) | (b << 16u) | (255u << 24u);
      ...
      return {
        ...
        entries: ctx._webgpuSceneCaptureModels?.models?.length ?? 0,
      };
    },
  ...
  fs.writeFileSync(
    path.join(
      OUT_DIR,
      `probe-tileset-capture-face-zoom-${captureOn ? "on" : "off"}.png`,
    ),
...
(async () => {
  ...
  console.log(
    `[tileset-face-zoom] ON: entries=${on.entries} records=${on.records} errors=${on.errs.length}`,
  );
  on.errs.slice(0, 6).forEach((e) => console.log("   ", e.t, e.text));
  console.log(
    `[tileset-face-zoom] OFF: entries=${off.entries} records=${off.records} errors=${off.errs.length}`,
  );
